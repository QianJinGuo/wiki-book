"""Fix content-diagram mismatch: replace architecture diagrams based on article CONTENT, not old template type.

Strategy:
1. Read article title + full content
2. Score against 20 topic profiles (keyword-weighted)
3. Pick the best-matching diagram template
4. Only replace if current diagram is clearly mismatched
"""
import re, os, sys
...
# Topic profiles: (profile_name, keywords with weights, diagram_key)
# Higher weight = stronger signal
PROFILES = [
    ('agent_react', {
        'react': 5, '推理+行动': 5, '反思循环': 5, 'reflection loop': 5, 'critic': 3,
        '观察-思考-行动': 5, '观察-行动': 4, '思想链': 3, 'chain-of-thought': 3,
        'reasoning trace': 4, 'trace': 2,
    }, 'agent_react'),
    ('agent_arch', {
...
# Diagram code — same rich templates as v4 but now content-matched
DIAGRAMS = {
...
def score_content(content):
    """Score content against all profiles, return best match."""
    lower = content[:3000].lower()  # Use first 3000 chars for speed
    title_m = re.search(r'^#\s+(.+)', content, re.MULTILINE)
    title = title_m.group(1).lower() if title_m else ''
    # Title gets extra weight
    combined = title + ' ' + title + ' ' + lower  # Double title weight
...
def get_current_diagram_type(content):
    """Identify what type of architecture diagram the article currently has."""
    blocks = list(re.finditer(r'\`\`\`mermaid\n(.*?)\`\`\`', content, re.DOTALL))
    for m in blocks:
        code = m.group(1).strip()
        if code.startswith('mindmap'):
            continue
        # Return the first non-mindmap block's start position and subgraph names
        subgraphs = re.findall(r'subgraph\s+"([^"]+)"', code)
        return m, subgraphs, code
    return None, [], ''


def is_generic_mismatch(current_subgraphs, content_diagram_key):
    """Check if current diagram subgraphs match the content-based diagram key."""
    ...
    mapping = {
        ...
        'multi_agent': ['编排层', 'agent 团队'],
        'harness': ['可观测性层', '护栏层'],
        'memory': ['工作记忆', '短期记忆', '长期记忆'],
        'rag': ['查询处理', '多路召回', '重排序'],
        'security': ['攻击面', '防御纵深', '检测响应'],
        'infra': ['边缘层', '服务层', 'agent 运行时'],
        'coding': ['意图理解', '代码生成', '验证闭环'],
        'training': ['数据准备', '训练阶段', '高效训练'],
        'inference': ['模型优化', '运行时优化', '调度策略'],
        'multimodal': ['编码器', '对齐层', '融合'],
        'robotics': ['感知层', '认知层', '决策层'],
        'data_pipeline': ['数据源', '采集层', '处理层'],
        'mlops': ['实验管理', '评估流水线', '部署流水线'],
        'product': ['发现', '验证', '增长'],
        'llm_core': ['输入处理', 'transformer block', '输出'],
        'opensource': ['项目生命周期', '质量门禁'],
        'cost': ['成本分析', '优化手段'],
        'compliance': ['法规要求', '实施层', '审计层'],
        'catchall': [],
    }
    
    expected = mapping.get(content_diagram_key, [])
    if not expected:
        return True  # catchall — always replace if mismatched
    
    # If any expected keyword appears in current subgraphs, it's a match
    for exp in expected:
        if exp in sg_str:
...
def fix_article(filepath):
    ...
    # Find current architecture diagram
    match, subgraphs, code = get_current_diagram_type(content)
    if match is None:
        return False
    
    # Determine what diagram SHOULD be here based on content
    correct_key = score_content(content)
    
    # Check if it's already correct
    if not is_generic_mismatch(subgraphs, correct_key):
        return False
    
    # Replace the mismatched diagram
...
for ch in ['ch01', 'ch03', 'ch04', 'ch05', 'ch07', 'ch09', 'ch11', 'ch12']:
    ...
    for fname in sorted(os.listdir(ch_dir)):
        ...
        if result:
            count += 1
            key = result[1] if isinstance(result, tuple) else 'unknown'
