...
# Pre-translated page dictionaries (generated by scripts/translate-all.py,
# tracked in git so CI needs no API key)
echo "=== Bundling pre-translations ==="
mkdir -p site/assets/tr
if [ -d translations ]; then
  cp translations/*.json site/assets/tr/ 2>/dev/null || true
  echo "Bundled $(ls translations | wc -l | tr -d ' ') dictionaries"
fi
