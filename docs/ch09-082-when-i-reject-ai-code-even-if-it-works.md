...
这篇文章本质上是在讨论 Harness Engineering 中的一个关键控制点：**人类审查回路**。在 Agent 系统中，harness 的作用是在 Agent 的自主性与系统可靠性之间建立边界。代码审查正是这个边界的具体体现——它是 Agent 输出进入生产系统的最后一道关卡。
...
文章提出的五条标准实际上是对传统代码审查标准的重新校准。在 AI 之前，「能否运行」很少成为审查的主要关注点，因为手动编写的代码通常在提交前已经过本地测试。AI 时代，「能否运行」成为了最容易达到的标准，而「是否应该合并」成为了新的核心问题。这与 Agentic Coding Workflow 中讨论的质量控制层直接相关。
...
- Harness Engineering — 代码审查作为 Agent 系统的控制层
- Agentic Coding Workflow — AI 辅助编码的完整工作流
