...
## 相关对照
- [AI Detection and Response](ch12-068-glasswasm-webassembly-malware-found-in-trojanized-open-vsx.html) —— 概念对照（云安全 vs 学术诚信，**底层方法论问题一致**）
...
## 关联阅读
- [AI Detection and Response (AIDR)](ch12-068-glasswasm-webassembly-malware-found-in-trojanized-open-vsx.html) —— 概念对照：云安全场景的 AI 检测方法论与学术诚信场景面临相似的黑箱校准问题，底层方法论缺陷一致（误判率不可审计、分布偏移导致性能漂移、闭源黑箱不可问责） 
- [Spotify LLM Evals](ch01-628-better-experiments-with-llm-evals-a-funnel-not-a-fork-s.html) —— 类比案例：Spotify 使用 LLM judges 进行 A/B 测试评估时，同样面临双层校准挑战——代理指标之上再增加代理指标，两层都可能漂移，这与 Pangram 在学术场景中的校准失效问题形成跨领域的方法论共鸣
...
- [Glm5 Scaling Pain](ch09-120-ai-coding-agent-token.html)
...
- [Noam Brown Ai Evaluation Reasoning Budget Performance Cost Curve](ch01-519-reasoning-lift-what-happens-to-ai-visibility-when-ai-thinks.html)
- [Aws Sagemaker Azerbaijani Lm](ch01-428-langchain-agent-3-2-5-agent.html)
...
**与 [Agent Harness Engineering](ch04-069-hermes-agent.html) 的呼应**：视频 Agent 化的核心瓶颈不是模型本身，而是**编排多种工具（扩散模型 + 传统剪辑）的 Harness 设计**——这正是 [Harness Engineering](ch04-160-kimi-work-beta-agent.html) 的能力被复用的领域。
...
- 与 [Agent Harness Engineering Survey](ch04-069-hermes-agent.html) 中描述的"AI 编程从一次生成走向多轮推理 + 调试 + 测试 + 提交 PR 的智能体系统"——Ethan 明确判断 **视频生成将经历完全相同的演化**。

### 与世界模型/机器人
[Yann LeCun JEPA 世界模型](ch04-363-temporary-cloudflare-accounts-for-ai-agents.html) + Fine-Tuning Cosmos + NVIDIA Gamma 世界模型——Ethan 在 Cosmos + Grok Imagine 的工作正是 **世界模型 + 视频生成** 的工程化实例，"实时长时程视频生成"是 [JEPA](ch04-363-temporary-cloudflare-accounts-for-ai-agents.html) 路线的 production 对应物。

### 与研究组织 / 算力
[AI Native 研发组织设计](ch05-018-ai-native-时代研发组织何去何从.html) + [Ai Native Rd Org Design Xiaobin](ch01-311-ai-native-时代-研发组织何去何从.html) 中都暗示了**算力 = 研究上限**的趋势。Ethan 的访谈是**第一手证据**——"GPU 富人也出来找算力"。

### 与 Agent 时代的人才竞争
- [中文 AI 实验室 Nathan 洞察](ch01-317-introducing-scheduled-tasks-2-0.html)——Ethan 的"算力-人才飞轮"是这些上层判断的**微观机制**

### 与上下文工程
[Agent Memory 架构](ch04-146-how-ai-agent-memory-works.html) + Agent Memory System Design + Context Management in Agent Systems——Ethan 揭示 LLM **不知道自己的上下文长度还剩多少**（"上下文到达 80% 时自动压缩触发，但模型在工作时并不知道这件事"），并指出 **OpenClaw 已经让模型具备时间感知能力**——这与 Context Management 中"让模型具备上下文自我感知"是同一方向。
...
- **复用 Harness 编排能力**：视频 Agent 的核心是工具编排（扩散模型 + 传统剪辑 + 资产库）——可复用 [Agent Harness Engineering](ch04-069-hermes-agent.html) 的实践
- **从"一次生成"到"生产级工作流"**：与 [画布 Agent 时代](ch04-442-ai-内容创作-开始进入-画布-agent-时代.html) + [AI 视频工具第三阶段](ch04-062-ai视频工具悄悄走到了第三阶段.html) 的演化趋势一致
...
- 与 [Agent Harness Engineering Survey](ch04-069-hermes-agent.html) 互补：Harness 在视频 Agent 时代的能力复用
- 与 [画布 Agent 时代](ch04-442-ai-内容创作-开始进入-画布-agent-时代.html) + [AI 视频工具第三阶段](ch04-062-ai视频工具悄悄走到了第三阶段.html) 互补：第一手研究人员视角补全产品视角
- 与 [Foundation Capital agent era 六洞察](ch04-173-web-bot-auth-agentcore-browser-tool-aws.html) 互补：算力-人才飞轮的微观机制
...
### 与 wiki 的关联
→ [Harness Engineering](ch05-041-harness-engineering.html) 的可观测性需求
...
## 相关实体
- [Natural Language Autoencoders — Anthropic 激活→文字可解释性方法](ch01-896-icml-2026.html)
- [QuickSight Dataset QA：NL直查S3 Iceberg](ch11-160-aws-quicksight-dataset-qa-natural-language.html)
