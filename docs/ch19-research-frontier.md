...
## 相关对照
- [AI Detection and Response](../ch12-068-ai-detection-and-response-aidr-a-zero-impact-operating-model) —— 概念对照（云安全 vs 学术诚信，**底层方法论问题一致**）
...
## 关联阅读
- [AI Detection and Response (AIDR)](../ch12-068-ai-detection-and-response-aidr-a-zero-impact-operating-model) —— 概念对照：云安全场景的 AI 检测方法论与学术诚信场景面临相似的黑箱校准问题，底层方法论缺陷一致（误判率不可审计、分布偏移导致性能漂移、闭源黑箱不可问责） 
- [Spotify LLM Evals](../ch01-628-better-experiments-with-llm-evals-a-funnel-not-a-fork-s) —— 类比案例：Spotify 使用 LLM judges 进行 A/B 测试评估时，同样面临双层校准挑战——代理指标之上再增加代理指标，两层都可能漂移，这与 Pangram 在学术场景中的校准失效问题形成跨领域的方法论共鸣
...
> **Background**：本文档基于 Recursive 团队 2026-06-11 发布的工程报告 *First Steps Toward Automated AI Research* 整理。Recursive 团队（与 [AlphaEvolve](../ch19-entities/alphaevolve-impact-deepmind) 同生态但不同公司）开发了一套自动化研究循环系统，瞄准三大 AI 进步杠杆：**训练算法 / 训练速度 / 硬件利用**。他们在三个 SOTA benchmark 上同时取得突破并开源 artifacts。
...
- 与 [Agent 自我改进六条路](../ch04-047-agent-自我改进的六条路) 中"对抗训练 + 编排自优化"两机制深度呼应

### 2. Long horizon context management
- 多个研究线程并行
- **保留前轮实验上下文**（不重置 prompt）
- 分支合并（promising branch combination）
- 工程实现上接近 [Hermes 自改进循环](../ch04-193-hermes-self-improving-闭环详解-winty) 的"持久记忆 + 进化搜索"
...
参考 [AlphaEvolve Impact](../ch04-372-alphaevolve-impact-deepmind) 和 [AlphaEvolve 一周年](https://github.com/QianJinGuo/wiki/blob/main/entities/alphaevolve交出一周年炸裂成绩单ai自我改进不再科幻.md) 了解 DeepMind 路线。
...
- **AI 进步已可被 AI 加速**：三个 SOTA 提升都不是"渐近修补"而是**数量级加速**（1.3× speedup、18% gap 缩小）。这与 [NanoGPT-Prime 递归自改进](../ch01-362-ai科研超越人类-prime-intellect递归自改进实验) 路线同向。
...
- 关联 entity：[Alphaevolve Impact Deepmind](../ch04-372-alphaevolve-impact-deepmind)、[Agent Self Improvement Six Mechanisms](../ch04-047-agent-自我改进的六条路)、[Ai Recursive Self Improvement Nanogpt Prime Intellect](../ch01-362-ai科研超越人类-prime-intellect递归自改进实验)、[Hermes Self Improving Loop Winty](../ch04-193-hermes-self-improving-闭环详解-winty)、[Deli Auto Research Skill V2 Continual Learning Self Improvement](../ch01-822-deepseek陈德里ai论文第二弹-从6分到8分-deliautoresearch-skill又进化了)
...
### 同范式生态
- [ICML 2026 NPR 文本原生并行推理](../ch01-884-icml-2026-告别-单线程-思维-智能体进化出了原生的并行推理大脑) — **同源**: 都推动"推理宽度扩展", NPR 在文本领域开辟
- [LASER ACL 2026 视觉推理](../ch01-879-laser-acl2026-latent-superposition-visual-reasoning) — **互补**: 同样针对 VLM, 但用 latent superposition 路线
- [DeepSeek 视觉原语](../ch01-213-deepseek-thinking-with-visual-primitives-深度解读) — **对比**: DeepSeek 用"视觉原语"做视觉推理的另一种思路

### 视觉/多模态相关
- [LLaVA-OneVision-2 全帧率 VLM](../ch01-600-对话灵感实验室-全帧率-vlm-低成本与分层部署) — VLM 架构
...
### 推理范式
- [LLM 语言思维机制](../ch01-476-llm-工作机制与可解释性) — 推理机制基础
- [Layered Thinking 分层思维](../ch03-050-ai-native-公司-dan-shipper-30-人做出-100-人产出-层次化思维-saas-6-维能力框架) — 推理范式

## 相关实体

- [Native Parallel Reasoner Icml2026](../ch01-884-icml-2026-告别-单线程-思维-智能体进化出了原生的并行推理大脑)
- [Laser Acl2026 Latent Superposition Visual Reasoning](../ch01-879-laser-acl2026-latent-superposition-visual-reasoning)
- [Llava Onevision 2 Full Frame Rate Vlm Glintlab](../ch01-600-对话灵感实验室-全帧率-vlm-低成本与分层部署)
- [Deepseek Visual Primitives Thinking](../ch01-213-deepseek-thinking-with-visual-primitives-深度解读)
- [Llm Language Thinking Mechanisms](../ch01-476-llm-工作机制与可解释性)- [count anything - 文本引导的通用目标计数框架](https://github.com/QianJinGuo/wiki/blob/main/entities/arxiv-2605-30846-count-anything-2026.md)
...
- [qwen-image-flash: beyond objective design — few-step distill](../ch01-866-qwen-image-flash-beyond-objective-design-few-step-distill)
- [对图像内容进行精确分析 — bedrock 多模态案例实践（汽车油表识别）](../ch11-233-对图像内容进行精确分析-bedrock-多模态案例实践-汽车油表识别)
...
- → [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/loopwm-looped-world-models.md)
...
- [Glm5 Scaling Pain](../ch09-120-glm-5-scaling-pain-推理复盘)
...
**与 [Agent Harness Engineering](../ch04-069-agent-harness-engineering-a-survey) 的呼应**：视频 Agent 化的核心瓶颈不是模型本身，而是**编排多种工具（扩散模型 + 传统剪辑）的 Harness 设计**——这正是 [Harness Engineering](../ch04-160-harness-engineering-让-coding-agent-可靠完成长程任务) 的能力被复用的领域。
...
- 与 [Agent Harness Engineering Survey](../ch04-069-agent-harness-engineering-a-survey) 中描述的"AI 编程从一次生成走向多轮推理 + 调试 + 测试 + 提交 PR 的智能体系统"——Ethan 明确判断 **视频生成将经历完全相同的演化**。

### 与世界模型/机器人
[Yann LeCun JEPA 世界模型](../ch04-363-yann-lecun-jepa世界模型与ami-labs) + [Fine-Tuning Cosmos](https://github.com/QianJinGuo/wiki/blob/main/entities/fine-tuning-cosmos.md) + [NVIDIA Gamma 世界模型](https://github.com/QianJinGuo/wiki/blob/main/entities/nvidia-gamma-world-multi-agent-world-model.md)——Ethan 在 Cosmos + Grok Imagine 的工作正是 **世界模型 + 视频生成** 的工程化实例，"实时长时程视频生成"是 [JEPA](../ch04-363-yann-lecun-jepa世界模型与ami-labs) 路线的 production 对应物。

### 与研究组织 / 算力
[AI Native 研发组织设计](../ch05-018-ai-native-时代研发组织何去何从) + [Ai Native Rd Org Design Xiaobin](../ch01-311-ai-native-时代-研发组织何去何从) 中都暗示了**算力 = 研究上限**的趋势。Ethan 的访谈是**第一手证据**——"GPU 富人也出来找算力"。

### 与 Agent 时代的人才竞争
- [中文 AI 实验室 Nathan 洞察](../ch01-317-所有实验室都怕字节-所有人都在夸deepseek-美国研究员36小时中国ai行)——Ethan 的"算力-人才飞轮"是这些上层判断的**微观机制**

### 与上下文工程
[Agent Memory 架构](../ch04-029-agent-memory-architecture-past-influence-future-ruofei) + [Agent Memory System Design](https://github.com/QianJinGuo/wiki/blob/main/concepts/agent-memory-system-design.md) + [Context Management in Agent Systems](https://github.com/QianJinGuo/wiki/blob/main/concepts/context-management-agent-systems.md)——Ethan 揭示 LLM **不知道自己的上下文长度还剩多少**（"上下文到达 80% 时自动压缩触发，但模型在工作时并不知道这件事"），并指出 **OpenClaw 已经让模型具备时间感知能力**——这与 [Context Management](https://github.com/QianJinGuo/wiki/blob/main/concepts/context-management-agent-systems.md) 中"让模型具备上下文自我感知"是同一方向。
...
- **复用 Harness 编排能力**：视频 Agent 的核心是工具编排（扩散模型 + 传统剪辑 + 资产库）——可复用 [Agent Harness Engineering](../ch04-069-agent-harness-engineering-a-survey) 的实践
- **从"一次生成"到"生产级工作流"**：与 [画布 Agent 时代](../ch04-442-ai-内容创作-开始进入-画布-agent-时代) + [AI 视频工具第三阶段](../ch04-062-ai视频工具悄悄走到了第三阶段) 的演化趋势一致
...
## 与现有 Wiki 的关系
- 与 [Claude Code 动态工作流多 Agent 编排](../ch04-409-agent-orchestration) 互补：AI 编程的 Agent 化 → 视频生成的 Agent 化
- 与 [Agent Harness Engineering Survey](../ch04-069-agent-harness-engineering-a-survey) 互补：Harness 在视频 Agent 时代的能力复用
- 与 [画布 Agent 时代](../ch04-442-ai-内容创作-开始进入-画布-agent-时代) + [AI 视频工具第三阶段](../ch04-062-ai视频工具悄悄走到了第三阶段) 互补：第一手研究人员视角补全产品视角
- 与 [Foundation Capital agent era 六洞察](../ch04-173-foundation-capital-agent-era-six-insights) 互补：算力-人才飞轮的微观机制
...
- [Alphaxiv Reinforcement Learning For Rlms](../ch01-636-reinforcing-recursive-language-models-alphaxiv)
...
## Ch19.011 Natural Language Autoencoders (Anthropic)
...
## 相关实体
- [Natural Language Autoencoders — Anthropic 激活→文字可解释性方法](../ch01-896-natural-language-autoencoders-anthropic-激活-文字可解释性方法)
...
- [Skill-RM: Reward Model as Agent Skill](../ch01-868-阿里qwen提出skill-rm-把奖励模型做成可复用agent-skill)
...
> 📊 Level ⭐⭐⭐⭐⭐ | 2.8KB | `entities/arxiv-2606-12683-from-agi-to-asi.md`

# From AGI to ASI

## 相关实体
- [mira + mpa：深度原理 ai scientist 递归自训练打造材料基座模型，40 项实验全面 sota](../ch01-183-mira-mpa-深度原理-ai-scientist-递归自训练打造材料基座模型-40-项实验全面-sota)
- [some ideas for what comes next, may 2026 (interconnects)](../ch01-910-some-ideas-for-what-comes-next-may-2026-interconnects)
- [agi 之路，可能从一开始就走错了（腾讯研究院·王鹏）](../ch01-258-agi-之路-可能从一开始就走错了-腾讯研究院-王鹏)

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/arxiv-2606-12683-from-agi-to-asi.md)

## 核心要点

1. **AGI 到 ASI 的 transition framework** — 论文提出 AGI（Artificial General Intelligence）向 ASI（Artificial Superintelligence）的转换路径，重点在递归自我改进（recursive self-improvement）的临界点。
2. **Recursive self-improvement 的能力评估** — ASI 不是比 AGI 强 的简单递进，而是 agent 能改进自身架构/训练数据/推理流程的临界能力。
3. **安全性 implications** — AGI 到 ASI 的过渡窗口（alignment window）是关键研究对象，过渡期的失控风险与人类监管机制设计。
4. **Capability ladder 与 economic displacement** — 从 AGI 到 ASI 的 capability 提升曲线，与劳动力市场替代速度的非线性关系。
5. **Open research questions** — 论文列出 ASI 路径上的开放问题：self-modifying architectures、constitutional AI 的规模化、跨 agent 协作的 emergent behavior。

## 与现有实体的差异化

- **Superintelligence / alignment 系列**：AGI 是已被广泛讨论的话题，本文聚焦 AGI 到 ASI 的转换临界点。
- **Recursive self-improvement 系列**：现有 entity 多在小规模 self-play 层面，本文是 AGI 级别的递归自我改进理论框架。

## 实践启示

- AGI 到 ASI 的过渡窗口是 alignment research 的核心目标，决定人类监管能否发挥作用。
- Recursive self-improvement 的能力评估需要新的 benchmark，超越现有的 static capability eval。
- ASI 不是单一技术突破，而是 architecture + data + compute + alignment 的系统协同。
- arxiv 2606.12683 的论文框架可作为后续 ASI 研究的方向地图。
