...
## 相关对照
- [AI Detection and Response](ch12/078-ai-detection-and-response-aidr-a-zero-impact-operating-model.html) —— 概念对照（云安全 vs 学术诚信，**底层方法论问题一致**）
...
## 关联阅读
- [AI Detection and Response (AIDR)](ch12/078-ai-detection-and-response-aidr-a-zero-impact-operating-model.html) —— 概念对照：云安全场景的 AI 检测方法论与学术诚信场景面临相似的黑箱校准问题，底层方法论缺陷一致（误判率不可审计、分布偏移导致性能漂移、闭源黑箱不可问责） 
- [Spotify LLM Evals](ch01/1274-llm.html) —— 类比案例：Spotify 使用 LLM judges 进行 A/B 测试评估时，同样面临双层校准挑战——代理指标之上再增加代理指标，两层都可能漂移，这与 Pangram 在学术场景中的校准失效问题形成跨领域的方法论共鸣
...
美团技术团队面向百万中小商家的真实需求，构建了覆盖"能生成、能编辑、能评判"的完整 AIGC 海报生成技术体系。三篇顶级会议论文构成了技术核心：PosterCraft（ICLR 2026）实现高美感端到端海报生成、PosterOmni（CVPR 2026）实现六类设计任务的统一建模、PosterReward（CVPR 2026）建立首个海报专用质量评估奖励模型。三者形成"生成-编辑-评判"的技术闭环，已在美团外卖套餐图生成、品牌 IP 袋鼠团团、点评信息流治理等业务场景中完成落地。

## 核心要点

1. **现实痛点驱动**：百万中小商家面临设计资源匮乏（外包数百至数千元/张）、时效性苛刻（分钟级交付 vs 传统 1-3 天）、内容同质化严重、批量生产质量失控等四重困境。

2. **技术闭环架构**：PosterCraft（端到端生成）→ PosterOmni（多任务统一编辑）→ PosterReward（质量评估），三者相互支撑、协同进化，构成完整的海报 AIGC 技术栈。

3. **全部开源**：三篇论文全部发表并开源（[MeiGen-AI](https://github.com/MeiGen-AI) 仓库），覆盖从基础生成到多任务融合到精准评估的全链路。

## 深度分析

### 挑战的多维交织：为什么海报 AIGC 不只是"文生图"

高质量商业海报生成面临**五大技术挑战**，这些挑战相互交织，远超普通的文生图任务：

**精准文字渲染**是海报生成的首要难题。海报上的文字要求"零容错"——任何错误、缺失或模糊都导致整张海报不可用。主流扩散模型在多行文字、中文字符和小字号文本上仍有明显短板，中文场景下的渲染困难尤为突出。

**和谐的版式布局**要求模型隐式学习对比、重复、对齐、亲密性等设计原则。这种"设计感"难以通过规则显式定义，更多依赖对大量优秀作品的隐式学习。不同行业的标准也迥异：餐饮要"食欲感"，美妆要"精致感"，科技要"未来感"。

**多任务场景的统一**是模型设计上的重大挑战。真实设计需求横跨"局部编辑"（文字排版叠加、局部填充）和"全局创作"（风格迁移、版式重组）两大范畴。需要在单一模型中同时支持所有场景。

PosterCraft、PosterOmni 和 PosterReward 这三项工作分别面对这些挑战的不同侧面，共同构成了完整解决方案。

### PosterCraft（ICLR 2026）：端到端高美感海报生成

PosterCraft 的核心创新是**摒弃模块化流水线，采用端到端统一优化**。传统方法采用"VLM 规划布局 → 背景生成 → 文字叠加"的模块化流水线，美学一致性难以保证。PosterCraft 通过四阶段级联优化工作流解决这一问题：
...
PosterOmni 解决了从 Text-to-Poster 到 Image-to-Poster 的范式转变。真实设计场景中更常见的起点是一张参考图、旧版海报或产品主视觉——设计目标不是完全重做，而是在保留核心主体的基础上完成编辑。

**核心创新**：单一模型覆盖六类设计任务（Extending、Filling、Rescaling、ID-driven、Layout-driven、Style-driven），采用"数据—蒸馏—奖励"闭环解决多任务冲突：

- 分别训练"局部编辑专家"和"全局创作专家"
- 通过任务蒸馏整合为统一学生模型（PosterOmni-SFT）
- 加入统一奖励与强化学习，对齐审美偏好、编辑准确性和指令遵循能力

**成果**：在 PosterOmni-Bench（首个面向多任务设计场景的统一测试基准）上，PosterOmni 在全部六类任务上均取得开源模型最佳表现，整体评分超越部分闭源模型。
...
参见 [MeiGen-AI 海报生成技术体系](ch05/094-ai.html) 中的论文解析。

### 技术闭环的协同进化

三个模块形成了自我进化的后训练系统：PosterCraft 建立端到端基础生成能力 → PosterOmni 拓展至多任务场景 → 营销海报结构化提供可解释设计规范 → PosterReward 将设计知识内化为端到端奖励信号，驱动生成模型持续进化。

这种闭环结构与 [Agent Harness 工程范式](https://github.com/QianJinGuo/wiki/blob/main/concepts/agent-harness-engineering-paradigm.md) 中的"反馈驱动进化"理念一脉相承——质量评估信号反哺生成模型优化，而非停留在单向的"生成→评估→结束"。

## 实践启示

1. **从闭源到开源的可行路径**：美团证明了在视觉生成领域，开源模型通过系统化的数据构建、多阶段训练和技术闭环，可以达到接近 SOTA 闭源商业系统的水平。这对于预算有限但具有技术能力的团队有重要参考意义。

2. **评估基础设施先于生成能力建设**：PosterReward 的建设先于 PosterOmni 的 RL 优化阶段。在构建生成能力之前先建立可量化的评估标准，可以避免"盲目优化"——这一原则同样适用于 Agent 系统的开发。

3. **合成数据的噪声控制是关键瓶颈**：PosterOmni-200K 的数据管线中最关键的环节不是提示词生成，而是多模态过滤——分层过滤机制（OCR 检查 + 图文一致性判断 + VLM 任务匹配度评估）是确保合成数据质量的核心。

4. **任务蒸馏解决多任务冲突**：当需要单一模型覆盖多个差异较大的任务时，先训练领域专家模型再通过蒸馏整合为统一学生模型的"拆分-整合"策略，比直接联合训练效果更好。这一经验可迁移到其他多任务 Agent 系统的设计。

5. **产业应用与学术发表并行的策略**：三项工作全部发表在顶级学术会议（ICLR/CVPR），同时完成了在美团多个业务场景中的实际落地。这种"学术深度的技术方案 + 产业验证的落地效果"的组合策略，是工程团队追求影响力的有效路径。

## 相关实体

- [MeiGen-AI 海报生成技术体系](ch05/094-ai.html) — 三项工作的论文级解析
- [美团海报生成 AIGC 生产实践](ch05/094-ai.html) — 工程落地方案
- [美团 LongCat 2.0](ch04/420-longcat-2-0.html) — 美团大模型技术栈
...
参考 [AlphaEvolve Impact](ch04/464-alphaevolve-impact-deepmind.html) 和 [AlphaEvolve 一周年](ch05/094-ai.html) 了解 DeepMind 路线。
...
- **AI 进步已可被 AI 加速**：三个 SOTA 提升都不是"渐近修补"而是**数量级加速**（1.3× speedup、18% gap 缩小）。这与 [NanoGPT-Prime 递归自改进](ch05/094-ai.html) 路线同向。
...
- 关联 entity：[Alphaevolve Impact Deepmind](ch04/464-alphaevolve-impact-deepmind.html)、[Agent Self Improvement Six Mechanisms](ch03/035-agent.html)、[Ai Recursive Self Improvement Nanogpt Prime Intellect](ch05/094-ai.html)、[Hermes Self Improving Loop Winty](https://github.com/QianJinGuo/wiki/blob/main/entities/hermes-self-improving-loop-winty.md)、[Deli Auto Research Skill V2 Continual Learning Self Improvement](ch04/271-skill.html)
...
- [Gaode Ai Companion Agent Architecture](ch05/094-ai.html)
...
> **Background**：本文基于机器之心报道 [Arxiv Independent Nonprofit 2026 07 02](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/arxiv-independent-nonprofit-2026-07-02.md) 综合整理。arXiv 于 2026 年 7 月 1 日正式脱离康奈尔大学，转型为独立非营利组织 arXiv, Inc.
...
### 同范式生态
- [ICML 2026 NPR 文本原生并行推理](ch01/913-20.html) — **同源**: 都推动"推理宽度扩展", NPR 在文本领域开辟
- [LASER ACL 2026 视觉推理](ch01/1279-laser-acl2026-latent-superposition-visual-reasoning.html) — **互补**: 同样针对 VLM, 但用 latent superposition 路线
- [DeepSeek 视觉原语](ch09/179-deepseek-visual-primitives.html) — **对比**: DeepSeek 用"视觉原语"做视觉推理的另一种思路

### 视觉/多模态相关
- [LLaVA-OneVision-2 全帧率 VLM](ch01/817-vlm.html) — VLM 架构
- [a16z 视觉 AI 下一个前沿是代码](ch05/094-ai.html) — 视觉 AI 趋势

### 推理范式
- [LLM 语言思维机制](ch01/1274-llm.html) — 推理机制基础
- [Layered Thinking 分层思维](ch05/094-ai.html) — 推理范式

## 相关实体

- [Native Parallel Reasoner Icml2026](ch01/913-20.html)
- [Laser Acl2026 Latent Superposition Visual Reasoning](ch01/1279-laser-acl2026-latent-superposition-visual-reasoning.html)
- [Llava Onevision 2 Full Frame Rate Vlm Glintlab](ch01/817-vlm.html)
- [Deepseek Visual Primitives Thinking](ch09/179-deepseek-visual-primitives.html)
- [Llm Language Thinking Mechanisms](ch01/1274-llm.html)- [count anything - 文本引导的通用目标计数框架](ch01/913-20.html)
...
## 相关实体
- [visual para-thinker: 视觉并行思考框架 (arxiv 2602.13310)](ch01/817-vlm.html)
- [qwen-image-flash: beyond objective design — few-step distill](ch01/1236-qwen-image-flash-beyond-objective-design-few-step-distill.html)
- [对图像内容进行精确分析 — bedrock 多模态案例实践（汽车油表识别）](ch11/299-bedrock.html)
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/2026-06-30-AI圈刚开始谈Loop-Engineering-两位95后博士已经盯上了人类闭环-机器之心.md)
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/loopwm-loop-world-models-2026-07-01.md)
...
**GenCeption** 是一篇 2026 年 7 月的 arXiv 论文（2607.09024），提出以文生视频（text-to-video）生成为视觉基础模型的预训练范式。论文将预训练的视频生成扩散模型转化为统一的、前馈的通用视觉感知模型——GenCeption，在深度估计、表面法线预测、相机姿态估计、分割和 3D 关键点等任务上达到或超越专门模型（SOTA）。Kaiming He 为合作者之一。
...
这与 [ELF (Embedded Language Flows)](ch05/094-ai.html) 共享 Kaiming He 在通用表征学习方面的研究脉络——ELF 提出语言流作为通用接口，GenCeption 则探索视频生成作为视觉预训练的接口。

## 技术架构

GenCeption 的架构包含两个阶段：
...
### 极端数据效率
视频生成预训练骨干在同等微调数据下胜过替代预训练范式（V-JEPA, VideoMAE V2）。最引人注目的：仅用 **D4RT 和 VGGT-Omega 的 1/7 到 1/500 训练数据**即可达到可比性能。

### 涌现行为
仅在合成人体视频上训练的模型能泛化到真实世界 footage 和分布外物体类别（动物、机器人）——展现出初级的 sim-to-real 迁移能力和模型缩放特性。

## 与现有生态的关系

- [Diffusion Model 一致性框架](ch01/913-20.html) — GenCeption 将多步扩散转化为单步前馈，是 consistency 方向的重要应用
- [ELF: Embedded Language Flows](ch05/094-ai.html) — 同一位研究者（Kaiming He）在通用接口方面的不同探索：语言流 vs 视频生成
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/genception-video-gen-models-general-purpose-vision-learners-arxiv-2607.md)
...
## 相关实体
- [Onereason Kuaishou Reasoning Recommender System](ch05/094-ai.html)
- [Glm5 Scaling Pain](ch05/094-ai.html)
- [Video Agent Paradigm Compute Talent Flywheel Ethan He 20260606](ch03/035-agent.html)
- [Noam Brown Ai Evaluation Reasoning Budget Performance Cost Curve](ch05/094-ai.html)
- [Aws Sagemaker Azerbaijani Lm](ch05/094-ai.html)
...
**与 [Agent Harness Engineering](ch03/035-agent.html) 的呼应**：视频 Agent 化的核心瓶颈不是模型本身，而是**编排多种工具（扩散模型 + 传统剪辑）的 Harness 设计**——这正是 [Harness Engineering](ch03/035-agent.html) 的能力被复用的领域。
...
- 与 [Agent Harness Engineering Survey](ch03/035-agent.html) 中描述的"AI 编程从一次生成走向多轮推理 + 调试 + 测试 + 提交 PR 的智能体系统"——Ethan 明确判断 **视频生成将经历完全相同的演化**。
...
### 与研究组织 / 算力
[AI Native 研发组织设计](ch05/094-ai.html) + [Ai Native Rd Org Design Xiaobin](ch05/094-ai.html) 中都暗示了**算力 = 研究上限**的趋势。Ethan 的访谈是**第一手证据**——"GPU 富人也出来找算力"。

### 与 Agent 时代的人才竞争
- [中文 AI 实验室 Nathan 洞察](ch05/094-ai.html)——Ethan 的"算力-人才飞轮"是这些上层判断的**微观机制**
...
- **复用 Harness 编排能力**：视频 Agent 的核心是工具编排（扩散模型 + 传统剪辑 + 资产库）——可复用 [Agent Harness Engineering](ch03/035-agent.html) 的实践
- **从"一次生成"到"生产级工作流"**：与 [画布 Agent 时代](ch05/094-ai.html) + [AI 视频工具第三阶段](ch05/094-ai.html) 的演化趋势一致
...
## 与现有 Wiki 的关系
- 与 [Claude Code 动态工作流多 Agent 编排](ch03/077-claude-code.html) 互补：AI 编程的 Agent 化 → 视频生成的 Agent 化
- 与 [Agent Harness Engineering Survey](ch03/035-agent.html) 互补：Harness 在视频 Agent 时代的能力复用
- 与 [画布 Agent 时代](ch05/094-ai.html) + [AI 视频工具第三阶段](ch05/094-ai.html) 互补：第一手研究人员视角补全产品视角
- 与 [Foundation Capital agent era 六洞察](ch04/186-foundation-capital-agent-era-six-insights.html) 互补：算力-人才飞轮的微观机制
...
-> [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/arxiv-2606-03979-language-models-need-sleep.md)
...
## 相关实体
- [Natural Language Autoencoders — Anthropic 激活→文字可解释性方法](ch01/1330-natural-language-autoencoders-anthropic.html)
- [QuickSight Dataset QA：NL直查S3 Iceberg](ch11/181-aws-quicksight-dataset-qa-natural-language.html)
...
魔芯科技正式发布 MoWorld，全球首个超高帧率交互式世界模型。该模型基于 14B 参数的 MoE 架构，在国产华为昇腾 NPU 平台上实现最高 50 FPS 实时推理，推理成本相比 GPU 降低 70%。MoWorld 首次在纯国产 NPU 上打通了世界模型从预训练、自回归蒸馏到实时推理部署的完整技术链路，标志着世界模型产业化的「DeepSeek 时刻」。

## 核心要点

- **Flash World Model 概念**：魔芯科技首次提出高帧率世界模型概念，认为 50 FPS 以上的帧率是世界模型在真实场景中可用的必要条件，突破了主流 30 FPS 以下的技术瓶颈。
- **国产 NPU 全栈打通**：MoWorld 在华为昇腾 NPU 平台上完成了预训练→蒸馏→推理的全栈优化，证明了国产 NPU 能够承载世界模型训练与大规模实时部署。
- **推理成本降低 70%**：经过 NPU 优化后的 MoWorld 推理成本相比 GPU 降低 70%，大幅降低了世界模型的部署门槛。
- **数据引擎与系统协同**：构建了行业领先的可扩展数据生产引擎与自动化数据治理管线，以及预训练→蒸馏→推理的三阶段协同优化体系。
- **开源与可获取性**：技术报告已发布，近期将开源权重和代码，并基于国产 NPU 超节点向公众提供服务。

## 技术架构

### 数据引擎：高质量 4D 世界建模资产

MoWorld 基于过去在 3D/4D 重建领域的成果，构建了可扩展的数据生产引擎。与传统视频生成模型不同，世界模型训练不仅需要视频和文本，还需要精确的相机轨迹与空间信息，以实现「4D 感知」。核心流程包括：

1. **多维质量筛选**：通过 3D 几何一致性、轨迹精度、多视图稳定性等多维标准筛选高质量训练数据
2. **几何补全与动态过滤**：对海量原始视频进行几何补全、动态区域过滤和轨迹验证
3. **自动化治理管线**：将原始视频高质量转换为世界模型训练数据

### 预训练—蒸馏—推理三阶段协同

**预训练阶段**：针对国产 NPU 集群高并行、长序列计算能力强的特点，设计了超密集注意力并行和输入 Token 并行两项关键技术，将注意力计算和超长视频序列拆分到多台 NPU 协同执行，支撑训练出长达 2 分钟的基础模型。
...
MoWorld 的出现可以被视为世界模型领域的「DeepSeek 时刻」，其核心类比在于：DeepSeek 证明了高效训练的大语言模型可以匹敌闭源前沿模型，而 MoWorld 证明了国产 NPU 承载的世界模型可以在性能与成本上达到产业可用级。两者的共同点是打破了「只有高端硬件/高成本才能实现前沿性能」的固有认知。MoWorld 的 14B MoE 模型在国产 NPU 上实现 50 FPS 实时交互，推理成本降低 70%，这一量级的变化正是从「实验室演示」到「产业应用」的关键跨越。

### 2. 帧率作为世界模型的核心性能指标

主流世界模型帧率长期停留在 30 FPS 以下，而 MoWorld 将这一指标提升到 50 FPS。这一提升的意义不仅是数字层面的增长——它触及了世界模型在实际决策系统中的根本角色：实时机器人控制、自动驾驶导航、交互式内容生成都需要极高的响应速度。高帧率意味着模型能够捕捉更精细的时序动态，为决策系统提供更密集、更准确的环境状态预测，从而提升整体系统的反应质量和安全性。
...
- 训练和推理的端到端自主可控路径已经走通
这不仅为国产 AI 硬件生态提供了标杆性应用案例，也意味着未来世界模型的规模化部署可以规避 GPU 供应链风险，加速物理 AI 和空间智能在中国的产业化进程。

### 4. 从生成质量到系统效率：世界模型竞争范式的转变

文章明确提出，世界模型的竞争正在从「谁生成得更像」转向「谁能在真实算力约束下更稳定、更实时、更经济地运行」。这一判断揭示了世界模型产业化面临的核心矛盾：生成质量的提升边际递减，而系统部署的成本和效率成为实际落地的关键瓶颈。MoWorld 的三层协同优化（流水线→并行→算子）代表了解决这一矛盾的工程化思路——不是一味追求模型能力的提升，而是通过系统级协同设计让现有能力在约束条件下发挥最大效用。
...
- [华为云昇腾具身智能生态](ch01/456-rlinf.html) — 同样基于昇腾 NPU 的具身智能框架优化
- [世界模型产业时代](ch01/686-70.html) — MoWorld 的产业落地视角
- **世界模型** — 世界模型概念与技术综述
...
Light Interaction 是浙江大学与 NVIDIA 合作提出的训练无关的交互式视频世界模型推理加速方法。核心思路是不修改模型参数、不重新训练，只在推理阶段根据用户交互状态（相机轨迹、局部动态等）自适应调度计算资源，在 HY-WorldPlay 上实现 **2.59× 加速**，在 Matrix-Game-3.0 上实现 **1.61× 加速**，同时降低峰值显存。

## 研究背景与动机

交互式视频世界模型正在从「一次性生成短片」走向「像游戏一样边操作边生成」。用户可以通过前进、后退、转向、回看等操作持续生成后续画面。但长轨迹交互会迅速放大三类基础开销：

- **历史信息更重**：为保持长时一致性需保留足够上下文，KV cache 显存压力难以忽略
- **注意力计算更重**：当前生成视频片段需参考更长历史，上下文越长注意力开销越高
- **去噪成本更重**：每个视频片段需多次运行 Transformer 完成逐步去噪

以 HY-WorldPlay 为例，单张 A100 GPU 上生成约 10 秒视频，模型主干推理耗时超过 200 秒。

## 核心技术

论文链接：https://arxiv.org/abs/2605.31158 | 项目主页：https://2843721358l-del.github.io/Light-Interaction-Project/ | GitHub：https://github.com/2843721358l-del/Light-Interaction-Project

Light Interaction 的核心观察是：**交互状态决定了哪些历史信息真正有用，也决定了哪些计算可以被安全省掉**。基于此，方法拆为三个组件：
...
时间上下文也不再使用固定窗口，而是根据近期稳定 latent 的变化程度自适应调整：局部动态较大时缩短时间窗口，减少冗余历史；变化平稳时保留更多上下文，增强连续性。
...
这使得 3D 稀疏注意力在自回归视频生成场景中真正跑出实际速度提升。

## 实验结果

团队在 HY-WorldPlay 和 Matrix-Game-3.0 两个开源交互式视频世界模型上进行评估，与 Sparse VideoGen、LongCat-Video Block Sparse Attention、TeaCache 等无需训练的加速基线对比。实验硬件为 NVIDIA A100 80GB GPU。
...
质量指标（PSNR、SSIM、LPIPS、VBench）显示 Light Interaction 并非简单「拿画质换速度」：在 HY-WorldPlay 上保持较好视觉质量并显著降低显存，在 Matrix-Game-3.0 上以竞争性质量取得最低延迟。

## 意义与展望

Light Interaction 的价值在于提出了一种更适合交互式生成的推理范式：**交互轨迹不只是生成条件，也可以成为系统调度信号**。
...
这让视频世界模型的推理过程从固定流程变成了随交互状态变化的动态系统。对于游戏模拟、虚拟场景探索、机器人训练和具身智能等应用场景，推理延迟和显存成本是能否走向实际使用的关键门槛，Light Interaction 提供了一条无需重新训练模型的现实路线。

## 作者信息

论文第一作者为浙江大学逯嘉程，通讯作者为浙江大学李渝研究员，并与 NVIDIA 谢恩泽研究员团队合作完成。作者团队关注视频生成模型推理优化、视频世界模型、软硬件协同加速等方向，涉及 KV 缓存管理、稀疏注意力、GPU 算子优化等系统问题。

## 相关概念

- [扩散模型架构](https://github.com/QianJinGuo/wiki/blob/main/concepts/diffusion-model-architecture.md) — Light Interaction 基于扩散模型架构的视频世界模型进行推理优化
- [推理优化](https://github.com/QianJinGuo/wiki/blob/main/concepts/inference-optimization.md) — 本文属于训练无关的推理阶段优化方法
- [注意力机制](https://github.com/QianJinGuo/wiki/blob/main/concepts/attention-mechanism.md) — 3D 稀疏注意力是核心加速手段之一
- [GPU 优化](https://github.com/QianJinGuo/wiki/blob/main/concepts/gpu-optimization.md) — Triton 算子融合与 HW/SW 协同设计是实际加速的关键
- [视频生成模型](https://github.com/QianJinGuo/wiki/blob/main/concepts/video-generation-models.md) — 交互式视频世界模型是视频生成的前沿方向
...
- [Skill-RM: Reward Model as Agent Skill](ch04/271-skill.html)
...
- [mira + mpa：深度原理 ai scientist 递归自训练打造材料基座模型，40 项实验全面 sota](ch05/094-ai.html)
- [some ideas for what comes next, may 2026 (interconnects)](ch01/1352-some-ideas-for-what-comes-next-may-2026-interconnects.html)
- [agi 之路，可能从一开始就走错了（腾讯研究院·王鹏）](ch01/221-agi.html)
...
- [ICML 2026 获奖论文](ch01/1295-icml-2026.html)
- [Claude Code 学术文献综述](ch03/077-claude-code.html)
- [Agent 技能写作指南](ch04/271-skill.html)
- [AI 科研助手从构思到应用](ch05/094-ai.html)
...
智源研究院（BAAI）悟界·RoboBrain Orca Team 发布的技术报告 Orca: The World is in Your Mind，提出 **Next-State Prediction（下一状态预测）**范式。

## 核心哲学

> 先让模型学习统一的世界状态表征，再从这个表征中读出理解、预测和行动能力。

Orca 不追求更好的 token 预测、帧生成或动作模仿，而是关注：**当前世界处于什么状态，以及这个状态在自然演化、事件条件或外部干预下，会如何转移到另一个状态。**

比喻：不让 3 岁小孩进工厂打 10 万小时螺丝——先理解世界，再去做具体任务。
...
两类学习共同构造 **world latent**——统一的世界状态表征空间。
...
基于自研 FlagScale 实现 **4.4× 加速**（H100 集群：0.66 → 2.91 Samples/Sec/GPU）。关键优化：FSDP2 灵活分片、分块交叉熵损失、前向/后向预取通信重叠。

## Three-in-One Readout

冻结 backbone，仅训练轻量 readout 模块，验证 world latent 是否真正承载多维能力：
...
# 唐杰内部信曝光：两年死磕ASI！ — 智谱ASI路线图

> 智谱创始人唐杰内部信披露两年ASI路线图，启动"Touch High（摸高）"计划，四大引擎同步推进：长程任务新记忆架构、数字员工智能体社会、合成数据工厂+沙盒代码重构、百亿级可解释性研究。这封内部信因首次将"AI意识"写入需要当作承诺兑现的公司内部文件而引发行业关注。

## 摘要

2026年7月，智谱创始人唐杰发布内部信《巨浪已来》，正式启动为期两年的"Touch High（摸高）"ASI冲刺计划。信中提出了从长程任务到自主智能体社会、再到无人公司、最终通向ASI的路线图，并披露了四大技术引擎。这封信的独特之处在于：它不仅是一份技术路线图，更是国产大模型头部公司首次将"AI意识"研究写入对员工负责的战略承诺。唐杰同时宣布GLM-5.2以MIT协议全量开源、支持1M上下文，作为"两手同时出牌"的安全对冲策略。

## 核心要点

1. **两年ASI时间表**：唐杰给出了一条从长程任务到ASI的清晰路线图，终点标注时间为两年，这是公开可查的国内最激进的ASI时间表
2. **四大Touch High引擎**：长程任务新记忆架构、数字员工智能体社会、合成数据工厂+沙盒代码重构、百亿级可解释性研究
3. **递归式自我改进已成型**：模型自己写代码、自己合成数据、自己训练自己——这是递归式自我改进的起点，一旦循环跑通，速度将挣脱人类工程师的物理限制
4. **三座山峰正在被攻克**：记忆（长期记忆）、持续学习（运行时更新认知）、自我评判（自主校准输出质量）这三个曾被研究者视为范式级革命的难题，正在被工程上消解
5. **从一人公司到无人公司**：智谱的目标是跳过"一人公司（OPC）"阶段，直接构建"无人公司（NPC）"——成千上万智能体7×24小时自动协作，全程无需人类干预
...
信中强调"AI训练AI已经成型"，这是整个ASI路线图中最具技术深度的断言。递归式自我改进（Recursive Self-Improvement）的核心机制是：
- AI 自己写训练代码、合成训练数据、运行训练流程
- 每轮迭代产生的下一代模型比上一代更擅长训练模型
...
唐杰对AGI的定义朴素而苛刻："AGI不是某一个天才的智慧，而是全人类智慧水平的总和，理应具备创造出'相对论'级别原创知识的能力。"这一标准远超当前主流定义（执行多种任务、自主学习新任务等），要求模型具备真正的知识创新能力。这与[Anthropic的AI意识研究](ch05/094-ai.html)中涉及的"自我认知"问题处于同一前沿——两者都触及AI是否能够具备原创性和自我意识的根本问题。

### 5. ASI对技术栈的穿透性影响

信中描绘了ASI到来对技术栈的全方位冲击：
- **应用层**：被重构为AI原生——不是现有应用加AI功能，而是用AI重新定义应用是什么
- **操作系统**：变成LLM OS——模型不再是跑在操作系统上的应用，而是成为调度一切的底层
- **冯·诺依曼体系**：被挑战——当计算逻辑从确定性指令变成概率性推理，现有计算机架构的基础假设开始动摇

这意味着ASI不仅是AI能力的飞跃，而是整个计算范式的重构。[AI原生组织转型](ch05/094-ai.html)将从优化现有流程，转变为彻底重建底层架构。

## 实践启示

1. **企业战略层面**：当技术跃进速度超常规时，"回归基础研究"可能比"产品变现"更具长期价值。在智能上界即将发生跃迁的窗口期，基础研究能力将成为最终竞争壁垒。公司是否敢于在上市后放弃短期产品变现、全面回归基础研究，是区分"真心赌ASI"和"借ASI营销"的核心信号。
...
5. **技术投资层面**：长程任务记忆、智能体社会、合成数据、可解释性——这四大引擎代表了ASI基础设施的投资方向，是比具体模型能力更具长期价值的底层能力。团队在制定AI路线图时，应将资源优先配置在这类"所有ASI路径都需要的共性基础设施"上。

## 相关实体

- [GLM-5.2](ch03/035-agent.html)
- [GLM Scaling Pain](ch05/094-ai.html)
- [Databricks GLM-5.2](ch09/047-coding-agent.html)
- [Anthropic AI意识研究](ch05/094-ai.html)
- [AREAL-2.0 Agent自演进](ch03/035-agent.html)
- [群聊Agent模式](ch03/035-agent.html)
- [钉钉招聘Agent](ch03/035-agent.html)
- [AI原生组织转型](ch05/094-ai.html)
