...
> 📊 Level ⭐⭐⭐⭐⭐ | 2.3KB | `entities/tangjie-zhipu-asi-internal-letter-2026.md`

# 唐杰内部信曝光：两年死磕ASI！ — 智谱ASI路线图

> 智谱创始人唐杰内部信披露两年ASI路线图，启动"Touch High（摸高）"计划，四大引擎同步推进：长程任务新记忆架构、数字员工智能体社会、合成数据工厂+沙盒代码重构、百亿级可解释性研究。

## ASI路线图

唐杰给出了一条清晰的超级人工智能ASI路线图：
...
1. **长程任务新记忆架构** — 解决大模型在长期任务中的记忆与推理问题
2. **数字员工智能体社会** — 成千上万「数字员工」构成的自主智能体协作体系
3. **合成数据工厂+沙盒代码重构** — AI训练AI，在沙盒内重构自身代码，实现递归式自我改进
4. **百亿级可解释性研究** — 投入百亿级资源研究AI可解释性

## 递归式自我改进

唐杰在信中强调「AI训练AI已经成型」——模型自己写代码、自己合成数据、自己训练自己。这是递归式自我改进的起点。一旦循环跑通，速度将挣脱人类工程师的物理限制。

## 相关 entities

- [智谱GLM-5.2](https://github.com/QianJinGuo/wiki/blob/main/entities/z-glm-5.2.md)
