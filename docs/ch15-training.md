...
- [Llm Post Training Full Guide](ch05/094-ai.html)
- [Yann Dubois Openai Post Training Interview](ch05/094-ai.html)
- [How Harnesses And Post Training Close The Open Weight Bug Finding Gap 20260606](ch05/094-ai.html)
...
- [Foundation Model Building Blocks](ch03/091-foundation-model-building-blocks.html) — 通用基础组件
- [750B MoE PD 分离推理 EFA vs RoCE](ch01/1243-0.html) — AWS 上的 MoE 推理对比
- [Microsoft / GitHub / AWS AI 算力承压](ch05/094-ai.html) — 超大规模算力承压事件
...
- [Olmo Hybrid and the Hybrid Architecture Wave (2026)](ch01/913-20.html) — 同一架构趋势下 GDN 3:1 混合的工业实践
- [最新开放模型快照](ch01/235-latest-open-artifacts-20-new-orgs-new-types-of-models.html) — Nemotron 3 Nano 线性架构对比
...
- [MoWorld 世界模型](ch01/1091-deepseek.html) — 同样基于昇腾 NPU 的世界模型推理优化
- **具身智能** — 具身智能核心概念
- **强化学习** — RLinf 的核心训练范式
...
- [Aws Sagemaker Ai Agent Guided Workflows Finetuning](ch05/094-ai.html)
...
- [Xai Grok Musk Training New Model Wechat](ch05/094-ai.html)
- [Video Agent Paradigm Compute Talent Flywheel Ethan He 20260606](ch03/035-agent.html)
- [奥特曼最险一战 前女Cto当庭翻脸 Openai权斗彻底打到台前 6Bf26E92E29B](ch05/094-ai.html)
...
- [SkillRM](ch04/271-skill.html) 关注 skill-level reward model，本文提供更底层的 rubric 评价框架 → 理论-实践互补
- [Harness Engineering](ch05/094-ai.html) 的评测循环可引入 rubrics 作为多维质量标准
...
- [2026 年面向 LLM 的 RL 方法总结](ch01/1274-llm.html) — 从 PPO 到 GRPO 的 RL 后训练全览
...
## 相关实体
- [Huawei Fuxi Recommendation System Ascend Npu Scaling Law](ch01/551-scaling.html)
...
这一发现对 [Skill Rm Qwen Agent Skill Reward Model](ch04/271-skill.html) 有直接启示：Skill-RM 的渐进式披露机制可以类比为 OPSD 的 per-token KL 分析——资源应该在需要时才激活，而不是一股脑全部暴露。
...
这与 [Skill Rm Qwen Agent Skill Reward Model](ch04/271-skill.html) 中"append resources 反而降分"的消融实验形成强烈呼应：把大量资源（rubric、checklist、verifier）一股脑 append 进 prompt，等价于对所有 token 无差别地增加信息密度，结果是 key signal 被稀释。OPSD 的 per-token clipping 机制和 Skill-RM 的渐进式披露机制，本质上是在解决同一个问题：如何让关键信息在需要的地方出现，而不是均匀分布在所有地方。
...
- [Skill Rm Qwen Agent Skill Reward Model](ch04/271-skill.html)
- [Skill Hub Organization Asset Winty](ch04/271-skill.html)
- [Skill Design Spec 8 Block Checklist Winty](ch04/271-skill.html)
...
- [Apo Autonomous Preference Optimization](ch01/493-apo-autonomous-preference-optimization-icml-2026.html)
...
- → [DeepSeek V4 Triton FP4 优化](ch01/1091-deepseek.html)
- → [ds4.c — DeepSeek V4 本地推理](ch01/515-ds4c-deepseek-v4-antirez.html)
...
- [Redis之父下场，给DeepSeek V4单独造了一台推理引擎](ch01/1091-deepseek.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/llm-core-technology.md)
...
- [Self-Taught RLVR](ch01/888-self-taught-rlvr.html) — 自监督 RLVR 训练方法
- [AWS GRPO RLVR](ch11/101-aws-grpo-rlvr-sagemaker-math-reasoning.html) — AWS 在 SageMaker 上实现的 RLVR
...
- [Navigating Eu Ai Act Requirements For Llm Fine Tuning On Amazon Sagemaker Ai](ch05/094-ai.html)
...
- [frontier post-training recipe review with finbarr timbers](ch05/094-ai.html)

## 相关主题
- [OpenAI 推理模型（o1/o3/o4-mini）](ch05/094-ai.html) — OpenAI 推理模型系列
...
在智能体任务 ALFWorld 上，层贡献度的跨领域一致性得到最强验证——尽管任务性质从数学推理完全切换到了多步交互式决策，且 RL 增益幅度远大于数学场景（83.7pp vs 6-10pp），中间层集中的结构仍然保持不变。这意味 agent 训练（如 [Deploying Multi Turn Rl Infrastructure For Amazon Nova On Amazon Sagemaker Hyper](ch11/305-amazon-nova.html) 中的多轮 RL）同样可以从层选择性训练中获益。
...
- [Deploying Multi Turn Rl Infrastructure For Amazon Nova On Amazon Sagemaker Hyper](ch11/305-amazon-nova.html) — 多轮 RL 基础设施
...
## Ch15.027 FAST：清华&滴滴自动驾驶强化学习并行训练框架
...
- [PiRL-PiPO：策略改进 RL 方法](ch01/913-20.html) — 并行策略优化的相关方法
...
## 相关实体
- [Opd Revisiting Failure Modes Simple Fixes Storm](ch05/094-ai.html)
- [Rlhf Dpo Grpo Alignment](https://github.com/QianJinGuo/wiki/blob/main/concepts/rlhf-dpo-grpo-alignment.md)
- [Deepseek V4 Training 58 Page Paper Deep Dive](ch05/094-ai.html)
...
这一发现与 [OpenAI 推理模型](ch05/094-ai.html) 中讨论的"推理时计算扩展"方向互补——前者关注外部搜索，后者关注内部生成策略的重塑。
...
- [OpenAI 推理模型](ch05/094-ai.html) — 推理时计算扩展与内部策略优化的互补关系
- [Diffusion LLM 推理加速](ch01/622-gemma-4.html) — 扩散模型在文本生成领域的推理优化
- **课程学习（Curriculum Learning）** — 渐进式难度递增的训练策略
- **掩码扩散 Transformer** — 扩散语言模型的架构基础 (@@参考 @@ [CoLa-DLM](ch01/341-cola-dlm.html))
- **ACL 2026 研究趋势** — 可参考 [CLaw: 长周期 Agent 评测](ch05/009-harness.html)
...
## Ch15.034 北航、北大和美团联合提出策略提升强化学习（PIRL/PIPO）
...
PIRL/PIPO 与在线策略蒸馏（如 [D-OPSD](ch01/1274-llm.html)）有相似的「回头验证」精神，但应用场景不同：D-OPSD 关注的是教师模型到学生模型的知识迁移过程中的验证，而 PIRL/PIPO 关注的是同一模型跨迭代的自我提升验证。两者可以协同使用——PIPO 作为外层闭环框架，内部可以接入蒸馏目标。
...
- [2026年LLM RL算法全景](ch01/1274-llm.html)
- [Agentic RL框架与实践](ch03/035-agent.html)
- [D-OPSD 扩散语言模型在线自蒸馏](ch01/1274-llm.html)
- [使用 TRL 进行微调](ch01/1325-trl.html)
- [大模型推理模型DoS攻击——浙大阿里HGA方法](ch01/913-20.html)
...
## Ch15.035 LLM Post-Training全景指南：从RLHF到GRPO再到AgenticRL
...
- [Llm Rl Algorithms Ppo Dpo Grpo Marl Evolution 2026](https://github.com/QianJinGuo/wiki/blob/main/concepts/llm-rl-algorithms-ppo-dpo-grpo-marl-evolution-2026.md)
- [Slim Cuhk Skill Lifecycle Agentic Rl](ch04/271-skill.html)
- [frontier post-training recipe review with finbarr timbers](ch05/094-ai.html)
...
- **Twinlight 混合推理架构**：强推理路径 + 高效路径，由模型自主选择推理路径（[Claude Managed Agents Developer Guide](ch04/710-claude-managed-agents.html) 中的混合推理思路类似但工程实现不同） 
...
## 交叉参考
- [Skill Design Patterns](ch04/271-skill.html) — Anthropic 14 模式中的 RL 后训练相关策略对比
- [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/baidu-wenxin-post-training-evolution.md)

## 相关实体
- [LLM Post-Training全景指南：从RLHF到GRPO再到AgenticRL](ch05/094-ai.html)
- [Llm Rl Algorithms Ppo Dpo Grpo Marl Evolution 2026](https://github.com/QianJinGuo/wiki/blob/main/concepts/llm-rl-algorithms-ppo-dpo-grpo-marl-evolution-2026.md)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/llm-core-technology.md)
...
- [Aws Grpo Rlvr Sagemaker Math Reasoning](ch11/101-aws-grpo-rlvr-sagemaker-math-reasoning.html)
- [Ai True Moat Not Llm But Organization](ch05/094-ai.html)
...
## Ch15.043 AlphaEvolve交出一周年炸裂成绩单！AI自我改进不再科幻
...
→ [GEPA优化框架](ch01/295-gepa-optimize-anything.html) — 包含AlphaEvolve等传统LLM进化框架的对比分析

---

## Ch15.044 Overcoming Reward Signal Challenges: Verifiable Rewards-based RL with GRPO on SageMaker AI
...
- [LLM RL 算法综述](ch01/1274-llm.html) — DPO 作为本文核心训练方法的算法背景
- [Amazon Nova Lite 微调](ch11/305-amazon-nova.html) — 另一个垂直领域微调的工程实践
- [递归强化语言模型](ch01/878-reinforcing-recursive-language-models-alphaxiv.html) — 奖励模型与偏好学习的理论框架
- [腾讯 Token 经济学](ch05/094-ai.html) — AI 模型的成本-质量权衡分析
...
## Ch15.046 EMO: Pretraining mixture of experts for emergent modularity | Ai2
...
## Ch15.047 MobileForge：无标注手机 GUI Agent 适配系统（快手、浙大）
...
## Ch15.051 LocalDPO — 面向视频扩散模型的局部细节偏好优化方法 (CVPR 2026)
...
- [Rlhf Dpo Grpo Alignment](https://github.com/QianJinGuo/wiki/blob/main/concepts/rlhf-dpo-grpo-alignment.md)
- [Diffusion Model Consistency Framework 2026 Survey](ch01/913-20.html)
- [A2Rd Agentic Autoregressive Diffusion Long Video](ch03/035-agent.html)
...
## 相关实体
- [Generalization Dynamics Of Lm Pre Training Jiaxin Wen](ch05/094-ai.html)
- [Generalization Dynamics Pre Training Jiaxin Wen](ch05/094-ai.html)
- [Generalization Dynamics Of Lm Pre Training Jiaxin Wen](ch05/094-ai.html)
- [New Ai Lock In](ch01/995-the-new-ai-lock-in.html)
- [Ai Driven Layoffs Business Sense Cio](ch05/094-ai.html)
...
## 相关实体
- [Generalization Dynamics Of Lm Pre Training Jiaxin Wen](ch05/094-ai.html)
- [Generalization Dynamics Lm Pretraining](ch05/094-ai.html)
- [Generalization Dynamics Of Lm Pre Training Jiaxin Wen](ch05/094-ai.html)
- [Yann Dubois Openai Post Training Interview](ch05/094-ai.html)
