...
- [Agent自我改进六条路](ch03/044-agent.md) — RL训练与Agent对齐
...
- **Twinlight 混合推理架构**：强推理路径 + 高效路径，由模型自主选择推理路径（[Claude Managed Agents Developer Guide](ch03/044-agent.md) 中的混合推理思路类似但工程实现不同） 
...
EMCES 是**首个将情景记忆引入可控扩散模型并用于指导强化学习样本合成**的工作。它与 [扩散模型](https://github.com/QianJinGuo/wiki/blob/main/concepts/diffusion-model-architecture.md) 架构相关，但核心创新在记忆驱动的可控生成范式。与 [MR-Agent (NUS ICML 2026)](ch03/044-agent.md) 同属 ICML 2026 记忆增强学习论文，但 EMCES 聚焦于样本合成场景，MR-Agent 聚焦于 memory-reconstructed retrieval。
