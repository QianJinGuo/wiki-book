...
对比：Cursor 从编辑器切入，Replit 从云开发环境切入，AgentCore 从云厂商基础设施切入——四条路径都在收敛到同一个目标（**企业级 Agent 控制平面**），但起点完全不同。Warp 的路径有「终端无关于 IDE」的优势，可以兼容 VS Code、Cursor、JetBrains 的用户。

### 与 AgentCore 的微妙差异

Oz 和 AWS AgentCore 在功能列表上有大量重叠，但定位有微妙不同：
...
- Agentcore Managed Harness — Managed Harness 定位
...
- Agent Harnesses Are Dead Long Live Agent Harnesses — Harness 演进观察
