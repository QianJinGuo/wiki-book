...
5. **评估 Figma Make 与现有 [低代码/框架 Agent 路线](ch03/006-agent.html) 的协同价值。** 如果组织已经在使用低代码或框架 Agent 构建内部工具，Figma Make 的设计优先编程能力可以作为这些 Agent 的上游输入源——设计师的视觉输出直接驱动代码生成，而非通过 prompt 工程触发 Agent
