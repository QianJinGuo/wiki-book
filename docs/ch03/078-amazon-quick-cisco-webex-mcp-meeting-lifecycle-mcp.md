...
- **[Agentic Incident Triage](ch03/035-agent.html)**（New Relic MCP）— SRE incident response 场景，时间敏感 / RCA brief 5 段
- **[FinOps Chat](ch03/035-agent.html)**（Billing MCP via Bedrock AgentCore）— 成本查询场景，对话式多账号 AWS 成本
...
这一 read/write scope 分层模型与 [New Relic MCP incident triage](ch03/035-agent.html) 的 "scoped OAuth (tasks:write only)" 思路一致——MCP 集成时代，**scope 最小化是 agent 安全的第一道防线**。
...
- [Amazon Quick + New Relic MCP incident triage](ch03/035-agent.html) — 同 Amazon Quick + MCP 编排模式，但 MCP server 家族和场景不同（SRE vs meeting）
- [Amazon Quick + Bedrock AgentCore FinOps](ch03/035-agent.html) — 同 Amazon Quick 对话模式，FinOps 成本查询场景
