...
> [!analysis]
> AgentScope 的双层记忆与 [上下文管理四档策略](ch04/503-agent.md) 中的第三代（Checkpoint + 记忆迁移）和第四代（结构化分维压缩）理念一致，但实现路径不同：AgentScope 采用"日流水账 + 周期性合并"的工程化思路，Claude Code 采用"分维保留"的结构化思路。两者都是对单层压缩局限性的超越。
...
- [OpenClaw Prompt/Harness](ch11/207-openclaw.md)
...
- [agentrun：阿里云多 agent 生产级协作方案（a2a 开放协议）](ch04/503-agent.md)
