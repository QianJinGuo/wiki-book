...
## Ch03.123 Fed Data Shows Cash Use Holding Steady
...
### 3. 现金的"应急储备"功能
调查发现 45% 的消费者存储平均 $364 的现金作为应急储备 。这揭示了现金的独特价值：不受网络故障、银行宕机或手机没电影响。在极端场景下，现金是唯一可靠的"最后支付手段"。O'Brien 将此描述为"公众对现金可用性的依赖"——现金不只是支付工具，更是一种金融韧性的保险机制。^[].md]

### 4. 现金的真正成本被严重低估
Crone Consulting 数据显示，现金的年度成本达 $124/人（ATM 费用、丢失/被盗风险等），政府端成本为 GDP 的 1-2% 。对于银行，现金管理涉及武装押运、安全传输、人工清点验证等高固定成本，支票处理成本是 ACH 的 8 倍 。然而，Celent 分析师 Gareth Lodge 指出，银行往往看收入而非成本，支付产品团队之间相互竞争收入，这解释了为何银行迟迟不淘汰支票和现金业务 。^[].md]

### 5. 数字支付替代的"隐藏速度"
美联储的调查将移动钱包绑定的信用卡/借记卡支付计入"卡类"，导致独立移动支付仅约 5%。但 Capital One 数据显示，61% 美国人用数字钱包网购，38% 在实体店使用 。实际数字支付渗透率远高于官方统计口径，"数字替代"的真实速度被低估了。^[].md]

### 6. 移动支付代际渗透的关键拐点
65 岁以上人群移动支付使用率从十年前的 2%（1/50）升至 10%（1/10）——虽然绝对比例仍低，但增速（5 倍）与人口基数意味着移动支付正在进入老龄群体。O'Brien 的判断是："年长群体确实在改变，但很缓慢" 。这个拐点一旦跨越，银发族数字支付将释放第二轮替代动能。^[].md]
...
→ [[raw/articles/fed-cash-use|原文存档]
