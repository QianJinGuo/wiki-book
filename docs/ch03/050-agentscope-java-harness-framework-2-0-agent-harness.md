...
> [!analysis]
> AgentScope 的双层记忆与 [上下文管理四档策略](../ch05/038-agent-harness.html) 中的第三代（Checkpoint + 记忆迁移）和第四代（结构化分维压缩）理念一致，但实现路径不同：AgentScope 采用"日流水账 + 周期性合并"的工程化思路，Claude Code 采用"分维保留"的结构化思路。两者都是对单层压缩局限性的超越。
...
- [OpenClaw Prompt/Harness](../ch11/214-openclaw.html)
...
- [agentrun：阿里云多 agent 生产级协作方案（a2a 开放协议）](../ch04/397-agentrun.html)
- [这个开源 agent 框架的核心设计，可能是目前最「聪明」的取舍](../ch04/331-pi-agent.html)
