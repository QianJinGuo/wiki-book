...
这套方案对应的不是「更好用的笔记」，而是「让每一次 Agent 对话都不是一次性消耗」。它和 [Agent 长期记忆架构](../ch04/506-perplexity-brain-self-improving-agent-memory-architecture.html)、Warp Oz 的 [cross-harness Agent Memory](../ch04/606-agent-orchestration.html) 处于同一个赛道——都是在解决「Agent 失忆」这个根本问题，只是 QMD 选择了「本地优先 + Markdown 原生」的路径。
