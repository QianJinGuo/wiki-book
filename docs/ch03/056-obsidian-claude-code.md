...
这套方案对应的不是「更好用的笔记」，而是「让每一次 Agent 对话都不是一次性消耗」。它和 [Agent 长期记忆架构](../ch04/329-perplexity-brain-self-improving-agent-memory-architecture.html)、Warp Oz 的 [cross-harness Agent Memory](../ch01/095-agent-orchestration.html) 处于同一个赛道——都是在解决「Agent 失忆」这个根本问题，只是 QMD 选择了「本地优先 + Markdown 原生」的路径。
...
- [Obsidian](../ch01/492-claude-code-memory-setup-obsidian-graphify.html) — Obsidian 实体页
- [Obsidian Llm Wiki Local Kytmanov](../ch01/422-llm.html) — 本地 LLM Wiki 实践
- [Claude Code 7 Layer Memory Architecture](057-claude-code.html) — Claude Code 的 7 层记忆架构
- [Agent Memory Architecture](../ch04/329-perplexity-brain-self-improving-agent-memory-architecture.html) — Agent 记忆架构综述
...
- [我用claude搭了个自动新闻简报，30天后比我刷了一年的信息还有用](../ch01/310-claude.html)
