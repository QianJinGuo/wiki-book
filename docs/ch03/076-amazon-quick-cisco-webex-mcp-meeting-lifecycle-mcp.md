...
- **[Agentic Incident Triage](ch03/045-agent.md)**（New Relic MCP）— SRE incident response 场景，时间敏感 / RCA brief 5 段
- **[FinOps Chat](ch03/045-agent.md)**（Billing MCP via Bedrock AgentCore）— 成本查询场景，对话式多账号 AWS 成本
- **[Time Series Market Intelligence](ch07/061-amazon-quick-mcp.md)**（KDB-X MCP）— 金融交易场景，时间序列分析
...
这一 read/write scope 分层模型与 [New Relic MCP incident triage](ch03/045-agent.md) 的 "scoped OAuth (tasks:write only)" 思路一致——MCP 集成时代，**scope 最小化是 agent 安全的第一道防线**。
...
- [Amazon Quick + New Relic MCP incident triage](ch03/045-agent.md) — 同 Amazon Quick + MCP 编排模式，但 MCP server 家族和场景不同（SRE vs meeting）
- [Amazon Quick + Bedrock AgentCore FinOps](ch03/045-agent.md) — 同 Amazon Quick 对话模式，FinOps 成本查询场景
- [Amazon Quick + KDB-X MCP 时序市场](ch07/061-amazon-quick-mcp.md) — 金融时序场景 MCP 集成
...
- [Amazon Bedrock AgentCore](ch05/009-harness.md) — 另一种 Amazon MCP 部署 runtime（vs Amazon Quick 原生 MCP 集成）
