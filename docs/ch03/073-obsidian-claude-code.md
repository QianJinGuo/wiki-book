...
## Ch03.073 Obsidian + Claude Code 完整集成指南：五种知识管理策略
...
这套方案对应的不是「更好用的笔记」，而是「让每一次 Agent 对话都不是一次性消耗」。它和 [Agent 长期记忆架构](ch04/502-agent.md)、Warp Oz 的 [cross-harness Agent Memory](ch04/502-agent.md) 处于同一个赛道——都是在解决「Agent 失忆」这个根本问题，只是 QMD 选择了「本地优先 + Markdown 原生」的路径。
...
- [Agent Memory Architecture](ch04/502-agent.md) — Agent 记忆架构综述
...
- [我用claude搭了个自动新闻简报，30天后比我刷了一年的信息还有用](ch04/502-agent.md)
