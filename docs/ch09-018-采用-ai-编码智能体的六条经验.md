...
- 模板代码 → 封装成 `npm package`（[天猫团队](../ch09-083-天猫新品营销技术团队-ai-编码实战指南)的创新做法）
...
- [规范每一行都对应一个历史失败案例](../ch05-077-harness-engineering-四根支柱与四要素架构)——凭空设计的"最佳实践"不如真实踩坑后的约束
...
- [马丁·福勒](../ch05-040-martin-fowler-的-ai-研发提醒-非确定性进了研发链路-harness-才真正开始承重)的核心观点：测试和重构不是旧时代包袱，而是 AI 时代的价值锚——AI 生成越快，确定性反馈环越值钱
...
- 用错误分类驱动 [Harness Engineering](../ch05-049-harness-engineering-系统梳理) 的迭代：Guides（事前防御）+ Sensors（事中检测）+ Garbage Collection（事后清理）
...
- [Claude Code 12 个设计模式](../ch04-090-深度拆解-claude-code-12-个可复用的-agentic-harness-设计模式) — 生产级 Agent 的架构参考
