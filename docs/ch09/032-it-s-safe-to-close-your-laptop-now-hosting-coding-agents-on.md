...
```mermaid
mindmap
  root(("Its safe to close your laptop now"))
    笔记本作为 Coding Agent 宿主的结构性缺陷
    AgentCore 的核心价值定位 不是容器 而是「远端工作站即服务」
    三种 Credential 模式 Bot On-behalf-of
    持久化 mntworkspace 设计重新定义了「Session」的语义
    将「Agent 必须在笔记本上运行」视为技术债 而不是物理约束
    平台团队应将 AgentCore Identity Gateway
    利用并行 Agent 对比实验建立团队内部的模型选择基准
    优先使用 On-behalf-of 认证模式而非 Bot 模式
```
