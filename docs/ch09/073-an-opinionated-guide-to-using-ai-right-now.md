...
Mollick 对 Chat/Agent/Wizard 三种模型的区分反映了 AI 产品设计的核心张力：**速度 vs 深度**。Agent 模型本质上是把 [Harness Engineering](../ch05-041-harness-engineering-概念框架/) 的理念内置到了模型推理过程中——它不只是回答问题，而是自主执行多步骤工作流。这与 [Claude Code](../ch01-734-两万字详解claude-code源码核心机制/) 的自主循环异曲同工，但以更轻量的方式呈现在普通用户面前。
