...
3. **GEP 协议是经验对象从"Prompt 片段"到"持久策略接口"的升格层**：没有协议，Gene 只是另一段 prompt——边界不稳、字段无法比较、不能累积。协议化后 Gene 变成可匹配、可替换、可修订、可组合的对象——这为 [OpenClaw Agent Loop 设计范式](../ch01/1224-openclaw-agent.html) 中多 Agent 之间的经验交换提供了标准化的接口规范。

4. **"经验回到模型那一刻，长什么形状"是 Gene 最本质的命题**：这个问题的答案决定了 Agent 在测试时能进化成什么样。[Harness 7 层](../ch05/050-harness-engineering.html)框架中，Gene 对应的是"控制对象层"——而这一层在之前的 harness 设计中几乎被完全忽视，所有设计努力都集中在 context 填充和 memory 系统上。
...
[Agent Skill 编写指南](../ch04/619-agent-skill.html)的渐进式披露设计（分阶段加载 Skill 内容）在这里是一个互补视角——Gene 解决的是"注入后是什么形态"，渐进式披露解决的是"何时加载"，两者结合才能构建完整的经验复用系统。
...
3. **强模型（Pro 级）上长 Skill 会压住固有能力**：在部署 Gene 时，对强模型优先使用 Gene 而非完整 Skill 包——这与 [Skill 高级实践](../ch04/619-agent-skill.html) 中"根据模型能力梯度选择注入内容"的设计原则一致
...
- [Agent Skill Writing](../ch04/619-agent-skill.html) — Agent Skill 编写指南（渐进式披露三阶段）
- [Agent Skill Writing Advanced](../ch04/619-agent-skill.html) — Skill 高级实践
- [Agent Skill Writing Evaluation](../ch04/619-agent-skill.html) — Skill 评估方法
- [Agent Skill Writing Guide](../ch04/619-agent-skill.html) — Skill 编写完整指南
- [Agent Skill Writing Practices](../ch04/619-agent-skill.html) — Skill 最佳实践
- [Anthropic Agent Skills Design Patterns 14](../ch04/315-anthropic-agent.html) — Anthropic 官方 14 个 Skill 设计模式
- [Darwin Skill 2 Huashu](../ch04/350-skill.html) — Darwin Skill 互优化
- [Hermes Agent Skill Crossover Optimization](../ch01/521-hermes-agent-skill.html) — Hermes Agent Skill 互优化（达尔文闭环）
- [Openclaw Agent Loop Design Patterns](../ch01/1224-openclaw-agent.html) — OpenClaw Agent Loop 设计范式
- [Harness Engineering 7 Layers Openclaw Hermes Claude Code P1Anu](../ch05/050-harness-engineering.html) — Harness 7 层 (OpenClaw/Hermes/Claude Code)
