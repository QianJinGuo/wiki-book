...
Coding 方面：SWE-Bench Pro 61.5（Opus 4.8 为 69.2），编程水平约为 [GLM 5.2](../ch03/036-agent.html) 同一档位。 Terminal-Bench 2.1 Meta 自测 80.0，第三方 Vals AI 测得 69.29，显示评测差异。Meta 内部编码基准 68.3，仅次于 Opus 4.8（69.0），高于 GPT-5.5（67.1）。
...
定价是其最具杀伤力的武器：输入 $1.25/1M tokens，缓存输入 $0.15/1M，输出 $4.25/1M。对比：Opus 4.8 为 $5/$25，Fable 5 为 $10/$50，[Grok 4.5](../ch05/104-ai.html) 为 $2/$6。 这意味着输入不到 Opus 四分之一、输出不到五分之一；综合成本约为 [Fable 5](../ch04/666-fable-5.html) 的十分之一。
...
**Meta Model API：** Meta 历史上第一次通过 API 开放自家最强模型。兼容 OpenAI 格式，只需改 endpoint 即可接入。早期合作伙伴包括 Replit、Box、[Cline](../ch04/573-cline-releases-open-source-agent-runtime-sdk.html)。普通用户可在 Meta AI 应用免费使用（Thinking 模式）。
...
1. **Agent 能力 > 通用能力的分化进一步确认。** Muse Spark 1.1 在 Agent 评测上碾压 Opus 4.8，在通用基准上掉出前十。模型设计进行了明确的「能力定向强化」，与 [Harness Engineering](../ch05/092-harness-engineering.html) 范式「场景定义能力」的理念高度吻合。
...
- [GPT-5.6 系列](../ch01/733-codex.html) — OpenAI 同日发布的降价系列
- [GLM 5.2](../ch03/036-agent.html) — 编程水平同档位的开源 Agent 模型
- [Fable 5](../ch04/666-fable-5.html) — Anthropic 旗舰，在 MedScribe/TaxEval 上被超越
- [Gemini 3.5](../ch01/091-gemini-3-5-frontier-intelligence-with-action.html) — Google 旗舰竞争模型
- [Sonnet 5](../ch01/1218-claude.html) — Anthropic 高性价比模型
...
- [Cline](../ch04/573-cline-releases-open-source-agent-runtime-sdk.html) — Meta Model API 早期合作伙伴
