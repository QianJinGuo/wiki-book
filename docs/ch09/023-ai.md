...
```mermaid
mindmap
  root(("采用 AI 编码智能体的六条经验"))
    经验一 先把任务切小从可独立验证的小任务下手
      问题
      解法
      实践要点
    经验二 把知识放回仓库Agent 看不见就等于不存在
      问题
      解法
      实践要点
    经验三 让验证先跑起来测试 类型约束 lint 架构边界检查
      问题
      解法
      实践要点
    经验四 权限按风险分层读写执行删除合并分级管理
      问题
      解法
      实践要点
    经验五 错误要分类分清参数错误 环境错误 权限错误 供应商错误等
      问题
      解法
      实践要点
    经验六 把经验写回 Harness每次失败后往系统里多塞一点确定性
      问题
      解法
      实践要点
    六条经验的内在逻辑
...
- 模板代码 → 封装成 `npm package`（[天猫团队](../ch05/111-ai-coding.html)的创新做法）
...
- [规范每一行都对应一个历史失败案例](../ch05/120-harness-engineering.html)——凭空设计的"最佳实践"不如真实踩坑后的约束
...
- [马丁·福勒](../ch05/009-harness.html)的核心观点：测试和重构不是旧时代包袱，而是 AI 时代的价值锚——AI 生成越快，确定性反馈环越值钱
...
### 实践要点
- [Claude Code 源码分析](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/claude-code-agentic-harness-design-patterns.md) 显示其权限控制细到很具体的粒度，远比大多数 Agent 框架更严格
...
- 用错误分类驱动 [Harness Engineering](../ch05/120-harness-engineering.html) 的迭代：Guides（事前防御）+ Sensors（事中检测）+ Garbage Collection（事后清理）
...
- [Claude Code 12 个设计模式](../ch03/070-claude-code-agent.html) — 生产级 Agent 的架构参考
- [Agent Context Management](../ch05/058-agent-harness.html) — 上下文管理的最佳实践
