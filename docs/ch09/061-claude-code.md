...
- **战略层面**：Anthropic 对 coding agent 的定位远超当前市场认知。Claude Code 不仅是代码补全工具，而是朝着全栈开发自动化方向演进。这一愿景与 [Codex 的五层架构](../ch01/533-codex.html) 中对「AI 协作的深层架构」的探索方向一致
...
Boris 100% 的工作由 Claude Code 完成这一事实，揭示了人类工程师角色的根本性转变——从「代码建筑师」向「AI 管理员」跃迁。编程不再是晦涩难懂的极客特权。这恰恰是 [Harness：一句话交付产品](../ch05/099-harness.html) 中「将意图转化为交付」理念的自然延伸——当 AI 能够处理代码层面的所有实现细节时，人类的核心价值将从「怎么写代码」转向「想要什么」。
