...
- [Agent Harness Engineering Survey 2026](../ch05/116-harness-engineering.html) — 学术 Survey 的 ETCLOVG 7 层分类（不同分类法）
...
- [Agent Production Harness Engineering](../ch05/116-harness-engineering.html) — 工程赤字 + Demo vs 生产型判别
- [Harness Engineering Systematic Framework](../ch05/116-harness-engineering.html) — Harness 工程系统化
- [Agent Skill Writing](../ch04/610-agent-skill.html) — Skill 编写实践（第三关深入）
...
"不评估，不准写代码"的 Pre-task Gating 纪律，本质上是将 [Harness Engineering Systematic Framework](../ch05/116-harness-engineering.html) 中的"工程赤字"概念落实为可执行规则。黄佳将其定位为"防呆"而非"审批"，是因为它防止的是 AI 在信息不完整时产生大量不可靠输出的情况——这种输出在长周期任务中的修复成本远高于前置评估的时间投入。

### 实践价值：ADPS 共同体将个人踩坑经验转化为组织资产

[Agent Skill Writing](../ch04/610-agent-skill.html) 解决个人级经验复用，而 ADPS 共同体解决跨组织级经验沉淀。Harness Engineering 的坑多数是共通的（上下文压缩、Token 成本、状态漂移），但行业内缺乏共享词汇表。ADPS 的价值在于建立共同的工程语言，使"某团队已解决的第 7 关问题"能快速映射为"另一团队的启动手册"。

## 实践启示

1. **优先建立五层记忆 + P0-P3 分诊，而非直接上 Agent**：在[Agent Production Harness Engineering](../ch05/116-harness-engineering.html) 的"Demo vs 生产型"判别中，上线前的第一件事就是建立上下文管理基础设施。没有 P0-P3 分诊的 Agent 等于没有内存管理的 CPU——看起来在跑，实际上在受罪。
...
3. **三层模型路由是 token 成本控制的第一优先级**：先用 Haiku 做路由分类（41% 查询根本不需要 Sonnet），再考虑压缩上下文。对应[Agent Skill Writing](../ch04/610-agent-skill.html) 的渐进式披露原则——系统应该先判断"这个问题需要多少智能"，再分配对应算力。

4. **HITL 人工审核不可省，特别是删除/部署/转账类操作**：第五关的爆炸半径分级是 Harness Engineering 的安全基线。[Agent Harness Engineering Survey 2026](../ch05/116-harness-engineering.html) 的 ETCLOVG 7 层框架中"安全层"与此呼应——高爆炸半径操作的 HITL 不是流程繁琐，而是防止不可逆损失的最后防线。
