...
mindmap
  root(("业务 Agent 增强层架构：复用通用 Agent 基座…"))
    概述
    第一性原理：通用 Agent 已经在做的，团队…
      通用 Agent 擅长的 vs 团队要补的（6…
      一句话总结取舍
    立项前判断链：跑裸基座 → 看真实能力 → 决…
      三步立项方法（强烈推荐）
      任务筛选判断表（4 个评估项）
    ...
    MVP 闭环：6 问清单 + 6 步执行协议
      MVP 至少要回答的 6 件事
      6 步执行协议（Read-Plan-Act-O…
    配置范例：business_agent_pro…
    知识库：不是资料仓库，是给 Agent 用的判…
      知识库最容易做偏的陷阱
...
[Agent Harness 12 组件 7 决策](../ch05/058-agent-harness.html) 提出 **Agent = Model + Harness**——本文是这公式在企业落地时的工程展开：**通用 Agent（Model + 基础 Harness）+ 业务增强层（业务能力）**。
