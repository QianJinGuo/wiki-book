...
这一设计理念与 [Agent Harness 上下文管理](../ch05/066-agent-harness.html) 中讨论的"上下文分层"技术完全一致——通过将信息从热（高频访问）到冷（低频访问）进行分级，优化上下文窗口利用率。
...
这种"先规格后实现"的模式避免了 AI 在方向未明确时盲目编写代码，减少了返工成本。与 [Claude Code Loop Engineering](ch09/181-claude-code-loop-engineering.html) 中的"验证-执行"循环互补。
...
与 [兔兔AGI Loop Engineering](ch09/181-claude-code-loop-engineering.html) 和 [黄佳 8 关卡](../ch03/085-claude-code.html) 互补：
...
- [Claude Code Loop Engineering](ch09/181-claude-code-loop-engineering.html) — 兔兔AGI 的 Loop Engineering 方法论
...
- [Agent Harness 上下文管理](../ch05/066-agent-harness.html) — 上下文分层与工作集管理技术
- [OpenSpec/Solo Spec 驱动开发](../ch05/057-openspec.html) — 规格先行在 AI 开发中的应用
