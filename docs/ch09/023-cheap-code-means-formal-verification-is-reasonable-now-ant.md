...
5. **关注工具间的互补性，而非互相取代**：AI Coding Agent 不取代 TLA+，TLA+ 也不取代测试套件。正确的架构是：TLA+ 发现架构级 bug，agent 用单元测试复现，然后生成精确的修复代码。每一层工具的输出都是下一层工具的输入。 [agent-formal-verification-ai-code.md:37-44](https://antfly.io/blog/agent-formal-verification)
