...
- [Agent Harness Context Management Working Set](../ch05/066-agent-harness.html) — 上下文管理的"工作集"视角
- [Agent Harness 12 Components 7 Decisions](../ch05/066-agent-harness.html) — 12 组件 + 7 决策框架
...
- [Agent Skill Writing](../ch04/619-agent-skill.html) — Skill 编写实践（第三关深入）
...
P0-P3 上下文分诊的核心价值在于将 LLM 视作 CPU、Context 视作内存、文件系统视作磁盘，从而借用 OS 虚拟内存的分页调度思想。**18K→2K Token 的实测压缩**不是 magic，而是"只调相关段"的工程必然。[Agent Harness Context Management Working Set](../ch05/066-agent-harness.html) 的"工作集"视角与此处 P0-P3 分诊本质同构，但黄佳的 OS 类比更易于向传统工程师传达。
...
Haiku 60% / Sonnet 30% / Opus 10% 的路由比例背后有数据支撑：41% 的查询只是 SQL 模板填空，根本用不上 Opus。**月账单 48 万→12 万**不是通过压缩质量实现，而是通过正确的任务-模型匹配实现。[Agent Harness 12 Components 7 Decisions](../ch05/066-agent-harness.html) 的 12 组件框架可为此路由决策提供系统性组件视角。
...
[Agent Skill Writing](../ch04/619-agent-skill.html) 解决个人级经验复用，而 ADPS 共同体解决跨组织级经验沉淀。Harness Engineering 的坑多数是共通的（上下文压缩、Token 成本、状态漂移），但行业内缺乏共享词汇表。ADPS 的价值在于建立共同的工程语言，使"某团队已解决的第 7 关问题"能快速映射为"另一团队的启动手册"。
...
3. **三层模型路由是 token 成本控制的第一优先级**：先用 Haiku 做路由分类（41% 查询根本不需要 Sonnet），再考虑压缩上下文。对应[Agent Skill Writing](../ch04/619-agent-skill.html) 的渐进式披露原则——系统应该先判断"这个问题需要多少智能"，再分配对应算力。
