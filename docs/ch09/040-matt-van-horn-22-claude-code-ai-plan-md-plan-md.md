...
```mermaid
mindmap
  root(("Matt Van Horn 的 22 个 Claude …"))
    核心反直觉
    22 个黑客技巧（六大类别）
      A. 计划循环（CE 系列）
      B. 输入与并行
    ...
    深度分析
      核心观点：80% 计划 + 20% 执行是 A…
      技术要点：'让 AI 写 plan.md 但不…
      实践价值：cmux 6 标签页并行 + 声音钩…
      深层博弈：Agent Cookie 让 CLI…
    实践启示
```
...
这个看似反直觉的做法有深刻的工程逻辑：plan.md 是 Agent 的作业而非人类的阅读材料。人类只需要扫一眼标题判断方向，然后 /ce-work 内联提问 TLDR/eli5。这意味着人类扮演的是**评审者而非消费者**——计划的生产者和计划的执行者是同一个 Agent，但人类只消费计划的摘要而非完整内容。这与 [Harness Engineering Core Patterns Claude Code](../ch05/120-harness-engineering.html) 中描述的 Harness Engineering 原则一致：人类定义验收标准，Agent 负责实现路径。
