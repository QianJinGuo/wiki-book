...
- 模板代码 → 封装成 `npm package`（[天猫团队](../ch05/076-ai-coding.html)的创新做法）
...
- [规范每一行都对应一个历史失败案例](../ch05/057-harness-engineering.html)——凭空设计的"最佳实践"不如真实踩坑后的约束
...
- [马丁·福勒](../ch05/008-harness.html)的核心观点：测试和重构不是旧时代包袱，而是 AI 时代的价值锚——AI 生成越快，确定性反馈环越值钱
...
- 用错误分类驱动 [Harness Engineering](../ch05/057-harness-engineering.html) 的迭代：Guides（事前防御）+ Sensors（事中检测）+ Garbage Collection（事后清理）
...
- [Claude Code 12 个设计模式](../ch03/048-claude-code-agent.html) — 生产级 Agent 的架构参考
- [Agent Context Management](../ch05/035-agent-harness.html) — 上下文管理的最佳实践
