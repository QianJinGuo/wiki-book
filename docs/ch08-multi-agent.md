...
## Related
- [Hermes Agent](../ch07-007-hermes-agent-满配-12-层配置完整指南-从裸装到-24h-agent-团队) — Nous Research 开源框架，核心基础
- [Hermes Agent 深度解析（阿里云）](../ch04-418-hermes-agent) — Self-Evolving/Prompt/Context/Harness 四维度技术解析
...
- **核心定位**：first truly multi-harness control plane for cloud agents——比 [AgentCore Harness](../ch04-408-amazon-bedrock-agentcore-harness-ga-两-api-调用生产级-agent-基础设施) 多了一层「harness 选择权」的抽象
...
这套架构与 [Agent Memory 架构](../ch04-029-agent-memory-architecture-past-influence-future-ruofei)和 [Agent 记忆系统框架](https://github.com/QianJinGuo/wiki/blob/main/concepts/agent-memory-systematic-framework.md)中讨论的「双向、长期、企业自有」三个理想性质完全对齐。它和 Obsidian + QMD 类型的本地方案（见 [Obsidian + Claude Code 集成指南](../ch03-072-obsidian-claude-code-完整集成指南-五种知识管理策略)策略 5）属于同一思路的两个层级：QMD 是个人工作站方案，Oz Agent Memory 是企业方案。
...
对比：Cursor 从编辑器切入，Replit 从云开发环境切入，[AgentCore](../ch04-206-agentcore-managed-harness) 从云厂商基础设施切入——四条路径都在收敛到同一个目标（**企业级 Agent 控制平面**），但起点完全不同。Warp 的路径有「终端无关于 IDE」的优势，可以兼容 VS Code、Cursor、JetBrains 的用户。

### 与 AgentCore 的微妙差异

Oz 和 AWS [AgentCore](../ch04-206-agentcore-managed-harness) 在功能列表上有大量重叠，但定位有微妙不同：
...
- [Agentcore Managed Harness](../ch04-206-agentcore-managed-harness) — Managed Harness 定位
...
- [Agent Memory Architecture](../ch04-029-agent-memory-architecture-past-influence-future-ruofei) — Agent Memory 架构综述
...
- [New And Improved Agent Governance Intelligent Workflows Connected App Exp](../ch04-459-new-and-improved-agent-governance-intelligent-workflows)
...
[Sub-Agent vs Agent Team 选型](../ch04-102-sub-agent-vs-agent-team-selection-guide) 的核心判断准则：按上下文边界设计，而不是按角色设计。Dynamic Workflows 更接近 Sub-Agent 模式——subagent 之间不直接通信，所有协调经过 workflow 脚本。但它引入了**阶段（phases）**概念，使得多步骤协作成为可能。
...
- [Sub-Agent vs Agent Team 选型](../ch04-102-sub-agent-vs-agent-team-selection-guide) — 上下文边界设计准则与五种编排原语
...
- [Meta Skill Skill Orchestration Opensquilla Jay](../ch07-022-meta-skill)（笨小葱 — 单 Skill 编排对照）
...
- **[Claude Code Dynamic Workflows 多Agent编排](../ch04-409-agent-orchestration)**（696 行 9 source）：jiagoux/thariq 视角（6 模式/3 失败/10 场景）↔ 古法程序员视角（spec 写作/skill 三层/gate 四态/edge 三种）。**互补不重复**
...
- **[Harness Engineering](../ch04-422-harness-engineering-让-coding-agent-可靠完成长程任务)**（290 行 5 source）：理论 + 5 制品 + 3 阵营；古法程序员的"skill 三层 + edge 三种 + gate 四态" = **Harness 概念的工程实现映射**
...
- [Hermes Agent 12 Layer Full Configuration Guide](../ch04-418-hermes-agent)
...
- [SkillOpt](../ch07-025-skillopt-微软训练-skill-文档的方法论) —— 训练 Skill 文档（与 Coze 3.0 的 Skill 包机制不同）
...
## 相关实体
- [OpenClaw 多智能体团队搭建实战经验](../ch04-038-openclaw-multi-agent-team-practice-v2)
- [Sub-Agent vs Agent Team 选型与编排原语](../ch04-102-sub-agent-vs-agent-team-selection-guide)
...
- [Harness Engineering](../ch04-422-harness-engineering-让-coding-agent-可靠完成长程任务) — Agent 时代的工程范式
...
## Ch08.023 γ-World: 多 Agent 世界建模（NVIDIA Research）
