...
- [Hermes Agent 深度解析（阿里云）](ch04/502-agent.md) — Self-Evolving/Prompt/Context/Harness 四维度技术解析
...
- **核心定位**：first truly multi-harness control plane for cloud agents——比 [AgentCore Harness](ch04/502-agent.md) 多了一层「harness 选择权」的抽象
...
这套架构与 [Agent Memory 架构](ch04/502-agent.md)和 [Agent 记忆系统框架](https://github.com/QianJinGuo/wiki/blob/main/concepts/agent-memory-systematic-framework.md)中讨论的「双向、长期、企业自有」三个理想性质完全对齐。它和 Obsidian + QMD 类型的本地方案（见 [Obsidian + Claude Code 集成指南](https://github.com/QianJinGuo/wiki/blob/main/entities/57U6XeKCGtVkQXnNqg9DJQ.md)策略 5）属于同一思路的两个层级：QMD 是个人工作站方案，Oz Agent Memory 是企业方案。
...
对比：Cursor 从编辑器切入，Replit 从云开发环境切入，[AgentCore](ch04/209-agentcore-managed-harness.md) 从云厂商基础设施切入——四条路径都在收敛到同一个目标（**企业级 Agent 控制平面**），但起点完全不同。Warp 的路径有「终端无关于 IDE」的优势，可以兼容 VS Code、Cursor、JetBrains 的用户。

### 与 AgentCore 的微妙差异

Oz 和 AWS [AgentCore](ch04/209-agentcore-managed-harness.md) 在功能列表上有大量重叠，但定位有微妙不同：
...
- [Agentcore Harness](ch04/502-agent.md) — AgentCore Harness 综述
- [Agentcore Managed Harness](ch04/209-agentcore-managed-harness.md) — Managed Harness 定位
- [Agent Harness Architecture](ch04/502-agent.md) — Agent Harness 架构
...
- [Agent Memory Architecture](ch04/502-agent.md) — Agent Memory 架构综述
...
- [AutoResearch 多Agent软件开发](ch04/502-agent.md) — Karpathy 风格，software engineering 场景（非science）
## 相关实体
...
- [Microsoft Agent Framework Tools Overview Provider Matrix](ch04/502-agent.md)
- [Agentscope Java Harness Framework Enterprise Distributed](ch04/502-agent.md)
- [New And Improved Agent Governance Intelligent Workflows Connected App Exp](ch04/502-agent.md)
...
这与 [Fan-Out 模式](ch04/502-agent.md) 的核心区别在于：Fan-Out 是"启动和收集分离"，Dynamic Workflows 则在此基础上增加了**多阶段（phases）和互相复查**的能力。

### 与 Agent Teams 的本质区别

[Sub-Agent vs Agent Team 选型](ch04/502-agent.md) 的核心判断准则：按上下文边界设计，而不是按角色设计。Dynamic Workflows 更接近 Sub-Agent 模式——subagent 之间不直接通信，所有协调经过 workflow 脚本。但它引入了**阶段（phases）**概念，使得多步骤协作成为可能。
...
- [四种 Sub Agent 模式](ch04/502-agent.md) — 内联工具/Fan-Out/Agent Pool/Teams 的控制粒度与状态保留对比
- [Sub-Agent vs Agent Team 选型](ch04/502-agent.md) — 上下文边界设计准则与五种编排原语
...
- [Hermes Agent 记忆系统 vs OpenClaw 记忆观](ch04/502-agent.md)
- [深度拆解 Hermes Agent 记忆系统](ch04/502-agent.md)
- [Agent Memory 架构本质](ch04/502-agent.md)
...
- [Claude Code 多智能体协作体系设计](ch03/073-claude-code.md)
- [Harness Engineering](ch05/061-harness-engineering.md)
- [Agent Loop 工程手册 8 个未解问题](ch04/502-agent.md)
- [jiagoux 架构师视角 Dynamic Workflows](ch03/073-claude-code.md)
- [Agent Harness 架构深度](ch04/502-agent.md)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/multi-agent-coordination.md)
...
- [Hermes Agent 12 Layer Full Configuration Guide](ch04/502-agent.md)
...
>- [CLI、MCP、API 选型：Agent 接入层决策指南](ch04/502-agent.md)

- [OpenClaw 多智能体团队搭建实战经验](ch04/502-agent.md)
- [龙虾装上了可以用来干啥 - OpenCLAW 多智能体团队搭建经验](ch04/038-openclaw-multi-agent-team-practice-v2.md)
...
- [Agentium — 从零实现 Agent 系统的开源框架](ch04/502-agent.md)
...
| 实体 | 来源 | 角度 |
|---|---|---|
| [扣子 3.0 多 Agent 协同实战](ch04/502-agent.md) | 网黑哥 2026-06-02 | 实战（开发小队 3 Agent / 品牌 4 风格 / 公众号 5 人 6 步） |
| [扣子 3.0 本地 Agent 项目编排](ch04/502-agent.md) | 技术角度 | coze-bridge / Claude Code / Codex CLI 接入 |
...
- [扣子 3.0 多 Agent 协同实战](ch04/502-agent.md) —— 同样是 Coze 3.0 多 Agent 协作主题，网黑哥从实战角度提供了开发小队/品牌设计/公众号流水线的完整案例，与本文的官方升级视角互为补充。
...
## 关联阅读
- [Owner Worker Verifier Architecture](ch01/901-owner-worker-verifier.md)
-
...
## 相关实体
- [OpenClaw 多智能体团队搭建实战经验](ch04/502-agent.md)
- [Sub-Agent vs Agent Team 选型与编排原语](ch04/502-agent.md)
...
## 相关实体
- [构建基于多智能体架构的深度思考交易系统](https://github.com/QianJinGuo/wiki/blob/main/entities/构建基于多智能体架构的深度思考交易系统.md)
- [Scalable Voice Agent Design With Amazon Nova Sonic Multi Agent Tools And Session](ch04/502-agent.md)
...
→ [AgentCore Payments 与 x402 协议](ch04/502-agent.md) — AWS 的对标方案
...
→ [AgentCore Payments 与 x402 协议](ch04/502-agent.md) — AWS AgentCore 支付方案
...
- → [AgentCore Payments 与 x402 协议](ch04/502-agent.md)
- → [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/ap2-agent-payments-protocol-hands-on-analysis.md)
...
## 参考
- [Minimax Agent Team Mavis](ch04/502-agent.md)
- [Owner Worker Verifier Architecture](ch01/901-owner-worker-verifier.md)
...
- → [Harness Context](ch04/502-agent.md) — 多 Agent 协作的上下文管理
...
- [Claude Managed Agents](ch04/502-agent.md) — 类似的 sandbox 架构
