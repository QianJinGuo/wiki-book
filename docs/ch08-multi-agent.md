...
- [Harness Engineering](ch05/066-harness-engineering.html) — Harness 是每个 Loop 节点的基础结构
...
## 相关实体
- [龙虾装上了可以用来干啥 - OpenCLAW 多智能体团队搭建经验](ch04/411-openclaw-multi-agent-team-practice-v2.html)
...
- [你不知道的 Agent 原理架构与工程实践](ch04/209-agent-principle-architecture-engineering-practice.html)
...
- [Factory Mission Multi Agent Architecture](ch01/607-factory-mission-multi-agent-architecture.html)
- [Openclaw Multi Agent Team Practice V2](ch04/411-openclaw-multi-agent-team-practice-v2.html)
...
## Ch08.009 Nature丨Google和FutureHouse同日登刊，把AI科学助理推到科研前线
...
- [Agent Orchestration](ch04/188-agent-orchestration.html)（AWS — 多 Agent 编排对照）
...
→ [原文存档](https://asymco.com/2026/05/11/the-great-memory-panic-of-2026/)
...
→ [ScienceDiscovery 原文存档](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw==&mid=2651050586&idx=2&sn=005d605a168bfbea6afdb2136fe639fe)
...
- [AI Agent 记忆系统架构](ch04/129-how-ai-agent-memory-works.html)
...
## Ch08.012 How Grab is Using AI Agents to Boost Team Productivity
...
- [Building Ai Agents For Business Support Using Amazon Bedrock](ch04/108-building-ai-agents-for-business-support-using-amazon-bedrock.html)
...
## Ch08.014 Sub-Agent vs Agent Team 选型与编排原语
...
## Ch08.015 Oz Multi-Harness Cloud Agent Orchestration (Warp)
...
对比：Cursor 从编辑器切入，Replit 从云开发环境切入，[AgentCore](ch04/357-agentcore-managed-harness.html) 从云厂商基础设施切入——四条路径都在收敛到同一个目标（**企业级 Agent 控制平面**），但起点完全不同。Warp 的路径有「终端无关于 IDE」的优势，可以兼容 VS Code、Cursor、JetBrains 的用户。

### 与 AgentCore 的微妙差异

Oz 和 AWS [AgentCore](ch04/357-agentcore-managed-harness.html) 在功能列表上有大量重叠，但定位有微妙不同：
...
- [Agentcore Managed Harness](ch04/357-agentcore-managed-harness.html) — Managed Harness 定位
...
- [Agent Harnesses Are Dead Long Live Agent Harnesses](ch04/196-agent-harnesses-are-dead-long-live-agent-harnesses.html) — Harness 演进观察
...
## Ch08.016 扣子 3.0 协作系统：项目化 + Agent 编排 + 工具链打通
...
- [Meta Skill](ch07/045-meta-skill.html) —— "Skill 的 Skill"（OpenSquilla 抽象层）
- [SkillOpt](ch05/058-skillopt.html) —— 训练 Skill 文档（与 Coze 3.0 的 Skill 包机制不同）
...
## Ch08.017 Orchestrating Self-Evolving Agents with CrewAI and NVIDIA NemoClaw
...
## Ch08.019 Claude Code Dynamic Workflows 多Agent编排
...
- [Agent Orchestration](ch04/188-agent-orchestration.html) — 多 Agent 编排的控制平面、状态管理、human-in-the-loop 审批
...
## Ch08.020 古法程序员复杂任务 Spec 写作：多 Agent 编排 + Skill 三层架构 + Gate 四态
...
- **[Harness Engineering](ch05/066-harness-engineering.html)**（290 行 5 source）：理论 + 5 制品 + 3 阵营；古法程序员的"skill 三层 + edge 三种 + gate 四态" = **Harness 概念的工程实现映射**
