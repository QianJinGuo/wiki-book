...
- **Hermes 生产案例系列**：继 [高德扫街榜 HermesAgent 配图系统](../ch01/295-hermes.html) 后第二个来自高德技术的 Hermes 生产案例。前者聚焦 VLM + Skill 化配图，本文聚焦多 Agent 协作 + 自进化 VOC 排查。两篇互补：前者展示混合架构（确定性 Pipeline + Agent），本文展示纯 Agent 多实例协作
...
- **路由精细化**：主调度 Agent 的模糊描述→精确领域映射，与 [Flow2Spec 结构化知识路由](../ch01/1181-spec.html) 的路由思想一脉相承
