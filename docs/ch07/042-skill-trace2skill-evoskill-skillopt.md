...
## Ch07.042 Skill自进化三路线：Trace2Skill归纳法 / EvoSkill验证闭环 / SkillOpt训练范式
...
SkillEvo 从**进化梯度的持续供给**角度补全自进化路线（腾讯系，投稿 ICLR，已落地生产持续运行一个月，arXiv 2608.13120）

- **核心论点**：Skill 自进化的瓶颈不在编辑能力或迭代次数，而在**评测反馈能否持续供给「可信的进化梯度」**。现有单轮评测驱动方法第一轮补上开场缺口后梯度立刻衰减，多轮追问才浮现的深层缺陷永远测不到。
- **可信反馈提供进化梯度**：打造可信的多轮交互范式，让深层缺陷在多轮追问中被持续暴露并转化为进化梯度
- **可控治理规定梯度方向**：治理从「标量分数一刀切」（只能拒收坏候选、定位不了也修不了）升级为可定位、可修复的治理，将 Skill 膨胀率从 16% 降到 2.8%
- **生产实测**：9 个生产级 skill、98 个 skill-ref 文档、近百万字符；Round0 解决率 30% → 4 轮进化达 81.8%；相比单轮交互范式 +15.4pp，相比自我反思进化 +23.0pp

与既有三路线的差异：既有路线（Trace2Skill/EvoSkill/SkillOpt）多聚焦单轮或一次性聚合的进化机制与验证门控，SkillEvo 则强调**多轮交互反馈的持续梯度供给**与**膨胀率治理**——这是对 Skill 自进化在生产场景落地的重要补充维度。
...
- [SkillOpt: 像训练神经网络一样训练 Skill](https://github.com/QianJinGuo/wiki-public/blob/main/entities/skillopt-microsoft-train-skill-like-neural-network.md)
- [Agent Skills 综合调查](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-skills-comprehensive-survey.md)
- [Hermes Agent 自进化](https://github.com/QianJinGuo/wiki-public/blob/main/entities/hermes-agent-self-evolving.md)
- [Agent 自改进六机制](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-self-improvement-six-mechanisms.md)
- [阿里 Agentic Cloud](https://github.com/QianJinGuo/wiki-public/blob/main/entities/alibaba-agentic-cloud.md)
