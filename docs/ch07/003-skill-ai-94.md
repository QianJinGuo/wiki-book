...
### 外部知识载体 > 模型记忆
TECH_SPEC.md 作为跨会话知识传承的外部文件，比依赖模型的内在记忆更可靠。这与 [Anthropic Long Running Agent Architecture 6H Retroforge](https://github.com/QianJinGuo/wiki-public/blob/main/entities/anthropic-long-running-agent-architecture-6h-retroforge.md) 的"文件系统 > 模型记忆"原则一致。
