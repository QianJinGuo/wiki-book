...
## Ch07.012 我把 Claude Design 做成了 Skill，人人都能成为顶级网站设计师

> 📊 Level ⭐⭐ | 19.7KB | `entities/claude-design-skill-web-design-engineer.md`

[[raw/articles/claude-design-skill-web-design-engineer.md]
...
**关键原则**：好的角色定位应该是**动态的**，根据具体任务切换专业身份。^[].md]
...
**设计哲学**：留白也是设计。大胆的留白，比十个平庸的板块更有表现力。^[].md]
...
**关键洞察**：用全新的上下文做验证，能有效打破"自己审自己"的确认偏误。^[].md]
...
### Skill 结构（约 400 行）
#### 1. 角色定义
定位：顶尖设计工程师，可以创造优雅、精致的 Web 作品。^[].md]

核心理念：目标直指"惊艳"，远超"能用"的底线。每个像素都有意义，每个交互都经过深思熟虑。^[].md]

继承动态角色切换思路：根据任务自动变身为 UX 设计师、动效设计师、数据可视化专家。^[].md]
...
- 图片缺失 → 带 ratio 信息的占位卡
- Logo 缺失 → 品牌名文字 + 简单几何形
**占位符 vs 假图**：占位符传递"这里需要真材料"；假图传递"我糊弄完了"。^[].md]
...
**核心逻辑**：给 AI 一个有品位的起点，比让它自由发挥好得多。^[].md]
...
- 克制感：每个元素都经过取舍
**关键差异**：从"把所有酷炫效果都用上"到"每个元素都经过取舍"^[].md]

### Demo 2：独立摄影师个人网站
**提示词仅一句**：帮我做一个独立摄影师的个人作品集网站首页。^[].md]

**无 Skill**：直接开干，深色霓虹调性、半透明背景、强行塞满"潜空间、创成式"等词汇，失去真实质感
**有 Skill**：先提问，确认后再实现^[].md]
...
## 五、核心结论
Skill 带来的是从"好用"到"好看"、从"完整"到"精致"、从"合格"到"有风格"的提升。^[].md]

每条规则效果微小，但叠加在一起，**量变产生质变**。^[].md]
...
→ [[raw/articles/claude-design-skill-web-design-engineer|原文存档]

**延伸阅读**：用户体验设计需要系统性的用户研究方法。[[entities/user-journey-map|User Journey Map] 提供了一种将用户行为、情感和痛点可视化的框架，帮助团队从"功能清单"转向"用户目标视角"。
