...
## Ch07.052 Agent Loop 架构三层模型：Loop + Skill + Orchestrator
...
```mermaid
mindmap
  root(("Agent Loop 架构三层模型 Loop Skill"))
    三层架构
      第一层 Loop cron 决策者
...
- [Inngest 产品概述](../ch01/815-inngest-ai-and-backend-workflows-orchestrated-at-any-scal.html)——本文是 CTO 的架构愿景，产品概述侧重功能特性
- [Loop Engineering (Addy Osmani)](../ch05/004-loop-engineering.html)——Osmani 拆解 loop 内部构成模块，本文把 loop 放进耐久执行层
- [Loop Engineering 反馈控制系统](../ch05/004-loop-engineering.html)——反馈控制的前提是耐久执行层提供可靠 checkpoint
- [Agent Loop 工程手册](../ch05/004-loop-engineering.html)——8 问框架与本文三层模型互补
- [Harness Engineering 框架](../ch05/120-harness-engineering.html)——三层中 Orchestrator 层对应 Harness 的运行时职责
- [Self-Harness](../ch05/009-harness.html)——自建 Skill agent 是 Self-Harness 的工程实现路径
- [Agentic Environment Engineering](../ch04/237-agentic.html)——Environment 决定反馈质量，Orchestrator 决定执行耐久性
