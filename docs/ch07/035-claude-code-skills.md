...
> 小 G (JavaGuide) 对 Claude Code Skills 的深度技术解析。与 [Claude Code Skills/MCP/Rules 源码分析](ch04/271-skill.html) 互补——该实体聚焦源码层实现，本实体聚焦用户层的发现机制、SKILL.md 编写、执行流程与安全限制。
...
- [Claude Code Skills/MCP/Rules 源码分析](ch04/271-skill.html) — 互补：该实体源码层（Rewrites/Loader/评分），本实体用户层（frontmatter/发现/安全）
- [Claude Code Skill Writing 指南](ch04/271-skill.html) — 互补：前者侧重编写方法，本实体侧重发现机制和执行原理
- [Hermes Skill System](ch04/271-skill.html)
