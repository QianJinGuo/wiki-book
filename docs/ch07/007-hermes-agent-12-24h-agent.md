...
```mermaid
mindmap
  root(("Hermes Agent 满配 12 层配置完整指南（从…"))
    核心定位：满配 ≠ 装满
    相关实体
    全文 20 章 + 4 部分结构
    12 层配置清单（核心框架）
    ...
    L2 输入系统：6 层协议栈（核心创新）
      3.1 SOUL.md：不要写玄学人格，要写工…
      3.2 USER.md vs MEMORY.m…
      3.3 记忆初始化：让 Hermes 通过访谈…
    4 阶段路线图（核心方法论）
    与已有实体的关系
```
...
- **多 Agent 协作的起点是"研究与执行分离"，而非功能分工**。根据 [Hermes Four Agents Setup](../ch03/035-agent.html) 的经验，多 Agent 架构的有效分工是按任务类型（研究 vs 执行）而非按功能模块（写代码 vs 写文档）。研究 Agent 需要长上下文和信息整合能力，执行 Agent 需要工具调用可靠性和步骤化执行能力，两者对模型能力的要求本质不同，混在一起会互相拖累。
