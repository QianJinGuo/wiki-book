...
```mermaid
mindmap
  root(("腾讯企业微信团队 Skill 流水线 AI代码生成率94的需求开发全流程"))
    背景 企业级移动端开发的六大痛点
    阶段 Skill 流水线
      设计稿阶段
      拆解阶段
...
企业微信团队面临 9000+ 源文件、跨多层调用的企业级移动端项目，AI辅助编码的六大真实堵点：
...
以「阶段·动作」命名约定贯穿，每个阶段有明确输入、产出和机器可校验的退出标准，形成一条严格顺序的流水线。
...
### 外部知识载体 > 模型记忆
TECH_SPEC.md 作为跨会话知识传承的外部文件，比依赖模型的内在记忆更可靠。这与 [Anthropic Long Running Agent Architecture 6H Retroforge](../ch01/989-anthropic.html) 的"文件系统 > 模型记忆"原则一致。
