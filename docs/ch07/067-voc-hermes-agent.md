...
- **自进化闭环**：Memory+Curator 机制与 [Hermes Memory 系统](../ch06/013-hermes-agent-memory.html) 一致——管理员纠正→写入 Memory→Curator 整理→下次更新
- **Hook 扩展**：无侵入定制策略与 [Hermes SOUL.md 人设系统](../ch03/098-hermes-agent.html) 互补——SOUL 定义行为，Hook 定义自定义逻辑
- **路由精细化**：主调度 Agent 的模糊描述→精确领域映射，与 [Flow2Spec 结构化知识路由](../ch01/1212-spec.html) 的路由思想一脉相承
