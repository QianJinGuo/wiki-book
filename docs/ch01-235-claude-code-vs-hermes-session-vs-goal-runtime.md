...
| 实体 | 角度 | 互补性 |
|------|------|------|
| [Harness 7 Layers OpenClaw/Hermes/Claude Code](/ch05-014-harness-到底是什么-看看-openclaw-hermes-claude-code-的演绎吧/) | 3 工具 × 7 层 harness 视角 | **互补** (本实体: 2 工具 × 6 维生命周期) |
| [Hermes 9 Module 架构](/ch01-888-hermes-agent-9-模块系统架构/) | Hermes 源码级 | 互补 (本实体: 跨工具对比) |
| [Claude Code Agentic Harness](/ch04-090-深度拆解-claude-code-12-个可复用的-agentic-harness-设计模式/) | CC 内部 harness 模式 | 互补 (本实体: CC vs Hermes 外部对比) |
| [Hermes Goal Runtime 架构](/ch04-365-hermes-agent-goal-长任务运行时架构拆解-状态持久化-judge-闭环与自主续航/) | Goal-in-loop 实现 | **直接相关** (本实体 Goal 概念的实现) |
...
[Harness Engineering](/ch05-041-harness-engineering-概念框架/) 关心的是"这一刀能不能切准"；Persistent Runtime 关心的是"这台机器能不能连续切一周，而且知道什么时候该停"。前者偏工程方法，后者偏系统架构。
