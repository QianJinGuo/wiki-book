...
- [system over model, tested: reproducing mythos's freebsd find](ch01/217-0.md)
- [from doer to director: the ai mindset shift](ch01/030-from-doer-to-director-the-ai-mindset-shift.md)
- [How my non-engineering team at Sentry learned to ship](ch01/217-0.md)
...
- **与 [Databricks Storage Ecosystem 开放共享治理](ch01/310-rag.md)**：都讲 Lakehouse + 开放表格式；Databricks 是"生态平台"阵营（Delta Lake 锁定），阿里云是"原生集成"阵营（中立兼容）——**3 大阵营中的两极**
...
- [Databricks Storage Ecosystem 开放共享治理](ch01/310-rag.md)
...
- [How Far Behind Are Open Models 2026](ch01/217-0.md)
...
OpenSharing 开源协议的本质是将 Databricks 的治理能力前移到存储层，同时避免数据复制。这是一个「协议层标准战」的战略——类比 MCP 协议在 Agent 工具调用领域的作用，OpenSharing 试图成为存储与计算分离架构下的标准连接协议。一旦成为事实标准，Databricks 就能通过 [Unity Catalog](ch01/606-llm.md) 统一治理所有实现 OpenSharing 的存储系统，无论供应商是谁。存储厂商只需实现协议接口即可加入生态，准入门槛低但 Databricks 对标准的主导权强——这是平台公司标准战略的典型打法。
...
「单一统一目录」是 SDS 生态的核心价值主张。[Unity Catalog](ch01/310-rag.md) 不再只是云端数据的治理层，而成为跨混合环境的数据治理平面。这意味着元数据管理、访问控制、血缘追踪和审计日志在混合环境下的一致性成为可能。结合 [数据 Agent 平台架构](https://github.com/QianJinGuo/wiki/blob/main/concepts/data-agent-platform-architecture.md) 的设计思路，治理平面的统一是实现「 enterprise data estate 一体化」的技术前提，对受监管行业的 CDO 来说是关键卖点。
...
- [Fine-tune LLM with Databricks Unity Catalog and Amazon SageMaker AI](ch01/606-llm.md) — 同 vendor 不同焦点（ML 训练 vs 数据治理）
- [Using Amazon EMR Serverless Storage](ch01/310-rag.md) — AWS 数据处理与存储成本优化参考
...
- [Karpathy Vibe Coding Agentic Engineering](ch04/131-karpathy-vibe-coding-agentic-engineering.md) — 同源访谈的另一标题版本
...
- [DiffusionGemma](ch01/217-0.md) — 生成模型与分数估计
...
## 相关实体
- [Lightfield Introducing Skills](ch04/256-skill.md)
...
- [aws graviton5 m9g/m9gd 实例 ga 公告](ch01/217-0.md)
...
## 相关实体
- [明星开源项目，为什么开始离开 github？](ch01/833-github.md)
- [cisa admin leaked aws govcloud keys on github](ch11/119-cisa-admin-leaked-aws-govcloud-keys-on-github.md)
- [1-click github token stealing via a vscode bug — ammaraskar](ch01/217-0.md)
