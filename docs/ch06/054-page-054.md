...
**检索增强**：将文档切分为可寻址 chunk 并建立索引，查询时按相关性取回片段注入窗口，把"模型自己找信息"变成"系统先把信息找出来"，是最通用方案；代价是检索误差（chunk 粒度、排序、重排），信息分散时可能漏召回。与上下文工程结合（见 [上下文工程：三种 Agent Memory 方案对比实验](ch06/015-memory.html)）是 Agent 系统的标准配置。
...
总趋势：注意力塌缩不会消失，但上下文管理重心正从模型内部（更长窗口、更好位置编码）向系统外部（检索、压缩、记忆、遗忘）迁移——正如 [How LLMs Actually Work: 0xkato Transformer Walkthrough](../ch01/1512-how-llms-actually-work-0xkato-transformer-walkthrough.html) 所揭示的"模型能力天花板由系统补足"的 Agent 时代范式。
...
- [How LLMs Actually Work: 0xkato Transformer Walkthrough](../ch01/1512-how-llms-actually-work-0xkato-transformer-walkthrough.html)
