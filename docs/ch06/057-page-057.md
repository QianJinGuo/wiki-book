...
**检索增强**：将文档切分为可寻址 chunk 并建立索引，查询时按相关性取回片段注入窗口，把"模型自己找信息"变成"系统先把信息找出来"，是最通用方案；代价是检索误差（chunk 粒度、排序、重排），信息分散时可能漏召回。与上下文工程结合（见 [上下文工程：三种 Agent Memory 方案对比实验](ch06/049-memory.html)）是 Agent 系统的标准配置。

**工作集压缩**：借鉴操作系统缓存思想，把上下文分为热/温/冷三层，通过 compaction 将冷数据移出窗口并保留摘要或指针，核心是 token 预算管理。[Agent Harness 上下文管理：工作集视角](../ch05/066-agent-harness.html) 正是从这一视角审视主流 Harness。
...
Harness 的本质职责之一是决定"模型在每一步看到什么"——上下文管理因此是 Agent 可靠性的地基。[Harness Engineering 系统性解读](../ch05/050-harness-engineering.html) 将上下文工程视为 Harness 的核心支柱：同一模型在不同上下文组织方式下的表现差异，往往大于不同模型之间的差异。

实践层面，[GSD 上下文管理工具：用 Plan 约束 Agent 行为边界](https://github.com/QianJinGuo/wiki/blob/main/entities/gsd-get-shit-done-context-management-tool.md) 用结构化 Plan 文件替代易腐烂的对话历史，[用好 Claude 的 18 个动作：搭一个个人 AI 工作台（Personal Harness）](../ch01/1487-claude.html) 展示通过动作编排控制上下文增长。注意力塌缩使长会话必然走向"上下文腐烂"（context rot），Harness 用 subagent 隔离、compaction 压缩、外部状态持久化，把不可控的注意力转化为可控的上下文编排。
...
总趋势：注意力塌缩不会消失，但上下文管理重心正从模型内部（更长窗口、更好位置编码）向系统外部（检索、压缩、记忆、遗忘）迁移——正如 [How LLMs Actually Work: 0xkato Transformer Walkthrough](../ch01/1519-how-llms-actually-work-0xkato-transformer-walkthrough.html) 所揭示的"模型能力天花板由系统补足"的 Agent 时代范式。
...
- [Harness Engineering 系统性解读](../ch05/050-harness-engineering.html)
- [用好 Claude 的 18 个动作：搭一个个人 AI 工作台（Personal Harness）](../ch01/1487-claude.html)
...
- [How LLMs Actually Work: 0xkato Transformer Walkthrough](../ch01/1519-how-llms-actually-work-0xkato-transformer-walkthrough.html)
