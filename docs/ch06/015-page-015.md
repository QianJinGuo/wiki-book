...
**长上下文模型**：窗口已走向百万级，但容量不解决利用率问题，评测界开始用 Lost in the Middle 类基准衡量"有效利用长度"。**KV Cache 优化**：MQA/GQA、KV 量化、cache eviction（H2O、StreamingLLM 的 attention sink）是在系统层对注意力分布做显式取舍。**记忆系统**：把上下文管理升级为"窗口外的架构"——工作/情景/长期记忆分层、检索按需取回，[Agent 上下文窗口管理对比](https://github.com/QianJinGuo/wiki-public/blob/main/entities/context-window-management.md) 与 Context Window Management Comparison 对此类方案有系统对比；主动遗忘是记忆策略的显式组成部分。
