...
这个表印证了 [VILA-Lab 1.6% vs 98.4%](ch03/074-claude-code.md) 的核心发现：**循环决策逻辑应该占代码总量的 < 10%，复杂度放在循环之外的基础设施里**。
...
- [Claude Code Dynamic Workflows Multi Agent Orchestration](ch03/074-claude-code.md)：Bun→Rust 75 万行移植使用的 Claude Code Dynamic Workflows（10 大实战场景 + Thariq 6 模式 + 3 类失败模式）
