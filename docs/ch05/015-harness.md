...
这条链路与 [Codex /goal Runtime](ch09/043-codex-goal.md) 的 `GOAL.md → PLAN.md → PROGRESS.md` 外部状态文件互补：本文关注**单个动作的提交闸门**，Codex /goal 关注**任务级别的状态文件**。两者不冲突，前者更细粒度。
...
这与 [六条经验](ch04/150-ai.md) 里的"测试和重构不是旧时代包袱，而是 AI 时代的价值锚——AI 生成越快，确定性反馈环越值钱"完全一致：Martin Fowler 与 OpenAI Harness Engineering 的共识在这里再次出现。
...
- **基础理论**：
  - [Harness Engineering Survey](ch03/045-agent.md)（CMU/Yale/Johns Hopkins，ETCLOVG 七层分类的源头）
  - [Harness Engineering — ETCLOVG Taxonomy](ch03/045-agent.md)（七层分类的独立条目）
- **同作者系列**：
...
- **互补实践**：
  - [Codex /goal Runtime](ch09/043-codex-goal.md)（任务级状态文件 GOAL.md/PLAN.md/PROGRESS.md）
  - [Anthropic 长时运行 Agent 架构](ch03/045-agent.md)（对抗式设计 + 合同谈判 + 审美量化）
  - [六条经验：让 AI 编码 Agent 变得可控](ch04/150-ai.md)（Martin Fowler 反馈环共识）
  - [Harness design for long running apps](ch05/017-harness-design-long-running-apps.md)（Anthropic 官方长任务 Harness 解读）
  - [Martin Fowler：非确定性进了研发链路](ch04/150-ai.md)（前馈/反馈原文）
- **概念图**：
...
若飞在本文和 5 张卡续篇里反复强调"记忆像预算，不像仓库"——长期记忆不是存储，而是选择性消耗认知资源的决策。这个框架与 [六条经验](ch04/150-ai.md) 里"AI 生成越快，确定性反馈环越值钱"共享同一个底层逻辑：资源（token 预算、注意力预算、判断预算）越稀缺，越需要把资源分配给高价值验证，而不是把所有中间结果都沉淀成"经验"。把临时绕路、失败猜测、一次性偏好写入长期记忆，是在预支明天的认知预算。
