...
mindmap
  root(("Harness 之后：状态边界与失败闭环（若飞续篇）"))
    与若飞 long-running-agent …
    核心框架 1：状态提交链路（候选 → 已验证 …
    核心框架 2：Trace 回写 + 前馈/反馈…
    核心框架 3：先做三件事（运行时契约 / 提交…
      3.1 运行时契约
      3.2 状态提交闸门
      3.3 失败回写
    三条分歧：模型 vs Harness 的边界
      分歧 1：模型会不会把 Harness 吃掉？
      分歧 2：同一个模型在不同 Harness 里…
...
这条链路与 [Codex /goal Runtime](../ch04/106-codex-goal-agent.html) 的 `GOAL.md → PLAN.md → PROGRESS.md` 外部状态文件互补：本文关注**单个动作的提交闸门**，Codex /goal 关注**任务级别的状态文件**。两者不冲突，前者更细粒度。
...
- **基础理论**：
  - [Harness Engineering Survey](ch05/120-harness-engineering.html)（CMU/Yale/Johns Hopkins，ETCLOVG 七层分类的源头）
  - [Harness Engineering — ETCLOVG Taxonomy](ch05/120-harness-engineering.html)（七层分类的独立条目）
- **同作者系列**：
