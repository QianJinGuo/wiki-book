...
## Ch05.076 Loss Function Development (LFD) — 损失函数开发与 /goal 循环（Elvis Sun）
...
- [Claude Code Vs Hermes Session Vs Goal Lifecycle](ch03/073-claude-code.md)（session vs goal lifecycle 对照）
- [Hermes Agent Goal Runtime Architecture State Persistence Judge Closed Loop](ch04/503-agent.md)（Hermes Agent Goal runtime 对照）
- [Claude Code 之父最新访谈编程已经结束Harness 将消失Claude Code 将只有 100 行代码Loop 才是未来](ch03/073-claude-code.md)（Claude Code 100 行 loop 同主线）
...
- [Snowflake Agentic Enterprise Summit 2026](ch04/503-agent.md)（Snowflake — 可审计治理同主线）
- [Hermes Agent Goal And Kanban](ch04/503-agent.md)（Hermes Goal + Kanban 对照）
