...
- Cloud Use 的 **四层模型 (Identity→Credential→Tool→Runtime)** 是 Agent 云工作负载的完整治理栈，补充了 [Qoder Cloud Agents 用云新范式](ch04/341-skill.html) 中 Skills 层的底层基础设施视角
- Cloud Use 的 **Credential Use 层**（Vault 注入、短期令牌、服务端代理）可视为 Agent 版本的 [阿里云 Agentic Cloud 战略](ch03/035-agent.html) 中任务级身份鉴权的具体实现
