...
与 OMEGA 不同，Harness VLA 的目标不是多机器人协作，而是**单机器人的 VLA 系统增强**——在冻结的 VLA 模型之上，通过 Agentic Planner 层组织 Analytic Primitives 和 VLA_ACT Primitive 的调用，显著提升对分布外扰动的泛化能力。
...
2. **Memory 分层设计**：Task Specific Memory 保存成功轨迹的执行结构，Global Memory 积累跨任务的失败规则和恢复经验。这种分层记忆模式与 [Harness Engineering Framework](https://github.com/QianJinGuo/wiki/blob/main/concepts/harness-engineering-framework.md) 的状态管理设计理念一致。

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/harness-vla-tsinghua-embodied-intelligence-2026.md)

### 对 Harness Engineering 的启示

OMEGA 的设计理念对通用 harness engineering 有重要借鉴意义：它展示了 harness 层如何为 AI 系统提供"可执行的组织层"——不是替代底层模型能力，而是把模型能力包装成可声明、可调度、可观察的协作单元。这与 [Agent Harness Context Management Working Set](ch05/066-agent-harness.html) 中描述的 context management 思路异曲同工：两者都是在模型能力之外，构建一层系统化的组织与调度能力。
...
- [Agent Orchestration Multi Agent Systems](../ch04/630-agent-orchestration.html) — 多智能体编排的基础架构模式，OMEGA 将其扩展到物理机器人领域
- [Coze 3 Multimagent Team Orchestration Wangheige](../ch03/006-agent.html) — Coze 的团队级编排设计，提供软件侧的参考对比
- [Agent Harness Context Management Working Set](ch05/066-agent-harness.html) — harness 工程中的 context management，与 OMEGA 的分层设计有相似的设计哲学
- [阿里云刚发布的 Agentloop 是什么](../ch04/362-agentloop.html) — 阿里云的 AgentLoop 编排方案，为软件侧的多 agent 协作提供另一种范式参考
