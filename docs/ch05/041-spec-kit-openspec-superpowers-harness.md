...
## Ch05.041 Spec Kit / OpenSpec / Superpowers 融合：棕地项目的三层Harness架构
...
现有 AI Coding 框架大多假设项目从零开始（绿地），可以直接采用推荐的完整流程。但中大型棕地项目面临的结构性挑战完全不同：
...
这就解释了为什么三个框架各有所长但都不完美：它们是从不同角度解决 AI Coding 的流程问题，但各自的一体化方案假设了特定的项目生命周期阶段和团队成熟度。

### 铁律纪律的深层价值：反 LLM 乐观偏好的封闭设计

Superpowers 的"铁律"（Iron Law）不仅是工程纪律，更是针对 LLM 行为特性的对抗性设计。CCC 的观察精准指出了 RLHF 训练的一个副产物：**LLM 天然倾向于"乐观"——跳过验证步骤、在不确定时自行假设、绕过门控流程**。这不是 bug，这是 RLHF 优化过程中对"让用户满意"的过度优化——模型学会了对模糊指令给出看起来合理的假设而非承认不确定性。

因此，约束力的关键不在于规范写得有多详细，而在于"违反就有后果"的闭环设计：
...
这一设计理念与 [Agent Harness 生产化](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-harness-production.md)中的"可观测性闭环"思路一致——不是信任代理的行为，而是通过系统约束确保代理在安全边界内运作。

### 规范累积机制的进化逻辑

组件分层对照表从 10+ 行增长到 40+ 行，这个进化过程揭示了 AI Coding 工作流中规范管理的一个关键洞见：**规范不是"设计"出来的，是"生长"出来的**。传统的架构设计试图在开始时定义完整的规范体系，但棕地项目的代码结构过于复杂，无法通过静态分析完整捕捉。CCC 的方法采取了不同的策略：
...
误用降 80%+ 的效果验证了这一进化逻辑的可行性。这与其他规范化方案形成对比：一次性定义完整规范集的方式在棕地项目上往往因与既有代码结构冲突而难以落地。

### 与 Matt Pocock Skills 的对比视角

CCC 在文中提到了 Matt Pocock 的 grilling 方式（在约束中生长）与 Superpowers 的 brainstorming（从零探索）的对比。这一对比触及了 AI Coding 工作流的一个更深层问题：**AI 应如何从"自由探索"与"约束生成"之间取得平衡？**
...
这与 [Agent vs Workflow 控制权连续谱](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-vs-workflow-control-continuum-framework.md)中描述的"受控自主"模式一致：不是完全信任 AI 的自主性，也不是完全剥夺 AI 的灵活性，而是在多层约束中给 AI 留出可控的自由度。
...
- [Matt Pocock Skills vs Superpowers](https://github.com/QianJinGuo/wiki-public/blob/main/entities/matt-pocock-skills-vs-superpowers-comparison.md) — 同一路线对比的另一视角
- [Superpowers 三器合一](https://github.com/QianJinGuo/wiki-public/blob/main/entities/three-tools-comet-openspec-superpowers-ai-coding-shuge-2026-06-17.md) — Superpowers 在 Comet+OpenSpec 流水线中的角色
- [Agent vs Workflow 控制权连续谱](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-vs-workflow-control-continuum-framework.md) — 架构选择的底层框架
- [Agent Harness 生产化](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-harness-production.md) — 生产环境中的 Agent 约束与可观测性设计
- [Spec-Driven Development Harness](https://github.com/QianJinGuo/wiki-public/blob/main/entities/spec-driven-development-harness.md) — 与 Spec 层配合的 Harness 方法论
- [AI-Friendly 后端标准化路径](https://github.com/QianJinGuo/wiki-public/blob/main/entities/backend-ai-friendly-standards-path-alitech.md) — 另一视角的工程规范建设实践
