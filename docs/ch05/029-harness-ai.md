...
## Ch05.029 Harness不是目的，知识才是护城河 —— 一个AI工程交付团队的知识沉淀实践

> 📊 Level ⭐⭐ | 16.5KB | `entities/tencent-ai-team-knowledge-mgmt-harness-moat.md`

[[raw/articles/tencent-ai-team-knowledge-mgmt-harness-moat.md]
...
**知识管理本身就是 Harness Engineering 的核心能力，而不是附属品。**^[].md]
...
└── contributions/
    ...
    └── conflicts/
```
**独立仓库的三个理由**：跨项目共享、生命周期独立（业务项目归档但知识不消失）、权限独立。^[].md]
...
这与 [[entities/harness-engineering-comprehensive-guide-conardli|Harness Engineering 全面解读] 中描述的"约束校验与失败恢复"支柱以及 [[entities/claude-code-large-codebase-enterprise-deployment|Claude Code 大型代码库最佳实践] 中的多层记忆系统形成互补——本文提供了知识层面的系统性方案，而行业实践提供了工具层面的具体实现路径。^
...
**关联阅读**
→ [[raw/articles/tencent-ai-team-knowledge-mgmt-harness-moat.md|原文存档]（本文为腾讯技术工程公众号下篇）
→ 腾讯知识沉淀体系（同一作者上篇）
→ （Harness 六层构成与行业实践）
→ （多层记忆系统与长时稳定性）
→ [[concepts/karpathy-llm-wiki-v2|Karpathy LLM Wiki V2]（渐进式知识披露模式）
→ [[entities/knowledge-mgmt-is-moat|知识管理即护城河]（知识管理作为竞争壁垒的核心论点）
