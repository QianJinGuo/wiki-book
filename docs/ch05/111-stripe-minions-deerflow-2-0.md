...
## Ch05.111 墙比模型更重要：Stripe Minions + 字节 DeerFlow 2.0 + 蚂蚁支小助 的同结论
...
- [Agent Harness Engineering: A Survey](ch05/068-harness-engineering.html) — 学术 7 层 ETCLOVG 分类法
- [AHE：Agentic Harness Engineering](ch05/068-harness-engineering.html) — 复旦/北大自动优化 Harness
- [长周期 Agent：Ralph Loop → 可接管 Harness](ch05/018-harness.html) — Ralph Loop 漂移治理 + 5 张卡框架

本文是**行业证据维度**的补充——把抽象框架落到三家不同行业公司的实际部署数据上。

→ [原文存档](https://github.com/QianJinGuo/wiki/blob/main/raw/articles/wall-not-model-harness-three-case-studies-stripe-deerflow-ant.md)
