...
AI 研发主线从代码生成更快转向非确定性进入工程系统。^[].md]

围绕这个核心，很多看上去特别热闹的新词，反而能各归各位：^[].md]
...
### Harness 是非确定性的适配层
**Harness 是把非确定性能力接入工程系统的那层适配层。**^[].md]
...
### 测试和重构不是旧时代的包袱
AI 生成代码越快，越要把变更切小。小切片还多承担了一件事：限制模型一次性发散的半径。^[].md]

Fowler 那句"不要让 LLM 做可以确定性计算的事"背后真正的工程味道：^[].md]
...
工程师从控制光标进入目标、边界、验证和系统演进的中间循环。^[].md]
