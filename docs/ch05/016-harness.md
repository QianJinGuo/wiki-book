...
## Ch05.016 Harness 之后：状态边界与失败闭环（若飞续篇）
...
这条链路与 [Codex /goal Runtime](../ch04/107-codex-goal-agent.html) 的 `GOAL.md → PLAN.md → PROGRESS.md` 外部状态文件互补：本文关注**单个动作的提交闸门**，Codex /goal 关注**任务级别的状态文件**。两者不冲突，前者更细粒度。
...
这与 [六条经验](../ch04/317-ai-coding-agent.html) 里的"测试和重构不是旧时代包袱，而是 AI 时代的价值锚——AI 生成越快，确定性反馈环越值钱"完全一致：Martin Fowler 与 OpenAI Harness Engineering 的共识在这里再次出现。
...
与 [Hermes Agent Operator上手 把一个 Agent 养成可运营系统 若飞](../ch03/091-hermes-agent.html) 的 "TERMINATION" 段直接相连——若飞把 Cowork 模板里的 `TERMINATION` 概念搬到了长任务 Agent Runtime。
...
这与 [Hermes Agent Memory System Three Layer Architecture](../ch03/091-hermes-agent.html) 的"记忆预算观"（注意力预算 + 上下文预算 + 判断预算）一致——若飞在 5 张卡续篇就提出过，本文再次强调。
...
- **基础理论**：
  - [Harness Engineering Survey](ch05/069-harness-engineering.html)（CMU/Yale/Johns Hopkins，ETCLOVG 七层分类的源头）
  - [Harness Engineering — ETCLOVG Taxonomy](ch05/069-harness-engineering.html)（七层分类的独立条目）
- **同作者系列**：
  - Ralph Loop 与可接管 Harness（2026-05-10）（2026-05-10，三类漂移 + 可接手标准）
  - Hermes 5 张卡治理框架（2026-06-01）（2026-06-01，don't automate slop）
  - [Hermes Agent Operator上手 把一个 Agent 养成可运营系统 若飞](../ch03/091-hermes-agent.html)（Cowork + TERMINATION 段）
  - [Agent Memory 架构：过去影响未来](../ch04/099-agent-memory.html)（记忆预算的更早版本）
- **互补实践**：
  - [Codex /goal Runtime](../ch04/107-codex-goal-agent.html)（任务级状态文件 GOAL.md/PLAN.md/PROGRESS.md）
  - [Anthropic 长时运行 Agent 架构](../ch01/1286-anthropic.html)（对抗式设计 + 合同谈判 + 审美量化）
  - [六条经验：让 AI 编码 Agent 变得可控](../ch04/317-ai-coding-agent.html)（Martin Fowler 反馈环共识）
  - [Harness design for long running apps](ch05/018-harness.html)（Anthropic 官方长任务 Harness 解读）
  - [Martin Fowler：非确定性进了研发链路](ch05/018-harness.html)（前馈/反馈原文）
- **概念图**：
...
若飞在本文和 5 张卡续篇里反复强调"记忆像预算，不像仓库"——长期记忆不是存储，而是选择性消耗认知资源的决策。这个框架与 [六条经验](../ch04/317-ai-coding-agent.html) 里"AI 生成越快，确定性反馈环越值钱"共享同一个底层逻辑：资源（token 预算、注意力预算、判断预算）越稀缺，越需要把资源分配给高价值验证，而不是把所有中间结果都沉淀成"经验"。把临时绕路、失败猜测、一次性偏好写入长期记忆，是在预支明天的认知预算。
...
参考若飞在 [Hermes Agent Operator上手 把一个 Agent 养成可运营系统 若飞](../ch03/091-hermes-agent.html) 里的 TERMINATION 段落，从"任务在哪里停"反推边界：目标、停止条件、输入来源、输出收件人、可读系统、可写动作、人确认动作、上一轮证据。契约不需要完美，但需要把"模糊地带"变成"明确条目"。这份契约应该落在 `AGENTS.md` 或项目入口文件里，让不同 Harness 读取但不依赖。
