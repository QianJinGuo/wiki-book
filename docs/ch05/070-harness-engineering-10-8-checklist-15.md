...
## Ch05.070 Harness Engineering 实践指南：10 步路线图 + 8 失败模式 + 设计 Checklist — 系列第 15 篇收官
...
Hermes Agent 已实现路线图中的大部分步骤：Step 1 = AGENTS.md + CLAUDE.md（目标边界）；Step 2 = tools/ 目录（原子工具）；Step 3 = context management + compaction；Step 4 = session persistence + memory tool；Step 5 = disallowed_tools + hooks；Step 6 = pre-commit hooks + wiki-lint；Step 7 = Langfuse integration；Step 8 = cron + kanban（长程执行）；Step 9 = model routing（OpenRouter）。Step 10 持续迭代通过 skill_manage 和 session_search 实现。 [Hermes Agent Skills Source Code Analysis Shuge](ch04/341-skill.html)
...
- [Production Harness 12 Components](ch05/085-harness.html)
- [Harness Engineering 14 步路线图](ch05/129-harness-engineering.html)
- [AI Agent Harness Construction — Akshay](ch05/100-ai.html)
- [Ralph Loop 长程执行](ch03/035-agent.html)
- [Hermes Agent Skills 源码分析](ch04/341-skill.html)
