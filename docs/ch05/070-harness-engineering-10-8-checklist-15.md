...
Hermes Agent 已实现路线图中的大部分步骤：Step 1 = AGENTS.md + CLAUDE.md（目标边界）；Step 2 = tools/ 目录（原子工具）；Step 3 = context management + compaction；Step 4 = session persistence + memory tool；Step 5 = disallowed_tools + hooks；Step 6 = pre-commit hooks + wiki-lint；Step 7 = Langfuse integration；Step 8 = cron + kanban（长程执行）；Step 9 = model routing（OpenRouter）。Step 10 持续迭代通过 skill_manage 和 session_search 实现。 [Hermes Agent Skills Source Code Analysis Shuge](../ch04/510-hermes-agent-skills.html)
...
- [AI Agent Harness Construction — Akshay](../ch01/1168-ai-agent-harness.html)
- [Ralph Loop 长程执行](ch05/085-harness.html)
- [Hermes Agent Skills 源码分析](../ch04/510-hermes-agent-skills.html)
