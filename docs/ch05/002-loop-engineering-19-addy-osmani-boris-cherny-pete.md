...
- **Evaluator 部件**与 [Harness 架构](ch05/009-harness.md)的"验证层"同源——Loop 把 Harness 验证层拉成独立部件
...
- **18 字段设计表**与 [agent-harness 12 components 7 decisions](ch05/009-harness.md)的"Harness 完整部件清单"互补——Harness 是"环境内规则"，Loop 是"环境外循环节奏"
...
- **熔断器类比**（本来源独家）与 [Harness Engineering」](ch05/009-harness.md) 的"多层重试"直接衔接：Harness 的多层重试从"工程落地"侧实现了熔断，本来源从"架构设计"侧给出了理论映射
