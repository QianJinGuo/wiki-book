...
6. **用"三个月后是参考还是噪音"过滤记忆**：写进长期记忆的每条决策，都要问它三个月后对 Agent 的判断是帮助还是干扰——记忆的价值在正确时刻被加载，不在总量。参见 [记忆主矛盾](../ch04/179-agent-memory.html) 与 [Working Set vs Long-Term Memory](https://github.com/QianJinGuo/wiki/blob/main/concepts/working-set-vs-long-term-memory.md)。
