...
1. **核心密码学代码的形式化验证 pipeline**（不是 TLA+ 系统建模）— 与现有 [antfly 博客](../ch03/019-agent.html)（AI agent 在 TLA+/Coq 证明上 hill-climb）属于**不同技术层**。前者是 *软件工程层*（让 AI 写出可验证代码），后者是 *密码学实现层*（用 ACL2/Sawmill 等证明助手验证汇编级恒定时间 + 正确性）。
...
- [Agent Formal Verification Ai Code](../ch03/019-agent.html) — 互补层：软件工程层（AI 写证明）vs 密码学实现层（人工 ACL2 证明）
- [Alphaevolve Deepmind Discovery Agent](../ch03/019-agent.html) — 同样在"AI × 形式化"边界，但走 discovery 路线而非验证路线
