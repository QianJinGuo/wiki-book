...
## Ch12.120 Semgrep Intercom Php Supply Chain

> 📊 Level ⭐⭐ | 3.6KB | `entities/semgrep-intercom-php-supply-chain.md`

# Malicious Intercom PHP Package Mini-Shai-Hulud
Semgrep security research: malicious Intercom PHP package spreading Mini-Shai-Hulud attack via Packagist/Composer. Attack chain, IOCs, code-level analysis.^[].md]

**Source**: [[raw/articles/semgrep-intercom-php-security.md|raw article] | **Review**: value=7 confidence=8^[].md]
...
PHP 生态的供应链攻击揭示了一个深层问题：Composer 的插件机制权限过大，缺乏沙箱隔离；包名验证完全依赖开发者肉眼识别而非加密签名验证。^[].md]
