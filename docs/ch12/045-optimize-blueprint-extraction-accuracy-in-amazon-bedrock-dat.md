...
- [amazon-bedrock-agentcore-runtime-deep-dive-and-scenario-analysis](ch04/502-agent.md) — Bedrock AgentCore 平台深度（非 BDA，但同 Bedrock 体系）
...
5. **与 [Process Financial Documents Using Amazon Bedrock Data Automa](ch11/240-bedrock.md) 联合使用** — 后者侧重金融文档的端到端提取场景，前者侧重 blueprint instruction 的自动化优化，两者构成"schema 设计 → instruction 优化 → 生产部署"的完整 IDP pipeline。
