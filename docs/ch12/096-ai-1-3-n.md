...
该体系与 [AI Gateway vs MCP Gateway](../ch11/091-ai-gateways-vs-mcp-gateways-what-security-teams-need-to-kno.html) 互补——飞连侧重于企业内部的办公安全治理（人+Agent+数据），而 AI Gateway 侧重外部 API 调用管控。同样涉及 [AI Tool Poisoning](../ch04/455-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s.html) 中提出的 Skill 投毒和 MCP 安全风险。
