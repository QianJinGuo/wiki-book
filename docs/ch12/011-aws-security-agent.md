...
```mermaid
mindmap
  root(("飞来汇借助 AWS Security Agent 构建跨…"))
    飞来汇借助 AWS Security Agen…
    **一、引言**
    **二、跨境支付行业的安全挑战**
      2.1 强监管与多地合规要求
      2.2 业务逻辑漏洞远比通用漏洞更致命
      2.3 渗透测试节奏跟不上发版节奏
      2.4 应用代码安全左移落地难
    **三、AWS Security Agent …
      3.1 核心能力一览
      3.2 多智能体协作架构
      3.3 与开发流程的原生集成
    **四、飞来汇的 Security Agent…
      4.1 整体架构
      4.2 增量代码扫描：让安全反馈与 Code …
      4.3 按需渗透测试：把”周级别”压缩到”小时…
      4.4 全量代码扫描：建立长期安全基线
    **五、飞来汇采用 Security Agen…
