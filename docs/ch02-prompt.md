...
> 与 [Skill 设计模式](../ch07-002-skill-设计模式) 在"skill 是知识注入而非工具生成"维度同源，但本文聚焦**触发失败诊断**而非模式选择。
...
> 与 [Agent Skill 进阶模式与治理](../ch01-319-agent-skill-进阶模式与治理) 在"description 是触发信号源"维度同源——但本文给的是**反模式 → 正模式**的具体对照，可作 description 写作 checklist。
...
> 与 [Agent 可靠性工程与持续改进](../ch04-054-agent-可靠性的工程解法-从-skillify-看持续改进机制) 的"产品级可观测"维度同源——本文聚焦"**可被反向审计的回执结构**"这一具体落地形态。
...
> 这是与 [Skill 工程化设计：把 Agent 当算法用](../ch07-056-当我把ai变成一个-算法-skill工程化设计的心路历程) 在"agent 链路是激活对象"维度上的**范式共振**——本文给出的是诊断视角，后者给出的是生产视角。
...
- **同 skill description / 触发信号**：
  - [Agent Skill 进阶模式与治理](../ch01-319-agent-skill-进阶模式与治理)（description + name + 触发机制）
  - [Skill 设计模式](../ch07-002-skill-设计模式)（5 种核心模式 + 1 特殊模式）
  - [Anthropic 官方技能最佳实践 14 模式](../ch01-677-anthropic)（5 类设计模式）
  ...
  - [Anthropic+Google Agent Skills 设计模式](../ch01-677-anthropic)
- **同 Codex / OpenAI**：
  - [Codex Goal 代理运行时](../ch04-094-codex-goal-长任务agent的目标运行时)
  - [Codex Goal 六小时运行](../ch09-052-codex-goal-six-hour-run)
  ...
  - [OpenAI Codex JasonLiu maxxing 攻略](../ch09-111-openai大神教你如何榨干codex)
  - [GPT-5.4 Codex Interconnects](../ch01-508-gpt-5-4-codex-评测-interconnects-的-agent-使用体验)
- **同 AGENTS.md / 角色边界**：
  - [Agent 可靠性工程与持续改进](../ch04-054-agent-可靠性的工程解法-从-skillify-看持续改进机制)
  - [Skill 工程化设计：把 Agent 当算法用](../ch07-056-当我把ai变成一个-算法-skill工程化设计的心路历程)
- **同 prompt 工程 / 范式**：
  - [Claude Managed Agents](../ch01-445-claude-managed-agents-官方-harness-平台指南)（prompt 工程的边界探索）
  - [AI 蜜罐：对抗 AI 智能体](../ch01-060-ai-powered-honeypots-turning-the-tables-on-malicious-ai-age)
...
"State is a projection of events" 这个命题在 Codex 中不是隐喻，而是工程实现。状态变化时追加新消息（而非原地修改）带来的两大收益：前缀不变 → 缓存命中；因果链保留 → 可推断意图。这与 [Claude Code Vs Codex Context Architecture 02](../ch01-286-claude-code-vs-codex-上下文架构-五层压缩管道-vs-容器文件系统) 中描述的 Claude Code 激进 Context Editing 路线形成鲜明对比。两种流派并非优劣之分，而是不同工程约束下的取舍：Append-only 更保守、工程友好，适合需要稳定缓存的生产系统；Context Editing 更激进、Token 精简，但前缀破坏带来工程复杂度。

### 实践价值：压缩能力的不对称是结构性的，应用层应聚焦可控部分

应用层压缩（Semantic Compression）与基础设施层压缩之间存在结构性信息不对称：厂商可访问注意力分布、海量真实对话数据、专用 Fine-tuned 压缩模型，应用层无法复制这一优势。理解这一点后，[Anthropic Prompt Caching Claude Code](../ch01-677-anthropic) 等应用层实践的价值在于：放弃追求"更好的压缩"，转而聚焦**缓存友好的 Prompt Layout + 显式的 Compress/Select 策略**，这是应用层真正能控制的部分。context-kit 开源工具正是这一哲学的教学实现。
...
尽管"/responses/compact"端点使用"latent understanding"措辞，但向量表征与模型架构深度耦合使得纯 Latent Compression 的工程可行性存疑。更合理的推断是：厂商做的是 Semantic Compression，但拥有关键信息优势（注意力分布、真实数据、专用压缩模型）。这与 [Harness Engineering](../ch04-422-harness-engineering-让-coding-agent-可靠完成长程任务) 实践中"在不确定黑盒内部机制时，聚焦可控部分"的原则一致。
...
5. **警惕压缩能力的结构性不对称**：应用层压缩永远受限于信息劣势。不要过度投入"更智能的摘要算法"，而是将资源投入到缓存友好的架构设计和规范的 [Harness Engineering](../ch04-422-harness-engineering-让-coding-agent-可靠完成长程任务) 实践中——这些是真正可控且可累积的工程优势。
...
HTML Artifacts 是 [Claude Code](../ch01-571-claude-code-架构解析) 中一种强大的输出形式，它使 Agent 能够生成富交互式的 HTML 文档，显著超越传统 Markdown 的表达能力。
...
-  — 本文讨论的主要工具
- [Claude Code 最佳实践](../ch01-248-claude-code-大型代码库最佳实践) — 相关实践指南
...
- [Agent Context Management Architecture Patterns](../ch04-460-智能体编排层中的上下文管理架构) — 上下文管理的架构模式总览（截断 / 总结 / 滑动窗口 / RAG）。Headroom 走**更激进的字节级 patch + cache-aware 路径**，是这些模式之外的"代理层"独立分支。
- [Openclacky Harness Prompt Cache](../ch05-039-openclacky-prompt-cache-命中率-90-的-harness-工程实践) — OpenClacky 提示词缓存策略。Headroom 与之同源思想（**关注 prompt cache key 稳定性**）但实现不同：OpenClacky 是 SDK 层策略，Headroom 是 proxy 层工具。
...
- [Claude Code Vs Codex Context Architecture 02](../ch01-286-claude-code-vs-codex-上下文架构-五层压缩管道-vs-容器文件系统) — Claude Code vs Codex 上下文架构对比。Headroom **同时支持两者**（作为 proxy 层），可以视为这两者的**第三方统一优化层**。
- [Agent Reliability Context Drift Tool Hallucination](../ch04-158-agent-reliability-context-drift-tool-calling-hallucinatio) — 上下文漂移导致工具幻觉。Headroom 的 cache stabilization 间接缓解上下文漂移（cache 命中率高 → 历史更稳定）。
- [Agentic Ai Infrastructure Practice Series Nine Context Engineering](../ch04-403-agentic-ai-infrastructure-practice-series-9-context-enginee) — Agent 上下文工程实践系列。Headroom 是**具体实现**之一，可作为该系列的"proxy 层"案例补充。
- [Cl Bench Life Tencent Context Learning](../ch01-902-腾讯混元-cl-bench-life-让大模型读懂你的日常生活) — 腾讯 CL-Bench 长上下文学习基准。Headroom 是**生产工具**（不是基准），与之形成"评估 vs 优化"对照。
- [Alibaba Eventhouse Enterprise Agent Context](../ch11-040-阿里云-eventhouse-企业级-agent-上下文构建五维框架) — 阿里云 EventHouse 企业级 Agent 上下文。Headroom 走**proxy 通用方案**，EventHouse 走**企业级云服务方案**。

## 相关主题

- 上下文工程 — [Agentic Ai Infrastructure Practice Series Nine Context Engineering](../ch04-403-agentic-ai-infrastructure-practice-series-9-context-enginee)
- 提示词缓存策略 — [Openclacky Harness Prompt Cache](../ch05-039-openclacky-prompt-cache-命中率-90-的-harness-工程实践)
- Agent 工具链 — [Agent Harness Architecture Design Production Guide](../ch04-021-agent-harness-架构设计与实现-生产级-agent-系统落地指南)
- VibeCoder / Vibe编码 公众号其他文章 — [Cli Mcp Skill Architecture Decision Vibecoder](../ch07-030-cli-mcp-和-cli-skill-应该如何选) / [Impeccable Vibe Design Philosophy Anomaly](../ch05-001-impeccable) / [Impeccable Vibe Design Philosophy Anomaly](../ch05-001-impeccable)
- MCP server 模式 — [Cli Mcp Skill Architecture Decision Vibecoder](../ch07-030-cli-mcp-和-cli-skill-应该如何选)
...
→ [Agent Context Management Architecture Patterns](../ch04-460-智能体编排层中的上下文管理架构) — Agent 上下文管理工程模式
...
- [Hermes Self-Improving 闭环详解（winty）](../ch04-193-hermes-self-improving-闭环详解-winty)
...
## Related
- [Hermes Agent](../ch07-007-hermes-agent-满配-12-层配置完整指南-从裸装到-24h-agent-团队) — Nous Research 开源框架（4万+ Stars），核心亮点自进化
...
- [Claude Code 架构](../ch01-571-claude-code-架构解析) — 同为 Agent 深度解析系列参照
...
**Logo 设计场景**：如果模型输出的是栅格图像，曲线错了，用户必须 mask、inpaint、重新生成或手动重画。如果输出是 SVG，用户可以直接编辑路径、原语、渐变、描边或文本元素。这已经是设计师在 [Quiver 等工具](../ch04-442-ai-内容创作-开始进入-画布-agent-时代) 上设计 Logo 的方式。
...
- [ICLR 2026: 英伟达/普渡用 Agent 闭环实现文生 3D](../ch01-417-scenethesis-iclr-2026-英伟达-普渡大学用-agent-闭环实现文生-3d) — 同一时期 3D 生成的 Agent 闭环探索
- [DeepSeek 视觉原语](../ch09-118-deepseek-visual-primitives-视觉原语作为思考媒介) — 视觉表征的另一思路：堆指代精度而非图像分辨率
- [ICLR Agent 3D 生成](../ch04-263-iclr-2026-英伟达-普渡大学-用agent闭环实现文生3d) — Agent 在 3D 生成中的另一研究路径
...
- [Ethan He: Cosmos / Grok Imagine / Latent Space 视频 Agent](../ch01-781-ethan-he-cosmos-grok-imagine-latent-space-video-agent-202606) — 视频与多模态生成的前沿
...
## 概述
FanOne 从源码角度解析 Claude Code 的 Prompt 提示词模块六大分层体系：Core System（静态/动态分离 + 优先级策略树）、Tool（自然语言行为协议）、Skill（渐进式加载 + Reading Guide）、Agent（强角色边界 SOP）、Context Management、Memory（四类分级存储）。与 [Claude Code 架构解析](../ch01-571-claude-code-架构解析) 构成完整源码解读系列。
...
- [Claude Code Skills 实践与 Superpowers](../ch01-408-claude-code-skills-superpowers-practice) — Skill 体系工程化实践
...
- [Agent Memory 架构](../ch04-029-agent-memory-architecture-past-influence-future-ruofei) — 与 CC Memory 的对比
- [Obsidian + Claude Code 集成指南](../ch09-097-obsidian-claude-code-integration-guide) — 五种集成策略
...
- [Unlocking asynchronicity in continuous batching](../ch03-083-unlocking-asynchronicity-in-continuous-batching)
- [Modal — Truly serverless GPUs](../ch11-100-how-to-achieve-truly-serverless-gpus)
- [Google Genkit Middleware](../ch04-496-announcing-genkit-middleware-intercept-extend-and-harden-you)
...
- [Claude Fable 5 安全寓言](../ch01-649-claude-fable-5-and-new-ai-safety-fables) — Nathan Lambert 的安全政策分析
- [Fable 5 on AWS Bedrock](../ch01-677-anthropic) — 企业部署视角
- [Mollick Fable 5 patron vs wizard](../ch01-242-claude-fable-5-ethan-mollick-hands-on-qualitative-evaluati) — 用户体验视角
- [阿里云云原生 安全护栏三域演进](../ch01-252-安全护栏的三域演进-阿里云云原生从-claude-fable-5-提炼的护栏设计原则) — 3 域对比（云资源约束 / AI 输出约束 / 模型路由约束）
- [Mythos bug hunting 营销](../ch01-677-anthropic)
- [System over Model tested](../ch01-754-system-over-model-tested-reproducing-mythos-s-freebsd-find) — 评测视角
- [Loop Engineering](../ch05-002-loop-engineering-不再写提示词-而是设计替你写提示词的循环-先写刹车再写循环-13-来源深度合并-ad) — 状态文件作为对话外可审计载体
- [Anthropic 12 MCP 生产模式](../ch01-677-anthropic)
- [Anthropic 14 Skill 模式](../ch01-677-anthropic)
- [Skill Issues Claude Code](../ch01-615-skill-issues-compromising-claude-code-with-malicious-skills) — 同样的 supply chain 攻击视角
...
- [AIMap Security Testing](../ch04-356-introducing-aimap-security-testing-for-ai-agent-bishop-f) — 同样聚焦 AI Agent 安全测试
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
- **扩展版本**：[Claude Code 12 Rules Karpathy Extension](../ch09-039-claude-md-12-条规则-karpathy-扩展模板)（新增 8 条覆盖 agent 编排场景）
...
- **[Claude Code Agent Engineering](../ch04-015-claude-code-的-agent-工程)**（SooKool）侧重 StreamingToolExecutor/主循环/压缩/小模型
...
- [fanling company as agent ai org reflection v2](../ch04-180-fanling-company-as-agent-ai-org-reflection-v2)
...
- [Skill 设计模式](../ch07-002-skill-设计模式) — 工作流模式与 Skill 设计的关系
- [Agent Skill 编写指南](https://github.com/QianJinGuo/wiki/blob/main/entities/agent-skill-writing.md) — Skill 格式规范参考
- [Anthropic MCP 重新定义](../ch01-677-anthropic) — Anthropic 官方对 Skill + MCP 互补关系的定义
...
## 子页面
- [高质量编写规范](../ch04-242-agent-skill-高质量编写规范) — 6 条核心编写原则
- [评估与迭代](https://github.com/QianJinGuo/wiki/blob/main/entities/agent-skill-writing-evaluation.md) — 触发测试、功能测试、基线对比方法论
- [进阶模式与治理](../ch01-319-agent-skill-进阶模式与治理) — Anthropic 5 种进阶模式、安装部署、YAML 完整规范、实战调试案例

## Related
- [Hermes Agent](../ch07-007-hermes-agent-满配-12-层配置完整指南-从裸装到-24h-agent-团队) — Skill 机制是 Hermes 的核心特性之一
...
- [Skill 设计模式](../ch07-002-skill-设计模式) — 5种设计模式系统指南
...
- [Harness + Post-Training bug-finding gap](../ch01-232-how-harnesses-and-post-training-close-the-open-weight-bug-fi) — Harness 与 Post-training 的协同效应
- [Qoder Skills 指南](../ch07-029-qoder-skills-完全指南) — 讨论 skill 约束与 prompt 边界
...
- [Kimi K2.5 架构创新](../ch01-882-kimi-k2-5-架构创新-1t-moe-一层路由-三能力跃迁) — 同样强调"模型层处理行为，harness 层处理任务"
...
- [Claude Code 源码中的 Agent Harness 构建之道](../ch01-167-深入理解-claude-code-源码中的-agent-harness-构建之道)
- [Claude Code 源码核心机制](../ch01-734-两万字详解claude-code源码核心机制)
- [Harness Engineering 概念解析](../ch05-005-一文带你弄懂-ai-圈爆火的新概念-harness-engineering)
...
- [Agent Self-Improvement](../ch04-047-agent-自我改进的六条路)
- [Agentic AI System Architecture](../ch04-197-mcp-skill-agent-llm-harness-一张图讲清-agentic-ai-系统如何真)
...
- [Claude Code开发负责人 为何放弃Rag而选择Agentic Search](../ch04-386-claude-code-开发负责人-为何放弃-rag-而选择-agentic-search)
- [Integrate Atlassian Confluence Cloud With Amazon Quick](../ch11-039-integrate-atlassian-confluence-cloud-with-amazon-quick)
...
- [滴滴 ibg 智能客服质检系统：3 管线（意图 86% / 合规 90%+ / voc）+ 企业 llm 落地方法论](../ch01-166-滴滴-ibg-智能客服质检系统-3-管线-意图-86-合规-90-voc-企业-llm-落地方法论)
...
- [Agent Harness Engineering Survey](../ch04-069-agent-harness-engineering-a-survey) — 推理系统的 harness 范式
- [高德 Marketing AutoResearch](../ch04-078-高德-marketing-autoresearch-营销增长-ai-native-经营托管框架) — 同样涉及算法模型迭代自动化
- [OpenClaw Agent Loop](../ch04-237-openclaw-与-claude-code-的-agent-loop-设计范式) — Fast-Slow Thinking 与 Loop 设计范式对照
