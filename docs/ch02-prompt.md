...
- [Cursor Harness Model Production Floor](ch01/454-cursor-harness-model-production-floor.html)
- [Fudan Peking Ahe Agentic Harness Engineering](ch05/129-harness-engineering.html)
- [From Agent Protocol To Harness Skill](ch04/499-from-agent-protocol-to-harness-skill.html)
...
- [is grep all you need? — 检索 × harness × 交付方式耦合三元组（pwc 论文 arxi](ch05/085-harness.html)
- [model-harness-fit-agent-harness](ch03/035-agent.html)
...
> 与 [Skill 设计模式](ch04/341-skill.html) 在"skill 是知识注入而非工具生成"维度同源，但本文聚焦**触发失败诊断**而非模式选择。
...
> 与 [Agent Skill 进阶模式与治理](ch04/341-skill.html) 在"description 是触发信号源"维度同源——但本文给的是**反模式 → 正模式**的具体对照，可作 description 写作 checklist。
...
> 与 [Agent 可靠性工程与持续改进](ch04/341-skill.html) 的"产品级可观测"维度同源——本文聚焦"**可被反向审计的回执结构**"这一具体落地形态。
...
> 这是与 [Skill 工程化设计：把 Agent 当算法用](ch04/341-skill.html) 在"agent 链路是激活对象"维度上的**范式共振**——本文给出的是诊断视角，后者给出的是生产视角。
...
- **同 skill description / 触发信号**：
  - [Agent Skill 进阶模式与治理](ch04/341-skill.html)（description + name + 触发机制）
  - [Skill 设计模式](ch04/341-skill.html)（5 种核心模式 + 1 特殊模式）
  - [Anthropic 官方技能最佳实践 14 模式](ch04/341-skill.html)（5 类设计模式）
  - [AI Skill Evolution Framework](ch04/341-skill.html)（skill 评估与度量）
  - [Anthropic+Google Agent Skills 设计模式](ch04/341-skill.html)
- **同 Codex / OpenAI**：
  - [Codex Goal 代理运行时](ch01/736-codex.html)
  - [Codex Goal 六小时运行](ch09/065-codex-goal-six-hour-run.html)
  - [Codex 上下文工程](ch01/736-codex.html)
  - [OpenAI Codex JasonLiu maxxing 攻略](ch05/100-ai.html)
  - [GPT-5.4 Codex Interconnects](ch01/736-codex.html)
- **同 AGENTS.md / 角色边界**：
  - [Agent 可靠性工程与持续改进](ch04/341-skill.html)
  - [Skill 工程化设计：把 Agent 当算法用](ch04/341-skill.html)
- **同 prompt 工程 / 范式**：
  - [Claude Managed Agents](ch04/775-claude-managed-agents.html)（prompt 工程的边界探索）
  - [AI 蜜罐：对抗 AI 智能体](ch05/100-ai.html)
...
HTML Artifacts 是 [Claude Code](ch03/084-claude-code.html) 中一种强大的输出形式，它使 Agent 能够生成富交互式的 HTML 文档，显著超越传统 Markdown 的表达能力。
...
-  — 本文讨论的主要工具
- [Claude Code 最佳实践](ch03/084-claude-code.html) — 相关实践指南
...
- [Agent Harness Context Management Working Set](ch03/035-agent.html) — Agent 上下文管理的 working set 模式（**主线程 vs 后台 subagent**）。Headroom 是 **proxy/中间层方案**，在工具输出层做字节级压缩。两者**问题域重叠但切入点完全不同**。
- [Agent Context Management Architecture Patterns](ch03/035-agent.html) — 上下文管理的架构模式总览（截断 / 总结 / 滑动窗口 / RAG）。Headroom 走**更激进的字节级 patch + cache-aware 路径**，是这些模式之外的"代理层"独立分支。
- [Openclacky Harness Prompt Cache](ch05/085-harness.html) — OpenClacky 提示词缓存策略。Headroom 与之同源思想（**关注 prompt cache key 稳定性**）但实现不同：OpenClacky 是 SDK 层策略，Headroom 是 proxy 层工具。
- [Ai Context Layer Kgc 2026](ch05/100-ai.html) — AI 上下文层（知识图谱增强）。Headroom 走**字节级压缩**而非**结构化知识层**，定位互补。
- [Claude Code Prompt Context Harness](ch03/084-claude-code.html) / [Claude Code Session Management 1M Context](ch03/084-claude-code.html) — Claude Code 的上下文管理实现。Headroom 是**通用 provider 方案**（覆盖 Claude Code / Codex / Cursor / Aider / Copilot），Claude Code 上下文管理是**单 provider 内部**方案。
- [Claude Code Vs Codex Context Architecture 02](ch03/084-claude-code.html) — Claude Code vs Codex 上下文架构对比。Headroom **同时支持两者**（作为 proxy 层），可以视为这两者的**第三方统一优化层**。
- [Agent Reliability Context Drift Tool Hallucination](ch03/035-agent.html) — 上下文漂移导致工具幻觉。Headroom 的 cache stabilization 间接缓解上下文漂移（cache 命中率高 → 历史更稳定）。
- [Agentic Ai Infrastructure Practice Series Nine Context Engineering](ch05/100-ai.html) — Agent 上下文工程实践系列。Headroom 是**具体实现**之一，可作为该系列的"proxy 层"案例补充。
- [Cl Bench Life Tencent Context Learning](ch01/1542-cl-bench-life.html) — 腾讯 CL-Bench 长上下文学习基准。Headroom 是**生产工具**（不是基准），与之形成"评估 vs 优化"对照。
- [Alibaba Eventhouse Enterprise Agent Context](ch03/035-agent.html) — 阿里云 EventHouse 企业级 Agent 上下文。Headroom 走**proxy 通用方案**，EventHouse 走**企业级云服务方案**。

## 相关主题

- 上下文工程 — [Agentic Ai Infrastructure Practice Series Nine Context Engineering](ch05/100-ai.html)
- 提示词缓存策略 — [Openclacky Harness Prompt Cache](ch05/085-harness.html)
- Agent 工具链 — [Agent Harness Architecture Design Production Guide](ch03/035-agent.html)
- VibeCoder / Vibe编码 公众号其他文章 — [Cli Mcp Skill Architecture Decision Vibecoder](ch07/001-mcp.html) / [Impeccable Vibe Design Philosophy Anomaly](ch05/001-impeccable.html) / [Impeccable Vibe Design Philosophy Anomaly](ch05/001-impeccable.html)
- MCP server 模式 — [Cli Mcp Skill Architecture Decision Vibecoder](ch07/001-mcp.html)
...
"State is a projection of events" 这个命题在 Codex 中不是隐喻，而是工程实现。状态变化时追加新消息（而非原地修改）带来的两大收益：前缀不变 → 缓存命中；因果链保留 → 可推断意图。这与 [Claude Code Vs Codex Context Architecture 02](ch03/084-claude-code.html) 中描述的 Claude Code 激进 Context Editing 路线形成鲜明对比。两种流派并非优劣之分，而是不同工程约束下的取舍：Append-only 更保守、工程友好，适合需要稳定缓存的生产系统；Context Editing 更激进、Token 精简，但前缀破坏带来工程复杂度。

### 实践价值：压缩能力的不对称是结构性的，应用层应聚焦可控部分

应用层压缩（Semantic Compression）与基础设施层压缩之间存在结构性信息不对称：厂商可访问注意力分布、海量真实对话数据、专用 Fine-tuned 压缩模型，应用层无法复制这一优势。理解这一点后，[Anthropic Prompt Caching Claude Code](ch01/405-anthropic-prompt-caching-claude-code.html) 等应用层实践的价值在于：放弃追求"更好的压缩"，转而聚焦**缓存友好的 Prompt Layout + 显式的 Compress/Select 策略**，这是应用层真正能控制的部分。context-kit 开源工具正是这一哲学的教学实现。
...
尽管"/responses/compact"端点使用"latent understanding"措辞，但向量表征与模型架构深度耦合使得纯 Latent Compression 的工程可行性存疑。更合理的推断是：厂商做的是 Semantic Compression，但拥有关键信息优势（注意力分布、真实数据、专用压缩模型）。这与 [Harness Engineering](ch05/129-harness-engineering.html) 实践中"在不确定黑盒内部机制时，聚焦可控部分"的原则一致。
...
2. **状态变化优先 Append-only 策略**：当 Agent 状态变化时（切换目录、审批模式、权限配置），追加新消息而非修改现有消息。这最大化 Prompt Cache 命中，同时保留完整因果链用于意图推断。结合 [Agent Harness Context Management Working Set](ch03/035-agent.html) 中的工作集管理实践，Append-only 是生产级 Agent 状态管理的推荐方案。
...
5. **警惕压缩能力的结构性不对称**：应用层压缩永远受限于信息劣势。不要过度投入"更智能的摘要算法"，而是将资源投入到缓存友好的架构设计和规范的 [Harness Engineering](ch05/129-harness-engineering.html) 实践中——这些是真正可控且可累积的工程优势。
...
- [反向审计 prompt 范式 — 从 vb 50 行 codex 自我蒸馏到 5 行核心](ch01/736-codex.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
→ [Claude Code Prompt Source Analysis](ch03/084-claude-code.html) — Claude Code 提示词体系对比
→ [Agent Context Management Architecture Patterns](ch03/035-agent.html) — Agent 上下文管理工程模式
→ [Agent Harness Context Management Working Set](ch03/035-agent.html) — Agent Harness 上下文管理

## 相关实体
- [Memento-Skills — 技能外部记忆让 Agent 自进化（arXiv 2603.18743）](ch04/341-skill.html)
...
- [Self-Evolving Agents 系统性综述](ch03/035-agent.html)
- [Hermes Self-Improving 闭环详解（winty）](ch01/294-hermes.html)
- [Agent 自我改进的六条路](ch03/035-agent.html)
- [SkillOS: Learning Skill Curation for Self-Evolving Agents](ch04/341-skill.html)
...
## Related
- [Hermes Agent](ch03/102-hermes-agent.html) — Nous Research 开源框架（4万+ Stars），核心亮点自进化
...
- [Claude Code 架构](ch03/084-claude-code.html) — 同为 Agent 深度解析系列参照
- [MemOS Hermes 插件](ch01/294-hermes.html) — 第三方记忆插件，与 Hermes 原生 Memory 形成互补
- [Agent Skill 编写指南](ch04/341-skill.html) — Skill 格式规范，与 Hermes 动态 Skill 沉淀机制高度相关
...
- [Google Genkit Middleware](ch03/035-agent.html)

- [development environments for your cloud agents](ch01/352-development-environments-for-your-cloud-agents.html)
...
**Logo 设计场景**：如果模型输出的是栅格图像，曲线错了，用户必须 mask、inpaint、重新生成或手动重画。如果输出是 SVG，用户可以直接编辑路径、原语、渐变、描边或文本元素。这已经是设计师在 [Quiver 等工具](ch05/100-ai.html) 上设计 Logo 的方式。
...
- [ICLR 2026: 英伟达/普渡用 Agent 闭环实现文生 3D](ch03/035-agent.html) — 同一时期 3D 生成的 Agent 闭环探索
- [DeepSeek 视觉原语](ch09/188-deepseek-visual-primitives.html) — 视觉表征的另一思路：堆指代精度而非图像分辨率
- [ICLR Agent 3D 生成](ch03/035-agent.html) — Agent 在 3D 生成中的另一研究路径
...
- [Ethan He: Cosmos / Grok Imagine / Latent Space 视频 Agent](ch03/035-agent.html) — 视频与多模态生成的前沿
...
## 概述
FanOne 从源码角度解析 Claude Code 的 Prompt 提示词模块六大分层体系：Core System（静态/动态分离 + 优先级策略树）、Tool（自然语言行为协议）、Skill（渐进式加载 + Reading Guide）、Agent（强角色边界 SOP）、Context Management、Memory（四类分级存储）。与 [Claude Code 架构解析](ch03/084-claude-code.html) 构成完整源码解读系列。
...
- [Claude Code Skills 实践与 Superpowers](ch07/032-claude-code-skills.html) — Skill 体系工程化实践
- [Claude Code Subagent 上下文卫生](ch03/084-claude-code.html) — Subagent = Harness 上下文隔离工具
- [Agent Memory 架构](ch03/035-agent.html) — 与 CC Memory 的对比
- [Obsidian + Claude Code 集成指南](ch03/084-claude-code.html) — 五种集成策略
...
- [Claude Code 源码深度解析（13 核心机制）](ch03/084-claude-code.html)
...
- [从Vibe Coding到Agentic Engineering：重构后台开发全流程 — 腾讯技术工程](ch04/268-tencent-vibe-coding-to-agentic-engineering-backend.html)
...
- [Thin Harness Fat Skills](ch04/341-skill.html)
...
- [using claude](ch03/084-claude-code.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
- [Claude Fable 5 安全寓言](ch01/1165-claude-fable-5-and-new-ai-safety-fables.html) — Nathan Lambert 的安全政策分析
- [Fable 5 on AWS Bedrock](ch01/1532-anthropic.html) — 企业部署视角
- [Mollick Fable 5 patron vs wizard](ch01/475-claude-fable-5.html) — 用户体验视角
- [阿里云云原生 安全护栏三域演进](ch05/100-ai.html) — 3 域对比（云资源约束 / AI 输出约束 / 模型路由约束）
- [Mythos bug hunting 营销](ch01/1532-anthropic.html)
- [System over Model tested](ch01/1429-0.html) — 评测视角
- [Loop Engineering](ch05/006-loop-engineering.html) — 状态文件作为对话外可审计载体
- [Anthropic 12 MCP 生产模式](ch07/001-mcp.html)
- [Anthropic 14 Skill 模式](ch04/341-skill.html)
- [Skill Issues Claude Code](ch04/341-skill.html) — 同样的 supply chain 攻击视角
- [System Prompt vs Post-Training](ch05/100-ai.html) — 行为约束应进训练而非 prompt
- [AIMap Security Testing](ch05/100-ai.html) — 同样聚焦 AI Agent 安全测试
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
## 相关实体
- [LBS-IntentBench — 首个真实出行隐式意图评测基准](ch04/342-lbs-intentbench.html)
- [Perplexity 内部 Skill 设计指南：四维体系与维护方法论](ch04/341-skill.html)
- [AI Skill 测评指标体系](ch04/341-skill.html)
...
这种生命周期划分启发了 [Loop Engineering](ch05/006-loop-engineering.html) 中的反馈控制设计——当检测到 Context Rot 信号时触发会话切换。
...
"清晰理想态"中的约束金字塔（示例 > 规则 > 需求）揭示了一个更深层的 Agent 工程原理：**Agent 不需要更多数据，它需要更好的约束**。 传统软件工程中，约束以类型系统、接口契约、测试用例的形式存在；AI Agent 时代，约束以自然语言示例、结构化 Spec、分层需求的形式存在。这两种范式本质相同——都在定义"什么可以做、什么不可以做"的边界。当约束足够精确时，Agent 的搜索空间被有效裁剪，返工轮次减少，Token 消耗自然下降。这与 [Spec-Driven Development](ch01/1188-spec.html) 的核心思想一致。
...
这与 [Harness Engineering](ch05/129-harness-engineering.html) 的模式三（子智能体与后台任务）一脉相承。
...
- [Loop Engineering](ch05/006-loop-engineering.html)
- [Skills For Real Engineers](ch04/341-skill.html)
- [Spec-Driven Development](ch01/1188-spec.html)
- [上下文管理](ch03/035-agent.html)
- [Agent 架构模式](ch03/035-agent.html)
...
- [Harness 工程核心模式](ch03/084-claude-code.html)
- [Harness Engineering](ch05/129-harness-engineering.html)
- [MCP 协议与工具编排](ch07/017-anthropic-mcp.html)
- [Skill 驱动开发](ch04/341-skill.html)
- [Working Set 上下文管理](ch03/035-agent.html)
...
- [Harness 工程实践 17 vs 6 Subagent](ch05/129-harness-engineering.html) — 探讨 Agent Skills 的工程化设计，与 Superpowers 的 prompt 微测方法论形成互补
...
- [Agent Loop 设计模式](ch03/035-agent.html) — 探讨 Agent 运行循环的设计模式，为理解 dispatch prompt 的上下文机制提供参考
- [Harness Engineering](ch05/129-harness-engineering.html) — Agent 工程化的广义框架，Superpowers 的微测方法是 prompt engineering 层面对 Harness Engineering 的一次实践验证
...
- [OpenClacky — Prompt Cache 命中率 90% 的 Harness 工程实践](ch05/085-harness.html)
- [Hermes Agent 自进化机制源码解析](ch03/035-agent.html)
...
## 相关维度深度分析
- **[Claude Code Prompt Context Harness](ch03/084-claude-code.html)**（飞樰）侧重 Claude Code 的 Prompt/Context/Harness 三维度，与本文同作者互补
- **[Claude Code Agent Engineering](ch03/084-claude-code.html)**（SooKool）侧重 StreamingToolExecutor/主循环/压缩/小模型
...
- [从 Prompt 到 Harness：最小实操指南](ch01/1442-claude.html)
...
- [AgentCore Managed Harness](ch04/751-agentcore-harness.html)
...
- [AI Agent 工程师能力地图](ch05/100-ai.html)
- [阿里云端到端业务需求专家 agent：multica 平台 + superai-* 技能集群 + tdd/pre-pus](ch03/035-agent.html)
- [fanling company as agent ai org reflection v2](ch05/100-ai.html)
...
## 相关链接
- [Llm Wiki Architecture](ch01/1476-llm.html)
...
- [Skills Refiner Design Quality Evaluation Framework](ch04/341-skill.html)
- [Tencent Vibe Coding To Agentic Engineering Backend](ch04/268-tencent-vibe-coding-to-agentic-engineering-backend.html)
...
- [Agentcore Managed Harness](ch04/291-agentcore-managed-harness.html)
- [Harness Engineering Framework](ch05/129-harness-engineering.html)
...
- [滴滴 ibg 智能客服质检系统：3 管线（意图 86% / 合规 90%+ / voc）+ 企业 llm 落地方法论](ch01/1188-spec.html)
...
- [Skill 设计模式](ch04/341-skill.html) — 工作流模式与 Skill 设计的关系
- [Agent Skill 编写指南](ch04/341-skill.html) — Skill 格式规范参考
- [Anthropic MCP 重新定义](ch07/017-anthropic-mcp.html) — Anthropic 官方对 Skill + MCP 互补关系的定义

## 相关实体
- [Lessons from Building Claude Code: Seeing like an Agent](ch03/084-claude-code.html)

- [Anthropic 官方 14 种 Skill 设计模式](ch04/341-skill.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
## 子页面
- [高质量编写规范](ch04/341-skill.html) — 6 条核心编写原则
- [评估与迭代](ch04/341-skill.html) — 触发测试、功能测试、基线对比方法论
- [进阶模式与治理](ch04/341-skill.html) — Anthropic 5 种进阶模式、安装部署、YAML 完整规范、实战调试案例

## Related
- [Hermes Agent](ch03/102-hermes-agent.html) — Skill 机制是 Hermes 的核心特性之一
- [OpenClaw 架构解析](https://github.com/QianJinGuo/wiki/blob/main/concepts/openclaw-architecture.md) — OpenClaw 内置 Skill 系统实现
- [MemOS Hermes 插件](ch01/294-hermes.html) — MemOS 的 Skill 管理能力
- [原始文章存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/agent-skill-writing-guide.md)
- [Skill 设计模式](ch04/341-skill.html) — 5种设计模式系统指南
...
- [gene/gep — evomap×清华 提出的「策略基因」经验对象框架（arxiv 2604.15097）](ch01/1429-0.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
- [LLM Post-Training 全景指南](ch05/100-ai.html) — Post-training 方法论基础（RLHF/GRPO/RLVR）
- [Harness + Post-Training bug-finding gap](ch05/100-ai.html) — Harness 与 Post-training 的协同效应
- [Qoder Skills 指南](ch04/341-skill.html) — 讨论 skill 约束与 prompt 边界
- [文心 Post-training 演进](ch05/100-ai.html) — Post-training 工程实践案例
- [Yann Dubois OpenAI Post-Training 访谈](ch05/100-ai.html) — Post-training 职业路径
- [Kimi K2.5 架构创新](ch01/1429-0.html) — 同样强调"模型层处理行为，harness 层处理任务"
...
- Harness Engineering
- [Karpathy: Vibe Coding 到 Agentic Engineering](ch04/185-karpathy-vibe-coding-agentic-engineering.html)
- [Claude Code 源码中的 Agent Harness 构建之道](ch03/084-claude-code.html)
- [Claude Code 源码核心机制](ch03/084-claude-code.html)
- [Harness Engineering 概念解析](ch05/100-ai.html)
...
## 相关实体
- [Llm Wiki Obsidian Wiki Gbrain Self Organization Self Evolution](ch05/100-ai.html)
...
## 相关实体
- [Llm Wiki Obsidian Wiki Gbrain Self Organization Self Evolution](ch05/100-ai.html)
- [Claude Code Prompt Context Harness](ch03/084-claude-code.html)
- [Claude Code Harness Deep Dive Founder Park](ch03/084-claude-code.html)
- [Openclaw Prompt Context Harness](ch11/247-openclaw.html)
- [Harness Engineering Framework](ch05/129-harness-engineering.html)
...
- [Hermes 9 Module Architecture Winty](ch01/294-hermes.html)
...
- [Harness Engineering](ch05/129-harness-engineering.html) — AINMM 的过程域对应 Harness 五层架构
- [高德 Harness/SDD 演进](ch05/100-ai.html) — 另一家团队 AI Native 团队级实践
...
- [AI Agent 评测 5 维体系](ch05/100-ai.html) — 评估维度的互补框架
...
- [Agentic AI System Architecture](ch07/001-mcp.html)
...
## 相关实体
- [Harness Engineering耗时一周我是如何将应用的Ai Coding率提升至90的](ch05/100-ai.html)
- [Wangyunhe Harness Optimization Agentsoul](ch03/035-agent.html)
- [Claude Code开发负责人 为何放弃Rag而选择Agentic Search](ch03/084-claude-code.html)
- [Integrate Atlassian Confluence Cloud With Amazon Quick](ch11/235-amazon-quick.html)
- [Rag Vs Llm Wiki Enterprise Knowledge Base](ch01/411-rag.html)

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/vivo-ai-sales-guide-ecommerce-agent.md)
- [电商 ai 操作系统崛起：从「工具人」到「all in one」+ 行业 knowhow skill 化 + 5 巨头](ch05/100-ai.html)
...
- [Claude Code 大型代码库 Harness 配置](ch03/084-claude-code.html) — Skills 作为 Harness 五扩展点之一，渐进式披露的工程视角
- [Anthropic Knowledge Work Plugins 分析](ch01/1532-anthropic.html) — 3 级渐进式披露的详细技术分析
- [Claude Code 为什么会忽略指令](ch03/084-claude-code.html) — CLAUDE.md 越写越糟的诊断，本文给出 Skills 作为解法
- [Claude Code 七种自定义方法](ch03/084-claude-code.html) — Skills 在七种方法中的定位
- [CLAUDE.md 12 条规则](ch01/342-01.html) — CLAUDE.md 喂法的代表，本文指出其局限
...
- **AI 把"系统结构"变成产品的一部分** — 印证了 [Agent Vs Workflow Control Continuum Framework](ch03/035-agent.html) 中控制权漂移的趋势：工程实现与产品设计的边界正在模糊
...
- [华为伏羲推荐系统](ch01/1409-scaling-law.html) — 另一个大规模生成式推荐实践
- [Agent Harness Engineering Survey](ch05/129-harness-engineering.html) — 推理系统的 harness 范式
- [高德 Marketing AutoResearch](ch05/100-ai.html) — 同样涉及算法模型迭代自动化
- [OpenClaw Agent Loop](ch03/035-agent.html) — Fast-Slow Thinking 与 Loop 设计范式对照
