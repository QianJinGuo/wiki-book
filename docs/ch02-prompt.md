...
2. **状态变化优先 Append-only 策略**：当 Agent 状态变化时（切换目录、审批模式、权限配置），追加新消息而非修改现有消息。这最大化 Prompt Cache 命中，同时保留完整因果链用于意图推断。结合 [Agent Harness Context Management Working Set](ch04/502-agent.md) 中的工作集管理实践，Append-only 是生产级 Agent 状态管理的推荐方案。
...
- [Agent Harness Context Management Working Set](ch04/502-agent.md) — Agent 上下文管理的 working set 模式（**主线程 vs 后台 subagent**）。Headroom 是 **proxy/中间层方案**，在工具输出层做字节级压缩。两者**问题域重叠但切入点完全不同**。
- [Agent Context Management Architecture Patterns](ch04/502-agent.md) — 上下文管理的架构模式总览（截断 / 总结 / 滑动窗口 / RAG）。Headroom 走**更激进的字节级 patch + cache-aware 路径**，是这些模式之外的"代理层"独立分支。
...
- [Agent Reliability Context Drift Tool Hallucination](ch04/502-agent.md) — 上下文漂移导致工具幻觉。Headroom 的 cache stabilization 间接缓解上下文漂移（cache 命中率高 → 历史更稳定）。
...
- [Alibaba Eventhouse Enterprise Agent Context](ch04/502-agent.md) — 阿里云 EventHouse 企业级 Agent 上下文。Headroom 走**proxy 通用方案**，EventHouse 走**企业级云服务方案**。
...
- Agent 工具链 — [Agent Harness Architecture Design Production Guide](ch04/502-agent.md)
...
→ [Agent Context Management Architecture Patterns](ch04/502-agent.md) — Agent 上下文管理工程模式
→ [Agent Harness Context Management Working Set](ch04/502-agent.md) — Agent Harness 上下文管理
...
- [ICLR 2026: 英伟达/普渡用 Agent 闭环实现文生 3D](ch04/502-agent.md) — 同一时期 3D 生成的 Agent 闭环探索
- [DeepSeek 视觉原语](ch09/130-deepseek-visual-primitives.md) — 视觉表征的另一思路：堆指代精度而非图像分辨率
- [ICLR Agent 3D 生成](ch04/502-agent.md) — Agent 在 3D 生成中的另一研究路径
...
- [Ethan He: Cosmos / Grok Imagine / Latent Space 视频 Agent](ch04/502-agent.md) — 视频与多模态生成的前沿
...
- [Agent Memory 架构](ch04/502-agent.md) — 与 CC Memory 的对比
...
- [Google Genkit Middleware](ch04/502-agent.md)
...
- [AgentEval：YAML驱动的Agent评测框架](ch04/502-agent.md)
...
- [Agent Harness 上下文管理：工作集视角](ch04/502-agent.md)
...
- [AgentCore Managed Harness](ch04/502-agent.md)
- [从 30 分钟手搓 Agent，到 Harness 成为"新后端"](ch04/502-agent.md)
...
- [agent 记忆注入实战：5 维框架（选什么/放哪里/怎么放/放多少/何时放）+ 4 前沿论文（memguide/sti](ch04/502-agent.md)
...
## 相关实体
- [Fudan Peking Ahe Agentic Harness Engineering](ch04/221-fudan-peking-ahe-agentic-harness-engineering.md)
- [Agent Evolution Four Stages Six Dimensions Aliyun](ch04/502-agent.md)
...
- [Agent Self-Improvement](ch04/502-agent.md)
...
- [Agent Harness Engineering Survey](ch04/502-agent.md) — 推理系统的 harness 范式
- [高德 Marketing AutoResearch](ch04/150-ai.md) — 同样涉及算法模型迭代自动化
- [OpenClaw Agent Loop](ch04/502-agent.md) — Fast-Slow Thinking 与 Loop 设计范式对照
