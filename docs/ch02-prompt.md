...
## 相关实体
- [Harness Engineering 第三代工程范式](ch05/009-harness.md)
...
- [model-harness-fit-agent-harness](ch05/009-harness.md)
...
2. **状态变化优先 Append-only 策略**：当 Agent 状态变化时（切换目录、审批模式、权限配置），追加新消息而非修改现有消息。这最大化 Prompt Cache 命中，同时保留完整因果链用于意图推断。结合 [Agent Harness Context Management Working Set](ch05/009-harness.md) 中的工作集管理实践，Append-only 是生产级 Agent 状态管理的推荐方案。
...
- [Agent Harness Context Management Working Set](ch05/009-harness.md) — Agent 上下文管理的 working set 模式（**主线程 vs 后台 subagent**）。Headroom 是 **proxy/中间层方案**，在工具输出层做字节级压缩。两者**问题域重叠但切入点完全不同**。
...
- [Cl Bench Life Tencent Context Learning](ch01/1171-cl-bench-life.md) — 腾讯 CL-Bench 长上下文学习基准。Headroom 是**生产工具**（不是基准），与之形成"评估 vs 优化"对照。
...
- Agent 工具链 — [Agent Harness Architecture Design Production Guide](ch05/009-harness.md)
...
- [Harness 工程实践 17 vs 6 Subagent](ch05/009-harness.md) — 探讨 Agent Skills 的工程化设计，与 Superpowers 的 prompt 微测方法论形成互补
...
- [Agent Harness 上下文管理：工作集视角](ch05/009-harness.md)
...
- [Wangyunhe Harness Optimization Agentsoul](ch05/009-harness.md)
- [Claude Code开发负责人 为何放弃Rag而选择Agentic Search](ch03/075-claude-code.md)
...
- [华为伏羲推荐系统](ch01/534-scaling.md) — 另一个大规模生成式推荐实践
- [Agent Harness Engineering Survey](ch05/009-harness.md) — 推理系统的 harness 范式
