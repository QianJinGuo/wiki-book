...
## 相关实体
- [Harness Engineering 第三代工程范式](ch05/085-harness.md)
...
- [is grep all you need? — 检索 × harness × 交付方式耦合三元组（pwc 论文 arxi](ch05/085-harness.md)
- [model-harness-fit-agent-harness](ch05/085-harness.md)
...
> 与 [Skill 设计模式](ch04/258-skill.md) 在"skill 是知识注入而非工具生成"维度同源，但本文聚焦**触发失败诊断**而非模式选择。
...
> 与 [Agent Skill 进阶模式与治理](ch04/258-skill.md) 在"description 是触发信号源"维度同源——但本文给的是**反模式 → 正模式**的具体对照，可作 description 写作 checklist。
...
> 与 [Agent 可靠性工程与持续改进](ch04/258-skill.md) 的"产品级可观测"维度同源——本文聚焦"**可被反向审计的回执结构**"这一具体落地形态。
...
> 这是与 [Skill 工程化设计：把 Agent 当算法用](ch04/258-skill.md) 在"agent 链路是激活对象"维度上的**范式共振**——本文给出的是诊断视角，后者给出的是生产视角。
...
- **同 skill description / 触发信号**：
  - [Agent Skill 进阶模式与治理](ch04/258-skill.md)（description + name + 触发机制）
  - [Skill 设计模式](ch04/258-skill.md)（5 种核心模式 + 1 特殊模式）
  - [Anthropic 官方技能最佳实践 14 模式](ch04/258-skill.md)（5 类设计模式）
  - [AI Skill Evolution Framework](ch04/258-skill.md)（skill 评估与度量）
  - [Anthropic+Google Agent Skills 设计模式](ch04/258-skill.md)
- **同 Codex / OpenAI**：
  - [Codex Goal 代理运行时](ch09/046-codex-goal.md)
  - [Codex Goal 六小时运行](ch09/057-codex-goal-six-hour-run.md)
  - [Codex 上下文工程](ch01/516-codex.md)
  - [OpenAI Codex JasonLiu maxxing 攻略](ch04/069-ai.md)
  - [GPT-5.4 Codex Interconnects](ch01/516-codex.md)
- **同 AGENTS.md / 角色边界**：
  - [Agent 可靠性工程与持续改进](ch04/258-skill.md)
  - [Skill 工程化设计：把 Agent 当算法用](ch04/258-skill.md)
- **同 prompt 工程 / 范式**：
  - [Claude Managed Agents](ch04/603-claude-managed-agents.md)（prompt 工程的边界探索）
  - [AI 蜜罐：对抗 AI 智能体](ch04/069-ai.md)
...
"State is a projection of events" 这个命题在 Codex 中不是隐喻，而是工程实现。状态变化时追加新消息（而非原地修改）带来的两大收益：前缀不变 → 缓存命中；因果链保留 → 可推断意图。这与 [Claude Code Vs Codex Context Architecture 02](ch03/075-claude-code.md) 中描述的 Claude Code 激进 Context Editing 路线形成鲜明对比。两种流派并非优劣之分，而是不同工程约束下的取舍：Append-only 更保守、工程友好，适合需要稳定缓存的生产系统；Context Editing 更激进、Token 精简，但前缀破坏带来工程复杂度。

### 实践价值：压缩能力的不对称是结构性的，应用层应聚焦可控部分

应用层压缩（Semantic Compression）与基础设施层压缩之间存在结构性信息不对称：厂商可访问注意力分布、海量真实对话数据、专用 Fine-tuned 压缩模型，应用层无法复制这一优势。理解这一点后，[Anthropic Prompt Caching Claude Code](ch01/275-anthropic-prompt-caching-claude-code.md) 等应用层实践的价值在于：放弃追求"更好的压缩"，转而聚焦**缓存友好的 Prompt Layout + 显式的 Compress/Select 策略**，这是应用层真正能控制的部分。context-kit 开源工具正是这一哲学的教学实现。
...
尽管"/responses/compact"端点使用"latent understanding"措辞，但向量表征与模型架构深度耦合使得纯 Latent Compression 的工程可行性存疑。更合理的推断是：厂商做的是 Semantic Compression，但拥有关键信息优势（注意力分布、真实数据、专用压缩模型）。这与 [Harness Engineering](ch05/091-harness-engineering.md) 实践中"在不确定黑盒内部机制时，聚焦可控部分"的原则一致。
...
2. **状态变化优先 Append-only 策略**：当 Agent 状态变化时（切换目录、审批模式、权限配置），追加新消息而非修改现有消息。这最大化 Prompt Cache 命中，同时保留完整因果链用于意图推断。结合 [Agent Harness Context Management Working Set](ch05/085-harness.md) 中的工作集管理实践，Append-only 是生产级 Agent 状态管理的推荐方案。
...
5. **警惕压缩能力的结构性不对称**：应用层压缩永远受限于信息劣势。不要过度投入"更智能的摘要算法"，而是将资源投入到缓存友好的架构设计和规范的 [Harness Engineering](ch05/091-harness-engineering.md) 实践中——这些是真正可控且可累积的工程优势。
...
- [反向审计 prompt 范式 — 从 vb 50 行 codex 自我蒸馏到 5 行核心](ch01/516-codex.md)
...
HTML Artifacts 是 [Claude Code](ch03/075-claude-code.md) 中一种强大的输出形式，它使 Agent 能够生成富交互式的 HTML 文档，显著超越传统 Markdown 的表达能力。
...
-  — 本文讨论的主要工具
- [Claude Code 最佳实践](ch03/075-claude-code.md) — 相关实践指南
...
- [Agent Harness Context Management Working Set](ch05/085-harness.md) — Agent 上下文管理的 working set 模式（**主线程 vs 后台 subagent**）。Headroom 是 **proxy/中间层方案**，在工具输出层做字节级压缩。两者**问题域重叠但切入点完全不同**。
- [Agent Context Management Architecture Patterns](ch03/045-agent.md) — 上下文管理的架构模式总览（截断 / 总结 / 滑动窗口 / RAG）。Headroom 走**更激进的字节级 patch + cache-aware 路径**，是这些模式之外的"代理层"独立分支。
- [Openclacky Harness Prompt Cache](ch05/085-harness.md) — OpenClacky 提示词缓存策略。Headroom 与之同源思想（**关注 prompt cache key 稳定性**）但实现不同：OpenClacky 是 SDK 层策略，Headroom 是 proxy 层工具。
- [Ai Context Layer Kgc 2026](ch04/069-ai.md) — AI 上下文层（知识图谱增强）。Headroom 走**字节级压缩**而非**结构化知识层**，定位互补。
- [Claude Code Prompt Context Harness](ch03/075-claude-code.md) / [Claude Code Session Management 1M Context](ch03/075-claude-code.md) — Claude Code 的上下文管理实现。Headroom 是**通用 provider 方案**（覆盖 Claude Code / Codex / Cursor / Aider / Copilot），Claude Code 上下文管理是**单 provider 内部**方案。
- [Claude Code Vs Codex Context Architecture 02](ch03/075-claude-code.md) — Claude Code vs Codex 上下文架构对比。Headroom **同时支持两者**（作为 proxy 层），可以视为这两者的**第三方统一优化层**。
- [Agent Reliability Context Drift Tool Hallucination](ch03/045-agent.md) — 上下文漂移导致工具幻觉。Headroom 的 cache stabilization 间接缓解上下文漂移（cache 命中率高 → 历史更稳定）。
- [Agentic Ai Infrastructure Practice Series Nine Context Engineering](ch04/069-ai.md) — Agent 上下文工程实践系列。Headroom 是**具体实现**之一，可作为该系列的"proxy 层"案例补充。
- [Cl Bench Life Tencent Context Learning](ch01/1182-cl-bench-life.md) — 腾讯 CL-Bench 长上下文学习基准。Headroom 是**生产工具**（不是基准），与之形成"评估 vs 优化"对照。
- [Alibaba Eventhouse Enterprise Agent Context](ch03/045-agent.md) — 阿里云 EventHouse 企业级 Agent 上下文。Headroom 走**proxy 通用方案**，EventHouse 走**企业级云服务方案**。

## 相关主题

- 上下文工程 — [Agentic Ai Infrastructure Practice Series Nine Context Engineering](ch04/069-ai.md)
- 提示词缓存策略 — [Openclacky Harness Prompt Cache](ch05/085-harness.md)
- Agent 工具链 — [Agent Harness Architecture Design Production Guide](ch05/085-harness.md)
- VibeCoder / Vibe编码 公众号其他文章 — [Cli Mcp Skill Architecture Decision Vibecoder](ch04/258-skill.md) / [Impeccable Vibe Design Philosophy Anomaly](ch05/001-impeccable.md) / [Impeccable Vibe Design Philosophy Anomaly](ch05/001-impeccable.md)
- MCP server 模式 — [Cli Mcp Skill Architecture Decision Vibecoder](ch04/258-skill.md)
...
→ [Claude Code Prompt Source Analysis](ch03/075-claude-code.md) — Claude Code 提示词体系对比
→ [Agent Context Management Architecture Patterns](ch03/045-agent.md) — Agent 上下文管理工程模式
→ [Agent Harness Context Management Working Set](ch05/085-harness.md) — Agent Harness 上下文管理

## 相关实体
- [Memento-Skills — 技能外部记忆让 Agent 自进化（arXiv 2603.18743）](ch04/258-skill.md)
...
- [Self-Evolving Agents 系统性综述](ch03/045-agent.md)
...
## Related
- [Hermes Agent](ch03/090-hermes-agent.md) — Nous Research 开源框架（4万+ Stars），核心亮点自进化
...
- [Claude Code 架构](ch03/075-claude-code.md) — 同为 Agent 深度解析系列参照
...
- [Agent Skill 编写指南](ch04/258-skill.md) — Skill 格式规范，与 Hermes 动态 Skill 沉淀机制高度相关
...
**Logo 设计场景**：如果模型输出的是栅格图像，曲线错了，用户必须 mask、inpaint、重新生成或手动重画。如果输出是 SVG，用户可以直接编辑路径、原语、渐变、描边或文本元素。这已经是设计师在 [Quiver 等工具](ch04/069-ai.md) 上设计 Logo 的方式。
...
- [ICLR 2026: 英伟达/普渡用 Agent 闭环实现文生 3D](ch01/216-0.md) — 同一时期 3D 生成的 Agent 闭环探索
- [DeepSeek 视觉原语](ch09/150-deepseek-visual-primitives.md) — 视觉表征的另一思路：堆指代精度而非图像分辨率
- [ICLR Agent 3D 生成](ch03/045-agent.md) — Agent 在 3D 生成中的另一研究路径
...
- [Ethan He: Cosmos / Grok Imagine / Latent Space 视频 Agent](ch01/216-0.md) — 视频与多模态生成的前沿
...
## 概述
FanOne 从源码角度解析 Claude Code 的 Prompt 提示词模块六大分层体系：Core System（静态/动态分离 + 优先级策略树）、Tool（自然语言行为协议）、Skill（渐进式加载 + Reading Guide）、Agent（强角色边界 SOP）、Context Management、Memory（四类分级存储）。与 [Claude Code 架构解析](ch03/075-claude-code.md) 构成完整源码解读系列。
...
- [Claude Code Skills 实践与 Superpowers](ch04/258-skill.md) — Skill 体系工程化实践
- [Claude Code Subagent 上下文卫生](ch03/075-claude-code.md) — Subagent = Harness 上下文隔离工具
- [Agent Memory 架构](ch03/045-agent.md) — 与 CC Memory 的对比
- [Obsidian + Claude Code 集成指南](ch03/075-claude-code.md) — 五种集成策略
...
- [Google Genkit Middleware](ch03/045-agent.md)

- [development environments for your cloud agents](ch09/103-development-environments-for-your-cloud-agents.md)
...
- [Claude Fable 5 安全寓言](ch01/820-claude-fable-5-and-new-ai-safety-fables.md) — Nathan Lambert 的安全政策分析
- [Fable 5 on AWS Bedrock](ch01/1022-claude.md) — 企业部署视角
- [Mollick Fable 5 patron vs wizard](ch01/1022-claude.md) — 用户体验视角
- [阿里云云原生 安全护栏三域演进](ch04/069-ai.md) — 3 域对比（云资源约束 / AI 输出约束 / 模型路由约束）
- [Mythos bug hunting 营销](ch01/855-anthropic.md)
- [System over Model tested](ch01/216-0.md) — 评测视角
- [Loop Engineering](ch05/006-loop-engineering.md) — 状态文件作为对话外可审计载体
- [Anthropic 12 MCP 生产模式](ch01/855-anthropic.md)
- [Anthropic 14 Skill 模式](ch04/258-skill.md)
- [Skill Issues Claude Code](ch04/258-skill.md) — 同样的 supply chain 攻击视角
- [System Prompt vs Post-Training](ch04/069-ai.md) — 行为约束应进训练而非 prompt
- [AIMap Security Testing](ch04/069-ai.md) — 同样聚焦 AI Agent 安全测试
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
## 相关实体
- [LBS-IntentBench — 首个真实出行隐式意图评测基准](ch04/262-lbs-intentbench.md)
- [Perplexity 内部 Skill 设计指南：四维体系与维护方法论](ch04/258-skill.md)
- [AI Skill 测评指标体系](ch04/258-skill.md)
...
- **扩展版本**：[Claude Code 12 Rules Karpathy Extension](ch03/075-claude-code.md)（新增 8 条覆盖 agent 编排场景）
...
- [Harness 工程实践 17 vs 6 Subagent](ch05/085-harness.md) — 探讨 Agent Skills 的工程化设计，与 Superpowers 的 prompt 微测方法论形成互补
...
- [Agent Loop 设计模式](ch03/045-agent.md) — 探讨 Agent 运行循环的设计模式，为理解 dispatch prompt 的上下文机制提供参考
- [Harness Engineering](ch05/091-harness-engineering.md) — Agent 工程化的广义框架，Superpowers 的微测方法是 prompt engineering 层面对 Harness Engineering 的一次实践验证
...
- [OpenClacky — Prompt Cache 命中率 90% 的 Harness 工程实践](ch05/085-harness.md)
...
## 相关维度深度分析
- **[Claude Code Prompt Context Harness](ch03/075-claude-code.md)**（飞樰）侧重 Claude Code 的 Prompt/Context/Harness 三维度，与本文同作者互补
- **[Claude Code Agent Engineering](ch03/075-claude-code.md)**（SooKool）侧重 StreamingToolExecutor/主循环/压缩/小模型
...
- [Agent Harness 上下文管理：工作集视角](ch05/085-harness.md)
...
- [从 30 分钟手搓 Agent，到 Harness 成为"新后端"](ch01/216-0.md)
- [harness-engineering-systematic-explainer](ch04/069-ai.md)
- [claude-code-7-layer-memory-architecture](ch03/075-claude-code.md)
- [AI Agent 工程师能力地图](ch04/069-ai.md)
- [阿里云端到端业务需求专家 agent：multica 平台 + superai-* 技能集群 + tdd/pre-pus](ch01/216-0.md)
- [fanling company as agent ai org reflection v2](ch04/069-ai.md)
...
- [agent 记忆注入实战：5 维框架（选什么/放哪里/怎么放/放多少/何时放）+ 4 前沿论文（memguide/sti](ch01/216-0.md)
...
## 相关链接
- [Llm Wiki Architecture](ch01/1171-llm.md)
...
- [Skills Refiner Design Quality Evaluation Framework](ch04/258-skill.md)
...
- [Skill 设计模式](ch04/258-skill.md) — 工作流模式与 Skill 设计的关系
- [Agent Skill 编写指南](ch04/258-skill.md) — Skill 格式规范参考
- [Anthropic MCP 重新定义](ch07/016-anthropic-mcp.md) — Anthropic 官方对 Skill + MCP 互补关系的定义

## 相关实体
- [Lessons from Building Claude Code: Seeing like an Agent](ch03/075-claude-code.md)

- [Anthropic 官方 14 种 Skill 设计模式](ch04/258-skill.md)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
## 子页面
- [高质量编写规范](ch04/258-skill.md) — 6 条核心编写原则
- [评估与迭代](ch04/258-skill.md) — 触发测试、功能测试、基线对比方法论
- [进阶模式与治理](ch04/258-skill.md) — Anthropic 5 种进阶模式、安装部署、YAML 完整规范、实战调试案例

## Related
- [Hermes Agent](ch03/090-hermes-agent.md) — Skill 机制是 Hermes 的核心特性之一
...
- [Skill 设计模式](ch04/258-skill.md) — 5种设计模式系统指南
...
- [gene/gep — evomap×清华 提出的「策略基因」经验对象框架（arxiv 2604.15097）](ch01/216-0.md)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/prompt-engineering-guide.md)
...
- [LLM Post-Training 全景指南](ch04/069-ai.md) — Post-training 方法论基础（RLHF/GRPO/RLVR）
- [Harness + Post-Training bug-finding gap](ch04/069-ai.md) — Harness 与 Post-training 的协同效应
- [Qoder Skills 指南](ch07/031-qoder-skills.md) — 讨论 skill 约束与 prompt 边界
- [文心 Post-training 演进](ch04/069-ai.md) — Post-training 工程实践案例
- [Yann Dubois OpenAI Post-Training 访谈](ch04/069-ai.md) — Post-training 职业路径
- [Kimi K2.5 架构创新](ch01/216-0.md) — 同样强调"模型层处理行为，harness 层处理任务"
...
- [Claude Code 源码中的 Agent Harness 构建之道](ch03/075-claude-code.md)
- [Claude Code 源码核心机制](ch03/075-claude-code.md)
- [Harness Engineering 概念解析](ch04/069-ai.md)
...
## 相关实体
- [Llm Wiki Obsidian Wiki Gbrain Self Organization Self Evolution](ch04/069-ai.md)
...
- [Agent Self-Improvement](ch03/045-agent.md)
- [Agentic AI System Architecture](ch04/258-skill.md)
...
## 相关实体
- [Harness Engineering耗时一周我是如何将应用的Ai Coding率提升至90的](ch04/069-ai.md)
- [Wangyunhe Harness Optimization Agentsoul](ch05/085-harness.md)
- [Claude Code开发负责人 为何放弃Rag而选择Agentic Search](ch03/075-claude-code.md)
- [Integrate Atlassian Confluence Cloud With Amazon Quick](ch11/202-amazon-quick.md)
- [Rag Vs Llm Wiki Enterprise Knowledge Base](ch01/1019-rag.md)
...
- [Claude Code 大型代码库 Harness 配置](ch03/075-claude-code.md) — Skills 作为 Harness 五扩展点之一，渐进式披露的工程视角
- [Anthropic Knowledge Work Plugins 分析](ch01/855-anthropic.md) — 3 级渐进式披露的详细技术分析
- [Claude Code 为什么会忽略指令](ch03/075-claude-code.md) — CLAUDE.md 越写越糟的诊断，本文给出 Skills 作为解法
- [Claude Code 七种自定义方法](ch03/075-claude-code.md) — Skills 在七种方法中的定位
- [CLAUDE.md 12 条规则](ch01/216-0.md) — CLAUDE.md 喂法的代表，本文指出其局限
...
- [华为伏羲推荐系统](ch01/544-scaling.md) — 另一个大规模生成式推荐实践
- [Agent Harness Engineering Survey](ch01/216-0.md) — 推理系统的 harness 范式
- [高德 Marketing AutoResearch](ch04/069-ai.md) — 同样涉及算法模型迭代自动化
- [OpenClaw Agent Loop](ch03/045-agent.md) — Fast-Slow Thinking 与 Loop 设计范式对照
