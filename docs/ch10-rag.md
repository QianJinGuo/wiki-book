...
## 相关实体
- [Identity Behavior Context Itdr Solution Teleport](/ch01-101-identity-behavior-context-itdr-solution-teleport/)
...
- [Architecture Data Foundations For Ai Powered Search](/ch01-653-architecture-data-foundations-for-ai-powered-search/)
...
- [Nvidia Extreme Co Design Agentic Systems](/ch04-040-nvidia-extreme-co-design-agentic-systems/)
...
- [RAG 深度解析：分块向量化召回重排](/ch01-327-rag-深度解析-分块向量化召回重排才是蒸馏同事-skill-的关键/)
- RAG 全链路技术详解
- RAG 分块向量化召回重排流水线
- 向量库 vs 知识图谱：RAG 的进阶路径
- [AI Agent 记忆系统工作原理](/ch04-145-how-ai-agent-memory-works/)
...
## 与 [Amazon Nova Lite 微调](/ch11-054-amazon-nova-lite-fine-tuning-高性价比的视觉检测模型微调案例与实践-亚马逊aws官方博/) 的关系
...
→ [Amazon Bedrock 模型推理无服务器架构案例](/ch11-067-amazon-bedrock-model-inference-serverless-architecture-case/)

→ Amazon Nova Sonic 可扩展语音代理设计

→ [Amazon Nova 2 内容审核提示工程](/ch01-387-prompting-amazon-nova-2-for-content-moderation/)

→ [Amazon Bedrock AgentCore 运行时深度解析](/ch11-068-快时尚电商行业智能体设计思路与应用实践-七-amazon-bedrock-agentcore-runtime-深度解析和/)
...
与 [Karpathy LLM Wiki V2 中文概述](/ch01-271-karpathy-llm-wiki-v2-2026/) 互补——原 entity 是中文入门解读，本文是 V2 深度技术分析 + 落地路线图 + 评测方法论。
...
- → [Karpathy LLM Wiki V2 中文概述](/ch01-271-karpathy-llm-wiki-v2-2026/) — 原版入门
- → LLM Wiki 范式 — 概念定义
- → 知识图谱 RAG — 图检索方法论
- → LLM Wiki 架构哲学
...
- → [Agent Memory 架构](/ch04-146-agent-memory-架构本质/) — 记忆系统设计模式
- → [Agent Memory 模块化框架](/ch01-238-agent-memory-模块化框架与评测-memory-in-the-llm-era-4-模块-10-方案对比/) — ICLR 2026 评测基准
- → [Mem0：Agent Harness 记忆现状](/ch04-130-state-of-memory-in-agent-harness-mem0-视角的九大-harness-横评/) — Mem0 等基线对比
- → Agent 记忆生命周期
- → [原文存档](https://github.com/QianJinGuo/wiki/blob/main/raw/articles/mragent-memory-reconstructed-not-retrieved-nus-icml2026.md)
...
- [5235705](/ch12-057-meta-u-turns-on-encryption-push-for-instagram-as-dms-go-plai/)
...
- RAG 与检索技术
- [From Silos To Service Topology Why Netflix Built A Real Time](/ch11-138-from-silos-to-service-topology-why-netflix-built-a-real-tim/)
...
Agentic browser 在传统浏览器基础上增加了 agent harness 层（参见 [Harness Engineering](/ch05-041-harness-engineering-概念框架/)），通过 tool calling 接口与浏览器引擎连接。用户通过 chat 界面发出指令，harness 将指令发送到远程模型进行推理，模型返回 tool call 指令由浏览器执行。关键的安全边界在于 agent 是否与用户共享 browsing context（密码、cookies、IndexedDB 数据）——如果共享，agent 就等同于用户本人，具备完整的代操作能力。
...
- **对开源社区**：参见 [ATH Agent Trust Handshake Protocol](/ch04-500-为了让agent更安全的工作-有多少人操碎了心/) 中关于 agent 身份验证的探索，这可能是解决信任问题的技术路径
