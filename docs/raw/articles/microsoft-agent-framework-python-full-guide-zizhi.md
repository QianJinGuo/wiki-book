---
type: source-card
title: "微软 Agent Framework 全栈指南：从 Hello Agent 到生产托管（Python）"
source: "智子AI社"
author: "智子AI社"
source_url: "https://mp.weixin.qq.com/s/xocGtHMHtlzLkdIuGtvGfw"
published: "2026-05-18"
collected: "2026-05-18"
license: "未发现可验证的再发布许可证；本仓库仅保留来源卡片"
---

# 微软 Agent Framework 全栈指南：从 Hello Agent 到生产托管（Python）

## 原创摘要

这份来源卡片记录一篇围绕“微软 Agent Framework 全栈指南：从 Hello Agent 到生产托管（Python）”的第三方资料，主题标签为microsoft、agent-framework、semantic-kernel。完整事实、论据、上下文与原文请以原始来源为准；公开仓库不保存正文副本。

> 公开版仅保留来源信息和原创摘要，不替代原始来源的阅读。
