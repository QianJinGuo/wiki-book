...
ingested: 2026-06-18
sha256: 66aeac8bd08bda2b6f8537abd37497498194ce76a39ab1cd9665af4e63acf245
---
...
URL Source: http://nathan.rs/posts/gzip-lm/
...
It turns out, kind of? It’s not exactly coherent text, but it clearly _knows_ something about the text. Much more than I expected `gzip` to _know_.[2](http://nathan.rs/posts/gzip-lm/#fn:2) So how can a compressor generate this?

## Compression is prediction[#](http://nathan.rs/posts/gzip-lm/#compression-is-prediction)
...
This is not a coincidence; it’s the core of information theory. The number of bits needed to encode a symbol is $- \left(log ⁡\right)_{2} p$, where $p$ is the probability the model assigns to it. High probability means few bits. So any compressor has a probability model hiding inside it, whether or not anyone wrote one down.
...
$$
\text{score} \left(\right. \text{candidate} \left.\right) = \mathtt{len}(\mathtt{gzip}(\mathtt{context} \textrm{ } + \textrm{ } \mathtt{candidate}))
$$
...
## Generating by beam search[#](http://nathan.rs/posts/gzip-lm/#generating-by-beam-search)
...
The fix is to look _ahead_ a whole span before committing. `gzipt` runs a [beam search](https://en.wikipedia.org/wiki/Beam_search) over byte sequences. At each step, the current context is:
...
1.   The paper did try this, but it ended up performing poorly. Adding beam search significantly improved generation quality (an idea they mentioned), which is discussed below.[↩︎](http://nathan.rs/posts/gzip-lm/#fnref:1)

2.   The code actually uses zlib instead of spawning a gzip process, but the name GziPT was too good. I believe they both use the same DEFLATE algorithm under the hood.[↩︎](http://nathan.rs/posts/gzip-lm/#fnref:2)
