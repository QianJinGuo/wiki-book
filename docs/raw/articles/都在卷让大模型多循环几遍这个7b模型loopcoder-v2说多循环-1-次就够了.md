...
ingested: 2026-07-01
sha256: 3dac4334f0c878bede3562cb66af54c1be52260731a5d67e1675f364c12e90c1
---
---
source: wechat
source_url: https://mp.weixin.qq.com/s/L_Bmxw44h4PFCQTO68lrpg
...
先看  收益侧  。研究者用三把「探针」——隐状态的演化、注意力的路由、输出分布的变化——同时观察每一次循环到底干了什么，只有三者一致时才下结论。结论是：  第 2 次循环几乎包办了所有「有用的精修」  ——隐状态朝同一方向稳步收敛、注意力重新分配、输出分布明显改善，表征多样性（effective rank）也在这一步达到峰值。再往后，更新越来越小、甚至开始来回震荡（方向反转），注意力路由基本「冻结」，后面的循环近乎空转。

再看  成本侧  。CLP 为了让循环能并行，做了一个取巧：从第 2 次循环起，每个 token 拿到的是「邻居」上一轮的状态，而不是它自己的。这就引入了一个固定的  「  位置错配税  」  ——团队把它量化成一个能直接从模型内部状态算出来的指标 Ω，并发现它在每一次循环几乎恒定不变。
