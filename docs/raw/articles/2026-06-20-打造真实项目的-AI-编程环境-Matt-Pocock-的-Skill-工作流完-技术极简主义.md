...
- •
  /grill-me
  ：适合非代码类决策，让 Agent 先追问模糊点。

- •
  /grill-with-docs
  ：适合代码场景，一边澄清需求，一边把领域语言和决策沉淀到文档里。
...
- •
  /to-issues
  ：把任务拆成端到端可验收的垂直切片，而不是按文件拆活。

- •
  /zoom-out
  ：进入陌生模块时，让 Agent 先站在系统层面解释代码。

- •
  /improve-codebase-architecture
  ：周期性扫描项目，找可以修剪、加深、收敛的模块。
...
- 要安装到哪个 Agent，比如 Claude Code、Cursor 或其他兼容环境。
...
推荐阅读：  深入理解 AI Agent 时代的驾驭工程：Harness Engineering