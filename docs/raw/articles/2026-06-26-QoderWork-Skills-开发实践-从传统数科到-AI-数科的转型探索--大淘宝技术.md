...
type: raw-article
tags: [raw, article]
sha256: 57c110e070b8f25f2cbfa8c5be18e007438f4b2b6e51c3193e3de33ba7525cfd
---
...
数科日常工作中有大量
重复但非标准化
的分析任务：每次做用户洞察要重新写分析代码，每次 AB 实验要手动跑一遍检验流程(如果没有接通天塔看板)，每次给产运出报告要重新排版。这些工作耗时但附加值有限。
...
- 反模式清单。 第 19-35 行不只说“要做什么”，还显式列出“不要做什么”——overused fonts（Inter、Roboto、Arial）、cliched color schemes（purple gradients on white）、predictable layouts。这是 给 AI 设置负向约束 ，因为 大模型的“模式坍缩”倾向会让它总是输出最高频的组合，必须显式阻断 。
...
- 对比 follow-builders：它需要 config 是因为用户的追踪偏好（语言、频率、投递方式）是 跨会话持久化 的。而 frontend-slides 每次生成演示文稿都是 独立的一次性任务 ，用完即走，天然不需要持久化配置。
...
- 没有 references/ 子目录，4 个知识文件直接和 SKILL.md 平级放在根目录——因为 Markdown 的 file 链接语法 在同级目录最简洁，减少路径出错的可能。
...
Skill 不是一个独立的工具或应用，而是
给 AI Agent 的一份"领域专家手册"
。可以把它理解为：
...
所有和具体业务相关的值（实验名称、指标列表、字段名）都不应该写死在 config 里，而是用
  auto
  或空值占位，运行时由检测逻辑或用户确认来填充。
