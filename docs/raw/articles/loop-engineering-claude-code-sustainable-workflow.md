...
type: raw-article
tags: [raw, article]
sha256: 991b891e2ad96cd862f0c83d555a68cfed75a5e0b67bffa42617ebb9ac3fadd1
---

sha256: 887e52358418d3b7b8037d10d3b97acd4760dc33268c7519b91062ece499c58a
---
source: wechat
source_url: https://mp.weixin.qq.com/s/cyAIYsdsPwWegGlt8D5ldQ
ingested: 2026-07-01
feed_name: 技术极简主义
...
在上一篇文章《 [ 一文看懂 AI 编程智能体工程化新范式：Loop Engineering ](<https://mp.weixin.qq.com/s?__biz=MjM5NzA1NzMyOQ==&mid=2247487109&idx=1&sn=e9a30d826ac13e32f2737e2569aa438e&scene=21#wechat_redirect>) 》中，我们介绍了 Loop Engineering 的核心设计理念。接下来，结合实际工程案例，看看它是如何落地应用的。
...
在 AI 智能体的圈子里，Loop Engineering 这个术语突然被广泛讨论。
...
你不需要在每个步骤点「继续」。Agent 自己跑完整条轨迹。就好像一个人反复修改自己的稿子，改到满意才交出来。这是最入门的形态，但已经能解放重复的手动操作。
...
核心风险  |  预算不设限的团队才玩得起；标准模糊时变成 slop machine  |  创造力被限制在框架内
...
Loop Engineering 不是凭空冒出来的概念。它扎根于两个重要的研究脉络。

###  ReAct：推理 + 行动

来自 Princeton 和 Google 联合研究的 ReAct 模式，是一种  ** 推理与行动交替进行  ** 的架构。
...
Reflexion 是 ReAct 的进阶版本，核心模式是  ** 把失败口头化，变成下一次尝试的依据  ** 。
...
「重复执行同一任务」的概念已经被直接内置到了 Claude Code 中。不需要从零造轮子。

命令  |  触发方式  |  适用场景  |  版本要求
---|---|---|---
` /loop  ` |  时间驱动，定时重复  |  轮询检查类任务（如「每 5 分钟检查一次构建状态」）  |  —
` /goal  ` |  目标驱动，持续工作直到满足条件  |  有明确完成标准的任务（如「修复所有失败的测试」）  |  v2.1.139（2026.5.11）
Dynamic Workflows  |  AI 自己编写工作流程，拆分为数十到数百个 Sub-agent 并行执行  |  超大规模复杂任务编排，通过  ` ultracode  ` 关键词触发  |  v2.1.154（2026.5.28）

三者的共同点：减少了「人给指令 → 看结果 → 再给指令」的来回拉扯。你把条件和流程提前设定好，剩下的 AI 自己搞定。

从  ` /goal  ` 开始试，最不容易出错。

##  Loop 的双层结构：Inner Loop 与 Outer Loop

Loop 有两层。理解这个区分，Loop Engineering 才算入了门。
...
关键点：强 Agent 和弱 Agent 用的是完全相同的工具基础设施。区别在于模型是否「选择」了去调用那个验证工具。
...
多个会话之间，Agent 从过去的经历中学习。
    ...
    Session 1：处理分页逻辑时失败了  
      ↓  
    把「分页最佳实践」记入 SKILL.md  
      ↓  
    Session 2：遇到同类任务  
      ↓  
    读取 SKILL.md，直接跳过上次踩的坑
...
Addy Osmani 总结了一个可运转的 Loop 由五个组件加记忆构成。这是搭建 Loop 的工具箱。
...
把「这个项目怎么写代码」这件事沉淀为可复用文件，不用每次从头解释：
...
Agent 从「给你建议」变成了「参与流程」。它可以自己看 Issue、自己发 PR、自己通知 Slack 频道。
...
循环的生命线。通过  ** 在对话之外持久化状态  ** ，突破上下文窗口的限制。
...
理论讲了这么多，不如动手搭一个。以「每天早上 6 点自动检查新 Issue、修复、发 PR」为例。
...
* •  ** 调整迭代次数  ** ：从  ` MAX_ITERATIONS=3  ` 起步，观察实际需求后再放宽
* •  ** 增加特定 Issue 类型的 Skill  ** ：比如常见的安全修复可以写一个  ` security-fix.md  ` skill，提高修复质量
* •  ** 设置告警通知  ** ：在  ` done  ` 循环里最后加一句 Slack webhook，完成后通知你
...
1. 1\.  ** 验证通过停止  ** ——所有测试绿灯，lint 零告警
2. 2\.  ** 达到上限停止  ** ——迭代次数到上限，自动挂起并通知人类
3. 3\.  ** 标记挂起  ** ——Agent 判断当前任务超出能力范围，将 Issue 标记为  ` needs-human  ` 并附上已完成的分析
...
Loop 跑起来之后，你最容易犯的错误就是「不管了，让它在后台跑就是了」。但 Loop 是杠杆——

** 它能放大一个工程师的判断力，也能放大一个工程师的缺席。  **

每隔一段时间，Review 一下你的 Loop 的产出。看它做的决策对不对，看它的经验教训写得有没有价值。设计 Loop 的人，仍然是最终对输出质量负责的人。

##  结语

从 2024 年大家琢磨「怎么写好 Prompt」，到 2025 年开始「同时跑多个 Agent」，再到 2026 年认真讨论「设计能自己转的循环」——每一年，人跟 AI 协作的方式都在往深里走一步。
