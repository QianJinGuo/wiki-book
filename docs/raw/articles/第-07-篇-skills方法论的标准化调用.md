...
source: wechat
url: https://mp.weixin.qq.com/s/9L7MXM2aNMbJ0UkcVI-uwg
...
它和提示词最大的区别就是 它是按需注入的 ，如果你把"怎么写日报"这段方法论塞到系统提示词里，那每次对话，哪怕用户问"你叫什么名字"，它都会被加载。模型看到一堆跟当前问题无关的指令，反而会被干扰。

如果你把它当作技能的话，这个提示词不会注入到每轮对话中，当用户需要完成写日报这种任务的时候，这个技能的提示词才会加载到给模型，让模型根据技能的提示词来完成任务。
