...
sha256: 78ddcf30d197f4e2bfb81cdbc95b28ca69b4dace208dffc20f3dd3919c90418e
---
...
**评估亮点** ：
  ...
  * **OfficeQA 主任务** ：基于美国财政部公告（89K 页扫描财务文档）的复杂数值推理任务。基线准确率 **60.6% → 进化后 67.9%（+7.3pp）** ，主要靠两条自学到的 Skill 撑场：
    *  _Data extraction validation_  —— 解决了之前表格解析时常见的"单元格错位"问题；
    *  _Quantitative analysis with checkpoints_  —— 在金融数值计算环节强制加入校验点。
  * **跨任务迁移** ：在 SealQA 上学到的 _search persistence protocol_ （搜索坚持协议）直接挪到 BrowseComp 任务上，**无需重新训练** 就带来 +5.3pp 的提升——这点跟后面 CoEvoSkills 的"transfer 不如自己 self-evo"结论形成有趣对比，说明**只要 Skill 抽象得足够通用，是可以跨任务迁移的** ，但前提是抽象层级要选对。
