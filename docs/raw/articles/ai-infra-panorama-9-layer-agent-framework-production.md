---
type: source-card
title: "AI Infra 全景图：Agent Framework、调度、编排、沙箱、记忆管理、Tracing 分层拆解"
source: "wechat"
author: "Knock"
source_url: "https://mp.weixin.qq.com/s/dmlwqGylzG0eQVUlcZocUQ"
published: "2026-06-30"
collected: "2026-07-02"
license: "未发现可验证的再发布许可证；本仓库仅保留来源卡片"
---

# AI Infra 全景图：Agent Framework、调度、编排、沙箱、记忆管理、Tracing 分层拆解

## 原创摘要

这份来源卡片记录一篇围绕“AI Infra 全景图：Agent Framework、调度、编排、沙箱、记忆管理、Tracing 分层拆解”的第三方资料，主题标签为ai-infra、agent-framework、production。完整事实、论据、上下文与原文请以原始来源为准；公开仓库不保存正文副本。

> 公开版仅保留来源信息和原创摘要，不替代原始来源的阅读。
