...
source: wechat
url: https://mp.weixin.qq.com/s/4fLgnwhIuJe76dX89Mv16A
...
HNSW 作为 最为流行的 ANN 算法之一，在机构测评和工程实现上取得了广泛的认可和验证。 目前 AliSQL 优先支持了基于 HNSW 算法的向量索引，向量搜索总体架构如下图所示。
...
【输入】目标节点 q 、返回结果数 ef 、候选集 C （包括 1 个或多个该层的节点）、结果集 W （初始为空）。

- 

【循环搜索】从候选集 C 中取出距离 q 最近的节点 c ，遍历其每个邻居 e ，若 e 未被访问且距离更优，则加入结果集 W ，也加入候选集 C ，重复此过程。

- 

【终止条件】当候选集 C 中最近距离的节点 c 还没有结果集 W 中最远距离的节点 f 的距离近，即可结束循环搜索。

- 

【输出】结果集 W 前 ef 个最近邻节点。
...
【搜索 L ~ l+1 层】从 HNSW 图的最高层 L 的随机节点开始，执行 ef 为 1 的 Search Layer 算法，即通过 NSW 图贪心搜索到离 q 局部最近的节点作为低一层的起点，直到 l+1 层完成搜索。

- 

【插入 l ~ 0 层】以 l+1 层找到的最近邻作为 l 层的起点，执行 ef 为 M 的 Search Layer 算法，在该层插入节点 q 并将其与搜索到的邻居节点进行双向连接，直到第 0 层，第 0 层不同于其他高层使用 ef=2M 。

- 

【邻居 shrink】对插入层的所有邻居节点，需要判断连接数若超过 M （第 0 层为 2M ），则通过 shrink 操作移除最远连接，确保图的紧凑性。
...
【快速跳转】从 L 到第 1 层执行 ef 为 1 的 Search Layer 算法。

- 

【精确搜索】在第 0 层搜索到 K 个最近邻返回。
...
gref 表示 graph ref 即辅助表主键，用于邻居节点的搜索，使用 InnoDB 的系统列 ROW_ID。

- 

layer 上建立一个 KEY，用于快速定位整张图的入口节点即 layer 最大的节点。

- 

tref 表示 table ref 即原表主键，用于向量搜索完成后进行回表，使用 server 层的存储格式。假设主表主键是一个值为 1 的 int 列，在 InnoDB 引擎层存储为 80 00 00 01，辅助表则使用 server 层的存储格式，将其存储成 01 00 00 00 的小端序 BINARY 列。

- 

vec 辅助表上的向量使用半精度 int16 格式存储，精度转换后，还需要记录转换的缩放因子 scale，存储为 float32 位于首 4 字节，该转换过程可以牺牲一定精度，有效降低存储空间并提升搜索效率。

- 

neighbors 包括节点从第 0 到最高层的每一层的邻居，每一层存储 1 字节邻居数量和每个邻居的 gref，算法流转时就可以在同层邻居之间进行导航搜索了。
...
3.3 DD 适配和 DDL 原子性保证

不同于普通表，向量索引辅助表对用户不可见，无法直接访问，与主表之间的关系如下图所示。

- 辅助表名使用主表的 table id 拼写为 vidx__00，主表可以利用辅助表名从数据字典（DD）找到辅助表的元数据信息。
...
通过辅助表 DD 构建辅助表的 TABLE_SHARE 并挂在主表的 hlindex 指针下，辅助表的还会创建一个公共的 Nodes Cache 通过指针 hlindex_data 访问。
...
辅助表的 TABLE_SHARE 不进入 table cache 中，随主表一起关闭。
