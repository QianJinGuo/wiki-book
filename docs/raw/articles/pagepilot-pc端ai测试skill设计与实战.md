...
sha256: 48c10aa15e2548878f7fca2ababb0266a11583bae591b05cba02bffc4f25d4ef
---
...
其次| components/| 平台级共享组件，所有业务通用| 平台提供的标准 category-selector.md，只做了基础 Cascader 逐级展开  
未命中| 无匹配| 标记 unmatched，提示用户新建| 新业务"棋盘密云"有个"私域用户筛选"操作，目前两个目录都没有对应组件  
...
---|---|---|---  
Phase 0| 执行环境 precheck| ODC MCP 连通 + yuque CLI 可用 + allowlist 配好| ODC 不通不执行——连数据库都查不了，跑下去验证也没意义  
Phase 1| 浏览器自动化执行| 登录成功 + 页面已到达| 登录失败不填表——没登录进去，后面填表点按钮都是空操作  
Phase 2| URL 提取 & ID 解析| 从 URL/拦截器拿到 brandId 或 astoreId| 无 ID 不查 DB——不知道查哪条记录，DB 查询无从下手  
Phase 3| 数据库查询| SQL 返回结果（至少 1 行）| SQL 无结果不比对——空结果比对什么？不是"比对失败"，是"没东西可比"  
...
第四波| 47 条 / 157 轮| 多轮对话| 89.2%| 发现 64 个问题（3类大问题）响应慢，无响应、响应空  
