...
这个转变的本质是：**从"提示"（一次性指令）转向"编排"（持续优化循环）**。这与 [Harness Engineering](../ch05/057-harness-engineering.html) 中关于"Agent 运行时"的理念完全一致——模型不再是独立的推理引擎，而是嵌入在一个包含反馈、记忆、验证的闭环系统中。
...
- [Harness Engineering Survey 2026](../ch05/057-harness-engineering.html) — Harness Engineering 总览
