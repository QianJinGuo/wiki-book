...
## Ch01.1257 淘宝内容生态：GrowBrain - 淘宝Agentic内容成长引擎
...
这两个问题在本质上反映了 Agent 架构设计中一个尚未被充分认知的原则：**LLM 的职责范围应当与其模型能力相匹配**。小模型的幻觉风险要求从「LLM 全程驱动」转向「框架精确控制 + LLM 在关键节点介入」。这与 [Agent 配置组合](../ch04/270-skill.html) 中「Skill 作为可控边界」的设计理念形成互文。
...
这三个设计原则构成了一个通用范式：**Agent 架构的「无状态化」是生产级部署的前提**。这与 [Agent Harness 上下文管理](../ch05/058-agent-harness.html) 中的工作集隔离设计不谋而合。
...
这种「职责分离 + 能力互补」的子 Agent 设计模式，与 [Marvis Multi-Agent 架构](../ch03/087-marvis.html) 中的多 Agent 分工协作理念高度一致，代表了生产级 Agent 系统的标准设计范式。
...
- [Agent 配置组合](../ch04/270-skill.html)
- [Agent Harness 上下文管理](../ch05/058-agent-harness.html)
- [Marvis 多智能体桌面助手](../ch03/087-marvis.html)
- [AgentCore Trip Allocation](../ch04/670-agentcore-harness.html)
- [Agent 评测方法论](../ch03/035-agent.html)
- [Harness Engineering Survey 2026](../ch05/118-harness-engineering.html)
- [Alibaba Agentic Cloud](../ch04/342-agentic-cloud.html)
