...
## Ch01.599 Anthropic Claude Managed Agents 平台正式发布
...
- [Anthropic 长时运行 Agent 架构：对抗式设计 + 合同谈判 + 审美量化](https://github.com/QianJinGuo/wiki-public/blob/main/entities/anthropic-long-running-agent-adversarial-architecture.md) — 另一篇技术解读，覆盖 API 细节和定价
...
VibeCoder 对 Claude Managed Agents 更新（scheduled deployments + vault env vars）的架构解读，提出「脑手分离」四层模型，并给出落地设计路径。原文：VibeCoder 原文存档
