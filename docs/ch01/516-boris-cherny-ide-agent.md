...
这与 [Harness Engineering Core Patterns](ch03/084-claude-code.html) 中"控制面应当外置给人类"的工程原则一致——Human-in-the-Loop 不是降级方案，而是 Agent 系统设计的核心约束。
...
这与 [Claude Code Harness 深度解析](ch03/084-claude-code.html) 中的 harness 设计哲学一致——harness 的核心价值就是为 Agent 提供持续运行的工程环境，而非一次性对话界面。
...
这与 [Karpathy Vibe Coding 到 Agentic Engineering](ch03/035-agent.html) 中的论述相互印证——"Vibe Coding"是 Agent 时代的临时形态，"Agentic Engineering"才是稳定形态。
...
- [深入理解 Claude Code Agent Harness 构建之道](ch03/084-claude-code.html)
