...
## Ch01.377 Noam Brown：推理预算应成为AI评估的基础变量

> 📊 Level ⭐⭐ | 10.8KB | `entities/noam-brown-ai-evaluation-reasoning-budget-performance-cost-curve.md`
...
```mermaid
mindmap
  root(("Noam Brown：推理预算应成为AI评估的基础变量"))
    一句话
    核心论点
      问题1: 传统成绩表低估新模型能力差距
      问题2: 性能平台期可能根本测不到
    ...
    深度分析
      1. 推理预算-性能的成本曲线
      2. '推理计算'是新的优化维度
      3. Reasoning budget 作为系…
      补充：推理档位工程机制（Sebastian R…
    实践启示
      1. 评估模型时画 performance-c…
      2. Agent 系统设计：推理预算可配置
      3. Test-time compute 是降…
```
...
- [Claude Code 深度解析](../ch03/078-claude-code.html) — agentic 系统的性能—成本权衡
...
Sebastian Raschka 从训练管线角度补充了推理预算控制的工程实现：**推理档位本质上是通过 system prompt + 长度为惩罚 + SFT 对齐实现的，而非模型内部的特殊开关**。
...
```
ChatGPT/Codex 界面的选择器只是把选择映射成这句话。
...
两条路可组合使用（作者推测 gpt-oss / GPT-5.6 即如此）。
...
1. **`<think>` 标签是装饰性的**：对推理能力无贡献，仅标记草稿起止供 UI 折叠。DeepSeek-R1 的 `R_format` 只是一条简单格式规则检查。
2. **档位边际递减**：最高档收益明显变小，中档是精度/成本/延迟甜点区。
3. **模型选择 vs 档位调节是两个正交 scaling 轴**：训练 scaling（换模型）和推理 scaling（调档位）的曲线会重叠——小模型开高档可追平大模型低档。
4. **"Reasoning Effort" 提示词不可跨模型复用**：DeepSeek V4 的 Think Max 提示词背后有专门训练支撑，照搬到其他模型无效。
5. **趋势**：近期档位仍会是显式 system prompt 输入，但 Agent harness 会越来越多地根据任务状态自动推断档位，同时保留手动覆盖。
