...
## Ch01.1105 BrowserBC：人类轨迹蒸馏为可复用技能，让小模型获得大模型的网页操作能力
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/browserbc-human-trajectory-skill-distillation-quantumbit-2026.md)

## 问题：Web Agent 的"从零摸索"

当前 Web Agent 不缺操作能力（能点击、输入、跳转），缺的是**每到陌生网站都要从零摸索**。摸索容易陷入循环导航、路径漂移、提前收手。更关键的是：这次摸索的经验随着对话一起蒸发，下次同类任务还要重踩同样的坑。
...
**关键设计**：只保留可迁移的过程性知识，剥离会变/会泄露的细节。"按语义标签找到字段、填入值、提交后确认成功状态"，而不是"点 (x,y)、再点那个 id 是某串字符的按钮"。

### 技能图管理

库组织为技能图，每个候选技能判断：新增 / 合并 / 特化。节点是技能，边是关系（时间依赖、特化、替代、互斥）。效果：重复演示合并为可复用节点；检索和更新只动相关局部；增量精炼只更新受影响的技能及邻居。
...
- [Autobrowse：浏览器 Agent 的失忆问题](https://github.com/QianJinGuo/wiki/blob/main/entities/autobrowse-browser-agent-persistent-skills-sense-ai.md) — 持久化探索 vs 人类轨迹蒸馏，互补方案
- [Browser Use：为 Agent 构建 Runtime Harness](https://github.com/QianJinGuo/wiki/blob/main/entities/browser-use-runtime-harness.md) — 浏览器 Agent 的运行时验证
