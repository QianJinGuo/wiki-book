...
> 📊 Level ⭐ | 11.8KB | `entities/agentic-design-system-from-chatbot-to-orchestration.md`
...
**Chatbot → Agent → Orchestration 的三层演进**^[].md]

作者清晰地勾勒出了 AI 在设计系统中应用的三层演进模型：^[].md]
...
**组件作为契约（Component as Contract）**^[].md]
...
**治理风险的三重挑战**
文章坦诚地列出了 agentic design system 的三大风险：^[].md]
...
**Design Debt 清理是 Agentic 化的前提条件**^[].md]
...
**Figma MCP 是设计系统通往 AI 工作流的桥梁**^[].md]
...
**"Structure is the moat, not the prompt"**^[].md]
