...
## Ch01.952 DREAM：用冻结 LLM 的自回归预测训练稠密检索器，无需标注正负样本
...
```mermaid
mindmap
  root(("DREAM 用冻结 LLM 的自回归预测训练稠密检索器 无需标注正负样本"))
    核心概述
    问题 检索器训练的标注瓶颈
    核心方法
      直觉
...
- [Agentic RL 六框架](../ch04/327-agentic-rl.html) — Agentic search 的检索质量是关键瓶颈
