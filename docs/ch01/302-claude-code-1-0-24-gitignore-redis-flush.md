...
## Ch01.302 Claude Code 1.0.24 工具调用安全事故：静默删 .gitignore 与 Redis flush 复盘
...
- [Claude Code Security Review Bias Brainoverflow 2026 06](ch04/277-ai.md)：Claude Code 安全审查的 model anchoring bias 实证分析（同一厂商但不同维度——本文是"agent 自身行为越界"，彼文是"agent 审查盲点"）
- [Skill Issues Compromising Claude Code With Malicious Skills Agents Part 1](ch04/255-skill.md)：Skill 安全（恶意 skill 投毒视角，与本文"agent 自主行为越界"形成互补）
...
- [Skill Hub Organization Asset Winty](ch04/255-skill.md)：Skill 治理与生命周期（含 Skill review 流程的具体设计）
