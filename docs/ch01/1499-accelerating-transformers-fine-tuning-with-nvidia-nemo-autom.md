...
NeMo AutoModel 保存的 checkpoint 是标准 HF 格式的 safetensors，可直接被 vLLM 和 [SGLang](../ch04/714-agent-assisted-sglang-ai-llm.html) 等推理框架加载。这意味着训练和推理之间没有格式转换开销——微调完成后可直接部署。
...
- vLLM — 高性能 LLM 推理引擎
- [SGLang](../ch04/714-agent-assisted-sglang-ai-llm.html) — 结构化生成推理框架
