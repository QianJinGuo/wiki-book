...
## Ch01.274 三器合一（Comet + OpenSpec + Superpowers）：用文件系统给 AI 编程上工程纪律 — 术哥源码级深度分析（WHAT/HOW/WHEN-NEXT 分工 + 9 平台 Hook + 上下文压缩 + 多 Agent 协作 + Shell+YAML 工程取舍）
...
- **vs [三器合一：gstack + Superpowers + OpenSpec](../ch05/081-ai-coding.md)**（AgentBuff, 2026-05-12）：第三器不同（gstack vs Comet），视角也不同（AgentBuff 偏「HARD-GATE 串联」+「TDD 与 /review 互补」，术哥偏「源码机制 + 平台适配 + 工程取舍」）。术哥文章更深入到 Shell 脚本 + YAML 状态机 + 9 平台 Hook + SHA256 追踪的具体实现
- **vs [Superpowers 深度拆解](../ch04/070-ai.md)**（开源圆桌）：单项目深度。本实体是三项目系统视角
- **vs [Hermes Agent Skills 源码分析](../ch04/395-hermes-agent-skills.md)**（术哥本人）：术哥对 Hermes 的源码分析风格一致（57 行 Shell、Markdown 为主、平台适配）。本实体是术哥对外部三项目（Comet/OpenSpec/Superpowers）的源码分析
- **vs [OpenSpec 规范驱动开发 SDD](../ch05/047-openspec.md)**：OpenSpec 单项目视角（Trae 原生支持）。本实体是三项目流水线视角
...
→ [Three Tools In One Gstack Superpowers Openspec Engineering Ai Coding](../ch05/081-ai-coding.md) — 同期同主题：gstack 作为第三器的视角
→ [Superpowers Deep Dive Kaiyuandakashuo](../ch04/070-ai.md) — Superpowers 单项目深度
→ [Openspec Spec Driven Development Trae Solo](../ch05/047-openspec.md) — OpenSpec 单项目深度
→ [Openspec 四步法深度复盘 流程完整不等于代码正确](../ch05/047-openspec.md) — OpenSpec 四步法实战复盘
→ [Ai Production Development Workflow Openspec Superpowers Gstack](../ch05/047-openspec.md) — gstack + OpenSpec + Superpowers 生产级实践
→ [Claude Code Skills Superpowers Practice](ch01/508-claude-code-skills-superpowers.md) — Claude Code + Superpowers 实践
→ [Hermes Agent Skills Source Code Analysis Shuge](../ch04/395-hermes-agent-skills.md) — 术哥本人对 Hermes Agent Skills 的源码分析（方法论参照）
→ [缝合怪识别与减法决策论：OpenSpec + Superpowers 融合方案下线记](../ch05/047-openspec.md) — 同期同作者 24h 后反思
