...
## Ch01.206 Trail of Bits: Skill Scanner Bypass 实证研究
...
### 与 [Skillclaw](ch04/271-skill.html) 的关系
...
### 与 [Agent Security Three Step Sequence Harness Governance Identity Crewai](ch05/094-ai.html) 的关系
...
1. **scanner 是必要不充分的防线**：单一 scanner 无法应对双载体攻击（自然语言 + 代码）。必须将 scanner 与 runtime sandbox、publisher reputation、audit trail 结合使用，形成纵深防御。参考 [Ai Agents Security Survey Attack Defense](ch05/094-ai.html) 中的多层防御模型。
...
5. **企业内 skill hub + 代码审计**：相比公共 marketplace，企业自建 skill hub 并对 skill 进行代码审计是更可靠的安全模型。可参考 [Skillsieve Agent Skill Security](ch04/271-skill.html) 中的企业级 skill 安全实践。

### 对 Agent 系统设计者

1. **将 skill 视为不受信的外部代码**：类似浏览器对第三方脚本的隔离策略，agent 系统应对每个 skill 应用最小权限原则，限制其对文件系统、网络、凭据的访问范围。

2. **建立 skill 的 provenance tracking**：记录每个 skill 的来源、版本、安装时间，并在 agent 运行时可查询。[Agent Security Three Step Sequence Harness Governance Identity Crewai](ch05/094-ai.html) 中的 identity 和 governance 框架可用于实现这一点。
...
- [Skillsieve Agent Skill Security](ch04/271-skill.html) — 企业级 skill 安全实践
- [Agent Security Three Step Sequence Harness Governance Identity Crewai](ch05/094-ai.html) — agent 安全的通用框架
