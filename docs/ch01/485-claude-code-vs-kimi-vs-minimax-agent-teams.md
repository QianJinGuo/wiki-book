...
## Ch01.485 Claude Code vs Kimi vs MiniMax：Agent Teams 到底拼的是什么？
...
这些问题的根因不在模型智力，而在缺乏系统化的控制平面——这正是 [Harness Engineering](../ch05/112-harness-engineering.html) 要解决的核心问题。
...
这与 [Claude Code 架构](../ch03/077-claude-code.html)中强调的「工程化协作」理念一致。限制在于：token 成本随 teammate 数量上涨，适合独立工作流并行，不适合同一个文件多人同时编辑。
...
- [Claude Code 架构分析](../ch03/077-claude-code.html) — Claude Code 的深层架构拆解
- [Harness Engineering](../ch05/112-harness-engineering.html) — Agent 控制平面的系统方法论
- [Agent Orchestration](../ch04/511-agent-orchestration.html) — 多 agent 编排模式
- [Kimi K2 Agent Swarm](ch01/400-kimi.html) — Kimi 并行 agent 方案的技术细节
