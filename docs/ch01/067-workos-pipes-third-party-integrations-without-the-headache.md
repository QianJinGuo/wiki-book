...
## Ch01.067 WorkOS Pipes: Third-party integrations without the headache
...
**OAuth Provider vs Pipes 的选择决策树：**^[].md]
...
### 共享凭证的使用策略
开发阶段充分利用共享凭证快速启动，但需提前规划向生产凭证的迁移路径：^[].md]
...
### 安全注意事项
虽然 WorkOS 负责 token 加密存储，但应用层仍需关注：^[].md]
...
→ [[raw/articles/workos-pipes-third-party-integrations|原文存档]
