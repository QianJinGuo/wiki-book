...
## Ch01.1250 万字详解 codex 全链路架构：Codex 不是一个 App，而是一套 Agent Harness Runtime

> 📊 Level ⭐⭐⭐ | 10.5KB | `entities/万字详解-codex-全链路架构codex-不是一个-app而是一套-agent-harness-runtime.md`
...
```mermaid
mindmap
  root(("万字详解 codex 全链路架构 Codex 不是一个 App"))
    Codex 产品形态全景
    App Server 关键底座
    三层会话模型
      Thread 线程
    ...
    沙箱与权限体系
    多端集成策略
    端是入口 Harness 才是身体多端统一架构的战略意义
    App Server 协议面的设计哲学 从聊天 API到任务运行时
    沙箱设计 权限分层而非权限开关
```
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/万字详解-codex-全链路架构codex-不是一个-app而是一套-agent-harness-runtime.md)
