...
这个转变的本质是：**从"提示"（一次性指令）转向"编排"（持续优化循环）**。这与 [Harness Engineering](../ch05/096-harness-engineering.html) 中关于"Agent 运行时"的理念完全一致——模型不再是独立的推理引擎，而是嵌入在一个包含反馈、记忆、验证的闭环系统中。
...
这个差异说明 Fable 5 不仅"更聪明"，而且**更有策略性**——它能跳出局部最优的思维陷阱，进行更高层面的探索。这与 [DeepSeek V3 推理模型架构](ch01/1456-moe-architecture.html) 中关于推理链长度与探索质量的关系研究有相通之处。
...
- [Harness Engineering Survey 2026](../ch05/096-harness-engineering.html) — Harness Engineering 总览
