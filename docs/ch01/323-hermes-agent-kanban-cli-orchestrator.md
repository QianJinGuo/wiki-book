...
## Ch01.323 Hermes-Agent 官方 Kanban 深度实测：让商业 CLI 工具当 Orchestrator
...
这种分层架构的优势在于：商业 CLI 工具（如 [Claude Code](ch03/073-claude-code.md)）具备深度推理和子代理分发能力，适合做认知层；而 Hermes-Agent 的 Kanban 系统提供稳定的多 worker 并发执行能力，适合做执行层。
...
- [Hermes Agent Goal And Kanban](ch04/503-agent.md) — Hermes Agent 基础教程（/goal + Kanban）
