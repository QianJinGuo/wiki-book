...
## Ch01.750 Claude Code Harness 深度分析
...
5. **在模型 API 层面防御蒸馏**：如果你的产品输出包含高价值推理过程，考虑实现 connector-text 摘要+签名机制，使外部调用无法获取完整推理链。这在 [Agent Harness Context Management Working Set](../ch05/057-agent-harness.html) 的安全设计中有类似体现 。
