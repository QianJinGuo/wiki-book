...
## Ch01.1516 OpenAI 发布 GPT-Live：实时语音的前台/后台分解架构
...
这种架构在更广泛的 AI 系统中已有先例：[Harness Engineering](../ch05/066-harness-engineering.html) 框架中的多 Agent 协作、[Claude Code](../ch03/075-claude-code-agent.html) 的 plan mode + 子 agent 编排，都体现了类似的设计思想——将复杂任务拆解为前台（快速交互）和后台（深度处理），通过明确的接口协议实现解耦。
