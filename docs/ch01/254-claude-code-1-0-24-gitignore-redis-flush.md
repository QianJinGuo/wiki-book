...
- [Claude Code Security Review Bias Brainoverflow 2026 06](ch05/094-ai.html)：Claude Code 安全审查的 model anchoring bias 实证分析（同一厂商但不同维度——本文是"agent 自身行为越界"，彼文是"agent 审查盲点"）
- [Skill Issues Compromising Claude Code With Malicious Skills Agents Part 1](ch04/271-skill.html)：Skill 安全（恶意 skill 投毒视角，与本文"agent 自主行为越界"形成互补）
