...
在 [Amazon Bedrock AgentCore](../ch04/506-amazon-bedrock-agentcore-harness-ga-api-agent.html) 中，可以将 OpenAI 模型配置为 Agent 的推理引擎，实现完整的工具调用、记忆管理和多步推理：
...
[OpenAI Codex on Bedrock](../ch11/162-amazon-bedrock.html) 可直接使用 Bedrock 作为推理运行时，Codex 的编码 Agent 能力与 Bedrock 的安全基础设施深度整合——适合需要企业级审计和合规的软件开发场景。
...
[Bedrock 模型选择器](../ch11/173-simplify-model-selection-in-amazon-bedrock-with-the-open-sou.html)可以帮助团队基于任务复杂度自动选择最优模型。结合 AWS 的成本管理工具，可实现：
- 按团队/项目维度的模型调用成本拆分
- 月度预算预警与自动限流
- 模型调用量的可视化和趋势分析
...
通过 [Bedrock AgentCore 安全加固](../ch11/260-securing-amazon-bedrock-agentcore-runtime-with-aws-waf.html)，可以为 OpenAI 模型的调用叠加多层防护：
...
Bedrock 的一大优势是可以在同一平台上混合使用 OpenAI 和 [Anthropic Claude](ch01/971-anthropic.html) 模型。典型场景：
...
- [OpenAI models and Codex on Amazon Bedrock - GA](../ch11/162-amazon-bedrock.html)
- [Amazon Bedrock AgentCore Harness GA](../ch04/506-amazon-bedrock-agentcore-harness-ga-api-agent.html)
- [AWS Bedrock Multi-Agent Collaboration Guide](../ch11/009-aws-bedrock.html)
- [Securing Bedrock AgentCore with AWS WAF](../ch11/260-securing-amazon-bedrock-agentcore-runtime-with-aws-waf.html)
...
- [Deep Agents on Bedrock AgentCore](../ch04/508-agent-orchestration.html)
