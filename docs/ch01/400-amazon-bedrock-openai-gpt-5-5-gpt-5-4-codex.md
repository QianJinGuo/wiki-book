...
在 [Amazon Bedrock AgentCore](../ch04/505-amazon-bedrock-agentcore-harness-ga-api-agent.html) 中，可以将 OpenAI 模型配置为 Agent 的推理引擎，实现完整的工具调用、记忆管理和多步推理：
...
[Bedrock 模型选择器](../ch11/248-simplify-model-selection-in-amazon-bedrock-with-the-open-sou.html)可以帮助团队基于任务复杂度自动选择最优模型。结合 AWS 的成本管理工具，可实现：
- 按团队/项目维度的模型调用成本拆分
- 月度预算预警与自动限流
- 模型调用量的可视化和趋势分析
...
Bedrock 的一大优势是可以在同一平台上混合使用 OpenAI 和 [Anthropic Claude](ch01/963-anthropic.html) 模型。典型场景：
...
- [Amazon Bedrock AgentCore Harness GA](../ch04/505-amazon-bedrock-agentcore-harness-ga-api-agent.html)
...
- [Deep Agents on Bedrock AgentCore](../ch04/507-agent-orchestration.html)
