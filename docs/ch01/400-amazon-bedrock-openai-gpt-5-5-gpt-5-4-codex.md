...
[Bedrock 模型选择器](../ch11/172-simplify-model-selection-in-amazon-bedrock-with-the-open-sou.html)可以帮助团队基于任务复杂度自动选择最优模型。结合 AWS 的成本管理工具，可实现：
- 按团队/项目维度的模型调用成本拆分
- 月度预算预警与自动限流
- 模型调用量的可视化和趋势分析
...
Bedrock 的一大优势是可以在同一平台上混合使用 OpenAI 和 [Anthropic Claude](ch01/969-anthropic.html) 模型。典型场景：
