...
## Ch01.648 Gemini 深度导读生成器 Prompt：让 AI 重写而非摘要
...
这条约束反映了一个深层原则：导读的"伪装"是"完整阅读"。一旦在输出中暴露"这是 AI 生成的摘要"，读者会自动降级信任，进入"挑刺模式"而非"理解模式"。这与 [Qy_Zacztcs1Ql3Bifmbmgg](ch01/334-1.html) 中"Subagent 隔离探索过程"的思路异曲同工——表面越像完整阅读，认知负担越低。
