...
## Ch01.826 Direct Connect (DX) 迁移最佳实践
...
部署多条 DX 专线成本较高，因此使用 Site-to-Site VPN 作为 DX 冗余备份是兼顾高可用与成本的方案。架构上由 VGW 终结 DX，第三方 VPN 软件（vCPE）承担 Site-to-Site VPN 功能（相关落地可参考 [AWS IPsec VPN 实施](../ch03/084-claude-code.html)）。
...
该场景的迁移步骤为：在新 DX 站点建立第二条连接 → 创建与主连接同类型的 VIF（若主连接关联在 Direct Connect Gateway 上，新 VIF 也应关联同一 DXGW）→ 配置 BGP 路由属性策略 → 关闭主连接 BGP 会话做故障转移测试（演练方法论可参考 [AWS 故障演练实战](../ch11/294-aws-devops-agent.html)）→ 验证后删除原连接。全程强烈建议在维护窗口内执行。
...
- [Direct Connect (DX) 迁移最佳实践（英文实体）](../ch11/047-direct-connect-dx.html)
- [AWS Network Firewall VGW BGP 流量检查](ch01/1181-spec.html)
...
- [AWS Glue 大规模迁移方法论](ch01/437-aws.html)
