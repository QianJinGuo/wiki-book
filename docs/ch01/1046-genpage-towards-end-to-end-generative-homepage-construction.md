...
## Ch01.1046 GenPage: Towards End-to-End Generative Homepage Construction at Netflix
...
GenPage 的训练目标被表述为"最大化用户满意度"（Maximize User Satisfaction），而非传统的离线指标（CTR、时长等）。这一目标的设定意味着模型需要在训练过程中学习用户的综合体验偏好，而不是孤立地优化每个元素的点击率。这与 [AI-Native 架构](../ch05/024-ai-friendly.html) 中"以用户意图为中心"的设计理念一致。
