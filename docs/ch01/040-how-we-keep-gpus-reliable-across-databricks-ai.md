...
Databricks 的实践为大规模 AI 设施运维提供了几个重要参考：(1) 监控体系必须覆盖从硬件传感器到应用层指标的全栈；(2) 静默故障的检测比显式崩溃更重要——这些是最隐蔽的成本漏洞；(3) GPU 的可靠性工程需要专门的工具和知识体系（如 DCGM、NCCL Flight Recorder），不同于传统服务器运维；(4) 故障恢复的自动化程度直接影响训练效率——从检查点恢复的速度和弹性训练的能力是核心竞争力。这些发现与 [Graviton 优化 Agentic RL Sandbox](ch03/045-agent.md) 中讨论的算力成本优化互补——可靠性问题直接影响 GPU 的有效利用率和单位算力成本。
