...
在[Stargate](https://mp.weixin.qq.com/s/I_qra45Jk-uKOI84BNSgnw)（星际之门）之前，OpenAI已与合作伙伴历时数年，共同开发、部署并维护了三代超级计算机。这段经历让OpenAI深刻认识到，要在Stargate这样的规模下高效使用算力，必须重新思考并大幅降低整个技术栈每一层的复杂度，网络设计也不例外。
...
OpenAI的Scaling团队与[AMD、Broadcom、英特尔、微软、英伟达](https://mp.weixin.qq.com/s/I_qra45Jk-uKOI84BNSgnw)合作，历时两年开发出了这个协议。MRC内置于最新的800Gb/s网络接口，可以把单次传输分散到数百条路径上，在微秒级别绕过故障，同时运行更简单的网络控制平面。
...
→ [原文存档](https://mp.weixin.qq.com/s/I_qra45Jk-uKOI84BNSgnw)
