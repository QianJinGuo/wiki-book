...
## Ch01.508 Scenethesis（ICLR 2026）英伟达 & 普渡大学用 Agent 闭环实现文生 3D
...
```mermaid
mindmap
  root(("Scenethesis ICLR 2026 英伟达 普渡大学用"))
    形式化重定义 3D 场景生成不是画图 是搭一个能用的世界
    两条已有路线的失败原因
    Scenethesis 的四阶段 Agent 闭环
    实验结果 空间关系和物理合理性同步改善
    「闭环生成」是 2026 年多模态生成的范式转移
    语言-视觉-物理的三方分工
    「资产库多样性」是真正的开放问题
    对具身智能的关键意义
```
