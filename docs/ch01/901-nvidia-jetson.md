...
如视选择了 Jetson Xavier NX 和 Jetson Orin Nano 两个层级，分别覆盖"移动场景"和"高精度场景"。这种分层硬件选型策略与 [后端 AI 友好标准化路径](ch05/100-ai.html) 中讨论的异构计算分层设计思路一致——不是追求单一硬件覆盖所有场景，而是根据任务需求匹配合适的算力层级。
...
这一模式与 [AI 友好架构](ch05/100-ai.html) 中的"智能端侧"理念吻合——将 AI 能力下沉到数据产生的地方，减少对中心化的依赖。
...
- [后端 AI 友好标准化路径](ch05/100-ai.html) — 异构计算分层设计与硬件选型策略
- [AI 友好架构](ch05/100-ai.html) — 端侧 AI 推理与中心化服务的架构权衡
