...
> 📊 Level ⭐⭐⭐ | 10.2KB | `entities/ai-friendly-architecture.md`
...
mindmap
  root(("AI 友好架构设计"))
    核心理念
      与传统架构的核心区别
    六大设计原则
      1. 可被 Agent 读写理解的接口（Int…
      2. 声明式配置优于命令式（Declarati…
      3. 自描述文档（Self-Describin…
      4. 结构化日志（Structured Log…
    实现层次
      L0：基础可观测性
      L1：接口增强
      L2：Agent 可操作性
...
[Agent 后端架构](../ch03/035-agent.html) 是 AI 友好架构的一种具体实现模式。如果说 AI 友好架构是"设计原则"，Backend for Agent 则是"架构模式"。前者回答"为什么"，后者回答"怎么做"。

### 与阿里技术标准

[阿里技术标准与规范](https://github.com/QianJinGuo/wiki/blob/main/entities/alitech-standards.md) 是 AI 友好架构在企业级落地的具体规范。阿里标准将上述原则转化为可执行的接口规范、日志规范和配置规范，是 AI 友好架构的"实施手册"。

### 与 Harness Engineering

[Harness 正在成为新后端](../ch05/009-harness.html) 是 AI 友好架构的演进方向——当 AI Agent 成为系统的主要使用者时，Harness（编排基础设施）将逐渐取代传统的后端层，成为新的运行时环境。

### 与 AI Friendly 架构设计

[AI Friendly 架构设计：后端系统面向无人值守开发时代的标准与路径](../ch05/022-ai-friendly.html) 提供了更详细的三范式框架（确定性→概率性、结构化→语义化、静态→动态），是这套原则的理论延伸。
...
L0→L4 的渐进式层次不仅是一个规划框架，更是实际落地经验的总结。绝大多数组织的 AI 友好改造在第一阶段（L0-L1）就能产生可衡量的收益——OpenAPI Schema 覆盖率达到 100% 后，Agent 的接口调用成功率从 60% 提升到 95%+。而 L3-L4 的全自主运维在 2026 年的实践中仍然是少数头部科技公司的专属，大部分团队停留在 L1-L2 即可覆盖 80% 的 AI 集成需求。这一发现与 [后端架构 AI Friendly 的标准与路径](../ch05/022-ai-friendly.html) 中的"80/20 法则"一致。
...
- [Agent 后端架构](../ch03/035-agent.html)
- [阿里技术标准与规范](https://github.com/QianJinGuo/wiki/blob/main/entities/alitech-standards.md)
- [AI Friendly 架构设计：后端系统面向无人值守开发时代的标准与路径](../ch05/022-ai-friendly.html)
- [后端架构 AI Friendly 的标准与路径](../ch05/022-ai-friendly.html)
- [Agent 架构关键变化：Harness 正在成为新后端](../ch05/009-harness.html)
- [AI Native 时代 —— 研发组织何去何从](../ch05/018-ai-native.html)
- [Harness 范式](../ch05/009-harness.html)
