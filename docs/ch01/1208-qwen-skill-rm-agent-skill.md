...
## Ch01.1208 阿里Qwen提出Skill-RM：把奖励模型做成可复用Agent Skill
...
- **[Token-in-Token-out Agentic RL](../ch04/684-agentic-rl-token-in-token-out-done-right.html)**：Skill-RM 是 agentic RL闭环中「奖励信号可审计化」的关键拼图——flat prompt 的奖励信号难复现，Skill化的评估轨迹可复查
...
- **vs [Evaluating Netflix with LLM-as-a-Judge](ch01/325-evaluating-netflix-show-synopses-with-llm-as-a-judge.html)**：Netflix案例是 judge 应用层；Skill-RM 是 judge框架层
- **vs [Multimodal Evaluators](ch01/350-mllm-as-judge.html)**：多模态 judge 是另一维度（输入模态）；Skill-RM 是单模态但评估协议层
