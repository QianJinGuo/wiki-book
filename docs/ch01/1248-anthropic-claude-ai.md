...
## Ch01.1248 刚刚，Anthropic切开Claude大脑！AI自发长出类人「意识器官」
...
3. **反事实训练是对齐技术的有力补充**。通过在假想反思场景上微调而非直接修改行为数据，可以更根本地改变模型的内在表征。这比 RLHF 式的行为约束更接近人类道德教育的本质——不是训练行为，而是塑造价值观。对于 [Hermes Agent](../ch03/093-hermes-agent.html) 等生产系统中的 safety guardrails，可以借鉴反事实训练机制，在「想象场景」中注入安全约束，而非仅在真实交互中施加惩罚。
...
- [Hermes Agent](../ch03/093-hermes-agent.html) — Agent 系统中的可解释性与安全对齐实践
