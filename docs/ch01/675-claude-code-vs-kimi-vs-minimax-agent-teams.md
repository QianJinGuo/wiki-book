...
这些问题的根因不在模型智力，而在缺乏系统化的控制平面——这正是 [Harness Engineering](../ch05/096-harness-engineering.html) 要解决的核心问题。
...
这与 [Claude Code 架构](../ch03/085-claude-code.html)中强调的「工程化协作」理念一致。限制在于：token 成本随 teammate 数量上涨，适合独立工作流并行，不适合同一个文件多人同时编辑。
...
- [Agent Orchestration](../ch04/631-agent-orchestration.html) — 多 agent 编排模式
