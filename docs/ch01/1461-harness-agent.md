...
## Ch01.1461 Harness 即后端：当Agent基础设施消解于统一原语
...
4. **递归扩展能力需要同步设计治理机制**：允许 Worker 创建 Worker 是强大的能力释放，但必须有配套的安全隔离（沙箱）、资源限额、审计日志和递归深度控制。否则系统可能因不受控的递归扩展而崩溃。[生产级 Agent Harness](../ch05/063-agent-harness.html) 的相关设计经验可作为参考。
