...
同一模型在不同 Harness 下的表现差异表明，**工作流设计和工具调用策略正在成为比模型本身更具区分度的因素**。这与 Harness Engineering 范式的基本观点高度一致——Agent 系统的工程化质量决定了模型能力的实际落地效果。参见 [Harness Engineering 的核心地位](../ch05/112-harness-engineering.html) 和 [Agent Harness 综述](../ch05/057-agent-harness.html)。
...
- [Agent Harness 综述](../ch05/057-agent-harness.html) — 同一模型在不同架构下的表现差异分析
- [SWE-bench Agent 评估](../ch04/685-swe-bench-agent.html) — 传统代码修复 Benchmark 的方法论
