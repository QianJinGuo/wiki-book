...
1. **MTP head 一体两用是训练-推理协同设计的典范** — 单一 Multi-Token Prediction head 同时服务预训练 auxiliary objective 和 speculative decoding draft model，避免了独立 draft model 的训练/维护成本。 这与 [Deepseek Moe Parallel Strategy](ch01/799-deepseek.html) 中 DeepSeek 对 MoE 通信层面的深度定制思路一致——都是通过工程化设计最大化已有资源的利用率，而非堆叠额外算力。
