...
## Ch01.330 安全护栏的三域演进 — 阿里云云原生从 Claude Fable 5 提炼的护栏设计原则
...
> 与 [NVIDIA Nemotron 3 内容安全](ch01/309-rag.md) 在"token-level 流式安全审核"维度同模式（Qwen3Guard-Stream 的对应英文实现）。
...
> 与 [Fable 5 AWS 中文译本](ch01/485-claude.md) 同 Fable 5 主题，但本文 80% 篇幅在 3 域对比 + 5 原则抽象，AWS 译本聚焦 Fable 5 产品本身（能力 + 接入 + 部署）。**互补不重叠**。
...
2. **token-level 流式安全**：高敏感场景（金融/医疗/未成年人）应采用 [Qwen3Guard-Stream 风格](ch01/309-rag.md) 逐 token 审核，而非等整条回复完毕再判定
...
- **同阿里云生态**：
  ...
  - [阿里云 CMS CLI 可观测](ch04/255-skill.md)
