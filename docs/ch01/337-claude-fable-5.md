...
## Ch01.337 安全护栏的三域演进 — 阿里云云原生从 Claude Fable 5 提炼的护栏设计原则
...
> 与 [Fable 5 AWS 中文译本](ch01/600-anthropic-claude.html) 同 Fable 5 主题，但本文 80% 篇幅在 3 域对比 + 5 原则抽象，AWS 译本聚焦 Fable 5 产品本身（能力 + 接入 + 部署）。**互补不重叠**。
...
- **同 Fable 5 主题**（互补不重叠）：
  - [Fable 5 AWS 中文译本](ch01/600-anthropic-claude.html)（产品视角）
  - [Claude Fable 5 Safety Fables (Lambert)](ch01/854-claude-fable-5-and-new-ai-safety-fables.html)（政策视角）
  - [Fable 5 Mollick hands-on](ch01/658-claude-fable-5.html)（用户体验视角）
- **同护栏 / 安全主题**：
  ...
  - [Enable Safe Agentic Payments with Built-in Guardrails](../ch04/601-enable-safe-agentic-payments-with-built-in-guardrails-using.html)
