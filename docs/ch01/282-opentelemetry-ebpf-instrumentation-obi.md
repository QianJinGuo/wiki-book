...
## Ch01.282 OpenTelemetry eBPF Instrumentation (OBI) — 零代码全栈可观测性的内核级实现
...
- **vs [Cilium Tetragon](ch01/1081-rag.html)**：Tetragon 聚焦 **运行时安全/拦截**（TracingPolicy + inline kill，syscall 级防护）；OBI 聚焦 **可观测性/追踪**（trace/metrics 生成、协议语义解析）。两者都基于 eBPF 但目标层不同
...
→ [Cilium Tetragon Kubernetes Runtime Security Ebpf](ch01/1081-rag.html) — 同为 eBPF 内核级方案，但聚焦运行时安全拦截
