...
- **vs [Cilium Tetragon](167-rag.html)**：Tetragon 聚焦 **运行时安全/拦截**（TracingPolicy + inline kill，syscall 级防护）；OBI 聚焦 **可观测性/追踪**（trace/metrics 生成、协议语义解析）。两者都基于 eBPF 但目标层不同
...
→ [Cilium Tetragon Kubernetes Runtime Security Ebpf](167-rag.html) — 同为 eBPF 内核级方案，但聚焦运行时安全拦截
