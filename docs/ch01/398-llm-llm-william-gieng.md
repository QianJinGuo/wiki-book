...
**LLM 输出的是带选择机制的模型输出，不是直接观测**——链条中的每一步（文本生成、模型识别、样本选择）都会影响该变量在下游因果模型中的含义，**而这些影响大多不会出现在拼接后的表格里**。 ^["[别把 LLM 提取的主题当成真实变量 — 数据派 THU 翻译
...
- [[entities/video-rag-chunking-strategy](https://mp.weixin.qq.com/s/mfJrT0HtMTvIhw9hf6HQ3Q)（文本 + AI 同源 — 视频 RAG 切片策略）
