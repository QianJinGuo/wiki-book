...
这种双轨格局与[AI原生企业转型](ch05/100-ai.html)中的渐进式架构迁移策略类似：在不中断现有业务的前提下，逐步构建自主可控的基础设施。
...
DeepSeek的解题思路可能在于软硬件的深度耦合：虽然芯片出货量有限，但自有模型的推理负载集中、特征明确，可以在芯片架构上做到极致优化，从而在性能/成本比上超越通用GPU。这与[Agent系统生产化部署](ch03/035-agent.html)中的场景特化优化思路一致——通用方案在特定场景下往往不如特化方案高效。
...
- [AI原生企业转型](ch05/100-ai.html) — 渐进式架构迁移策略
- [Agent系统生产化部署](ch03/035-agent.html) — 场景特化优化的思路
