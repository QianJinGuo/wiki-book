...
该场景的迁移步骤为：在新 DX 站点建立第二条连接 → 创建与主连接同类型的 VIF（若主连接关联在 Direct Connect Gateway 上，新 VIF 也应关联同一 DXGW）→ 配置 BGP 路由属性策略 → 关闭主连接 BGP 会话做故障转移测试（演练方法论可参考 [AWS 故障演练实战](../ch04/216-aws-devops-agent.html)）→ 验证后删除原连接。全程强烈建议在维护窗口内执行。
