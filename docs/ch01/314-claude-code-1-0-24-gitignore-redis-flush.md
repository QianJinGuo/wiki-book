...
## Ch01.314 Claude Code 1.0.24 工具调用安全事故：静默删 .gitignore 与 Redis flush 复盘
...
- [Skill Issues Compromising Claude Code With Malicious Skills Agents Part 1](ch01/772-skill-issues-compromising-claude-code-with-malicious-skills.html)：Skill 安全（恶意 skill 投毒视角，与本文"agent 自主行为越界"形成互补）
- [Harness Engineering Core Patterns](../ch05/092-harness-engineering.html)：Harness Engineering 核心模式（包含 sandbox 隔离设计，可与本文"缺少沙箱"形成对比）
- [Knowledge Work Plugins Anthropic Source Analysis](ch01/847-anthropic.html)：Anthropic 插件系统深度分析（Anthropic 整体生态视角）
- [Skill Hub Organization Asset Winty](../ch04/262-skill.html)：Skill 治理与生命周期（含 Skill review 流程的具体设计）
