...
## Ch01.300 Boris Cherny 新访谈：开发工具正在从 IDE 变成 Agent 控制台
...
这与 [Harness Engineering Core Patterns](ch03/074-claude-code.md) 中"控制面应当外置给人类"的工程原则一致——Human-in-the-Loop 不是降级方案，而是 Agent 系统设计的核心约束。
...
这与 [Claude Code Harness 深度解析](ch03/074-claude-code.md) 中的 harness 设计哲学一致——harness 的核心价值就是为 Agent 提供持续运行的工程环境，而非一次性对话界面。
...
这些边界问题在 [Agent 记忆系统工程实践](ch03/044-agent.md) 中被称为"过程资产"——Agent 系统不仅要完成任务，还要留下可审查、可重放、可学习的中间表示。
...
这与 [Karpathy Vibe Coding 到 Agentic Engineering](ch03/044-agent.md) 中的论述相互印证——"Vibe Coding"是 Agent 时代的临时形态，"Agentic Engineering"才是稳定形态。
...
- [深入理解 Claude Code Agent Harness 构建之道](ch03/074-claude-code.md)
