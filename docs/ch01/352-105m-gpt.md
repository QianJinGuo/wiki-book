...
## Ch01.352 何恺明首个语言模型：105M参数，不走GPT自回归老路
...
```mermaid
mindmap
  root(("何恺明首个语言模型 105M参数 不走GPT自回归老路"))
    ELF到底做了什么
    核心技术细节
      把token变成连续embedding
    ...
    实验对比
    连续扩散路线的到底哲学 一次算不算连续
    x-prediction替代v-prediction
    推理时零额外模块的设计美学
    Self-CFG 无推理开销的条件生成机制
    连续 embedding 扩散项目的架构选型
```
...
- [Gaode Ai Companion Agent Architecture](../ch03/035-agent.html)
