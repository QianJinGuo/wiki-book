...
这与 [Harness Engineering Core Patterns](ch03/084-claude-code.html) 中"Harness 是 Agent 系统的工程价值所在"的论断一致——模型是引擎，harness 是底盘、传动、刹车、仪表盘的整套工程组合。
...
这与 [Factory Mission](ch05/100-ai.html) 的"串行 + 定点内部并行"策略有异曲同工之妙——但 Mission 是 Agent 间的串行，Claude Code 是工具间的串行。
...
这与 [Claude Managed Agents 企业自托管](ch03/035-agent.html) 中关于"Hybrid Control Plane"的设计哲学一致——**控制是分层的，不是二元的**。
...
这与 [Claude Code Harness Deep Dive](ch03/084-claude-code.html) 中关于"流式交互是 Agent 体验核心"的论述一致——用户感受到的"逐字输出"本质就是 `StreamEvent` 实时推送的结果。
...
- [Claude Code Harness 深度解析](ch03/084-claude-code.html)
...
- [Harness Engineering Core Patterns](ch03/084-claude-code.html)
- [Harness 状态边界与失败闭环](ch05/085-harness.html)
- [Factory Mission Multi-Agent 系统](ch05/100-ai.html)
- [Claude Managed Agents 企业自托管](ch03/035-agent.html)
...
- [AI Agent Harness 构建](ch05/100-ai.html)
