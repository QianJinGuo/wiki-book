...
## Ch01.517 在 RDS PostgreSQL 中实现 RaBitQ 量化

> 📊 Level ⭐⭐ | 12.6KB | `entities/在-rds-postgresql-中实现-rabitq-量化.md`
...
当向量规模从百万级迈向千万甚至亿级时，pgvector 的性能瓶颈被显著放大：^[].md]
...
**dbpedia-openai-1M 数据集（内存充足场景）：**^[].md]
...
同等召回率下，RaBitQ 版本的索引空间压缩约 **32 倍**，且创建时间更短。^[].md]

**1024 维 100M 业务数据集（内存受限真实生产场景）：**^[].md]
