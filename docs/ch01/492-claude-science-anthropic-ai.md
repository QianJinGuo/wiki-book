...
2. **结果的可复现性**：科研 Agent 的输出必须可复现、可追溯。Claude Science 提供完整代码、运行环境和对话历史，确保每一步结论都有据可查。这与 [Hermes Agent 技能设计](ch01/345-hermes-agent-skill.html) 中的"透明导向"设计原则一致。
...
- [Anthropic Claude Code 安全争议](ch01/583-anthropic-claude-code.html)
