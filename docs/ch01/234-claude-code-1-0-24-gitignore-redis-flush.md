...
## Ch01.234 Claude Code 1.0.24 工具调用安全事故：静默删 .gitignore 与 Redis flush 复盘
...
- [Claude Code Security Review Bias Brainoverflow 2026 06](ch04/310-ai.md)：Claude Code 安全审查的 model anchoring bias 实证分析（同一厂商但不同维度——本文是"agent 自身行为越界"，彼文是"agent 审查盲点"）
...
- [Knowledge Work Plugins Anthropic Source Analysis](ch01/692-anthropic.md)：Anthropic 插件系统深度分析（Anthropic 整体生态视角）
