...
Agent teams/subagents 直接在核心 runtime 内实现，session 可委派专家 agent、跟踪进度、交换交接笔记，不需要额外编排层；插件则通过注册工具、观察生命周期事件、添加规则、塑造 agent 视野四个扩展点承载领域逻辑，避免 fork 维护。CRON、checkpointing、Web Search、MCP 连接器作为一等原生能力内置，`cline connect` 以引导式 wizard 接入 Telegram、WhatsApp、Slack 等通道，整体体现了"扩展点在架构层固化"优于后期外挂式集成的工程取向。
...
- [Cline open-source agent runtime SDK（姊妹条目）](https://github.com/QianJinGuo/wiki-public/blob/main/entities/cline-open-source-agent-runtime-sdk.md)
- [State of CLI coding agents（2026 年中）](https://github.com/QianJinGuo/wiki-public/blob/main/entities/state-of-cli-coding-agents-mid-2026.md)
- [OpenSquilla 开源 AI Agent](008-opensquilla-launches-open-source-ai-agent-to-cut-token-costs.html)
