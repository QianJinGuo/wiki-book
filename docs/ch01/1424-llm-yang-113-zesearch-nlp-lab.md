...
## Ch01.1424 LLM 自我提升系统综述 — Yang 等 113 页四阶段闭环框架（Zesearch NLP Lab）
...
> 与 [Hermes 自我改进闭环](ch01/294-hermes.html) 在"运行时改进"维度高度共振——Hermes 是工业级实现（基于 Hermes Agent 的 SKILL.md 自我迭代），本综述是学术级分类。
...
| **科学研究（Science）** | 自主假设提出、实验执行 | [Prime Intellect / nanoGPT Opus 4.7 突破人类纪录](../ch05/100-ai.html) |
...
| **问题域** | LLM 自我提升的**系统级闭环** | 多为单点机制（[六条路](../ch03/035-agent.html)）/ 单点实验（[nanoGPT](../ch05/100-ai.html)）/ 单点产品（[LangSmith](../ch03/035-agent.html)） |
...
1. **自动评估是闭环的控制层而非末端节点**：传统观点把评估视为"一次性打分"的末端验证环节，但这篇综述揭示评估应该是**贯穿全程的持续反馈机制**——动态基准和交互环境评估共同构成控制层，实时指导数据获取/筛选/优化的方向。这与 [CrewAI + NVIDIA Nemotron](../ch04/281-self-evolving-agents.html) 的多 Agent 仿真环境设计在"持续演化的评估"维度高度共振。
...
5. **动态基准是控制层的关键**：静态 benchmark 容易过拟合/污染，需配合 [持续演化的评估环境](../ch04/281-self-evolving-agents.html)（如 CrewAI 多 Agent 仿真环境）
...
- **同主题不同 artifact**：
  ...
  - [AI 科研超越人类 — Prime Intellect 递归自改进实验](../ch05/100-ai.html)（单点实验：nanoGPT 突破人类纪录）
  - [Lossy Self-Improvement](ch01/474-lossy-self-improvement.html)（自噬数据风险的理论分析）
  - [编排自演化 Agent — CrewAI + NVIDIA Nemotron](../ch04/281-self-evolving-agents.html)（多 Agent 仿真环境）
  - [MUSE AutoSkill — 字节 ByteBrain 自演化 Agent](../ch04/758-self-evolving-agent.html)（arXiv 2605.27366 工业级实现）
  - [LangSmith Engine — 基于链路追踪的自改进 Agent](../ch03/035-agent.html)（工业级 trace-based 自改进）
  - [Hermes 自我改进闭环](ch01/294-hermes.html)（Winty 视角：基于 SKILL.md 自我迭代）
- **同领域框架**：
  - [Agent Harness Engineering 综述](../ch05/129-harness-engineering.html)（harness 视角）
  - [Harness 演化论文集](../ch05/085-harness.html)（harness 论文集合）
