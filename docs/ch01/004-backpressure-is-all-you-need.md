...
**Review Agent 的引入揭示了一个讽刺的分工陷阱。** 当第一个 LLM 的输出由第二个 LLM 来审查，而审查结果需要人类手动粘贴回第一个 LLM 时，人类实际上变成了「昂贵的剪贴板」——这个观察直接指向了当前 AI 辅助开发中最常见的低效场景。这与 [Harness Engineering Core Patterns](../ch05/026-harness-engineering.html) 中描述的多层验证机制形成对比——在 harness 框架中，门禁是自动串联的，而非人肉传递。
...
这与 [Harness Engineering Core Patterns](../ch05/026-harness-engineering.html) 中描述的「多层验证门禁」思想一致，也呼应了 [Production Agent Engineering](https://github.com/QianJinGuo/wiki-public/blob/main/concepts/production-agent-engineering.md) 中「渐进式可信度建立」的方法论——AI 系统的可靠性不是通过单一机制一次性保证，而是通过层层筛选逐步建立。
...
- [原文存档](https://www.lucasfcosta.com/blog/backpressure-is-all-you-need)
- [Harness 工程核心模式](../ch05/026-harness-engineering.html) — 多层验证门禁的设计思想
