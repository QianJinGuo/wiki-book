...
## Ch01.1042 Anthropic puts Claude agents on a meter across its subscriptions
...
## 技术洞察
**AI 代理计费的范式转变**：
这篇文章的核心洞察是：**从 token 计费到 agent-aware 计费的转变**。^[].md]

传统 AI API 定价：按输入/输出 token 计费^[].md]

新型 Agent 定价：追踪代理执行的操作、任务完成数、操作复杂度^[].md]

这种定价模式反映：
1. **AI 能力成熟** — 从简单文本生成到复杂多步骤任务执行
2. **价值衡量变化** — 一个完成复杂研究的代理比一个简单问答更有价值
3. **商业模式创新** — 为企业客户提供更可预测的成本结构
市场影响：可能为 AI 代理服务商业化设立新标准^[].md]

→ [[raw/articles/anthropic-claude-agents-meter-infoworld.md|原文存档]
...
## 深度分析
**从 Token 计费到 Agent-Aware 计费的范式转变**^[].md]
