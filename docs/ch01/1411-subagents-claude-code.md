...
## Ch01.1411 Subagents 详解：Claude Code 如何避免上下文污染

> 📊 Level ⭐⭐⭐ | 43.5KB | `entities/subagents-详解claude-code-如何避免上下文污染.md`

[[raw/articles/subagents-详解claude-code-如何避免上下文污染.md]
...
* * *
昨天在梳理 Agent Harness 的上下文管理，我一直在想一个很小但很真实的场景：^[].md]
...
Subagent 把探索过程留在独立窗口，只把结果交回主会话^[].md]
...
##  同一条线索
这两个月翻 Claude Code 相关的讨论，会看到不少表面不同、底子相近的经验。^[].md]
...
把这些材料放在一起看，我反倒不太想把本文写成一篇 Subagents 功能介绍。^[].md]
...
##  长会话为什么会变脏
做过稍长一点 Claude Code 任务的人，大概都有过类似体感。^[].md]

刚开始，窗口很清爽。用户目标、项目结构、关键文件、约束条件都还清楚。^[].md]

跑着跑着，模型开始探索：
    grep -R "AuthService" .^[].md]

    find . -name "*.ts"
    ls packages/api/src
    npm test -- auth
这些动作没有问题。甚至可以说，它们是 Agent 真正能干活的前提。^[].md]

问题在于，每一次工具调用的输入和输出，都会进入会话历史。^[].md]

短任务里这不算什么。模型看一眼目录，读两个文件，做一个小修复，窗口足够用。^[].md]
...
Subagent 正好挡住了其中一类污染：那些必须做、但做完之后不值得长期留在主窗口里的探索过程。^[].md]
...
主 Agent 把探索、计划、审查交给不同 Subagent，最后只收回结果^[].md]

这件事真正值钱的地方，可以分成三层来看。^[].md]
...
原帖给了一个代码审查的例子，大致是这种形态：^[].md]

    ---
    name: code-reviewer
    description: Review code quality, security, and maintainability after code changes.
    tools: Read, Grep, Glob, Bash^[].md]

    model: sonnet
    ---
    You are a senior code reviewer.^[].md]
...
这比「code reviewer」四个字有用得多。^[].md]

因为它不仅告诉模型「这是审查」，还告诉模型「实现后使用」「不负责规划」。边界越清楚，路由越稳。^[].md]
...
这两个内置子代理背后的设计，挺值得借鉴。长任务里最「脏」的部分，往往在探索阶段，倒不一定在执行阶段。^[].md]
...
但我不会把 fork 当默认。
原因也很简单：fork 复制的不只是有用背景，也会复制噪音。^[].md]

如果父窗口已经很脏，fork 只是把这份脏工作集复制给更多子代理。每个子代理看似「知道得更多」，实际可能一起被旧状态、无关日志和过期判断拖住。
所以我更倾向于这样用：
场景  |  更适合的方式
---|---
查找代码模式、搜索影响面、阅读一批文件  |  fresh Subagent^[].md]

安全审查、性能审查、测试覆盖率检查  |  fresh Subagent + 明确任务描述^[].md]

已经有很长项目背景，子任务必须继承这些约束  |  按需 fork^[].md]

想从同一个起点并行比较几个方案  |  fork 可以考虑^[].md]

子任务之间需要持续共享中间状态  |  不适合普通 Subagent，考虑主循环或共享状态结构^[].md]

父窗口已经很乱，只是想并行加速  |  先清理主任务，再考虑拆分^[].md]
...
##  我自己常放的几个 Subagents
如果要往项目里真的放几个  ` .claude/agents/  ` ，我倾向于先从很少的几个开始。^[].md]
...
一个我比较喜欢的 Subagent 模板，会刻意写清四件事：^[].md]

    ---
    name: backend-impact-analyzer^[].md]
    ...
    tools: Read, Grep, Glob^[].md]

    model: sonnet
    ---
    You analyze impact scope for backend changes.^[].md]
    ...
    Do not edit files.
    Do not propose broad refactors.^[].md]
...
* • 明确不要做大重构，减少子代理跑偏。
这类约束看起来啰嗦，但对 Agent 很有用。不显式写出来，它常常会自己补一套更模糊的版本。^[].md]

* * *

##  最容易踩的坑
Subagent 本身不复杂，真正容易出问题的是使用姿势。^[].md]

** 第一个坑，是把委派写得太含糊。  **^[].md]

「帮我看一下这个模块有没有问题」这种任务，子代理大概率会发散。更好的写法是：「只检查认证模块最近 diff 中的安全风险，重点看 token 校验、权限绕过和敏感日志，返回 P0/P1/P2 级别问题。」
任务越具体，子代理越像工具。任务越含糊，子代理越像另一个会跑偏的聊天窗口。^[].md]

** 第二个坑，是让子代理返回太多过程。  **^[].md]

如果子代理把所有搜索结果、完整日志、读过的文件都倒回主窗口，隔离价值就没了。^[].md]

主 Agent 需要的是结论、证据、下一步动作。必要时带 2 到 3 个文件锚点就够了。^[].md]

** 第三个坑，是把需要共享状态的任务硬切出去。  **^[].md]
...
如果说 2025 年很多人还在比较模型聪不聪明，2026 年更值得盯的，可能是这些看起来朴素的东西：^[].md]
...
这些东西不热闹，但它们让 Agent 从「能演示」走向「能长期用」。^[].md]
...
回头看，长任务跑不稳，原因往往不是少了一个更聪明的模型，更多是窗口里堆了太多本该用完就丢的东西。^[].md]
