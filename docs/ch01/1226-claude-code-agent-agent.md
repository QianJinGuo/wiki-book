...
## Ch01.1226 Claude Code 多智能体协作体系设计：从单Agent到多Agent工作流
...
```mermaid
mindmap
  root(("Claude Code 多智能体协作体系设计"))
    为什么需要多Agent协作
    多Agent协作的核心架构模式
      父子层级架构 Parent-Child Hierarchy
      水平分工架构 Horizontal Division
      流水线架构 Pipeline Architecture
    多Agent协作的核心设计模式
      模式一 上下文隔离子Agent Context-Isolated
      模式二 分支-合并并行 Fork-Join Parallelism
      模式三 研究与执行分离 Research-Execution
    多Agent通信协议
      消息传递模式
      共享状态模式
      事件驱动模式
...
- [Claude Code Prompt Context Harness](../ch09/061-claude-code-prompt.html) - Claude Code Prompt/Context/Harness设计
- [Hermes Agent Memory System](../ch03/096-hermes-agent.html) - Hermes记忆系统
