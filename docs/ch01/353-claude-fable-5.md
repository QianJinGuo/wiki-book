...
## Ch01.353 安全护栏的三域演进 — 阿里云云原生从 Claude Fable 5 提炼的护栏设计原则
...
> 与 [AI Gateway vs MCP Gateway 安全分析](../ch11/078-ai-gateways-vs-mcp-gateways-what-security-teams-need-to-kno.html) 在"网关层安全"维度同框架。
...
> 与 [NVIDIA Nemotron 3 内容安全](../ch03/045-agent.html) 在"token-level 流式安全审核"维度同模式（Qwen3Guard-Stream 的对应英文实现）。
...
> 与 [Fable 5 AWS 中文译本](ch01/1262-anthropic-claude.html) 同 Fable 5 主题，但本文 80% 篇幅在 3 域对比 + 5 原则抽象，AWS 译本聚焦 Fable 5 产品本身（能力 + 接入 + 部署）。**互补不重叠**。
...
2. **token-level 流式安全**：高敏感场景（金融/医疗/未成年人）应采用 [Qwen3Guard-Stream 风格](../ch03/045-agent.html) 逐 token 审核，而非等整条回复完毕再判定
...
- **同 Fable 5 主题**（互补不重叠）：
  - [Fable 5 AWS 中文译本](ch01/1262-anthropic-claude.html)（产品视角）
  - [Claude Fable 5 Safety Fables (Lambert)](ch01/836-claude-fable-5-and-new-ai-safety-fables.html)（政策视角）
  - [Fable 5 Mollick hands-on](ch01/168-claude-fable-5.html)（用户体验视角）
- **同护栏 / 安全主题**：
  ...
  - [Enable Safe Agentic Payments with Built-in Guardrails](../ch04/069-enable-safe-agentic-payments-with-built-in-guardrails-using.html)
  - [Nemotron 3.5 Content Safety](ch01/509-nemotron-3-5-content-safety-customizable-multimodal-safety.html)
  - [阿里集团 AI 代码评审安全带](../ch05/081-ai.html)
- **同阿里云生态**：
