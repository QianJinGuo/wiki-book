...
## Ch01.1034 GPT-5.6 Sol/Terra/Luna 分层定价，Codex 合并入 ChatGPT，ChatGPT Work 发布
...
GPT-5.6 Sol, Terra, and Luna are generally available on Amazon Bedrock, running on Mantle, the next-generation inference engine. Pricing matches OpenAI first-party rates, and usage counts toward existing AWS commitments.

### Bedrock Inference Engine
- **Durable state capture**: Every request captures its full state continuously; if hardware fails or a node restarts mid-call, the request picks back up where it left off instead of starting over
- **Isolated queue**: Each customer gets their own isolated queue with automated capacity management, ensuring predictable performance under heavy load
- **In-Region inference**: Requests stay within the specified AWS Region for data residency
...
### Security
- **Zero-Operator Access (ZOA)**: Enforced at the chip level — no AWS operators can access prompts or completions
...
### ChatGPT Work & Codex
Alongside GPT-5.6 on Bedrock, OpenAI launched **ChatGPT Work** — an agent for larger, multi-step tasks in the ChatGPT desktop app. The updated desktop app brings Chat, Work, and Codex together. Users can configure the app to use GPT-5.6 through the Responses API on Amazon Bedrock.

→ [量子位报道](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/gpt-56-正式上线codex-和-chatgpt-合并顶级-agent-能力开始按任务定价.md)
→ [机器之心报道](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/刚刚gpt-56全面上线codex被合并生产力工具chatgpt-work来了.md)
→ [AWS Bedrock GA announcement](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/openai-gpt-56-sol-terra-and-luna-are-now-generally-available.md)
