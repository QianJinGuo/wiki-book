...
DeepSeek V4 的 CSA/HCA 代价更高（丢失 token 级细节），但通过 CSA（轻压缩+稀疏选择）和 HCA（重压缩+密集注意力）的交替使用，在效率和建模质量间取得平衡。 See also [Context Window Management](../ch06/009-context-window-management-comparison.html)
