...
## Ch01.118 PyTorch 2.12 Release Blog – PyTorch
...
→ [[raw/articles/pytorch212releaseblogpytorch.md|原文存档]
...
**3. Microscaling 量化支持打通 LLM 部署最后一公里**^[].md]
...
**3. 多 GPU 训练调试应利用 Profiler 增强的事件 API**^[].md]
...
**4. ROCm 用户应评估 FlexAttention 流水线配置**^[].md]
...
**5. 使用 torch.compile 的优化器密集型训练需验证数值正确性**^[].md]
