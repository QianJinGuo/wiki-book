...
环境与工具链问题也在迁移过程中占据重要比重：Docker 缓存不一致、端口连接问题、Maven Wrapper 和构建工具问题等操作层面的困扰，往往在源代码迁移本身就绪后仍会延迟验证。IBM 的故障模式分布分析显示：迁移失败涉及构建系统、部署环境、依赖注入、数据库、端点、断言和基础设施等多个维度。这印证了 [CODA Bench](../ch09/093-coda-bench-code-agent.html) 中也强调的一个发现——代码 Agent 的能力评估不能仅关注代码生成质量，还需要考虑其对开发环境、构建工具和运行时行为的理解能力。
...
- [CODA Bench 代码 Agent 基准测试](../ch09/093-coda-bench-code-agent.html)
