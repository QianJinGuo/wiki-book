...
## Ch01.1258 淘宝内容生态：GrowBrain - 淘宝Agentic内容成长引擎
...
这两个问题在本质上反映了 Agent 架构设计中一个尚未被充分认知的原则：**LLM 的职责范围应当与其模型能力相匹配**。小模型的幻觉风险要求从「LLM 全程驱动」转向「框架精确控制 + LLM 在关键节点介入」。这与 [Agent 配置组合](../ch04/273-skill.html) 中「Skill 作为可控边界」的设计理念形成互文。
...
这三个设计原则构成了一个通用范式：**Agent 架构的「无状态化」是生产级部署的前提**。这与 [Agent Harness 上下文管理](../ch05/057-agent-harness.html) 中的工作集隔离设计不谋而合。
...
- [AgentCore Trip Allocation](../ch04/683-agentcore-harness.html)
