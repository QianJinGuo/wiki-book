...
## Ch01.1246 OpenAI 发布 GPT-Live：实时语音的前台/后台分解架构
...
这种分级既是技术选择（不同场景需要不同的推理策略），也是商业策略（为用户提供性价比最优的选择，同时为大模型推理资源做差异化管理）。这与 [GPT-5.6 的分层定价模型](ch01/512-codex.html) 一脉相承。
