...
## Ch01.1244 ICML 2026 | 不用文本编码器推理，脉冲神经网络也能看懂3D开放世界

> 📊 Level ⭐⭐⭐ | 11.1KB | `entities/icml-2026-不用文本编码器推理脉冲神经网络也能看懂3d开放世界.md`
...
```mermaid
mindmap
  root(("ICML 2026 不用文本编码器推理 脉冲神经网络也能看懂3D开放世界"))
    背景与挑战
      SNN 的核心优势与局限
      现有方法的不足
    核心技术贡献
      多尺度三元对齐 Multi-scale Tri-modal
      可重参数化视觉-语言集成 Reparameterized
      首个全脉冲驱动的点云 Transformer
    零样本 3D 分类
    D 字幕生成与开放世界问答
    下游任务迁移
    SNN 从能效优势到能力优势的转折点
...
这使得模型在保持全脉冲驱动、硬件兼容的同时，实现了**无文本编码器的轻量级推理**——这是 SNN 首次在开放世界理解中摆脱对文本编码器的依赖。
...
Rep-VLI 的"训练时用大模型学习，推理时压缩为轻量权重"的思路，本质上是一种**跨模态知识蒸馏与重参数化的融合**。这与 [Harness Engineering](https://github.com/QianJinGuo/wiki/blob/main/concepts/harness-engineering-framework.md) 中"训练/推理分离"的设计哲学一致——训练阶段可以使用大型、昂贵的组件完全解锁学习能力；推理阶段则通过结构重参数化转换为轻量、高效的架构。这种模式对于任何有部署场景约束的 AI 系统都有参考价值。
...
- [国产视觉基模](ch01/686-70.html)
- [Hermes Agent 上手](../ch03/096-hermes-agent.html)
- 多模态学习
- 神经形态计算

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/icml-2026-不用文本编码器推理脉冲神经网络也能看懂3d开放世界.md)
