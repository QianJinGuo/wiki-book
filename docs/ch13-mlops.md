...
> 📊 Level ⭐⭐⭐⭐ | 29.3KB | `entities/meituan-turing-agent-evaluation-methodology-2026-08-06.md`
...
美团图灵团队 2026-09-10 发布《Agent 评测白皮书》系列第一篇《评测全览》——系列定位从「评测是什么」转向「怎么落地」（先做什么后做什么、每步产出物长什么样、做到什么程度算达标、常见坑怎么处理），计划四篇（全览/冷启动/扩量/自进化），本篇为全览。它以「一套统一的语言对齐行业内尚在发散和纠缠的概念」，把完备评测体系收敛为**四个模块、三种能力、两条 Loop、一套资产**。该全景框架（四模块职责边界 / 两条 Loop 咬合机制 / 端到端 vs 过程评测集双分类 / 归因双路径 / 成熟度 L0-L3 自查表 / 三种「部分完备」形态）在本文（《漫谈》）与库内其他评测实体中零覆盖——本文讲的是方法论内容，白皮书补的是体系结构：概念如何划分、模块如何咬合、成熟度如何分级。

**两条相互咬合的 Loop。** 体系被描述为一套持续运转的系统：**评测体系迭代 Loop**（在线评测/监控产生异常信号 → 线上真实样本经 Case 挖掘进 Case 池 → 归因判断问题性质 → 「评测集没覆盖」则补样本、「标准判错了」则迭代 Metrics 与 Rubric）与 **Agent 迭代 Loop**（同一批样本经挖掘归因定位到 Prompt/Skill 描述/上下文管理/模型能力等 Agent 侧问题 → 研发变更 → 评测集回测确认无退化 → 上线 AB）。两条 Loop 共享同一批线上真实样本、在 Case 挖掘与归因处咬合；第二个咬合点是评测资产本身——评测集越完善，回测门禁价值越高，上线后暴露的新问题越少，形成正反馈。

**四个模块与三种能力。** 四模块=离线评测（守住已知，回答「变更是否安全」）/ 在线评测与在线监控（发现未知）/ Case 挖掘与归因（定位与分派，是体系枢纽也是「Agent 自进化」的核心切入点）/ 观测基建（前三者的地基）。三种能力=发现问题、定位问题、驱动演进，「缺一项，体系就会在对应环节卡住」。作者坦承在线评测/在线监控/观测/Case 挖掘与归因之间存在概念模糊地带（Arize AI、LangFuse、Braintrust 的划分互不相同，有的把 Online Evaluation 与 Monitoring 合并，有的把 Trace 采集归 Observability、Trace 分析归 Evaluation），故显式声明四模块是「为讨论对齐而做的切分」，团队可按自身组织方式重切，只需不漏职责。

**评测集结构与分类。** 广义评测集由（问题、参考答案、评价标准）三元组构成（问题+参考答案=评测样本），随长程化迁移为（prompt、expected_behavior、trace）；每一行三元组即一个 Task。评测集分**端到端**（回答「事有没有办成」，从用户视角按功能模块分别建立）与**过程**（回答「中间哪一步出了问题」）两类，两者都可建黄金集/必过集且彼此独立不冲突；过程评测集可继续下钻为知识库评测集/Skill 评测集，收益是**责任清晰**（分数下降能直接定位负责人）。关键经验：「端到端回答要不要拉响警报，过程回答警报响了该找谁」；拆解边界是「演进的结果不是设计的起点」，先跑核心场景再按真实问题细化，而非被完备性焦虑驱动。

**回测执行的三个工程考量。** ①**多次试验（Pass@k）**：大模型输出有随机性，用通过率而非单次通过与否作结论；②**分层门禁**：安全类/数据准确性类检查项一票否决（任一不过即阻断发布），体验类/加分项设阈值；③**接入研发流程**：把回测嵌进 CI/CD 或发布流程，「没有接入流程的门禁本质上只是一个建议」。同时必须承认离线评测的固有局限——只能发现已知问题，评测集里没有的场景永远测不出来，故离线守已知、在线（AB/影子模式/巡检）发现未知。

**巡检：两个模块的交集。** 巡检位于在线评测与在线监控的交集，把「线上质量观察」变成「持续可追踪过程」，做法是从离线评测集抽取代表性 Task 周期性检查（也可单建巡检评测集）。它同时具备两种属性：**评测属性**（看某类能力是否持续稳定，属质量判断）与**监控属性**（把分数变化/失败样本/异常趋势沉淀为 Case 挖掘的输入，属信号产出）。在线监控维度分三类：可用性（Skill/Tool 成功率、失败率、超时率）、成本（Token、平均步数、平均耗时）、行为监控（任务类型分布、Skill 调用频次、平均轮次——用于发现用户结构变化，某类新任务占比快速上升往往意味着评测集该扩充）。

**Case 挖掘四来源、Case 池分化与归因双路径。** 四类来源（线上反馈/监控/规则挖掘/随机采样）必须搭配使用，因为**盲区不同**——只靠反馈只见用户愿抱怨的问题，只靠监控只见能被指标刻画的问题，只靠规则只见自己已想到的风险，而「随机采样是唯一能发现你根本没想到的问题的手段，虽然成本最高、命中率最低，但不可省略」。Case 池分化为 Good Case（定义「好」的程度下限 → 黄金集/挑战集）与 Bad Case（→ 错题集，回报最直接），断言「没有黄金集，团队讨论质量时只能各说各话」。归因靠 Trace 全链路铺开定位（与看日志查 bug 高度雷同），正向 Case 入黄金集/挑战集，负向 Case 走**两条路径**：修 Agent（Agent 迭代 Loop）或校验并迭代 Rubric、Case 流入错题集（评测体系迭代 Loop）。作者点出一个隐蔽风险：多数团队只走第一条路径，默认「评测说不行就是 Agent 不行」，结果 Agent 被优化成「迎合评测标准」而非「解决用户问题」——评测标准本身有偏差时，优化越努力偏离越远，故**定期检查「是不是评测判错了」是体系自我校准的必要动作**。

**观测基建的「最小可归因」底线。** 主张观测指标与基建应「基于需求反向设计」而非项目启动就建完备系统，判断信号是「想自动化判断某件事但数据里没有」即补埋点；冷启动阶段的底线是「最小可归因」——至少拿到完整的模型调用输入输出与 Skill 调用记录，能凭 Trace 复现一个 Bad Case 到底发生了什么。

**成熟度自查表与「三种部分完备」形态。** 白皮书给出可直接打分的 L0-L3 成熟度自查表，并按业务阶段定合理水位：冷启动=观测 L1 + 评测集 L1 + 回测门禁 L1（最小闭环）；扩量=评测标准与在线评测推到 L2、少量指标 L3；全量运营才考虑 L3 自动化与规模化。核心用法是看**短板与错配**——「体系的能力上限由最短的那块板决定」，例如评测标准已 L3（机评规模化）但 Case 挖掘仍 L0（靠用户投诉），则投入应转向补齐挖掘能力。三种「部分完备」形态中，第三种最值得警惕：既有评测集又有监控看板、汇报数据齐全，但缺中间枢纽导致两套体系割裂（线上问题回不到评测集，评测集解释不了线上现象），投入多而收效有限。

**FAQ：Agent 为什么不打榜，以及必过集 vs 挑战集。** 本篇 FAQ 补充了一个与基座模型评测的分野判断：基座模型提供通用能力、可拆解出通用指标（其评价体系是三年论文堆砌而来），而 Agent 服务于垂直业务，「最佳的 Agent 指标就是业务指标」，故上线 AB 看业务指标才是评价 Agent 能力的黄金标准。评测集拆为必过集（核心功能，已知问题必须尽可能解决才可上线）与挑战集（垂直领域构建极难、极易跟不上版本迭代）；实践中常见「必考题都会了、难题又见不到」，所以离线评测更多用于回测，真实效果由线上 AB 判定；另附警告：设了挑战集就必须及时更新以拟合线上真实分布，否则评测集不置信。
...
## Ch13.004 美团海报生成 AIGC 技术体系：PosterCraft/PosterOmni/PosterReward（ICLR/CVPR 2026 三连发）
...
- [Inngest 2026 AI 评测报告](ch01/327-inngest-ai-in-production-the-2026-benchmark-report.html)
...
## Ch13.011 07—AI Skill 测评体系完整进阶指南：5 大能力缺口与填补路径
...
> 📊 Level ⭐⭐⭐⭐⭐ | 15.2KB | `entities/agent-skill-writing-evaluation.md`
...
腾讯轻量云 SkillHub（作者 Elisedai）整理 Google DeepMind 工程师 Philipp Schmid 围绕 Gemini API 与 Agent Skill Eval 的实践，把 Skill 评测从「发布前跑一次」推进为**发布门槛 + 持续回归 + 消融退休**的完整工程链路。本页前述内容回答「怎么测」，本节补上「凭什么允许发布、什么时候可以退休」。

### 为什么 Vibe Check 不够：SkillsBench 的两组数据

SkillsBench 索引了 GitHub 上约**五万个 Skill，几乎没有完整 eval**；许多 Skill 是 AI 生成的，「人工看一眼，再跑几次 vibe check 就发布了」。但同一 Agent 面对同一任务第一次成功、第二次失败并不罕见——没有固定测试集就没有稳定基线，也无法归因失败。SkillsBench 另一组结果更关键：Skill 在约**一百个编码与生产力任务中平均带来约 15% 的提升，但人工编写的 Skill 表现最好，AI 自动生成的 Skill 有时反而降低表现**——「Skill 有效，不等于任何 Skill 都有效」。

判断依据是行为依赖而非文档：**Skill 一旦被加载就占用上下文并影响 Agent 决策，它不是静态说明文档，而是进入运行链路的行为依赖**——既然能改变行为，就应像代码、提示模板和工具调用一样接受测试。

### 触发边界：描述是「触发分类器」的一部分

开发者在 Cursor / Claude Code 里知道有哪些 Skill，模型没触发就补一句提示或直接命令调用；但**产品里的最终用户不知道内部 Skill 名称、不会替系统修正触发方式，只会说自己的任务**。Skill 通过三层渐进披露暴露信息（① 名称+描述 → ② 正文 → ③ 按需读取的脚本/示例/参考资料），模型在读取完整内容前往往只看到名称与描述——**因此描述不是简介，而是触发分类器的一部分**。实践数据中**约一半的失败来自 Skill 没有被正确触发**：描述太弱、太宽，或只含 API 术语而没覆盖用户真实说法，会让 Skill 该出现时缺席、不该出现时抢走任务。一段可测试的描述至少要回答「为什么用、什么时候用、怎样使用」，并说明哪些相似任务**不应该**用；「构建多轮聊天应用时使用 Interactions API」是可执行指令，而「Interactions API 适合多轮聊天」只是背景信息，仍要求模型自己推断下一步。

### 两类 Skill，两种生命周期（能力型可退休）

从生命周期看 Skill 分两类，**eval 目标不同**：
...
因此 **eval 不只是上线前的成绩单，也定义了 Skill 的维护周期**。

### 四个收敛动作：先让 Skill 变得可评测

内容本身模糊，后面的 eval 只能测到「一团模糊行为」。动手收敛四步：①**写指令，不写被动说明**（指令明确告诉 Agent 在什么条件下采取什么动作；被动背景把判断压力留给模型）；②**正文精简，详细资料按需放进参考文件**（描述频繁进入调用上下文，正文触发后也产生 token 成本；SKILL.md 超过约 500 行就该认真检查是否拆分）；③**固定流程写成脚本**（装依赖、移文件、执行发布命令等确定性步骤不必每次重新推理——Skill 更适合表达目标、约束、相关文件与可用参考，让 Agent 在边界内判断）；④**删除不改变行为的 no-op**（「编写高质量代码」「仔细检查结果」听起来正确却难形成可观察行为差异，既耗上下文又干扰「Skill 究竟增加了什么」的判断）。

### 最小测试集与隔离 Runner

不必先搭复杂评测平台，而是尽早建立**小而真实**的测试集：最小起步为**五条 happy path + 五条 negative case**，之后每出现一个真实失败（客户反馈 / 生产轨迹 / 团队实际使用）就沉淀为长期保留的**回归用例**——测试集因此不是「发布前写完就丢掉的文档」，而是随产品使用不断增长的**行为规格**。每条用例至少记录 **用户提示 / 语言 / should_trigger / 可检查的 expected_checks**。**负例尤其重要**：只测「该用时能不能用」会把 Skill 推向过度触发，加入「不该用时是否保持沉默」才能把边界真正固定下来。

最小 eval harness 只有两块：**一份机器可读的测试集 + 一个运行 Agent 的脚本**。Runner 读取提示、在**干净工作区**执行 Agent、取回最终结果与必要轨迹再运行检查；**每次运行必须隔离**，避免 Agent 从旧对话、残留文件或上一次输出中「偷到答案」；测试声明还可包含工作区文件、依赖与启动命令，让环境本身也可重复。检查方法**优先便宜、确定的手段**（regex / 普通程序断言，速度快成本低且适合持续回归）；只有代码风格、设计质量或复杂任务完成度这类难以写死的问题才交给**带明确 rubric 的 LLM-as-a-Judge**——「关键不是用了一个更聪明的 Judge，而是把可确定的部分尽量确定化，LLM Judge 应该是较小的补充层，而不是所有测试的默认入口」。Interactions API 案例的检查项即为四类确定性断言：是否用了正确 SDK / 是否选择当前模型 / 是否调用正确方法 / 是否仍出现已淘汰的旧模式。**该案例最终积累 117 条真实与合成用例，把有效代码表现提高到接近 90%**——产生价值的不是某条神奇提示，而是测试集、Runner、断言和迭代形成的闭环。

### 评结果不锁路径：双指标与跨 Harness

Agent 可能第一轮没加载 Skill、第五轮才加载但最终完成任务——**评测若要求固定轨迹，就会把合理行为误判为失败**，故应优先判断最终结果而非锁死执行路径。但只看一次也不够：**每个案例运行 3–5 次**，并分别看两个问题——**「多次运行中是否至少成功一次」（能力上限）与「是否能够连续稳定成功」（用户实际感受到的可靠性）**。同一个 Skill 还要在真实用户会使用的**不同模型与 Agent Harness** 上分别测试：与某个模型/环境组合有效不代表换 Harness 仍成立，**跨环境测试的目的不是追求一张统一分数，而是明确它在哪些组合中可靠、在哪些组合中不应该发布**。

### 回归门禁与消融退休

一次评测只能证明一个时间点。要让 eval 真正约束工程行为，就要**让它与 Skill 一起进入版本库、每次修改自动重跑**；合理的**合并门槛**是：这次变更**要么改善了结果，要么针对新发现的失败增加了新的 eval**——真实失败一旦进入回归，就不能在下一次重构中悄悄回来。最后做**消融**：在同一组测试、同一模型、同一 Harness、相同运行条件下比较**加载 Skill 与不加载 Skill** 的结果——稳定提高表现即证明增量价值；**基础模型已达到相同表现，就该考虑退休 Skill**。**退休不等于删除 eval**：能力型 Skill 可以消失，eval 应继续保留——它既证明了为什么可以退休，也能在模型或 Harness 未来退化时发出信号，帮助团队判断是否需要重新引入。

该文给出的「周一就能开始」最小行动八步：写五条来自真实使用的测试提示 → 补上该触发与不该触发的边界 → 删除不改变行为的 no-op → 用 JSON/YAML 保存测试声明 → 写一个最小 Runner 并优先加入确定性断言 → 在隔离环境重复运行 → 比较 Skill ON 与 OFF 的结果 → 把第一次失败变成第一条回归用例。核心立场句：**「没有 eval 的 Skill，不是可靠能力，只是未经验证的行为。」**
