...
1. **威胁可见性是 CNI 安全底线**：5% 的监控覆盖率几乎等同于完全无监控，任何依赖"被动发现"的安全策略都不应被接受。企业应建立基于攻击面管理（ASM）、EDR/NDR 部署、日志聚合（SIEM）的全栈可见性体系，参考 Agent 安全架构 与 Agent 可观测性 中强调的"持续验证"原则。
...
- Agent 安全架构 — 零信任与持续验证原则在 AI Agent 时代的延伸
