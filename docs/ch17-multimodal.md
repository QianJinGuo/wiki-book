...
- [Anthropic Computer Use 最佳实践](ch01/1189-anthropic-computer-use.html)
...
演进方向上，三个趋势值得关注。其一，多模态大模型的统一化——像 GPT-Image-2 这类模型同时具备图像生成与编辑能力，未来"静态图→视频"与"文本→视频"很可能合并进同一 pipeline，对单一功能的图生视频工具形成挤压。其二，推理加速与降本——[视频生成推理加速](ch01/1429-0.html)、Sana Video 2 混合线性注意力等方向正把生成成本推向"实时级"。其三，可控性与编辑能力——[AI 视频工具的第三阶段](ch05/100-ai.html) 已从"能生成"转向"能精确控制"，可控编辑与剪辑工作流深度集成成为下一轮竞争焦点。
...
- [GPT-Image-2 完全指南](ch04/341-skill.html)
- [AI 视频工具走到了第三阶段](ch05/100-ai.html)
...
- [Cvpr 2026 Highlight 让Ai像电影人一样 看 视频 8B小模型反超Gpt 5与Gemini 3 1 Pro](ch01/1429-0.html)（视频 8B 反超同主题 — 多模态 8B 起步）
- [A2Rd Agentic Autoregressive Diffusion Long Video](ch03/035-agent.html)（多模态生成对照）
- [Openai Realtime Api Architecture](ch05/100-ai.html)（OpenAI Realtime API — 实时语音对照）
- [刚刚Openai 放出三个语音模型顺便杀死了同传](ch05/100-ai.html)（OpenAI 语音模型同主题）
- [Gpt 5级推理能力塞进语音模型Openai把同传翻译成本砍穿地板价](ch05/100-ai.html)（OpenAI GPT-5 语音同主题）
- [Amazon Bedrock Model Inference Serverless Architecture Case Study](ch11/310-bedrock.html)（Bedrock 多模态推理对照）
- [Nvidia Nemotron 3 Agents Rag Voice Safety](ch03/035-agent.html)（Nemotron 语音 + 智能体对照）
- [Snowflake Agentic Enterprise Summit 2026](ch03/035-agent.html)（Snowflake 真实感场景对照）
- [Loss Function Development Elvis Sun Goal Loop 2026](ch01/1429-0.html)（LFD 质量 > 体量同源思想）
...
- vs **[AI 视频工具悄悄走到了第三阶段](ch05/100-ai.html)** — 那是**行业历史阶段综述**(20KB,花叔 2026-05-07),本文是**单一项目深测**。两者互补:阶段综述给宏观背景,本文给工程细节。
- vs **[Video Agent 范式迁移与算力-人才飞轮](ch03/035-agent.html)** — 那是**底层视频模型视角**(nvidia Cosmos + xAI Grok Imagine),本文是**pipeline 编排层视角**。两层视角互补。
- vs **[JoyAI-Echo:京东长视频框架](ch05/100-ai.html)** — 那是**长视频(5 分钟一致性)底层生成框架**(DMD 蒸馏 + Director Agent),本文是**短视频 pipeline 装配**。时长 / 抽象层完全不同。
- vs **[Fastlane 短视频内容](ch03/116-fastlane-create-winning-short-form-content-in-seconds.html)** — 另一款短视频工具,但**未开源**;Pixelle-Video 是 Apache 2.0 开源,可二开。
- vs **[Agentium Agent Framework](ch03/035-agent.html)** — 同为 pipeline 编排思路,但 Agentium 偏**通用 agent 编排**,Pixelle-Video 偏**视频生成专精**。
- vs **[纳德拉「Token 资本」论](ch05/100-ai.html)** — Pixelle-Video 是该战略宣言"模型可替换性"哲学的**工程范本**。
- vs **[800 行 OpenClaw tool 消息总线子 agent 管理架构](ch03/035-agent.html)** — 两者都体现"**装配工胜过生成器**"的工程哲学(OpenClaw 是 agent 工具总线装配)。

## 深度分析

**1. 装配工哲学的崛起:从"赌单个模型"到"编排即壁垒"**

Pixelle-Video 的出现印证了一个正在多领域复现的规律:当单点生成模型(图像、视频、语音)的能力逐渐同质化,真正的工程壁垒从"谁能训练出更好的模型"迁移到"谁能更聪明地把模型串起来"。这是一种自下而上的范式转移——在 [纳德拉的 Token 资本论](ch05/100-ai.html) 框架里,这正是"模型可替换性"的核心洞察:企业的 AI 主权不在于拥有最强模型,而在于能够随时替换底层模型而不丢失已沉淀的业务逻辑。Pixelle-Video 是这个哲学在视频生成领域的第一批工程范本之一。
...
- [AI 视频工具悄悄走到了第三阶段](ch05/100-ai.html)
- [Video Agent 范式迁移与算力-人才飞轮](ch03/035-agent.html)
- [JoyAI-Echo:京东长视频框架](ch05/100-ai.html)
- [Fastlane 短视频内容](ch03/116-fastlane-create-winning-short-form-content-in-seconds.html)
- [Agentium Agent Framework](ch03/035-agent.html)
- [纳德拉「Token 资本」论](ch05/100-ai.html)
- [800 行 OpenClaw tool 消息总线](ch03/035-agent.html)
- [Harness Engineering 7 层架构](ch03/084-claude-code.html)
- [A²RD 长视频一致性框架](ch03/035-agent.html)
- [Anthropic 缓存 Token 经济](ch01/1532-anthropic.html)
...
[已有 Meituan AIGC Poster 实体](ch05/100-ai.html) 和 [PosterCraft 实体](ch05/100-ai.html) 覆盖了相关技术，本文补充了大规模生产部署的实践经验与性能数据。生产环境下，该系统在保证设计质量的同时将单张海报生成成本降低了 80%。
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/美团海报生成-aigc-技术创新与实践.md)
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/automatically-redact-pii-in-images-with-amazon-nova.md)
...
The M-a-O pattern aligns with broader trends in [Agent Harness production deployment](ch03/035-agent.html), where "a model that delegates to tools outperforms a model that tries to do everything." The key insight: Nova is not required to be pixel-perfect at segmentation or character-perfect at OCR — it only needs to be good enough at **deciding what to route where and when**.
...
5. **The coordinator model doesn't need to be the strongest model**: Nova 2 Lite is a cost-optimized model, not the most powerful in the Nova family. The M-a-O architecture means the coordinator only needs sufficient reasoning capability to route tasks correctly — the specialized tools handle precision work. This has implications for model selection in [AI system architecture](ch05/100-ai.html) more broadly.

## Related Entities

- [Agent Harness Production Deployment](ch03/035-agent.html)
- [AI Security & Telemetry](ch03/084-claude-code.html)
- [AI System Evaluation](ch05/100-ai.html)
- [Harness Engineering Framework](https://github.com/QianJinGuo/wiki/blob/main/concepts/harness-engineering-framework.md)
- [Agent Routing Patterns](ch03/035-agent.html)
...
1. **双料 Fellow**：Daniel Povey 于 2023 年当选 IEEE Fellow，2026 年再获 ISCA Fellow，成为语音领域罕见的"双料 Fellow"
2. **Kaldi 生态奠基人**：Kaldi 是全球使用最广泛的语音识别开源框架，几乎所有语音团队都曾或正在使用它
3. **新一代 Kaldi 四大子项目**：加入小米后推出 k2、Lhotse、Icefall、Sherpa 四大子项目，在语音识别与合成方向持续取得里程碑式进展
4. **学术影响深远**：谷歌学术引用超 6 万次，提出 MPE 序列训练、时延神经网络（TDNN）、X-vectors、LF-MMI 等语音识别核心方法
5. **OpenViking 生态联动**：新一代 Kaldi 的开源架构与小米"人车家全生态"深度耦合，并赋能全球开发者社区
...
1. **k2**：语音识别 FST（加权有限状态转换器）框架
2. **Lhotse**：语音数据准备与增强工具
3. **Icefall**：语音识别 recipes 与示例
4. **Sherpa（sherpa-onnx）**：端云一体推理引擎，支持高通/昇腾/瑞芯微等主流 NPU，覆盖 Android、iOS、嵌入式与服务端

**关键技术突破**：
- **Zipformer & Zapformer 编码器**：Zipformer 是首个严格超越 Google Conformer 的语音编码器，被 ICLR 2024 接收为 Oral 论文（前 1.2%）；2026 年升级版 Zapformer 将识别精度再提升 10%-15%。Zipformer 可在树莓派上实时运行
- **OmniVoice 多语言 TTS**：2026 年发布支持 600+ 语言/方言的零样本语音合成模型（0.8B 参数，Qwen3 初始化），仅需 3-10 秒参考音频即可跨语言克隆音色，中英文基准达 SOTA，开源 3 个月 Huggingface 下载量超 500 万次
- **CR-CTC（Consistency-Regularized CTC）**：ICLR 2025 接收，让纯 CTC 模型性能比肩 Transducer，在 LibriSpeech、AISHELL-1、GigaSpeech 等基准上刷新 SOTA
- **ZipVoice**：基于 Flow Matching + Zipformer 骨干的零样本语音合成，CPU 单线程可达实时
- **FlashAR+**：生成效率较原始自回归范式提升近 83 倍的推理加速方案
...
Daniel Povey 的影响力之所以超越一般学者，根本原因在于他构建了**语音识别领域的开源基础设施**。第一代 Kaldi 解决了"语音识别算法可复现"的问题——在深度学习爆发的前夜，它为全球研究者提供了一个统一、可扩展的实验平台。新一代 Kaldi 则转向了"全栈部署"——从模型训练到边缘推理，从单一 ASR 到多语言 TTS，从学术界到工业界。
...
1. **开源基础设施是 AI 时代的核心战略资产**：Kaldi 证明了构建行业级开源基础设施的长期价值——其影响力远超单篇论文或单个产品，能够定义整个技术领域的发展方向。
2. **"学术 + 工程 + 开源"三位一体的科学家模式**：Daniel Povey 同时具备学术深度（超 6 万次引用）、工程能力（构建可用的工具链）和开源运营（构建活跃的社区），这种复合能力是 AI 领域最稀缺的。
3. **语音交互是 AI 时代的默认交互方式**：从新一代 Kaldi 的技术布局看，多语言、低延迟、全平台、零样本——语音交互正在从"可用"向"无处不在"演进，这对 Agent 产品的交互设计具有重要启示。
4. **中国企业吸引国际顶级科学家的能力正在增强**：Daniel Povey 选择加入小米并持续迭代 Kaldi，表明中国 AI 企业已经具备了吸引国际顶尖人才的技术土壤和开源文化。

## 相关实体

- [小米 Harness Engineering 实践](ch05/129-harness-engineering.html)
- [小米零售 AI 工程化](ch05/100-ai.html)
...
本文从**透明度/可解释性**角度分析 DiffusionGemma，与现有 [DiffusionGemma 技术架构](ch01/1429-0.html) 实体（侧重模型架构、MoE 设计、推理加速）形成互补。
...
## 相关实体
- [AI Detection and Response (AIDR): A Zero-Impact Operating Model](ch12/081-ai-detection-and-response-aidr-a-zero-impact-operating-model.html)
- [AWS Model Agility: 6步LLM跨代际迁移框架](ch11/171-aws-generative-ai-model-agility-framework.html)
...
- [用 Kiro构建 AI：基于 AWS 基础设施快速构建企业级 Agentic AI 平台 | 亚马逊AWS官方博客](ch05/100-ai.html)
- [AI 驱动的跨云网络搭建：用 Claude Code 和 Kiro CLI 实现 AWS-腾讯云 IPSec VPN 双隧道互联 | 亚马逊AWS官方博客](ch05/100-ai.html)
- [在 Amazon Bedrock 上为 Claude 应用设计稳健的 Prompt Cache 策略](ch01/1442-claude.html)
...
- [AMap Abot Earth 0.5 3D Native World Model](ch01/1429-0.html)
...
## 五类记忆分层结构
与 [Agent Memory 架构本质](ch03/035-agent.html) 的六维度记忆单元相比，PersonaVLM 的记忆分层更贴近认知心理学框架：
| 类型 | 功能 |
|------|------|
| 性格画像 | 大五人格量化追踪，动态更新 |
| 核心记忆 | 用户基础属性（身份、职业） |
...
## 相关页面
- [ChatGPT Memory](ch01/550-chatgpt-memory.html) — OpenAI 的记忆实现对比
...
## 相关实体
- [Liteframeefficientvisionencodersunlockframescalinginvideollms](ch01/611-scaling.html)
...
- [Aws Sun Finance Ai Id Extraction Fraud Detection](ch05/100-ai.html)
...
- [PilotDeck：清华系Agent操作系统](ch03/035-agent.html) — 同团队（OpenBMB/面壁智能/清华）的 Agent OS 项目
- [EdgeClaw — 端云两栖龙虾框架](ch04/619-edgeclaw.html) — 同团队的开源框架
...
- [LLaVA-OneVision-2](ch01/1065-vlm.html)（同类全帧率视频语言模型）
- [Video RAG 分块策略](ch01/411-rag.html)（视频检索的互补方向）
...
[原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/别让格式杀死思想logics-parsing-v2定义文档解析新边界.md)
...
## 相关实体
- [Context Not Free Long Document Agent Architecture Raunak](ch03/035-agent.html)
...
- [Xiaomi Ai Icml 2026 11Papers](ch05/100-ai.html)
- [Sensnova U1 Deep Dive Jiqizhixin D8602Ded5C51](ch01/1429-0.html)
...
## 相关实体
- [Helloworldmedia.Notion Self Filming Guide By Hello World Media 2F60Dfa5E2E180Cfa](ch01/141-self-filming-guide-by-hello-world-media.html)
- [Self Filming Guide By Hello World Media 2F60Dfa5E2E180Cfa6Efcef23C882E57](ch01/141-self-filming-guide-by-hello-world-media.html)
- [How To Build Audio Transcription Agent](ch04/561-how-to-build-audio-transcription-agent.html)
...
## 相关实体
- [gemma 4 模型发布 — google deepmind 开源权重家族在 amazon bedrock 上线](ch01/853-gemma-4.html)
...
## 相关对照
- [Microsoft Build 2026](ch05/100-ai.html) —— 大厂云端模型（MAI-Thinking-1）
- [ANOLISA v0.3](ch03/035-agent.html) —— 阿里 Agentic OS（可在本地跑多模态模型）
...
这一范式在 OOD 设置下的表现尤其突出——当物体位置变化、环境扰动或首次抓取失败时，具备世界状态表征的模型可以理解"任务还没有结束""物体仍然存在""当前状态距离目标还有差距"，从而继续行动。这与 [Agent落地真相 协议 成本与进化 关于智能体从能跑通到能投产的讨论](ch03/035-agent.html) 中讨论的"鲁棒性来自对状态空间的理解"高度一致。
...
- [Baai Orca Next State Prediction World Model](ch05/100-ai.html) — BAAI Orca 在实体化世界模型方向的研究
- [Agent落地真相 协议 成本与进化 关于智能体从能跑通到能投产的讨论](ch03/035-agent.html) — 从能跑到能投产的工程落地
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/智源悟界robobrain-orca多模态表征世界模型.md)
→ [新智元补充报道](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/智源悟界robobrain-orca多模态表征世界模型-2026-07-08.md)
...
ICRDrag 的重要创新在于将区域拖拽重新定义为**上下文学习任务**。模型不再是一个「端到端生成器」，而是一个「阅读理解器」——一次性输入原图（上下文）、源区域掩码（问题指示）、目标区域掩码（期望位置），然后直接输出编辑结果。
...
ICRDrag 的模态专属 LoRA 为图像分支和掩码分支使用独立的低秩适配参数，使两个模态可以独立调优。这一设计的经济性在于：LoRA 仅需微调少量参数（原参数的 0.1-1%），即可实现模态特异性学习，而无需为每个模态训练完整的独立网络。这也是 参数高效微调（PEFT，参见 [MoE 架构](https://github.com/QianJinGuo/wiki/blob/main/concepts/moe-mixture-of-experts-2025.md)） 方法在视觉编辑领域的一个重要应用。
...
这种设计体现了将产品级容错性纳入模型训练的思路：不是要求用户精确操作，而是让模型理解用户的「大致意图」。这与 [Harness Engineering](https://github.com/QianJinGuo/wiki/blob/main/concepts/harness-engineering-framework.md) 中的「容错设计」原则一致——系统应该对非精确输入保持鲁棒性。

## 实践启示

1. **从单点控制到区域控制是 AI 交互的普遍趋势**。图像编辑如此，[Agent 系统](ch03/102-hermes-agent.html) 的任务控制亦然——提供精确的约束（掩码）比模糊的指示（点对）更容易获得预期的结果。在设计 Agent 交互界面时，应优先考虑「约束性输入」而非「自由文本提示」。

2. **双向注意力约束比单向更适合空间一致性任务**。ICRDrag 的源-目标双向对应约束确保了编辑前后的一致性。在 [多 Agent 系统](https://github.com/QianJinGuo/wiki/blob/main/concepts/multi-agent-systems.md) 中，双向通信比单向指令能更有效地维持系统状态的一致性。
...
- [扩散模型](https://github.com/QianJinGuo/wiki/blob/main/concepts/diffusion-model-architecture.md) — 图像生成与编辑的基础框架
- [注意力机制](https://github.com/QianJinGuo/wiki/blob/main/concepts/attention-mechanism.md) — Transformer 中的核心组件
- [Hermes Agent](ch03/102-hermes-agent.html) — Agent 系统中的交互控制设计
...
- [扩散模型架构](https://github.com/QianJinGuo/wiki/blob/main/concepts/diffusion-model-architecture.md) — 扩散模型在图像生成领域的基础理论与方法
...
## 相关实体
- [Elf Embedded Language Flows Hekaiming](ch05/100-ai.html)
...
- [Claude Code](ch03/084-claude-code.html) — 主要工作台（被补上眼睛和耳朵的那位）
- [Agent 输入侧瓶颈背景](ch03/035-agent.html)
## 相关实体
- [Video Rag Chunking Strategy](ch01/411-rag.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/vision-multimodal.md)
...
## 相关实体
- [Olmoearth V1 1 Efficiency](ch01/200-1.html)
- [Kamacoder Agent Context Drift Tool Hallucination](ch03/035-agent.html)
- [Olmo Hybrid Gdn Wave 2026](ch01/1429-0.html)
- [How Llms Actually Work 0Xkato](ch01/1473-how-llms-actually-work-0xkato-transformer-walkthrough.html)
...
- [VLX-Seek](ch05/100-ai.html) — 细粒度视觉感知与定位
- [VLX-Go](ch05/100-ai.html) — 具身导航与执行
...
同期工作 Vision Banana 证明：把图像生成模型拿来做视觉理解，效果比专门训练的判别式模型还强——"会画画的 AI 天然就会认东西"。ACE-ViDiHand 把同样的逻辑从图像推到视频、从 2D 推到 4D：**会生成视频的 AI，天然就懂手怎么动**。这与 [Zhuji Dynamics Pre Ipo Embodied Ai 2026](ch05/100-ai.html) 中张巍强调的"手部操作数据不足是具身智能最大短板"形成呼应——ACE-ViDiHand 恰好给出了解决方案。
...
2. **视频生成模型的内部表征是尚未充分挖掘的数据金矿**。ACE-ViDiHand 证明了 DiT 的中间层包含可用于感知任务的丰富几何信息。对于 [Agent Harness Dingtalk Recruitment](ch03/035-agent.html) 等需要环境感知的 agent 系统，同样可以思考如何利用大模型的"副产物"表征。
...
- [Zhuji Dynamics Pre Ipo Embodied Ai 2026](ch05/100-ai.html) — 逐际动力强调手部操作数据瓶颈，与 ACE-ViDiHand 的技术路径互补
...
- [Lingbot Vla 2 60000H Open Source Vla](ch12/036-vla.html) — 开源 VLA 模型，与手部动捕结合可形成完整感知-控制闭环
...
MSL 架构的核心创新在于图像和视频共享同一骨干网络，仅在输入/输出层有差异。这种设计大幅简化了训练和部署流程，使得同一个预训练模型可以同时支持生成和视频理解任务。与 [DiffusionGemma](ch01/1429-0.html) 等纯自回归扩散模型不同，MSL 在多尺度潜在空间上学习表示，既能捕捉图像的精细局部细节，又能理解视频帧之间的时间动态

### 2. Agentic 生成范式的转变

Muse Image 最引人注目的设计是「Agentic Image Generation」——将 LLM 作为图像生成的核心控制器。这与 [Meta 的 Agent 生图模型](ch03/035-agent.html) 方向一致，代表了图像生成从「模型中心」走向「Agent 中心」的范式转变。LLM 不再只是文本理解模块，而是生成流程的总指挥：
...
这种从「OpenAI → 一线大厂」的人才回流，与 [Agent Harness 招聘实践](ch03/035-agent.html) 中人才流动的宏观趋势相呼应。
...
- [VLX-Flow](ch05/100-ai.html) — 系列开篇：流式视频理解 VLM
- [VLX-Seek](ch05/100-ai.html) — 系列第二弹：3B 细粒度感知 VLM
...
## 相关实体
- [Bonsai Image 4B Quantization](ch05/100-ai.html)
...
- [Openai Models Codex Amazon Bedrock Ga](ch05/100-ai.html)
- [Stochastic Parrot Language Models And Meaning](ch01/648-language-models-and-meaning.html)
- [MOC](https://github.com/QianJinGuo/wiki/blob/main/moc/vision-multimodal.md)
...
这一模式与 [知识代理](ch03/035-agent.html) 的思路异曲同工——在特定领域注入结构化知识的小模型可以超越通用大模型。
...
这与 [模型规模推演](ch01/808-model-size-scaling-in-2023-2031.html) 中关于 sparsity 作为参数放大器的讨论形成有趣对比——Moebius 走的是另一条路：不是增加总参数并稀疏化，而是直接在架构层面大幅压缩。
...
- [模型规模推演](ch01/808-model-size-scaling-in-2023-2031.html) — 模型大小与硬件约束的系统分析
- [知识代理超越前沿模型](ch03/035-agent.html) — 小模型+领域知识超越大模型的另一范式
...
- [Nvidia Gamma World Multi Agent World Model](ch03/035-agent.html)
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/diffusion-model-consistency-survey-ustc-2026.md)
...
相比前代 FLUX 1/2（参见 [图像生成模型](ch03/035-agent.html)），在复杂 prompt 处理、多语言文字渲染方面显著提升。
...
2. **端侧效率**：两次完整的生成式大模型推理难以满足移动设备功耗预算
...
1. **交替训练策略**：在不同训练阶段分别强化画质恢复能力和虚化渲染能力，让模型逐步学会在两种目标间取得平衡。

2. **对焦区域感知的注意力机制**：引入由深度信息生成的 defocus map，通过掩码注意力机制区分对焦区域与非对焦区域，在注意力计算中加入空间约束，减少超分和虚化间的干扰。

3. **退化感知的深度估计**：针对高倍变焦图像中常见的噪声、模糊和细节缺失，设计退化感知模块，在低质量输入上仍能获得稳定深度先验，为后续虚化提供可靠空间结构。
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/vivo-magicbokeh-cvpr-2026-generative-bokeh-diffusion.md)

> 参见 [CVPR 2026 DGAF-VSR 视频超分辨率](ch01/1429-0.html)、[CVPR 2026 Omni2Sound 多模态音频](ch01/1429-0.html) 等同届 CVPR 2026 论文实体。
...
传统机器人世界模型通常接收关节角、末端位姿或控制向量，这些信号与具体机械结构绑定。Masked Visual Actions（掩码视觉动作）将机器人动作表示为**视频中随时间逐帧变化的时空像素掩码**——保留实体在整段视频中的时空像素区域，其余区域统一置为灰色背景。外形、位置、尺度和接触过程都直接可见。
...
该方法可归类为[基于视频的世界模型](https://github.com/QianJinGuo/wiki/blob/main/concepts/ai-coding-agent-from-helloworld-to-production.md)的新范式——不是用关节角或控制向量表示状态，而是直接用像素级时空掩码统一世界预测和行为生成。与 [BAAI ORCA 状态预测世界模型](ch05/100-ai.html)、[轻交互世界模型推理](https://github.com/QianJinGuo/wiki/blob/main/entities/light-interaction-world-model-inference.md)、[循环世界模型](https://github.com/QianJinGuo/wiki/blob/main/entities/loopwm-looped-world-models.md)等方案相比，其核心差异在于**动作表示由数值向量变为视觉掩码**，大幅降低了对具体机械结构的依赖。
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/feifei-li-masked-visual-actions-world-model-2026.md)
...
- [LASER: ACL 2026 视觉推理潜叠加](ch01/1482-laser-acl2026-latent-superposition-visual-reasoning.html)
- [Flat Feedforward Latent Triangle Splatting](https://github.com/QianJinGuo/wiki/blob/main/entities/flat-feedforward-latent-triangle-splatting.md)

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/colt-eccv-2026-latent-thought-chain-multimodal-reasoning.md)
...
- **与 [SaaS-Bench](ch03/035-agent.html)** 互补：SaaS-Bench 评测 Agent 在真实系统中工作能力；ABot-Earth 0.5 生成 Agent 训练所需的 3D 世界
- **与 [Agent 六机制](ch03/035-agent.html)** 呼应：六机制中"环境仿真"的具体实现 —— 指数级训练场景
- **与 [Anthropic 生物学 Agent 数据基础设施](ch03/035-agent.html)** 平行：都揭示"非合成数据是真实世界 AI 的必要条件" —— 真实时空数据 / 真实生物数据 vs 合成数据
