...
- [A2Rd Agentic Autoregressive Diffusion Long Video](ch04/502-agent.md)（多模态生成对照）
...
- [Snowflake Agentic Enterprise Summit 2026](ch04/502-agent.md)（Snowflake 真实感场景对照）
...
- vs **[Video Agent 范式迁移与算力-人才飞轮](ch04/502-agent.md)** — 那是**底层视频模型视角**(nvidia Cosmos + xAI Grok Imagine),本文是**pipeline 编排层视角**。两层视角互补。
...
- vs **[Agentium Agent Framework](ch04/502-agent.md)** — 同为 pipeline 编排思路,但 Agentium 偏**通用 agent 编排**,Pixelle-Video 偏**视频生成专精**。
- vs **[纳德拉「Token 资本」论](ch04/150-ai.md)** — Pixelle-Video 是该战略宣言"模型可替换性"哲学的**工程范本**。
- vs **[800 行 OpenClaw tool 消息总线子 agent 管理架构](ch04/502-agent.md)** — 两者都体现"**装配工胜过生成器**"的工程哲学(OpenClaw 是 agent 工具总线装配)。
...
- [Video Agent 范式迁移与算力-人才飞轮](ch04/502-agent.md)
...
- [800 行 OpenClaw tool 消息总线](ch04/502-agent.md)
- [Harness Engineering 7 层架构](ch03/073-claude-code.md)
- [A²RD 长视频一致性框架](ch04/502-agent.md)
...
## 五类记忆分层结构
与 [Agent Memory 架构本质](ch04/502-agent.md) 的六维度记忆单元相比，PersonaVLM 的记忆分层更贴近认知心理学框架：
| 类型 | 功能 |
|------|------|
| 性格画像 | 大五人格量化追踪，动态更新 |
| 核心记忆 | 用户基础属性（身份、职业） |
...
- [PilotDeck：清华系Agent操作系统](ch04/502-agent.md) — 同团队（OpenBMB/面壁智能/清华）的 Agent OS 项目
...
## 相关实体
- [Context Not Free Long Document Agent Architecture Raunak](ch04/502-agent.md)
...
- [ANOLISA v0.3](ch04/502-agent.md) —— 阿里 Agentic OS（可在本地跑多模态模型）
...
- [Claude Code](ch03/073-claude-code.md) — 主要工作台（被补上眼睛和耳朵的那位）
- [Agent 输入侧瓶颈背景](ch04/502-agent.md)
## 相关实体
...
- [Kamacoder Agent Context Drift Tool Hallucination](ch04/502-agent.md)
...
- [Agent Reliability Context Drift Tool Hallucination](ch04/502-agent.md)
...
这一模式与 [知识代理](ch04/502-agent.md) 的思路异曲同工——在特定领域注入结构化知识的小模型可以超越通用大模型。
...
- [知识代理超越前沿模型](ch04/502-agent.md) — 小模型+领域知识超越大模型的另一范式
...
- [Nvidia Gamma World Multi Agent World Model](ch04/502-agent.md)
...
- **与 [SaaS-Bench](ch04/502-agent.md)** 互补：SaaS-Bench 评测 Agent 在真实系统中工作能力；ABot-Earth 0.5 生成 Agent 训练所需的 3D 世界
- **与 [Agent 六机制](ch04/502-agent.md)** 呼应：六机制中"环境仿真"的具体实现 —— 指数级训练场景
- **与 [Anthropic 生物学 Agent 数据基础设施](ch04/502-agent.md)** 平行：都揭示"非合成数据是真实世界 AI 的必要条件" —— 真实时空数据 / 真实生物数据 vs 合成数据
