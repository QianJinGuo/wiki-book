...
## Ch17.037 vivo MagicBokeh — CVPR 2026 Best Paper Finalist，统一扩散框架长焦虚化
...
## Ch17.038 20种机器人本体通吃！蚂蚁新一代VLA具身大脑刚刚开源了
...
## Ch17.039 Om AI VLX-Flow: 流式视频理解 VLM — VLX 系列开篇
...
## Ch17.041 ShotStream: Streaming Multi-Shot Video Generation (ECCV 2026, 港中文&快手可灵)
...
## Ch17.042 ICRDrag：ECCV 2026 首个上下文区域拖拽图像编辑模型
...
## Ch17.044 智源悟界·RoboBrain Orca：多模态表征世界模型
...
## Ch17.045 维纳智能登上Nature通讯：AI不只会回答问题，开始生成高精度行业数据
...
## Ch17.046 24G显卡可跑！南大PRISM工具包来了，多模态持续微调开箱即用
...
## Ch17.047 CVPR 2026 | DGAF-VSR: 重思基于扩散模型的视频超分辨率

> 📊 Level ⭐⭐⭐ | 2.1KB | `entities/cvpr-2026-dgaf-vsr-video-super-resolution-diffusion-taobao.md`

# CVPR 2026 | DGAF-VSR: 重思基于扩散模型的视频超分辨率 — 利用对齐特征的稠密引导

CVPR 2026 接收论文 DGAF-VSR 提出一种基于扩散模型的视频超分辨率方法，发现特征域比像素域更适合提供稳定的时序引导，且在高分辨率尺度下进行光流变形能更有效保留高频细节（但存在最优尺度）。

## 核心创新

- **光流引导变形模块（OGWM）**：在上采样特征空间执行对齐以减少细节损失，利用高分辨率尺度下的光流变形保留高频细节，同时避免最优尺度之外的效果退化。
- **特征级时序条件模块（FTCM）**：在扩散过程中注入稠密的跨帧特征引导，实现更精准的高频恢复与帧间一致性。

## 实验结果

在 REDS4、Vid4 和 VideoLQ 等多个合成与真实数据集上全面超越现有方法：
- **LPIPS**（感知质量）：SOTA
- **PSNR**（重建保真度）：SOTA
- **tLPIPS**（时序一致性）：SOTA

显著提升视频纹理自然度、结构稳定性和运动连贯性，为短视频、直播等场景的实用化超分技术提供了突破。
