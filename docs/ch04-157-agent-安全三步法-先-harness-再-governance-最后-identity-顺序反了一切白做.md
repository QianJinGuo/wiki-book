...
## Ch04.157 Agent 安全三步法：先 Harness、再 Governance、最后 Identity（顺序反了一切白做）
...
**关键互补点**：
- `[Ai Tool Poisoning Exposes A Major Flaw In Enterprise Agent Security](/ch04-277-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s/)` — MCP supply chain 攻击模型（**怎么攻击**）
- `[Secure Ai Agents Policy Lambda Interceptors Aws](/ch04-026-secure-ai-agents-with-policy-and-lambda-interceptors-in-amaz/)` — Policy + Lambda 实现 runtime interception（**怎么防御**）
...
- `[Agent Development Crawl Walk Run Crewai Iterative](/ch04-166-agent-开发应小步快跑-第一个-agent-只需做一件事-哪怕很烂/)` — 同作者上一篇（CrewAI 迭代方法论）：先做一件事、上生产、积累数据
- `[Agent Harness Engineering Survey 2026](/ch04-069-agent-harness-engineering-a-survey/)` — Harness Engineering 综述：harness 是什么、为什么重要
- `[Ai Agents Security Survey Attack Defense](/ch07-005-ai-agents-security-survey-attack-and-defense/)` — Agent 安全 attack/defense survey（不同问题域）
