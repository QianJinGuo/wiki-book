...
2. **Anthropic 80% / 8x 一手叙述 + 17x SSRN 论文互证** — 这是 2026-06 出现的**首次由著名 AI 研究者引用的双源基准**，把"AI 写大多数代码"从传闻升级为可引用的工程现实。区别于 [Ai Recursive Self Improvement Nanogpt Prime Intellect](135-ai.html) 的"自举训练"叙事，Mollick 强调的是**"AI 在生产环境直接生产商品代码"**的工程落地（与 harness/skill 体系互补但维度不同）。
...
- [Ai Recursive Self Improvement Nanogpt Prime Intellect](135-ai.html) — 同一时间窗口的 AI 自主研究叙事（自举训练 vs 自主编码生产）— 互补视角
- [Stochastic Parrot Deep Mystery Llms](../ch01/448-llm.html) — 引述 Mollick 关于"next-word prediction 是否模拟思维"的挑战 — 同一作者不同议题
