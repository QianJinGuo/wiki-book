...
## Ch04.391 MemSlides — 层级记忆驱动的 AI Slide 生成 Agent
...
这一视角转换与 [Agent 记忆架构](ch04/504-perplexity-brain-self-improving-agent-memory-architecture.html) 中讨论的"记忆即状态管理"理念高度一致。在 LLM-based Agent 系统中，记忆的核心挑战从来不是"能否记住"，而是"该记住什么、该遗忘什么、何时从哪层记忆读取"。MemSlides 的三层分级（Profile ↔ Working ↔ Episodic）给出了一个针对创作领域的具体实现方案。
...
- [Agent 记忆架构](ch04/504-perplexity-brain-self-improving-agent-memory-architecture.html) — 层级化 Agent 记忆设计的通用框架，MemSlides 是其幻灯片生成领域的具体实例
