...
## Ch04.135 Hermes Agent 记忆系统深度拆解

> 📊 Level ⭐⭐ | 18.9KB | `entities/hermes-agent-memory-system-vs-openclaw.md`

[[raw/articles/hermes-agent-memory-system-vs-openclaw.md|原文存档]
...
**取舍**：牺牲一点即时性，换来更稳定的缓存命中和更可控的提示词结构。^[].md]
...
### 3. 压缩前的 memory flush
长会话一定会遇到压缩。Hermes 在压缩**前**先做一次 memory flush：^[].md]
...
OpenClaw 和 Hermes 代表了两种 Agent 架构思路：^[].md]
