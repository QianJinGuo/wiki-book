...
文章强调"框架无关，模型无关"，这不是营销话术，而是面对 Agent 技术快速迭代的战略选择。专用压缩模型（[Anthropic Prompt Caching Claude Code](../ch01/284-anthropic-prompt-caching-claude-code.md)）与通用基础设施的对比在这里有直接意义：锁定单一模型/框架意味着将组织的技术演进路线绑定到供应商的发布周期。分层标准化（接口/身份/可观测性） + 保持灵活性（模型/框架/Prompt）是兼顾控制力和演进速度的最优解，参考 [Agent Harness Architecture Design Production Guide](../ch05/038-agent-harness.md) 中的生产级设计原则。
