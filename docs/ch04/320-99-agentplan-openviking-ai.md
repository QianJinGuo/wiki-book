...
3. **"编排 + 记忆"正在成为 Agent 架构的标准模式**：AgentPlan 作为编排中枢、OpenViking 作为记忆体——这套架构与 [如何利用 Agentcore Openviking 快速搭建具备高效记忆的 Agent](../ch03/006-agent.html) 中介绍的 AWS Bedrock AgentCore + OpenViking 方案高度一致，说明行业正在收敛到类似的架构模式。
...
- [火山引擎 Agentic 全栈数据管理](ch04/757-agentic.html)
