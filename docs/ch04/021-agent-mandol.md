...
```mermaid
graph TB
    subgraph Before["传统: 碎片化记忆"]
        V[向量数据库] --- G[图数据库] --- R[关系型存储]
        V -.->|噪声| Q1[被动 RAG 匹配]
        G -.->|跨库开销| Q1
        R -.->|遗漏关联| Q1
    end
    subgraph After["Mandol: 凝聚式内存原生"]
        U[统一内存语义] --> L[分层记忆模型]
        L --> S[智能量化检索]
        S --> O[Token 预算控制]
    end
...
在 LoCoMo 和 LongMemEval 两项公开长对话记忆评测基准上，Mandol 在所比较的代表性开源记忆系统中取得最优总体准确率；在以 GPT-4.1-mini 作为回答生成模型的设置下，其整体准确率分别达到 92.21% ...
