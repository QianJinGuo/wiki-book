...
这与 [Claude Code Agent Teams 任务分解](../ch01/383-claude-code-agent-teams.md) 中"任务分解策略决定 Agent 协作模式"的判断相互印证——并行不是越多越好，而是要在不引入冲突的边界内最大化吞吐。
...
这与 [AgentOps on Bedrock](ch04/228-agentops-operationalize-agentic-ai-at-scale-with-amazon-bed.md) 中关于"多模型编排降低单点故障"的设计哲学一致——异构性是鲁棒性的来源。
...
这与 [Harness Engineering Core Patterns](../ch05/092-harness-engineering.md) 中关于"声明式优先于命令式"的设计原则形成强对应——编排层应当尽量薄，复杂逻辑让模型在 prompt 中处理，这样模型升级能自动带动系统升级。
...
- [Factory Missions Multi-Agent Shipping](ch04/161-factory-missions.md)——同主题的姊妹篇
- [Claude Code Agent Teams 任务分解](../ch01/383-claude-code-agent-teams.md)——任务分解策略的另一视角
- [Harness Engineering Core Patterns](../ch05/092-harness-engineering.md)——声明式编排的工程哲学
- [OpenClaw 多 Agent 团队实践](ch04/047-openclaw-multi-agent-team-practice-v2.md)——多 Agent 落地的国内实践
- [Claude Managed Agents 企业自托管](ch04/485-claude-managed-agents.md)——Multi-Agent 的企业部署形态
- [Agent 记忆系统工程实践](../ch03/045-agent.md)——过程资产积累的方向
- [Harness 状态边界与失败闭环](../ch05/009-harness.md)——边界即文档的工程哲学
- [AgentOps on Bedrock](ch04/228-agentops-operationalize-agentic-ai-at-scale-with-amazon-bed.md)——多模型编排降低单点故障
