...
## Ch04.360 Defense at AI speed: Microsoft's new multi-model agentic security system
...
MDASH 的设计哲学是**管道是产品，模型是输入之一**。整个系统分为五个阶段 ：^[].md]

**Prepare 阶段**：摄入代码目标，构建语言感知索引，分析历史提交以绘制攻击面和威胁模型。^[].md]

**Scan 阶段**：specialized auditor agents 对候选代码路径进行审计，输出带有假设和证据的候选发现。
**Validate 阶段**：第二组 agents（debaters）辩论每个发现的可达性和可利用性。
**Dedup 阶段**：基于语义等价性（如基于补丁的分组）合并重复发现。^[].md]

**Prove 阶段**：在漏洞类型允许的情况下构造并执行触发输入，动态验证前条件并生成 PoC。^[].md]
...
→ [[raw/articles/defense_at_ai_speed_microsofts_new_multi|原文存档]
