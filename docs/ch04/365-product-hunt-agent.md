...
## Ch04.365 Product Hunt 日榜第一！把任何网站变成可被 Agent 复用的技能
...
```mermaid
mindmap
  root(("Product Hunt 日榜第一 把任何网站变成可被 Agent"))
    从页面操作界面到浏览器执行层的抽象跃迁
    浏览器自动化的技能化范式
    与现有自动化方案的差异化定位
```
...
- [Agent Harness 钉钉招聘](../ch05/058-agent-harness.html) — 生产环境中 Agent 工作流编排的实际案例
- [Browser Use v13](../ch05/009-harness.html) — 浏览器 Agent 能力框架
- [MCP 协议](https://github.com/QianJinGuo/wiki/blob/main/concepts/model-context-protocol-mcp.md) — Agent 工具调用的标准化协议
- [Hermes Agent 操作手册](../ch03/096-hermes-agent.html) — Agent 操作系统的实践指南
