...
## Ch04.675 rd-signal-2：生产规模下的 Agent 行为二元分类（Frontier Classification at Production Scale）
...
- [Trace 即 Evals（张雁飞）](ch04/438-ai-agent-trace-evals.html) — 同为 trace 驱动评估范式；rd-signal-2 给出生产规模实现路径
- [YAML 驱动 Agent 评估框架](../ch03/037-agent.html) — 声明式评估对照
- [Agent 改进的数据挖掘视角](../ch03/037-agent.html) — trace 数据的另一消费方式
- [Agent 可观测性五层架构](../ch03/037-agent.html)
- [Agent 落地真相：协议、成本与进化](../ch03/037-agent.html) — 成本治理上下文
