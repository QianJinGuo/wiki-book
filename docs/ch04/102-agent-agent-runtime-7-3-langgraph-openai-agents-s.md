...
- vs **[Harness Engineering Framework](ch05/085-harness.md)** — Harness 是 Runtime 的**外壳**;Runtime 是 Harness 的**内脏**。Runtime 7 职责 = Harness 的实现细节
- vs **[Agent Harness 架构设计与生产实践](ch05/085-harness.md)** — Production 视角更全;本文是入门视角
...
- vs **[AgentCore Harness](ch05/085-harness.md)** / **[AgentCore Managed Harness](ch04/216-agentcore-managed-harness.md)** — AWS 实现的 8 职责具体形态
...
本文将 Router 定性为"复杂 Agent 扩展层"而非第 8 个必备职责，这一判断具有重要的工程意义：**Router 的必要性是 Agent 复杂度阈值的结果，而不是功能丰富度的标志**。这与 [Agent Harness 架构设计与生产实践](ch05/085-harness.md) 的"按需引入复杂度"原则一致 — 过早引入 Router 会增加不必要的状态空间，早期 Agent 应该用提示词 + 工具选择承担简单分发。
...
本文核心洞察 — **Runtime 把 LLM 差异封装在执行层，让业务不被任一模型锁定** — 与 [纳德拉「Token 资本」论](ch04/069-ai.md) 的"模型可替换性是主权测试"哲学在工程层面高度吻合。[Harness Engineering Framework](ch05/085-harness.md) 将 Runtime 定位为 Harness 的内脏，而 Runtime 的模型隔离能力正是 Harness 层实现"模型无关性"的底层机制。
...
- [Agent Harness 架构设计与生产实践](ch05/085-harness.md)
...
- [AgentCore Harness](ch05/085-harness.md)
