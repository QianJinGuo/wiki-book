...
- vs **[AgentCore Harness](ch04/758-agentcore-harness.html)** / **[AgentCore Managed Harness](ch04/294-agentcore-managed-harness.html)** — AWS 实现的 8 职责具体形态
...
本文清晰区分了 **Trace（记录）和 Observability（分析）**：Trace 提供原始执行链路数据，Observability 在这些数据之上构建监控与告警。[langgraph-state-machine-under-the-hood](ch04/266-langgraph.html) 等框架的实践也印证了这一点 — 没有 Trace，Observability 就是无源之水；没有 Observability，Trace 只是无人阅读的日志。两者是 Runtime 可观测性职责的不可分割两面。
...
- [AgentCore Harness](ch04/758-agentcore-harness.html)
