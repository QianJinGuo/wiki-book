...
## Ch04.513 Agent 时间性策略（Temporal Policies）：基于轨迹的有状态授权架构
...
- [Policy + Lambda interceptors](ch04/581-ai-agent.html)：stateless Cedar policy 的先行方案；temporal policy 是其有状态扩展（同一 Policy engine 之上）
- [AI 工具投毒](ch04/459-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s.html)：工具调用层面的攻击面；temporal policy 的 output-integrity 规则正是防御此类攻击的授权层手段
- [AgentCore Harness](ch04/775-agentcore-harness.html)：temporal policies 运行于 AgentCore Gateway 外围，是 harness 安全边界的组成部分
