...
> [!contradiction]
> 与 [Hermes 记忆系统深度拆解](../ch03/098-hermes-agent.html) 一文的判断一致：Hermes 的核心优势在于 cache-aware runtime 与分层记忆设计，而非 OpenClaw 式的控制面架构。两文从不同维度（功能定位 vs 记忆工程）得出相同的互补性结论。
...
- [DeerFlow vs Hermes vs OpenClaw 深度对比](ch04/579-deerflow-hermes-openclaw.html)
