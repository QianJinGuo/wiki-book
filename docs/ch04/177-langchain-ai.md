...
## 相关主题
- [Harness Generator Evaluator Anthropic](ch01/692-anthropic.md) — Generator-Evaluator 架构
- [Agent Harness Engineering Survey 2026](ch04/503-agent.md) — Agent Harness 工程综述
- [Pi Openclaw Coding Harness](ch03/012-openclaw.md) — OpenClaw Coding Harness
...
Adish Jain 的观点揭示了一个被忽视的问题：无论选择哪种架构，如何验证智能体在沙盒内实际执行的操作都是核心挑战。这与 [Harness Generator Evaluator Anthropic](ch01/692-anthropic.md) 中 Anthropic 强调的"eval 优先"理念形成呼应——沙盒提供了隔离，但隔离不等于可控。两种模式都需要额外的行为验证层（如 trajectory logging、deterministic feedback sensors），这是当前架构讨论中缺失的关键维度。
...
## 关联阅读
- [Harness Generator Evaluator Anthropic](ch01/692-anthropic.md) — Anthropic 的 Generator-Evaluator Harness 架构，提供了智能体评测闭环的设计思路，与沙盒行为验证问题直接相关
