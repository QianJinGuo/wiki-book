...
- [Agent Harness Engineering Survey 2026](ch04/502-agent.md) — Agent Harness 工程综述
...
Adish Jain 强调的行为测试应该成为沙盒集成的标准环节，而非事后考虑。建议实现：工具调用轨迹记录、沙盒内文件变更审计、执行结果确定性校验。这与 [Agent Harness Engineering Survey 2026](ch04/502-agent.md) 中 Verification 层的最佳实践一致。
...
- [Agent Harness Engineering Survey 2026](ch04/502-agent.md) — Agent Harness 工程的系统性综述，包含沙盒（Execution environment）层的完整分类，对理解两种沙盒架构的行业上下文很有价值
