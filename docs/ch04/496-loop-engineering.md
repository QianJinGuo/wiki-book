...
- [Claude Code Loop Engineering 完整攻略](ch03/077-claude-code.html) — 同为 Loop Engineering 方法论，但本实体是阿里云真实生产环境的实战数据
- [Loop Engineering 半年实战（claude-ship）](ch01/976-claude.html) — 同为实战案例，但本实体聚焦于日志监控→自动修复这一垂直场景
