...
## Ch04.484 企业 BI 洞察 Agent 可信推理链路与决策闭环
...
```mermaid
mindmap
  root(("企业 BI 洞察 Agent 可信推理链路与决策闭环"))
    「可审计推理」是企业 BI Agent 与传统 ChatBI 的本质区别
    仪表板上下文 被忽视的企业知识金矿
    「数据血缘 业务拆解逻辑」是 BI Agent 设计的关键洞见
    语义层卡点 Agent 时代的老问题新解法
```
...
传统 ChatBI 的核心范式是「自然语言 → SQL → 图表」，本质是查询工具的 NLU 升级。而观远洞察 Agent 提出的三层推理架构，其核心突破在于将**推理过程本身作为可审计资产**。每一层都有明确的输入输出规范——指标监控触发后，系统按「异常幅度检查 → 多维度拆解 → 逐维度假设验证 → 结论关联知识 → 建议关联策略库」的固定链路执行，而非让 LLM 自由发挥。这种结构化推理链路是企业级决策工具的核心要求，因为自由联想在数据领域意味着不可靠。
...
→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/insight-agent-trustworthy-reasoning-guandata.md)
