...
**关键互补点**：
- `[Ai Tool Poisoning Exposes A Major Flaw In Enterprise Agent Security](ch05/100-ai.html)` — MCP supply chain 攻击模型（**怎么攻击**）
- `[Secure Ai Agents Policy Lambda Interceptors Aws](ch05/100-ai.html)` — Policy + Lambda 实现 runtime interception（**怎么防御**）
...
- `[Agent Development Crawl Walk Run Crewai Iterative](ch05/100-ai.html)` — 同作者上一篇（CrewAI 迭代方法论）：先做一件事、上生产、积累数据
- `[Agent Harness Engineering Survey 2026](ch05/129-harness-engineering.html)` — Harness Engineering 综述：harness 是什么、为什么重要
- `[Ai Agents Security Survey Attack Defense](ch05/100-ai.html)` — Agent 安全 attack/defense survey（不同问题域）
