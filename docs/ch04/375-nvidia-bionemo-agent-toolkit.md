...
每种技能都是一个标准化接口，Agent 只需知道"输入什么、输出什么"，而不必理解底层的科学计算细节。这与 [Agent 团队协作模式](../ch01/638-claude.html) 中讨论的"工具抽象层"理念一致——Agent 通过标准化工具接口获取领域能力，而非内化所有领域知识。
...
- [Agent Teams 与群聊模式](../ch01/638-claude.html) — Agent 团队协作与工具抽象层设计
