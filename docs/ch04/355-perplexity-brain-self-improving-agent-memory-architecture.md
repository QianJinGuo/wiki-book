...
这一区分直接回应了 [Agent Memory Architecture](/ch04-146-agent-memory-架构本质//) 中讨论的"记忆应服务于 agent 能力提升而非用户粘性"这一核心论点。
...
该图随每次交互演化，形成一个复利型知识库。与 [Agent Memory Modular Framework](/ch01-238-agent-memory-模块化框架与评测-memory-in-the-llm-era-4-模块-10-方案对比//) 的模块化架构不同，Brain 采用的是 **单一图结构** 而非分层存储。
