...
## Ch04.467 Hermes Agent Operator 上手：把一个 Agent 养成可运营系统
...
常驻 Agent 的风险会跨时间、跨入口、跨介质悄悄传播（延迟触发）。常见场景：
...
若飞/ Shann 的思路是**先养再拆**：].md]

1. 先在主 Agent 里跑通流程（第一道门）].md]
2. 用真实任务暴露不稳定因素（第二道门）].md]
3. 确认流程稳定后再独立部署（第三道门）].md]
4. 验证长期可维护性后才上 cron（第四道门）].md]
...
很多团队第一次建 Agent 时的错误是：**上来就设计完美架构**。结果是：].md]
...
1. **止血**：让一个 Agent 先跑通一个真实任务].md]
2. **记录**：把纠偏过程记下来（task-result.md）].md]
3. **健身**：从纠偏记录里提炼 Skill].md]

### 控制室建设的最少必要知识

不需要第一天就写完 5 个文件。根据任务风险程度逐步完善：].md]
...
如果无法明确回答这三个问题，**先不要拆**：].md]

1. 这个 Agent 需不需要独立的 API 凭据？].md]
2. 这个 Agent 的记忆会不会污染其他 Agent？].md]
3. 这个 Agent 的失败会不会拖垮其他 Agent？].md]
...
路由策略确定后，写进 inventory.md，下次换模型时知道影响哪些任务类型。].md]
