...
- vs **[Anthropic Claude Fable 5 on AWS:Mythos 级功能](../ch01/786-anthropic-claude.html)** — 那是**AWS 官方产品介绍**(功能/部署/内置保护),本文是**架构师工程化拆解**(Runtime 协议层)。两者互补。
...
- vs **[Claude Fable 5 — Mollick patron vs wizard](../ch01/225-claude-fable-5.html)** — Mollick 的**hands-on 用户体验视角**(4 用例 + patron vs wizard 框架),本文是**架构师工程视角**。两者对应"产品体验"vs"产品架构"。
- vs **[Claude Fable 5 提示词泄漏 — Runtime Control Plane 安全工程启示](../ch01/225-claude-fable-5.html)** — 同样用"Runtime"概念,但**VibeCoder 重点在安全工程** (Prompt 不能当保险箱 / 攻击面像系统 / 分类器组合风险),**本文重点在工程协议** (Task Brief / 能力路由 / 状态账本 / 治理层)。两者**完全互补**: 一个看 Runtime 怎么被攻击,一个看 Runtime 怎么被设计。

**若飞同系列延伸**(本文是若飞"Agent 治理"的 Runtime 工程化主轴):

- vs **[若飞 5 张卡治理 (Hermes)](161-hermes-agent.html)** — 若飞把 Hermes Agent 长跑治理拆为 5 张卡,**侧重"治理框架"**;本文把 Fable 5 Runtime 拆为 9+8+5+8 维度,**侧重"运行时契约"**。两者是**"治理框架 vs 运行时协议"** 同主题不同切面。
...
- vs **[Harness Engineering Deletable Worksite](../ch05/054-harness-engineering-deletable-worksite-ruofei.html)** — 那是 **Harness 可删工作位**的精简原则;本文是 **Runtime 4 层**(包含 Harness 作为"工具路由层"的子集)。
- vs **[若飞 agent memory architecture](133-agent-memory.html)** — 那是 **Memory 架构**;本文**执行状态账本** 5 类中包含 memory。
...
长任务 Agent 不能只靠聊天记录记住自己做过什么——这个判断来自 Anthropic 官方文档,被若飞拆解为 5 类状态账本(memory/Artifact storage/文件账本/工具历史/进度声明校验)。这与 [若飞 agent memory architecture](133-agent-memory.html) 中的"记忆架构决定 Agent 能否真正完成任务"高度相关。核心洞察是:**状态管理不只是"有没有 memory",而是"状态是否可以被验证和交接"**。进度声明必须回到本轮工具结果校验,这是一个朴素的工程原则,但在 AI Agent 场景下被放大——模型的"自信"可能掩盖实际的工具调用失败,而没有结构化账本,工程师无法事后重建真实执行路径。
...
- [Claude Fable 5 提示词泄漏 — Runtime Control Plane](../ch01/225-claude-fable-5.html)
- [Anthropic Claude Fable 5 on AWS](../ch01/786-anthropic-claude.html)
- [Claude Fable 5 and new AI safety fables](../ch01/592-claude-fable-5-and-new-ai-safety-fables.html)
- [Claude Fable 5 — Mollick patron vs wizard](../ch01/225-claude-fable-5.html)
...
- [Harness Engineering Deletable Worksite](../ch05/054-harness-engineering-deletable-worksite-ruofei.html)
