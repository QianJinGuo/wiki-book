...
5. **技术范式判断**：AI 视频增强的关键从「哪里不清楚」转向「这段视频是怎么生成出来的」——增强算法需要理解生成过程本身（内容语义、边界约束、时序一致性），这是区别于传统超分/修复的范式级差异，与 [AI MediaKit 生产套件](../ch03/006-agent.html) 的交付层能力（画质增强保留艺术风格同时重建高频细节）一致。
