...
## Ch04.549 CMU PACE — Proxy for Agentic Capability Evaluation
...
这意味着 PACE 适合作为**筛选和排序工具**（筛 checkpoint、比版本、决定值不值得跑完整评测），但不能完全替代完整评测。这与 [Agent Harness 架构](../ch05/063-agent-harness.html) 中强调的评测标准化重要性相互印证——没有标准化的 Harness，代理评测的参考价值也受限。
