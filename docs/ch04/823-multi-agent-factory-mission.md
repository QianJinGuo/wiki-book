...
## Ch04.823 Multi-Agent 架构：Factory Mission 系统的方法论
...
这一设计与 [Harness 状态边界与失败闭环](../ch05/080-harness.html) 中强调的"边界即文档"的工程哲学一致——Agent 系统的可靠性不来自 Agent 自身的记忆，而来自跨边界时强制落盘的中间表示。
...
这与 [Harness Engineering Core Patterns](../ch05/096-harness-engineering.html) 中关于"声明式优先于命令式"的设计原则形成强对应——编排层应当尽量薄，复杂逻辑让模型在 prompt 中处理，这样模型升级能自动带动系统升级。
