...
## 相关实体
- [Agentic RL: Token-In Token-Out Done Right](ch04/615-agentic-rl-token-in-token-out-done-right.html)（相关：Token-In/Token-Out RL 训练框架，但 APPO 进一步细化到决策点）
- [港中文 SLIM：动态技能生命周期管理](ch04/538-agentic-rl.html)（相关：另一条 Agentic RL 主线——技能生命周期管理）
