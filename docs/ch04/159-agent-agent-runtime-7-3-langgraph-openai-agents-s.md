...
## Ch04.159 Agent 的骨架:Agent Runtime 7 大职责 + 3 主流框架对比 (LangGraph / OpenAI Agents SDK / 自研)
...
- vs **[阿里云云原生安全护栏三域演进](../ch05/101-ai.html)** — 那是从云资源到 AI 模型到模型间路由的**三域护栏**;本文的"风险控制"职责是 Guardrail 的**单点实现**视角
...
- vs **[Agent Harness 架构设计与生产实践](../ch05/063-agent-harness.html)** — Production 视角更全;本文是入门视角

**框架生态**(本文 3 主流框架):

- vs **[Google Agent Executor Distributed Runtime](ch04/005-agentrun.html)** — Google 自家 Runtime 实现;与本文 LangGraph / OpenAI SDK 平行
- vs **[Anthropic Claude Managed Agents Platform](../ch01/398-anthropic-claude-managed-agents.html)** — Anthropic Managed Agents 视角
- vs **[Amazon Bedrock AgentCore Runtime 深度分析](ch04/639-amazon-bedrock-agentcore.html)** — AWS Bedrock AgentCore 视角;与本文 LangGraph / OpenAI SDK 平行
- vs **[AgentCore Harness](ch04/755-agentcore-harness.html)** / **[AgentCore Managed Harness](ch04/290-agentcore-managed-harness.html)** — AWS 实现的 8 职责具体形态
...
本文将 Router 定性为"复杂 Agent 扩展层"而非第 8 个必备职责，这一判断具有重要的工程意义：**Router 的必要性是 Agent 复杂度阈值的结果，而不是功能丰富度的标志**。这与 [Agent Harness 架构设计与生产实践](../ch05/063-agent-harness.html) 的"按需引入复杂度"原则一致 — 过早引入 Router 会增加不必要的状态空间，早期 Agent 应该用提示词 + 工具选择承担简单分发。
...
本文核心洞察 — **Runtime 把 LLM 差异封装在执行层，让业务不被任一模型锁定** — 与 [纳德拉「Token 资本」论](../ch12/003-token.html) 的"模型可替换性是主权测试"哲学在工程层面高度吻合。[Harness Engineering Framework](../ch05/057-harness-engineering.html) 将 Runtime 定位为 Harness 的内脏，而 Runtime 的模型隔离能力正是 Harness 层实现"模型无关性"的底层机制。
...
- **未涉及 prompt injection / 越权 / 隐私等高级 Guardrail**: 本文风险控制仅 1 段;深度安全参考 [阿里云安全护栏三域](../ch05/101-ai.html)
...
- [阿里云安全护栏三域](../ch05/101-ai.html)
- [阿里云 Agent 演化四阶段](../ch03/035-agent.html)
- [Harness Engineering Framework](../ch05/057-harness-engineering.html)
- [Agent Harness 架构设计与生产实践](../ch05/063-agent-harness.html)
- [Google Agent Executor Runtime](ch04/005-agentrun.html)
- [Anthropic Claude Managed Agents](../ch01/398-anthropic-claude-managed-agents.html)
- [Amazon Bedrock AgentCore Runtime](ch04/639-amazon-bedrock-agentcore.html)
