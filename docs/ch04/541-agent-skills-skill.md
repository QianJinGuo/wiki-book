...
## Ch04.541 Agent Skills 无依赖设计：Skill 之间不传数据的哲学与实践
...
```mermaid
mindmap
  root(("Agent Skills 无依赖设计 Skill"))
    核心设计原则
      个自描述字段 零个指向外部
      Progressive Disclosure 三阶段
      availableskills 是平铺清单 不是 DAG
    与四种体系的反差对比
    编排责任推给了 agent 层
...
- → [Agent Skill 规范、构建与设计模式](ch04/269-agent-skill.html) — 更多关注如何构建 skill（Skill-Creator、设计模式等）
- → [Hermes Agent Skills 源码级拆解](ch04/439-hermes-agent-skills.html) — 同一作者（术哥）的源码级分析
