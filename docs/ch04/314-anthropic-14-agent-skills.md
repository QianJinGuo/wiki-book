...
## Ch04.314 Anthropic 官方技能最佳实践：14 个可复用的 Agent Skills 设计模式

> 📊 Level ⭐⭐⭐⭐ | 34.9KB | `entities/anthropic-14-skill-patterns-best-practices.md`
...
百亿补贴详情助手（bybt-detail-assistant，50+ commit）采用"决策树路由 + 远程 Agent API 桥接"架构：本地负责信息收集/商品查询/结果呈现（薄客户端），日志搜索/截屏分析等深度分析交给远程 Agent。桥接脚本 xiaomi.py 定义稳定协议，远程 Agent 升级时本地 Skill 无需修改。

### 决策树按需加载与注意力聚焦

SKILL.md 划分为共享全局约束→决策树→三条逻辑互斥路径。埋点解读逻辑不塞 prompt 只描述调用协议，Python 脚本固化逻辑比 LLM 快 7 倍。
