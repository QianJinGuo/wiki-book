...
每个 Trace 事件携带 step 和 status 信息，可审计模型每一步做了什么。这在生产环境中至关重要——当回答出现问题时，可以通过 Trace 回放定位是规划出错、检索遗漏还是综合错误。这与 [Bedrock AgentCore 运行时](ch03/035-agent.html) 的可观测性设计一脉相承。
...
Agentic Retrieval 将这些统一为服务端能力，借助 AWS 的 IAM 集成和 CloudWatch 日志，安全模型和可观测性开箱即用。这体现了 [Bedrock 平台化](ch11/299-bedrock.html) 的核心理念——让开发者专注业务逻辑，而非基础设施编排。
...
- [Bedrock AgentCore 运行时](ch03/035-agent.html)
- [Bedrock 跨区域推理](ch11/299-bedrock.html)
- RAG 框架对比
- [三种 RAG 架构](ch03/035-agent.html)
- RAG 检索增强生成
