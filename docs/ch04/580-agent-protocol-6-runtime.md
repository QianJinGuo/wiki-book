...
## Ch04.580 Agent Protocol 不变层：跨框架的 6 个稳定 Runtime 对象
...
- [Agent Runtime 7 大职责](ch03/044-agent.md) — 从 Runtime 职责视角（状态持久化、工具编排、可观测性等）出发，与本文的 Protocol 视角互补
- [From Agent Protocol to Harness Skill](ch04/354-from-agent-protocol-to-harness-skill.md) — 从 MCP/A2A 协议到 Harness Skill 的演进路径，与本文的 Runtime Protocol 抽象视角不同
- [Google Agent Executor](ch03/044-agent.md) — 分布式 Runtime 实现案例
- [Agent Harness 架构](ch03/044-agent.md) — Harness 作为 Runtime 的上层封装
- [Agent 架构关键变化：Harness 成为新后端](ch03/044-agent.md) — Harness 作为 Runtime 演进方向
- [MCP · Skill · Agent · LLM · Harness](ch04/245-skill.md) — 高层架构关系图
