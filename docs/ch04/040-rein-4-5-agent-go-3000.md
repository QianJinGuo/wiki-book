...
## Ch04.040 Rein：4 模块 + 5 类型边界防止 agent.go 膨胀到 3000 行
...
### vs Claude Code 源码分析（[Claude Code 20000 Char Source Analysis](../ch03/057-claude-code.html)）
- Claude Code 98.4% 基础设施 + 1.6% AI 决策
- Rein 用 4 模块 + 5 类型边界把 3000 行结构化

### vs Agent Harness 上下文管理（[Agent Harness Context Management Working Set](../ch05/035-agent-harness.html)）
- 工作集视角：logical_messages 原样保留 + 投影 = Rein 的"存储完整，发送压缩"

### vs wow-harness v3 事件溯源（[Wow Harness V3 Governance Protocol](../ch05/008-harness.html)）
- v3 = 跨 session 事件时间线 + 概念图
- Rein = session 原子写入 + 完整 logical_messages
- **共同点**：完整历史 + 不可篡改 + 确定性回放

### vs PilotDeck 白盒记忆（[Pilotdeck Agent Os Openbmb Tsinghua](../ch03/004-agent.html)）
...
## 相关对照
- [Claude Code 20000 字符源码分析](../ch03/057-claude-code.html)
- [Agent Harness 上下文管理](../ch05/035-agent-harness.html) —— 工作集视角
- [wow-harness v3](../ch05/008-harness.html) —— 事件溯源 + 概念图
- [PilotDeck](../ch03/004-agent.html) —— 白盒记忆
- [Agent Harness 架构](../ch05/035-agent-harness.html) —— 7 层 harness 模型
- [17 种 agent 架构演进](440-17-agent.html) —— 类型边界视角
