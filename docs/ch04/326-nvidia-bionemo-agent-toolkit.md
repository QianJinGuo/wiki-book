...
这一策略与 [后端 AI 友好标准化路径](ch05/094-ai.html) 中讨论的"平台化基础设施"思路一致——通过定义标准化的 Agent 技能接口，让不同模型（Claude、GPT、Nemotron 等）都能接入同一套科学工具集。
...
这些要求与 [Harness Engineering 实践](ch03/035-agent.html) 中讨论的生产级 Agent 设计原则高度一致——可靠性、可审计性、可复现性是科学 Agent 从原型走向落地的关键。
