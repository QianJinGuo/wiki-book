...
每种技能都是一个标准化接口，Agent 只需知道"输入什么、输出什么"，而不必理解底层的科学计算细节。这与 [Agent 团队协作模式](../ch01/965-claude.html) 中讨论的"工具抽象层"理念一致——Agent 通过标准化工具接口获取领域能力，而非内化所有领域知识。
...
这些要求与 [Harness Engineering 实践](../ch05/115-harness-engineering.html) 中讨论的生产级 Agent 设计原则高度一致——可靠性、可审计性、可复现性是科学 Agent 从原型走向落地的关键。
...
- [Agent Teams 与群聊模式](../ch01/965-claude.html) — Agent 团队协作与工具抽象层设计
- [Harness Engineering 实践](../ch05/115-harness-engineering.html) — 生产级 Agent 的可靠性设计原则
