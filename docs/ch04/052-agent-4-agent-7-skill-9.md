# 一个多 Agent 零人工运维系统的设计复盘：4 个 Agent、7 个 Skill 与 9 条工程判断

## Ch04.052 一个多 Agent 零人工运维系统的设计复盘：4 个 Agent、7 个 Skill 与 9 条工程判断

> 📊 Level ⭐ | 0.8KB | `entities/opspilot-zero-multi-agent-ops-9-judgments.md`

> -> [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/opspilot-zero-multi-agent-ops-9-judgments.md)

OpsPilot Zero —— 一个刻意做小的多 Agent 零人工运维 Demo，面向 GOAI 大赛 Agent Infra 赛道的最小参考实现。

## 来源

- 原文: [一个多 Agent 零人工运维系统的设计复盘：4 个 Agent、7 个 Skill 与 9 条工程判断](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/opspilot-zero-multi-agent-ops-9-judgments.md)
- 原始链接: : "https://mp.weixin.qq.com/s/oxIuX6WJhSmzh_0gdxFi8w

---

