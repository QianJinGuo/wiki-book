...
## Ch04.360 Perplexity Brain: Self-improving Agent Memory Architecture
...
这一区分直接回应了 [Agent Memory Architecture](ch04/503-agent.md) 中讨论的"记忆应服务于 agent 能力提升而非用户粘性"这一核心论点。
...
该图随每次交互演化，形成一个复利型知识库。与 [Agent Memory Modular Framework](ch04/503-agent.md) 的模块化架构不同，Brain 采用的是 **单一图结构** 而非分层存储。
