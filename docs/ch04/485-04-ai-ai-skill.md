...
## Ch04.485 04—为什么不能让 AI 自己评审自己？AI Skill 四层验证体系完整解析
...
这个问题本质上是 **执行者与评审者未分离** 导致的确认偏误（confirmation bias）。参见 [对抗式验证](https://github.com/QianJinGuo/wiki-public/blob/main/entities/adversarial-verification.md) 中关于 Worker-Verifier 对抗关系的论述——Verifier 若与 Worker 共享上下文，就会丧失独立校验能力。
...
- **（一）** [AI Skill 测评的底层逻辑](https://github.com/QianJinGuo/wiki-public/blob/main/entities/ai-skill-evolution底层逻辑.md) — 入门篇，介绍自判卷偏差、随机性、负向增益三大核心问题
- **（二）** AI Skill 触发与控制 — 待关联
- **（三）** AI Skill 防过拟合 — 待关联
- **（四）** 本文 — 四层验证体系完整解析

> **关联**：四层验证体系是 [Harness Engineering](../ch05/096-harness-engineering.html) 在 AI Skill 评测场景的具体实现——Layer 0-1 对应 Harness 的执行基础设施，Layer 2a/2b 对应评估回路，Layer 3 对应可观测性管道。
