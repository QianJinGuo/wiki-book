...
## Ch04.335 MemSlides — 层级记忆驱动的 AI Slide 生成 Agent
...
这一视角转换与 [Agent 记忆架构](ch04/417-perplexity-brain-self-improving-agent-memory-architecture.html) 中讨论的"记忆即状态管理"理念高度一致。在 LLM-based Agent 系统中，记忆的核心挑战从来不是"能否记住"，而是"该记住什么、该遗忘什么、何时从哪层记忆读取"。MemSlides 的三层分级（Profile ↔ Working ↔ Episodic）给出了一个针对创作领域的具体实现方案。
...
这与 [Agent 记忆力工程税](ch04/102-agent-memory.html) 中讨论的"经验迁移"挑战直接相关。工具记忆的实验结果（strict verification +72%）表明，结构化执行经验的复用对 Agent 可靠性的提升远大于简单的 Prompt 优化。
...
- [Agent 记忆架构](ch04/417-perplexity-brain-self-improving-agent-memory-architecture.html) — 层级化 Agent 记忆设计的通用框架，MemSlides 是其幻灯片生成领域的具体实例
- [Harness 上下文管理工作集](../ch05/058-agent-harness.html) — 上下文优先级分层与 MemSlides 的三层记忆优先级设计一致
- [Agent 记忆力工程税](ch04/102-agent-memory.html) — 讨论经验迁移的结构化成本，与工具记忆复用直接相关
