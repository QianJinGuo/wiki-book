...
## Ch04.632 Agentic Abstention—Agent 能及时停止吗？华盛顿大学/Allen AI 28000+ 任务基准评测
...
与 [Loop Engineering 反馈控制系统](../ch05/005-loop-engineering.html) 中的停止条件讨论直接互补——该实体从架构角度讨论停止条件设计，本文提供实证数据证明当前 Agent 的停止能力严重不足。

> 工程启示：你在 AbstentionBench 上刷高的分数不能直接外推到 ReAct 式 Agent 的停止质量。花大价钱堆模型、加 reasoning、换 scaffold，Agent 依然可能在不可行任务上无效 burn token。

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/agentic-abstention-washington-allen-2026.md)
