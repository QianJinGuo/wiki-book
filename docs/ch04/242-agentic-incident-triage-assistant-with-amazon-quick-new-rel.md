...
- [introducing the mdn mcp server](../ch01/897-20.md)
- [从 pi-main 源码拆解：顶尖 ai agent 的工程设计（17 维度全解）](../ch03/045-agent.md)
- [万级实时推理的商品领域agent实践思考和总结](../ch03/045-agent.md)
...
这种"单 agent + 多 tool 类别"的模式是当前 agentic AI 落地的标准形态，与 [Claude Code Dynamic Workflows Multi Agent Orchestration](../ch09/078-claude-code-dynamic-workflows.md) 描述的动态 fan-out 模式形成对比——Amazon Quick 提供的是 **预定义工具集的稳定编排**，而非 Claude Code 的动态 sub-agent 生成。
...
与 [Hermes Agent Tool System Architecture](../ch03/090-hermes-agent.md) 等"工具架构"主题 entity 的差异：本 entity 关注 **incident response 流程标准化** 而非 agent 内部工具系统设计。

### 4. 异步补全 vs 实时编排的对比

本架构中 **Amazon Quick 是同步编排**（单 prompt 触发 → 5 工具调用 → RCA 输出 → Asana 任务创建），与 [Ai Infra Auto Driven Skills V0 Bbuf Giantpanda](../ch01/884-ai-infra.md) 描述的"长时自驱动 agent"模式相反：
...
- [Amazon Bedrock Agentcore Gateway Mcp Extension](ch04/499-amazon-bedrock-agentcore.md) — Amazon Bedrock AgentCore 的 MCP gateway 扩展（MCP 在 AWS 的另一面）
...
- [Ai Infra Auto Driven Skills V0 Bbuf Giantpanda](../ch01/884-ai-infra.md) — 长时自驱动 agent 模式（与本 entity 的同步编排模式形成对比）
