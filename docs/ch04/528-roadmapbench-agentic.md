...
## Ch04.528 RoadmapBench — 跨版本升级的长周期 Agentic 软件开发评估基准
...
RoadmapBench 揭示了一个根本性矛盾：现有 Agent 架构大多为短周期交互设计（单次工具调用→结果反馈→下一步决策），而长周期任务需要 Agent 在数百次工具调用中维持对项目全局的理解和一致的目标导向。 这与 [Agent Harness 上下文管理](../ch05/062-agent-harness.html) 中讨论的上下文窗口滑动问题密切相关——当修改跨越 51 个文件时，Agent 无法将所有相关代码同时保持在上下文中，必须依赖外部持久化机制。
...
RoadmapBench 填补了从 SWE-Bench（单 bug fix）到 [Codex 六小时目标运行](../ch09/065-codex-goal-six-hour-run.html)（端到端功能开发）之间的评估空白。 它的定位类似 SWE-Bench 的"跨版本升级"增强版，关注的是 Agent 在已知代码库上进行系统性演进的能力，而非从零开始的创作能力。
...
RoadmapBench 的结果表明，单纯提升模型能力不足以解决长周期开发问题。Agent Harness 需要提供更强大的工作流管理、断点续传、版本控制和中间产物验证能力。 这与 [Agent Harness 架构](../ch05/062-agent-harness.html) 中讨论的生产级 Harness 设计要求高度一致。
...
- [Agent Harness 架构](../ch05/062-agent-harness.html) — 生产级 Agent 运行基础设施的设计原则
- [Codex 六小时目标运行](../ch09/065-codex-goal-six-hour-run.html) — 长链 Agent 执行的实际案例
- [Agent Harness 上下文管理](../ch05/062-agent-harness.html) — 长周期任务中的上下文窗口管理技术
