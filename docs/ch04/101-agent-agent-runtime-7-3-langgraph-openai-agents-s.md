...
## Ch04.101 Agent 的骨架:Agent Runtime 7 大职责 + 3 主流框架对比 (LangGraph / OpenAI Agents SDK / 自研)
...
- vs **[若飞 Fable 5 Runtime Contract 工程化拆解](../ch01/342-claude-fable-5.html)** — 若飞文是**Runtime Contract 框架**(Task Brief 9 字段 / 能力路由 8 维度 / 状态账本 5 类),**深度工程协议视角**;本文是**7 职责概念入门视角** + **3 主流框架对比**。两者**完全互补**: 若飞 = Runtime **如何被设计** (契约层);二曲线 = Runtime **包含什么职责** + **用什么框架实现** (职责 + 工具层)
- vs **[阿里云云原生安全护栏三域演进](ch04/070-ai.html)** — 那是从云资源到 AI 模型到模型间路由的**三域护栏**;本文的"风险控制"职责是 Guardrail 的**单点实现**视角
...
- vs **[Harness Engineering Framework](../ch05/062-harness-engineering.html)** — Harness 是 Runtime 的**外壳**;Runtime 是 Harness 的**内脏**。Runtime 7 职责 = Harness 的实现细节
- vs **[Agent Harness 架构设计与生产实践](../ch05/038-agent-harness.html)** — Production 视角更全;本文是入门视角

**框架生态**(本文 3 主流框架):

- vs **[Google Agent Executor Distributed Runtime](ch04/397-agentrun.html)** — Google 自家 Runtime 实现;与本文 LangGraph / OpenAI SDK 平行
- vs **[Anthropic Claude Managed Agents Platform](../ch01/272-anthropic-claude-managed-agents.html)** — Anthropic Managed Agents 视角
- vs **[Amazon Bedrock AgentCore Runtime 深度分析](ch04/501-amazon-bedrock-agentcore.html)** — AWS Bedrock AgentCore 视角;与本文 LangGraph / OpenAI SDK 平行
- vs **[AgentCore Harness](ch04/464-amazon-bedrock-agentcore-harness-ga-api-agent.html)** / **[AgentCore Managed Harness](ch04/219-agentcore-managed-harness.html)** — AWS 实现的 8 职责具体形态
...
7 大职责（工具管理 / 上下文组装 / 状态管理 / 终止判断 / 风险控制 / Trace / 可观测性）本质上是 **Agent Runtime 的概念性契约** — 无论用 LangGraph、OpenAI Agents SDK 还是自研 Runtime，都必须回答这 7 个问题。这一结论与 [若飞 Fable 5 Runtime Contract](../ch01/342-claude-fable-5.html) 的"工程契约"思路同源：二曲线给出**职责层面的概念契约**（"要做什么"），若飞给出**协议层面的工程契约**（"怎么做"）。两者结合构成完整的 Runtime 设计图谱。
...
本文将 Router 定性为"复杂 Agent 扩展层"而非第 8 个必备职责，这一判断具有重要的工程意义：**Router 的必要性是 Agent 复杂度阈值的结果，而不是功能丰富度的标志**。这与 [Agent Harness 架构设计与生产实践](../ch05/038-agent-harness.html) 的"按需引入复杂度"原则一致 — 过早引入 Router 会增加不必要的状态空间，早期 Agent 应该用提示词 + 工具选择承担简单分发。

### 4. Trace ↔ 可观测性 是同一数据流的两个齿轮,必须配套设计

本文清晰区分了 **Trace（记录）和 Observability（分析）**：Trace 提供原始执行链路数据，Observability 在这些数据之上构建监控与告警。[langgraph-state-machine-under-the-hood](ch04/195-langgraph.html) 等框架的实践也印证了这一点 — 没有 Trace，Observability 就是无源之水；没有 Observability，Trace 只是无人阅读的日志。两者是 Runtime 可观测性职责的不可分割两面。

### 5. Runtime 是模型可替换性的工程底座,与"Token 资本"哲学异曲同工

本文核心洞察 — **Runtime 把 LLM 差异封装在执行层，让业务不被任一模型锁定** — 与 [纳德拉「Token 资本」论](../ch12/003-token.html) 的"模型可替换性是主权测试"哲学在工程层面高度吻合。[Harness Engineering Framework](../ch05/062-harness-engineering.html) 将 Runtime 定位为 Harness 的内脏，而 Runtime 的模型隔离能力正是 Harness 层实现"模型无关性"的底层机制。
...
- **未涉及 prompt injection / 越权 / 隐私等高级 Guardrail**: 本文风险控制仅 1 段;深度安全参考 [阿里云安全护栏三域](ch04/070-ai.html)
...
- [若飞 Fable 5 Runtime Contract](../ch01/342-claude-fable-5.html)
- [阿里云安全护栏三域](ch04/070-ai.html)
- [阿里云 Agent 演化四阶段](../ch03/045-agent.html)
- [Harness Engineering Framework](../ch05/062-harness-engineering.html)
- [Agent Harness 架构设计与生产实践](../ch05/038-agent-harness.html)
- [Google Agent Executor Runtime](ch04/397-agentrun.html)
- [Anthropic Claude Managed Agents](../ch01/272-anthropic-claude-managed-agents.html)
- [Amazon Bedrock AgentCore Runtime](ch04/501-amazon-bedrock-agentcore.html)
- [AgentCore Harness](ch04/464-amazon-bedrock-agentcore-harness-ga-api-agent.html)
