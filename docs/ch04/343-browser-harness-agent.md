...
与 [AgentBrowser](ch09/147-agent-browser-claude-code-qoderwork.md)、Playwright/Selenium 等方案不同，Browser Harness 不构建厚重抽象层，而是让 Agent 直接通过 CDP WebSocket 与浏览器原生状态交互。
...
与 [CLI-Anything](ch03/094-cli-anything.md)/[OpenCLI](ch03/084-opencli.md)/[AutoCLI](ch03/117-autocli.md) 等 CLI 化方案不同，Browser Harness 走的是**浏览器原生操作**路线而非命令行封装，二者互补而非替代。
