...
## Ch04.700 CPU 缓存类比下的 Agent 上下文管理：L1/L2/L3 层级架构与 execute_code 单工具设计
...
```mermaid
mindmap
  root(("CPU 缓存类比下的 Agent 上下文管理：L1/L2…"))
    一句话总结
    核心命题
    L1/L2/L3 三层架构
      L1 — 常驻内容（80% 覆盖）
      L2 — 按需加载（15% 覆盖）
      L3 — 兜底路径（5%+ 覆盖）
      几乎所有优化都围绕'信息压缩'与'发现速度'之…
    execute_code 单工具原则（vs A…
      核心论断
      三个机制
      与 Anthropic 工具数量陷阱ch…
    L1 实战：读取区间的三重压缩
      压缩 1：公式别名化
      压缩 2：自动补充行列语义
      压缩 3：样式压缩
