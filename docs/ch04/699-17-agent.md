...
## Ch04.699 17种Agent架构演进：控制流设计的完整演化史
...
graph LR
    subgraph "单Agent层"
        R["1. Reflection<br/>Generator→Critic→Refiner"]
        T["2. Tool Use<br/>LLM↔Tool闭环"]
    end
    subgraph "推理循环层"
        RA["3. ReAct<br/>Thought→Action→Observation"]
        P["4. Planning<br/>显式步骤清单"]
    end
    subgraph "多Agent层"
        MA["5. Multi-Agent<br/>协作/委派/协商"]
        SW["6. Swarm<br/>分布式涌现"]
    end
    subgraph "生产级层"
        L["7. LATS<br/>树搜索+价值函数"]
        H["8. Harness<br/>运行时治理"]
    end
    R -->|需要外部能力| T
    T -->|需要动态推理| RA
    RA -->|控制流需可审计| P
...
[AI Native 研发组织设计](../ch05/018-ai-native.html) 则从组织维度探讨了 Multi-Agent 时代的架构演化。
