...
## Ch04.571 LoHoSearch — 下一代搜索智能体评测基准
...
LoHoSearch 的核心创新是**让机器自己出题**——以大规模知识图谱为基础，自动生成难度可控的搜索智能体评测题目。
...
2. **双维度难度控制**：
   ...
   针对这两个维度设计了树结构（放大搜索空间）和图结构（叠加结构复杂度）。

3. **质量把关**：三层筛选——从子图抽取+改写为自然语言问题 → 自动验证 → 人工复核。自动化流程 75.5% 直接通过，22.3% 微调后接受，仅 2.2% 被丢弃。

4. **最终数据集**：**544 道**经人工核验的题目，覆盖音乐、地理、影视、体育等 11 个主题领域。

## 关键发现

**模型表现**：最强模型 GPT-5.5 准确率仅 34.74%；DeepSeek-V4-Pro、Claude-Opus-4.6、Kimi-K2.6 集中在 15.53%–15.99%，与 BrowseComp 上 80%+ 的表现形成鲜明对照。

**工具调用增长**：解一道 LoHoSearch 题目平均工具调用从 BrowseComp 的 35 次增至 61 次（+74%）。图结构题目准确率 8.01%，远低于树结构的 11.89%。

**重复采样收益**：pass@N 从 N=1 的 9.3% 升至 N=16 的 38.3%，但仍有六成以上题目无法攻克。best-of-N 仅 24.6%，说明模型置信度校准存在不足。

**上下文管理策略失效**：最优策略（Discard-all + Verify）仅提升 6.8 个百分点，远低于在 BrowseComp 上的 14.03%，表明 LoHoSearch 需要更长的推理链，简单轨迹压缩无法解决信息丢失。
...
- **面向上下文管理研究的挑战性平台**，推动下一代技术研究

已全面开源：Paper (arXiv 2606.12837) | [HuggingFace 数据集](https://huggingface.co/datasets/meituan-longcat/LoHoSearch)
