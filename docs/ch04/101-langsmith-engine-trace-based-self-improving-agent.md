...
- [Hermes Agent Operator](../ch03/098-hermes-agent.html) — "模型侧自改进"路径（定时任务把对话总结成 Skill），与本文"系统侧自改进"互补
...
- [Claude Code 深度解析](../ch05/086-claude-code-harness.html) — Claude Code 的评估观测层工程实践
- [深入理解 Claude Code Harness](../ch05/042-agent-harness.html) — Plan Mode + Tasks 系统的"避免问题复发"机制
- [OpenClaw 完整指南](../ch11/253-openclaw.html) — 开源 Agent 的故障处理与自我恢复机制
- [Agent 记忆系统实践](../ch03/037-agent.html) — Agent Overview 是 Memory 模块的诊断侧应用- [langchain × fireworks 100x cheaper trace judge — 通用 trace 评估](../ch01/347-langchain.html)
