...
Worktree 可能是 Loop 中最不起眼却最重要的工程决策。当多个 Agent 同时修改同一个仓库，最容易出问题的地方不是模型能力，而是文件冲突。Git worktree 给每个 Agent 一个独立 checkout，共享仓库历史但在不同目录工作，从根本上避免了「两个 Agent 改同一个文件」的问题。这与 [Beautiful Article Skill](../ch05/110-harness.html) 中的「一节一文件」和多 Agent 并行安全设计思路一致——都是通过**空间隔离**来解决协作冲突，而非依赖 Agent 的自我约束。
...
- [Harness 实践将任何文字编辑成精美的文章](../ch05/110-harness.html) — Beautiful Article Skill 的 8 Phase Harness 与 Loop 架构相互印证
