...
## Ch04.102 深度拆解 Claude Code：12 个可复用的 Agentic Harness 设计模式
...
```mermaid
mindmap
  root(("深度拆解 Claude Code：12 个可复用的 Ag…"))
    记忆与上下文
      1\. 持久化指令文件模式（Persisten…
      2\. 作用域上下文组装模式（Scoped C…
      3\. 分层记忆模式（Tiered Memor…
      4\. 记忆整合模式（Dream Consol…
    工作流与编排
      6\. 探索-规划-行动循环模式（Explor…
      7\. 上下文隔离子智能体模式（Context…
    深度分析
      1. 这 12 个模式本质是一个协同系统，不是…
      2. '系统兜底' vs '模型负责' 的边界…
      3. 模式 4（Dream Consolida…
      4. 分支-合并并行模式（模式 8）的实现复杂…
    实践启示
      1. 用生命周期钩子实现「强制门禁」，不要放在…
      2. 渐进式工具扩展要从「最小可用集」开始，而…
      3. 在 Monorepo 中优先实现目录级的…
      4. 对话超过 20 轮时主动触发渐进压缩，不…
    关联阅读
    Related
```
...
原文提到 `autoDream` 在后台做记忆整合，但实际上这个过程有一个根本矛盾：负责「整理记忆」的 Agent 也需要上下文来理解哪些记忆是重要的、哪些是冗余的——但它的上下文同样会受到记忆膨胀的影响。这形成了一个递归困境：记忆太多导致整理困难，但整理本身又需要消耗上下文。Claude Code 的解法可能是让这个过程非常轻量（只处理索引层而不是全量记忆），但这个细节原文没有展开。对比 [OpenSquilla 的 Dream Consolidation](../ch01/011-opensquilla-launches-open-source-ai-agent-to-cut-token-costs.html) 实现，会发现两者都承认这个整合步骤是必须的，但也都承认整合本身有成本。这是一个长程 Agent 的 open problem，没有银弹。
...
Bilgin Ibryam 能从 Claude Code 源码里提炼出这 12 个模式，说明生产级 Agent 已经广泛使用它们。但大多数开源 Agent 框架（比如 LangChain Agents、AutoGPT）只实现了其中少数几个（主要是模式 1 和 6）。这种错位的后果是：社区里大量关于「Agent 太不可靠」的抱怨，往往不是模型的问题，而是 Harness 缺失的问题。[Harness Engineering 系统化框架](../ch05/120-harness-engineering.html) 试图把这个领域的共识整理成可操作的检查清单，12 个模式可以视为这个清单的源码级证据。
