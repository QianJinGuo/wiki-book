...
HITL 审批把「关键决策」变成显式暂停点，直到人类签核才继续执行——这是素材明确列出的学习目标，也是企业合规的最低要求。失败处理则交给 DAG 式编排（MWAA/Airflow、Temporal、Prefect、Orkes 等），在复杂多步流水线中提供重试、补偿与优雅降级。企业实践中，审批闸门同时承担审计职责：谁在何时批准了什么，本身就是合规证据链的一部分。凭证治理是另一个常被忽视的隔离维度——Agent 不应持有数据库直连凭证，相关模式见 [Agent 数据治理与凭证模式](../ch03/019-agent.html)。
...
- [在数据所在处构建 Agent: CrewAI + Snowflake 企业级 Agent 部署](../ch03/019-agent.html)
...
- [Stop Giving Your Agents Database Credentials — Agent Data Governance Patterns](../ch03/019-agent.html)
