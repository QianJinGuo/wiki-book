...
素材呈现的是两种互补范式而非替代关系：Step Functions 代表确定性编排——状态机显式管理状态、重试、分支与并行，适合步骤边界清晰的业务流程；Bedrock Agents 代表推理驱动协调——内置工具调用、记忆与动态路由，适合路径不固定的开放任务。真实企业编排层通常把两者叠放：用确定性的外层定义合规边界，用推理的内层保留灵活性。这种「工作流引擎 vs 自主 Agent 框架」的张力在开源生态中同样存在，控制流设计的更完整谱系参见 [17种Agent架构演进](ch04/786-17-agent.html)。
