...
- [Asana Agentic Work Management Platform Lettertwo](ch04/603-agentic.html)（Asana — 另一个企业级 Agent 平台视角）
- [Building Multi Tenant Agents With Amazon Bedrock Agentcore](ch04/527-amazon-bedrock-agentcore.html)（AWS Bedrock AgentCore — 对比平台）
- [Anthropic 官方 Agent Harness 平台Claude Managed Agents 完整指南](ch04/513-claude-managed-agents.html)（Anthropic Managed Agents — 另一家厂商视角）
...
- [Ai Tool Poisoning Exposes A Major Flaw In Enterprise Agent Security](ch04/430-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s.html)（企业 Agent 安全风险同主题）
