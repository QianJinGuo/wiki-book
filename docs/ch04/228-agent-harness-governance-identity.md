...
**关键互补点**：
- `[Ai Tool Poisoning Exposes A Major Flaw In Enterprise Agent Security](ch04/529-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s.html)` — MCP supply chain 攻击模型（**怎么攻击**）
- `[Secure Ai Agents Policy Lambda Interceptors Aws](ch04/237-ai-agent.html)` — Policy + Lambda 实现 runtime interception（**怎么防御**）
...
- `[Ai Agents Security Survey Attack Defense](ch04/237-ai-agent.html)` — Agent 安全 attack/defense survey（不同问题域）
