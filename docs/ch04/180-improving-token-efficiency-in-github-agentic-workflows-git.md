...
## Ch04.180 Improving Token Efficiency in GitHub Agentic Workflows — GitHub 内部 Agent 工作流 Token 优化实践
...
这与 [Anthropic MCP 优化](../ch07/016-anthropic-mcp.html) 中 Tool Search 减少 85% tool definition token 的思路一脉相承。
...
- **Subagent 模式**：生成专门负责数据获取的子 agent，只返回结构化数据
  **效果**：将大部分 GitHub 数据获取移出 LLM 推理循环。
  这与 [Claude Code Subagent 上下文卫生](ch04/326-claude-code-subagent.html) 中 Subagent 作为上下文隔离工具的理念相通。
...
- → [Prompt Caching 工程实践](../ch01/289-anthropic-prompt-caching-claude-code.html)：缓存策略是 token 优化的另一维度
- → [Agent Harness 12 组件与 7 个关键决策](../ch05/039-agent-harness.html)：工具选择决策树与本文的 MCP vs CLI 选型呼应
- → [Agentic Systems Extreme Co-Design（NVIDIA）](ch04/235-agentic.html)：从硬件/推理引擎角度审视 token 效率，与本文的 workflow 层优化形成互补
...
## 相关实体
- [Improving token efficiency in GitHub Agentic Workflows](ch04/235-agentic.html)

- [Token Efficiency](ch04/235-agentic.html)
````
