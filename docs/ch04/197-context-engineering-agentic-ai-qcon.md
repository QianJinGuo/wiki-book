...
## Ch04.197 一点天下：Context Engineering 与 Agentic AI (QCon)
...
## 四、主动注入：Hook 化的主动推送
传统"按需自取"模式的根本缺陷：**模型不知道自己不知道什么**。^[].md]
...
**效果**：把"记忆"从被动资料库升级为主动副驾驶——知识在真正需要之前就已到位。^[].md]
...
**效果**：单次注入 Token 消耗下降约 **80%**，L2 完整内容始终"一键可达"。^[].md]
...
**当前规模：**近百个不同职能 Agent 活跃运行，覆盖营销业务、技术运维、客户服务等多个维度。^[].md]
