...
- **多模型调度层**：不绑定单一模型体系，而是将模型能力以可切换、可调度的形式存在，根据任务类型、响应效果或成本条件动态调用不同的模型。这与 [Agent Harness 招聘场景](../ch05/043-agent-harness.html) 中提到的"多模型路由"架构一致。
...
- [Agent Harness 招聘场景](../ch05/043-agent-harness.html)
