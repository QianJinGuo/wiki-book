...
## Ch04.682 rd-signal-2：生产规模下的 Agent 行为二元分类（Frontier Classification at Production Scale）
...
- [Trace 即 Evals（张雁飞）](ch04/444-ai-agent-trace-evals.html) — 同为 trace 驱动评估范式；rd-signal-2 给出生产规模实现路径
