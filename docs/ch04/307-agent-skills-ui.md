...
## Ch04.307 Agent Skills 终于有 UI 了
...
**同一 Skill 可混合使用两种模式**，按场景选择。^[].md]

## 实战案例：Frontend-design Skill 加设计方向面板
### 第一轮：生成初版（6 秒）
Agent 分析 Skill 意图，渲染出包含五个区域的面板：^[].md]
...
用户需在调性、配色、字体三项各选一个后，"Start Generation" 按钮激活。^[].md]

### 第二轮：加调性视觉预览
原文：Aesthetic Tone 当前没有卡片预览图，你帮我生成。^[].md]

→ Agent 调用图像生成模型，为 9 种调性各生成 mood-board 风格预览图。

### 第三轮：加自定义品牌色
原文：配色方案加一个自定义选项，让用户输入品牌色。^[].md]
