...
## Ch04.214 Agent Skills vs Workflow低代码平台：选型分析
...
## 核心洞察
1. **Skill是Workflow的另一种表达**：HR简历筛选skill的执行步骤（解析→提取→评分→写入）本身就是Workflow。没有Skills工程机制前，Agent稳定性更差。
2. **Skills不会淘汰Workflow平台**：会给压力倒逼升级（Coze已在用AI Coding方式实现Workflow搭建，不再手动拖拉拽）。两边会向中间靠拢：Workflow平台Agent化，Skills系统平台化/可治理化。
3. **企业选型原则**：个人/小团队→Skills；企业生产→Workflow（涉及隐私/权限/合规/协作/可追溯）
4. **最终竞争本质**：不是Workflow/Skills/Agent的工具形态竞争，而是业务KnowHow承载方式的竞争
...
文章将KnowHow承载方式定义为终极竞争要素，这值得进一步分层：
...
- [Anthropic Google Agent Skills Design Patterns](https://github.com/QianJinGuo/wiki-public/blob/main/entities/anthropic-google-agent-skills-design-patterns.md)
- [Agent Skills Teams Architecture Evolution Selection Guide](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-skills-teams-architecture-evolution-selection-guide.md)
- [从 Anthropic 到 Googleagent Skills 正在进入设计模式阶段](https://github.com/QianJinGuo/wiki-public/blob/main/entities/从-anthropic-到-googleagent-skills-正在进入设计模式阶段.md)
- [Cong Anthropic Dao Googleagent Skills Zhengzai Jinru Sheji Moshi Jieduan](https://github.com/QianJinGuo/wiki-public/blob/main/entities/cong-anthropic-dao-googleagent-skills-zhengzai-jinru-sheji-moshi-jieduan.md)
