...
5. **默认安全、显式授权**：高风险（block 级）工具必须由用户显式放行，且权限只能调松不能调紧——把不可逆破坏性操作的最终责任交给人类决策者，可对照 [Agent 安全三步序列](../ch05/091-harness.html) 的治理思路。
...
- [MCP 工具设计权衡](../ch01/780-anthropic.html) — 工具接口设计的 Anthropic 视角
