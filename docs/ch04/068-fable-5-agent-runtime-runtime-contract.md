...
- vs **[Anthropic Claude Fable 5 on AWS:Mythos 级功能](../ch01/1218-anthropic-claude.html)** — 那是**AWS 官方产品介绍**(功能/部署/内置保护),本文是**架构师工程化拆解**(Runtime 协议层)。两者互补。
- vs **[Claude Fable 5 and new AI safety fables](../ch01/810-claude-fable-5-and-new-ai-safety-fables.html)** — Nathan Lambert 的**政策分析**(数据保留/prompt 过滤/用户未告知模型修改),本文**不**涉及政策。
- vs **[Claude Fable 5 — Mollick patron vs wizard](../ch01/342-claude-fable-5.html)** — Mollick 的**hands-on 用户体验视角**(4 用例 + patron vs wizard 框架),本文是**架构师工程视角**。两者对应"产品体验"vs"产品架构"。
- vs **[Claude Fable 5 提示词泄漏 — Runtime Control Plane 安全工程启示](../ch01/342-claude-fable-5.html)** — 同样用"Runtime"概念,但**VibeCoder 重点在安全工程** (Prompt 不能当保险箱 / 攻击面像系统 / 分类器组合风险),**本文重点在工程协议** (Task Brief / 能力路由 / 状态账本 / 治理层)。两者**完全互补**: 一个看 Runtime 怎么被攻击,一个看 Runtime 怎么被设计。
...
- vs **[Claude Code agent teams task decomposition ruofei](../ch01/385-claude-code-agent-teams.html)** — 那是**任务分解**(具体执行);本文是**任务协议** (前置契约)。
- vs **[Harness Engineering Deletable Worksite](../ch05/083-harness-engineering-deletable-worksite-ruofei.html)** — 那是 **Harness 可删工作位**的精简原则;本文是 **Runtime 4 层**(包含 Harness 作为"工具路由层"的子集)。
- vs **[若飞 agent memory architecture](ch04/096-agent-memory.html)** — 那是 **Memory 架构**;本文**执行状态账本** 5 类中包含 memory。

**Runtime / Agent 架构** 主题簇:

- vs **[dangling 已删除]** — 那个 entity 把 Harness 推为新后端;**本文**把 **Runtime 协议** 推为新运行时,Runtime ⊃ Harness
- vs **[Harness Engineering Framework](../ch05/062-harness-engineering.html)** — Runtime Contract 是 **Harness 的契约形式**;后者是工程化框架
...
长任务 Agent 不能只靠聊天记录记住自己做过什么——这个判断来自 Anthropic 官方文档,被若飞拆解为 5 类状态账本(memory/Artifact storage/文件账本/工具历史/进度声明校验)。这与 [若飞 agent memory architecture](ch04/096-agent-memory.html) 中的"记忆架构决定 Agent 能否真正完成任务"高度相关。核心洞察是:**状态管理不只是"有没有 memory",而是"状态是否可以被验证和交接"**。进度声明必须回到本轮工具结果校验,这是一个朴素的工程原则,但在 AI Agent 场景下被放大——模型的"自信"可能掩盖实际的工具调用失败,而没有结构化账本,工程师无法事后重建真实执行路径。

### 5. Fable 5 事件是 Agent 治理从"产品内嵌"走向"显性协议"的拐点

Anthropic 因美国政府出口管制指令暂停 Fable 5 访问,这件事在表面上是一个合规事件,但深层看是**供应商锁定风险的极端暴露**:当模型供应商不可用,整个 Agent 产品的业务连续性立即中断。若飞在"设计对象变了"一节列出的 8 个新问题(任务协议/工具路由/状态交接/产物验证/权限审计/成本解释/拒答降级/供应商依赖)中,最后一个问题在 Fable 5 事件中变成了真实的业务中断。这与 [Agent 架构关键变化:Harness 正在成为新后端](../ch05/009-harness.html) 的判断形成呼应——**当模型层不可靠,Runtime/Harness 层必须承担起稳态责任,包括降级和交接**。
...
- [Claude Fable 5 提示词泄漏 — Runtime Control Plane](../ch01/342-claude-fable-5.html)
- [Anthropic Claude Fable 5 on AWS](../ch01/1218-anthropic-claude.html)
- [Claude Fable 5 and new AI safety fables](../ch01/810-claude-fable-5-and-new-ai-safety-fables.html)
- [Claude Fable 5 — Mollick patron vs wizard](../ch01/342-claude-fable-5.html)
- [若飞 5 张卡治理](../ch03/090-hermes-agent.html)
- [若飞 Ralph loop 状态交接](../ch05/009-harness.html)
- [Claude Code agent teams task decomposition](../ch01/385-claude-code-agent-teams.html)
- [Harness Engineering Deletable Worksite](../ch05/083-harness-engineering-deletable-worksite-ruofei.html)
