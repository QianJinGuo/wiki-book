...
## Ch04.293 Hermes Agent 记忆系统

> 📊 Level ⭐⭐ | 10.7KB | `entities/hermes-agent-memory-system-architecture.md`
...
仓库：github.com/NousResearch/hermes-agent^[].md]
...
**代价**：本次会话写入的记忆在本次会话内不可见。Agent 通过工具返回值感知实时状态。^[].md]

**判断**：牺牲一次会话内的记忆可见性，换取整个生命周期的 API 成本稳定。^[].md]
...
## 安全扫描
记忆写入前正则扫描 `_MEMORY_THREAT_PATTERNS`：^[].md]
...
```
同一文件系统内的 rename 是原子的，读者永远看到完整的旧版本或完整的新版本。^[].md]
