...
与传统的单轮 Prompt 生成方案相比，MemSlides 解决了 Slide 生成中的 stateful 多轮交互问题——用户可以在已有 PPT 基础上进行精确的局部修改，而不是每次都需要重新生成整体内容。这与 [Agent 记忆架构](ch03/045-agent.md) 和 [Harness 上下文管理工作集](ch05/009-harness.md) 中的层级化记忆设计思路一脉相承。
