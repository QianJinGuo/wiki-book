...
## Ch04.042 Hermes Agent Eval Harness：可验证 Skill 进化的 7 模块闭环（TaskSet/HarnessAdapter/TrajectoryStore/Verifier/Selection Gate/Regression Gate/Report & Replay + 6 类 Verifier + 3 类 Regression + 6 阶段落地路径）
...
[Skillopt](ch05/048-skillopt.md)（Microsoft + SJTU SkillOpt 论文 arxiv 2605.23904）提出了"把 SKILL.md 当成冻结模型之外的可训练状态"——通过 rollout、bounded edits 和 held-out validation gate 优化 agent skills。但 SkillOpt 学术框架在生产环境落地需要完整的工程实施：
...
| [Skillopt Microsoft Train Skill Like Neural Network](ch04/255-skill.md) | **SkillOpt 产品视角** | 别再手写 Skill！像神经网络一样训练（58 行）：SkillOpt 产品介绍 |
| [Skillopt Microsoft Train Skill Hyman'S Blog](ch04/255-skill.md) | **SkillOpt Hyman 视角** | 微软等 SkillOpt 解读（64 行） |
| [Hermes Agent Skill Crossover Optimization](ch04/255-skill.md) | **Hermes Skill 互优化** | SkillEvolver × Darwin × EmbodiSkill 4 轮闭环（277 行）— Skill 之间的相互优化 |
| [Skill Self Evolution Three Approaches](ch04/255-skill.md) | **Skill 自进化 3 路线** | Trace2Skill 归纳法 / EvoSkill 验证闭环 / SkillOpt 训练范式（45 行）；本文是 SkillOpt + EvoSkill 的工程化展开 |
...
| [Darwin Skill 2 Huashu](ch04/255-skill.md) | **Darwin Skill 2** | Darwin Skill 进化 |
...
| [Slim Cuhk Skill Lifecycle Agentic Rl](ch04/255-skill.md) | **SLIM Skill 生命周期** | CUHK SLIM Skill 生命周期 + Agentic RL |
| [Deli Auto Research Skill V2 Continual Learning Self Improvement](ch04/255-skill.md) | **Deli Auto Research** | Continual learning + self-improvement |
