...
> [!contradiction]
> 与 [Hermes 记忆系统深度拆解](ch03/045-agent.md) 一文的判断一致：Hermes 的核心优势在于 cache-aware runtime 与分层记忆设计，而非 OpenClaw 式的控制面架构。两文从不同维度（功能定位 vs 记忆工程）得出相同的互补性结论。
...
- [Hermes Agent 深度解析](ch03/045-agent.md)
- [OpenClaw 架构八部总结](ch11/210-openclaw.md)
