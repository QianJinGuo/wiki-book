...
## Ch04.107 AI伴行技术解析：基于空间智能的高可用Agent架构实践

> 📊 Level ⭐⭐ | 24.6KB | `entities/gaode-ai-companion-agent-architecture.md`
...
高德选择ReAct，但原生ReAct框架在导航场景下表现远未达标，进行了一系列首创性优化：^[].md]
...
### 3.2 核心循环架构
ReAct推理引擎的核心是一个受控的"思考→行动→观察"循环，设计了多重判停机制：^[].md]
...
高德的创新：针对导航场景的实时性要求，设计了一套面向token效率优化的极简结构化输出范式。^[].md]

**效果**：相比JSON格式，输出token开销降低约40%，线上单轮推理延迟降低约300ms。^[].md]
...
#### 3.4.4 两阶段训练：SFT + RL
基于Qwen3-Next-80B-A3B基座模型，采用两阶段训练策略：^[].md]
...
#### 3.4.5 训练效果
经过完整训练优化流程后，模型在出行场景下的工具调用能力实现全面跃升：^[].md]
...
业界通行做法是将全部规则硬编码到System Prompt中，但这在实时交互场景下会导致三重困境：^[].md]
...
### 5.2 四维上下文模型
将上下文拆成四个维度：空间、时间、用户、环境。^[].md]

这套模型的价值在于把用户交互的真实物理世界建模为可推理、可裁剪、可复用的时空状态实体：^[].md]
...
### 6.4 端到端体验优势
首包时延（秒）上，伴行Agent相较对照**显著更短**，完美解决4秒体验魔咒[1]。^[].md]
...
→ [[raw/articles/05-11-the-great-memory-panic-of-2026.md|原文存档]
