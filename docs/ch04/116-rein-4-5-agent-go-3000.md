...
## Ch04.116 Rein：4 模块 + 5 类型边界防止 agent.go 膨胀到 3000 行
...
### vs Agent Harness 上下文管理（[Agent Harness Context Management Working Set](ch03/035-agent.html)）
- 工作集视角：logical_messages 原样保留 + 投影 = Rein 的"存储完整，发送压缩"

### vs wow-harness v3 事件溯源（[Wow Harness V3 Governance Protocol](ch05/085-harness.html)）
