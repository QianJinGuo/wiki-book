...
## Ch04.325 Snowflake Agentic Enterprise — 迈向可信的 Agent 平台
...
1. 利用 AI 函数从非结构化数据中**提取信息** ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
2. 提取为**结构化信息** ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
3. 运行**精确的分析查询** ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
4. 返回基于非结构化内容的**精确分析结果** ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
...
未来企业数据平台不仅要回答"谁能访问什么数据"，还要回答： ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
...
**"开放底座、深度协同"的逻辑**： ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
...
这种**用户驱动的命名 + 关系导向的命名**，比厂商自创技术名词更有亲和力。Christian 在台上笑着说出"Denise 说..."本身就传递了**Snowflake 对用户反馈的尊重**。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

### 3. 治理的"民主化"与"动态化"

**传统治理** = 写复杂策略脚本，由安全专家维护 — 治理是"少数人的事"。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

**Snowflake 意图驱动治理** = 自然语言意图 → 自动分类 + 找敏感信息 + 创建策略 + 持续监控 — **每个业务负责人都可直接施加控制力**。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

**传统治理 = 静态**（写好规则不再变）。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

**Snowflake 治理 = 动态**（智能体行为可审计、可回溯、可追责）。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

这是企业 AI 治理从"合规"升级为"运营"的关键变化。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]

### 4. 智能协作经验是新平台价值

**开放 vs 中心化** 不矛盾——Snowflake 看似强调开放（Iceberg / Polaris / Open sharing），但实际通过**让 AI / 治理 / 协作能力进入更多外部系统**（Query across / 与 Salesforce / SAP 集成）把自己放到更多数据和 AI 工作流的交汇处。 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
...
1. **试点 CoCo + CoWork**：在受控数据集上验证 24% → 83% 的 Cortex Sense 提升是否在自己的数据上成立 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
2. **设计 Agent Identity 策略**：先识别"哪些操作必须区分人类 vs 智能体上下文" ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
3. **定义 Data Movement Policy 标签体系**：从最敏感的数据标签开始（如 VIP 客户 / 财务数据） ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
4. **设置 Multi-party Approval 流程**：对高敏感操作强制双人审批 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
5. **建立意图驱动治理模板**：把常见治理意图（如"找敏感信息 + 保护"）写成自然语言模板 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
6. **跨数据源联邦**：评估 Query across 能否替代部分 ETL 流水线 ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
7. **用户记忆 + 调度任务**：从高频分析需求开始沉淀 User Memory ^["InfoQ 奇遇旧金山: Snowflake 迈向 Agentic Enterprise 的关键一跃"]
