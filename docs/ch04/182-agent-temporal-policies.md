...
- [Policy + Lambda interceptors](423-ai-agent.html)：stateless Cedar policy 的先行方案；temporal policy 是其有状态扩展（同一 Policy engine 之上）
- [AI 工具投毒](225-ai-tool-poisoning-exposes-a-major-flaw-in-enterprise-agent-s.html)：工具调用层面的攻击面；temporal policy 的 output-integrity 规则正是防御此类攻击的授权层手段
