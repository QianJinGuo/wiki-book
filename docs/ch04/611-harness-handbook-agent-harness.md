...
已有大量 [agent harness 架构](ch03/035-agent.html) 实体覆盖 harness 的设计模式、组件和工程实践。Harness Handbook 提供了不同的角度——**以 behavior 为核心的导航系统**（而非以组件/模块为核心）。它与以下实体互补：
...
- [Agent Harness 12 Components](ch03/035-agent.html)（组件架构）
