...
## Ch04.420 RoadmapBench — 跨版本升级的长周期 Agentic 软件开发评估基准
...
RoadmapBench 填补了从 SWE-Bench（单 bug fix）到 [Codex 六小时目标运行](../ch09/057-codex-goal-six-hour-run.html)（端到端功能开发）之间的评估空白。 它的定位类似 SWE-Bench 的"跨版本升级"增强版，关注的是 Agent 在已知代码库上进行系统性演进的能力，而非从零开始的创作能力。

### 模型能力的差异化分析

13 个模型的评测结果显示，模型性能并非简单地随参数量增长。Claude-Opus-4.7 的 39.1% 与最弱模型的 5.2% 之间差距悬殊，暗示长周期任务对模型的指令遵循、长上下文理解和错误恢复能力提出了复合要求。 这与 [CMU PACE 代理评估](../ch03/046-agent.html) 中揭示的能力指纹概念相互印证——不同基准测试对模型能力维度的需求分布不同。
...
3. **关注能力指纹而非单一分数**：结合 [CMU PACE](../ch03/046-agent.html) 等代理评估方法，分析模型在长周期任务上的具体短板——是指令遵循、规划还是错误恢复？针对性地设计 prompt 策略。
...
- [Codex 六小时目标运行](../ch09/057-codex-goal-six-hour-run.html) — 长链 Agent 执行的实际案例
- [Agent Harness 上下文管理](../ch05/039-agent-harness.html) — 长周期任务中的上下文窗口管理技术
- [CMU PACE 代理评估](../ch03/046-agent.html) — 低成本 Agent 能力评估方法
