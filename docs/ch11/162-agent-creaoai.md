...
## Ch11.162 云端 Agent 基础设施两条硬经验：CreaoAI 联合创始人的状态/代码解耦 + 凭据隔离
...
- [Cloudflare + Claude Managed Agents](../ch01/119-announcing-claude-managed-agents-on-cloudflare.html) = "脑手分离"架构：推理在 Anthropic 平台 + 代码执行在 Cloudflare Sandboxes——**关注架构分工**，本文关注 **平台自身的 hot-swap + 凭据隔离**
...
- [OpenClaw → Bedrock AgentCore 多租户迁移](https://github.com/QianJinGuo/wiki-public/blob/main/entities/ai-agent-的迁移与现代化-使用-amazon-bedrock-agentcore-将-openclaw-从单机改造为多租户-serverless-架构-.md) = 阿里云/AWS 的多租户 serverless 路径，**关注迁移路径**
- [AgentScope Builder](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agentscope-builder-enterprise-self-evolving-agent-harness.md) = 阿里云 Harness 框架 + 平台化，**关注 harness 框架**
- [Stripe Agent 经济基础设施 5 套图谱](https://github.com/QianJinGuo/wiki-public/blob/main/entities/stripe-agent-economic-infrastructure-5-products.md) = 关注 **agent 经济层基础设施**（支付/钱包/订阅），**与本文 runtime 隔离互补**

### 与 RAG / Agent 评估
- [RAG 技术框架的演进方向](https://github.com/QianJinGuo/wiki-public/blob/main/entities/rag技术框架的演进方向.md) = 关注 **retrieval 充分性**（充分上下文智能体 = 信息的"够不够"判断）
...
### 与阿里云 Agentic Cloud
- [阿里云 Agentic Cloud](https://github.com/QianJinGuo/wiki-public/blob/main/entities/alibaba-agentic-cloud.md) = 关注 **云平台级别的 agent-as-a-service**（Bailian/Qwen/MaaS）——**关注平台对外的能力**
