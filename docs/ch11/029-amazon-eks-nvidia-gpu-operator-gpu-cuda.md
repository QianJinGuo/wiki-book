...
- [mountpoint s3 vs s3 files：eks 上 s3 数据接入的两种方案实战对比](ch01/310-rag.md)
...
| 实体 | 焦点 | 与本文关系 |
|------|------|----------|
| [Building Blocks For Foundation Model Training And Inference On Aws](ch04/277-ai.md) | AWS 训练/推理基础设施通用构件 | 上层基础设施视角，本文专注 GPU 节点驱动管理 |
| [Foundation Model Building Blocks](ch03/087-foundation-model-building-blocks.md) | 训练/inference building blocks | 同上，更上层 |
| [在 Amazon Ec2 Gpu 实例上部署 Nvidia Nemoclaw 以 Amazon Bedrock 作为推理](ch11/253-bedrock.md) | Nemoclaw 安全沙箱 + Bedrock 混合推理 | 应用层，本文是底层 driver 管理 |
