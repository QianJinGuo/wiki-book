...
- [Aliyun Agentrun](../ch04/476-agentrun.html) / [Aliyun Agentrun 5Min Quickstart](../ch04/476-agentrun.html) / [Aliyun Agentrun 2Line Integration](../ch04/476-agentrun.html) — **AgentRun** 是阿里云 AI Agent 运行时（运行 Agent 的平台）。**alibabacloud-cms-manage Skill 是观测接入工具**（为已部署的 AI Agent 接入 CMS 监控）。两者**关系**：Agent 跑在 AgentRun 上 → 通过 cms-manage Skill 接入 CMS 监控 → 通过 LoongSuite 上报 Agent 行为数据。
- [Skill Development Guide Aliyun 2026](../ch04/270-skill.html) — Aliyun 团队发布的 **Skill 开发保姆级教程**（教你如何写 Skill）。**alibabacloud-cms-manage Skill 是 Skill 教程落地的一个具体生产 Skill 案例**——是 Skill 教程的"现身说法"。
...
- [Cli Mcp Skill Architecture Decision Vibecoder](../ch04/270-skill.html) — VibeCoder 的 **CLI vs MCP vs CLI+Skill** 架构决策指南。本实体是**CLI+Skill 模式**的具体生产案例（CMS CLI 6 步流程 → Skill 化），是该架构决策的真实落地示例。
- [Deeppotential Alibabacloud Agentrun Scientific Ai](../ch04/476-agentrun.html) / [Agent Evolution Four Stages Six Dimensions Aliyun](../ch03/035-agent.html) — 阿里云 AgentRun 生态。本实体与之是**同一厂商不同产品线**（AgentRun vs CMS Skill）—— 都是阿里云 AI 工具链。

## 相关主题

—
- Agent Skill 生态 — [Agent Skills Comprehensive Survey](../ch04/404-agent-skills.html)
...
- [opentelemetry ebpf instrumentation (obi) — 零代码全栈可观测性的内核级实现](../ch01/900-20.html)
