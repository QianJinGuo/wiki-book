...
- [Aliyun Agentrun](ch03/044-agent.md) / [Aliyun Agentrun 5Min Quickstart](ch03/044-agent.md) / [Aliyun Agentrun 2Line Integration](ch03/044-agent.md) — **AgentRun** 是阿里云 AI Agent 运行时（运行 Agent 的平台）。**alibabacloud-cms-manage Skill 是观测接入工具**（为已部署的 AI Agent 接入 CMS 监控）。两者**关系**：Agent 跑在 AgentRun 上 → 通过 cms-manage Skill 接入 CMS 监控 → 通过 LoongSuite 上报 Agent 行为数据。
...
- [Alibaba Eventhouse Enterprise Agent Context](ch03/044-agent.md) — 阿里 EventHouse 企业 Agent 上下文。本实体是 CMS 接入（**事件**层）—— EventHouse 处理**状态上下文**，CMS 处理**可观测性**。
...
- [Agent Evolution Four Stages Six Dimensions Aliyun](ch03/044-agent.md) — Agent 演化的四个阶段六维度（阿里云视角）。本实体的 Skill + 自然语言接入模式属于该框架中"**降低接入门槛**"维度的具体实现。
...
- [Deeppotential Alibabacloud Agentrun Scientific Ai](ch04/150-ai.md) / [Agent Evolution Four Stages Six Dimensions Aliyun](ch03/044-agent.md) — 阿里云 AgentRun 生态。本实体与之是**同一厂商不同产品线**（AgentRun vs CMS Skill）—— 都是阿里云 AI 工具链。
