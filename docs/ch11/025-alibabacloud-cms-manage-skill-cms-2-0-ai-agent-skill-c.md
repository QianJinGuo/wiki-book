...
- [Aliyun Agentrun](../ch04/473-agentrun.html) / [Aliyun Agentrun 5Min Quickstart](../ch04/473-agentrun.html) / [Aliyun Agentrun 2Line Integration](../ch04/473-agentrun.html) — **AgentRun** 是阿里云 AI Agent 运行时（运行 Agent 的平台）。**alibabacloud-cms-manage Skill 是观测接入工具**（为已部署的 AI Agent 接入 CMS 监控）。两者**关系**：Agent 跑在 AgentRun 上 → 通过 cms-manage Skill 接入 CMS 监控 → 通过 LoongSuite 上报 Agent 行为数据。
