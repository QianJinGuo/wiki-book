...
### 与知识库的连接
- 容量感知推理Fallback机制（[Aws Sagemaker Capacity Aware Inference Fallback](../ch01/526-aws-sagemaker-capacity-aware-inference-fallback.html)）体现了GPU资源管理的另一面——当容量受限时如何优雅降级
- 两者共同构成GPU资源管理的"预订-使用-降级"完整链路
## 相关实体
- [SageMaker容量感知推理：实例池+优先级Fallback](../ch01/526-aws-sagemaker-capacity-aware-inference-fallback.html)
