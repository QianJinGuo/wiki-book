...
## Ch11.218 记忆体系工程实战：从设计选型到生产落地 — 存储分层、诊断框架与架构模式
...
与 [Hermes Agent 记忆系统架构](../ch03/095-hermes-agent.html) 互补——该实体聚焦 Hermes 记忆系统（SQLite/FTS5/sqlite-vec/Markdown）的具体源码实现，本实体提供通用的 **存储选型诊断框架与架构决策指南**。
...
```mermaid
mindmap
  root(("记忆体系工程实战：从设计选型到生产落地 — 存储分层、诊…"))
    概念导图
    核心命题
    四类诊断症状
    记忆类型访问模式（五层模型）
...
```mermaid
mindmap
  root(("记忆体系工程实战 从设计选型到生产落地 存储分层 诊断框架与架构模式"))
    四类诊断症状
    记忆类型访问模式 五层模型
    五种存储技术对比
    三种典型架构方案
...
- [Hermes Agent 记忆系统架构](../ch03/095-hermes-agent.html) — 互补：该实体聚焦 Hermes 源码实现，本实体提供通用选型框架
- [Agent Memory 架构分析(Ruofei)](../ch04/101-agent-memory.html) — 互补：前者侧重记忆架构概念，本实体侧重存储工程实践
- [存之有序治之有矩 — Agent 记忆系统工程实践](../ch03/035-agent.html) — 互补：前者侧重记忆系统整体工程，本实体聚焦存储选型这一具体维度
