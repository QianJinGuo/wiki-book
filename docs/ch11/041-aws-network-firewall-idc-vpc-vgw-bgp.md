...
## Ch11.041 AWS Network Firewall 审查 IDC-VPC 流量：VGW 架构 + BGP 路由传播实验
...
9. **NFW 日志需开启 Flow + Alert 双通道** — Flow 日志记录所有流量会话，Alert 日志记录触发拦截/告警的规则匹配事件；两者结合才能完整还原一次攻击链。参见 [AWS NFW 规则冲突 AI 检测](ch11/140-aws-network-firewall-ai.html)。
...
- [AWS NFW 规则冲突 AI 检测](ch11/140-aws-network-firewall-ai.html) — 姐妹篇（AI 集成）
- [LiteLLM ECS/EKS 部署](../ch01/1517-llm.html) — 同样部署在 VPC 中，受 NFW 保护
- [QuickSight Dataset Q&A](ch11/199-aws-quicksight-dataset-qa-natural-language.html)
