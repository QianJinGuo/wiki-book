...
> 📊 Level ⭐⭐⭐ | 5.8KB | `entities/agent-memory-storage-engineering-practical-guide.md`

> 一枚后端工程狮的 Agent 记忆系统存储工程实战指南。聚焦 **存储选型** 这一 Agent 记忆系统的核心工程问题——诊断四类症状、匹配五种存储技术、提供三种架构模式。

与 [Hermes Agent 记忆系统架构](https://github.com/QianJinGuo/wiki-public/blob/main/entities/hermes-agent-memory-system-architecture.md) 互补——该实体聚焦 Hermes 记忆系统（SQLite/FTS5/sqlite-vec/Markdown）的具体源码实现，本实体提供通用的 **存储选型诊断框架与架构决策指南**。
...
hermes-agent 采用 **方案 A 变体**：SQLite FTS5 + sqlite-vec（混合检索，70% 向量 + 30% BM25）+ Markdown 文件树（程序性记忆）。未引入 Redis，用进程内状态管理会话上下文——不跨进程共享，但对单进程个人助手完全够用；收益是**零依赖**。[^1]
...
- [Hermes Agent 记忆系统架构](https://github.com/QianJinGuo/wiki-public/blob/main/entities/hermes-agent-memory-system-architecture.md) — 互补：该实体聚焦 Hermes 源码实现，本实体提供通用选型框架
- [Agent Memory 架构分析(Ruofei)](https://github.com/QianJinGuo/wiki-public/blob/main/entities/agent-memory-architecture-ruofei.md) — 互补：前者侧重记忆架构概念，本实体侧重存储工程实践
- [存之有序治之有矩 — Agent 记忆系统工程实践](https://github.com/QianJinGuo/wiki-public/blob/main/entities/存之有序治之有矩agent-记忆系统的工程实践与演进.md) — 互补：前者侧重记忆系统整体工程，本实体聚焦存储选型这一具体维度
