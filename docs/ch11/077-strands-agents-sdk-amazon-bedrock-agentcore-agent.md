...
对企业用户来说，AgentCore 的价值是「不用自己搭 Agent 运行平台」，对应 [AgentCore Managed Harness](../ch04/219-agentcore-managed-harness.html) 的定位。但代价是供应商锁定——业务逻辑通过 `@tool` 装饰器和 AgentCore 抽象耦合，迁出 AWS 需要重写工具桥接层。
...
- [Agentcore Harness](../ch04/464-amazon-bedrock-agentcore-harness-ga-api-agent.html) — AgentCore Harness 综述
- [Agentcore Managed Harness](../ch04/219-agentcore-managed-harness.html) — Managed Harness 定位与权衡
- [Amazon Bedrock Agentcore Runtime Deep Dive And Scenario Analysis](../ch04/501-amazon-bedrock-agentcore.html) — AgentCore Runtime 深度分析
- [Agentcore Payments X402 Agentic Commerce](ch11/065-agentcore-payments.html) — AgentCore 在支付场景的应用
- [Agentic Scheduler With Strands Agentcore For Multi Region Gpu Inference](../ch03/045-agent.html) — Strands + AgentCore 调度案例
...
- [deep agents + bedrock agentcore：多 agent 编排 + 隔离基础设施的端到端研究 ag](../ch04/466-agent-orchestration.html)
