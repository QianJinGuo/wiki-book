...
### 核心Tools设计
系统定义了六个核心Tools，覆盖了成本分析的主要场景：
1. **成本异常检测**（使用AWS Cost Anomaly Detection API）：基于机器学习算法识别异常成本模式，计算总影响金额并提供根因分析
2. **多账户成本聚合**：为Payer账户设计的跨账户成本管理，支持 Organizations API集成、权限处理、数据聚合与排序
3. **预算监控**（get_all_budgets）：获取所有AWS预算的状态和利用率，控制预算超支风险
4. **成本预测**（get_cost_forecast）：基于历史数据预测未来成本趋势
5. **服务成本分析**（get_service_costs）：按AWS服务维度分析成本分布
6. **账户成本比较**（compare_account_costs）：比较不同账户间的成本差异
...
→ [原文存档](https://aws.amazon.com/cn/blogs/china/intelligent-cost-analysis-and-alerting-system-powered-by-bedrock-agentcore/)
...
## 相关实体
- [Dify集成Amazon Bedrock AgentCore Browser  实现更强大的信息获取和分析能力 | 亚马逊AWS官方博客](../ch04/392-amazon-bedrock-agentcore.html)
- [快时尚电商行业智能体设计思路与应用实践（七）Amazon Bedrock AgentCore Runtime 深度解析和场景分析 | 亚马逊AWS官方博客](../ch04/392-amazon-bedrock-agentcore.html)
...
- [AgentCore质量优化飞轮：推荐-验证-部署闭环](046-bedrock-agentcore.html)
