...
1. **LiteLLM AI Gateway 的本质是"统一接口层"而非简单代理** — 它不只是把请求转发给 Bedrock，而是承担了 [成本治理](ch01/1476-llm.html)（预算控制、速率限制）、Key 管理（Master Key + 用户级 Key）、响应缓存（Redis）和负载均衡多副本等企业级职责。内部开发者只需要对接一个 OpenAI 兼容 API，底层切换模型或 Provider 对上游完全透明。
...
- [LiteLLM Bedrock 成本治理](ch01/1476-llm.html) — 姐妹篇（运行时限额 + 审计）
- [LiteLLM + QuickSight 可视化](ch01/1476-llm.html) — 姐妹篇（运维监控）
- [AWS NFW AI 冲突检测](ch05/100-ai.html) — NFW 与 ECS/EKS 部署组合使用
