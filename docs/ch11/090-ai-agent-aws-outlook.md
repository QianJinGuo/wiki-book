...
```mermaid
mindmap
  root(("用 AI Agent 自动化日常办公工作流 — 在 AW…"))
    概念导图
    四层架构
    各层职责
      1. Agent 运行时
    ...
    深度分析
      ...
      3. 模型可替换性作为中国区合规约束下的关键设…
      4. 草稿确认门作为外发动作合规基线的设计意图
    实践启示
      1. 为 Skill 文件建立 Git 版本控…
      2. 为每个 MCP Server 实现最小权…
...
```mermaid
mindmap
  root(("用 AI Agent 自动化日常办公工作流 在 AWS 上构建"))
    四层架构
    各层职责
      Agent 运行时
      模型层
    ...
    工程实践要点
    价值
    Skill 文件作为业务语言层的架构意图
    MCP 协议作为企业级异构系统接入标准的价值
    模型可替换性作为中国区合规约束下的关键设计选择
    草稿确认门作为外发动作合规基线的设计意图
```
