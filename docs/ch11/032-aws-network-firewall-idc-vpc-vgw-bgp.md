...
## Ch11.032 AWS Network Firewall 审查 IDC-VPC 流量：VGW 架构 + BGP 路由传播实验
...
> 来源：[原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/使用-aws-network-firewall-服务审查-idc-和云上-vpc-间的流量-vgw-架构的设计和实验.md)
...
9. **NFW 日志需开启 Flow + Alert 双通道** — Flow 日志记录所有流量会话，Alert 日志记录触发拦截/告警的规则匹配事件；两者结合才能完整还原一次攻击链。参见 [AWS NFW 规则冲突 AI 检测](https://github.com/QianJinGuo/wiki/blob/main/entities/aws-network-firewall-ai-conflict-detection-bedrock.md)。
10. **合规审查场景中"流量可见性"是前提** — NFW 不仅拦截流量，更是合规审计的数据源；所有 IDC↔VPC 双向流量均需经过 NFW 并留存日志，以满足等保/GDPR 等监管要求。

## 关键引用清单

- [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/使用-aws-network-firewall-服务审查-idc-和云上-vpc-间的流量-vgw-架构的设计和实验.md)
- [AWS NFW 规则冲突 AI 检测](https://github.com/QianJinGuo/wiki/blob/main/entities/aws-network-firewall-ai-conflict-detection-bedrock.md) — 姐妹篇（AI 集成）
- [LiteLLM ECS/EKS 部署](https://github.com/QianJinGuo/wiki/blob/main/entities/litellm-aws-ecs-eks-ai-gateway-architecture.md) — 同样部署在 VPC 中，受 NFW 保护
- [QuickSight Dataset Q&A](https://github.com/QianJinGuo/wiki/blob/main/entities/aws-quicksight-dataset-qa-natural-language.md)
