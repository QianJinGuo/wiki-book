...
5. **透明路由理由是用户接受度的关键驱动力** — 对比实验显示，仅返回单一预测结果的 fine-tuned 模型用户信任度远低于提供每条路由 Rationale 的方案。这也与 [Claude Code Governance Soft Rules](ch03/077-claude-code.html) 中强调的「可解释性建立信任」原则一致，也呼应了 [Wangyunhe Harness Optimization Agentsoul](ch03/035-agent.html) 中 human-in-the-loop 反馈的校准价值。
