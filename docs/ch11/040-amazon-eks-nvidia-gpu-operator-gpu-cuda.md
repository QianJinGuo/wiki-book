...
## Ch11.040 在 Amazon EKS 上使用 NVIDIA GPU Operator 管理自定义 GPU 驱动与 CUDA 工作负载
...
- [mountpoint s3 vs s3 files：eks 上 s3 数据接入的两种方案实战对比](../ch01/1268-rag.html)
...
| 实体 | 焦点 | 与本文关系 |
|------|------|----------|
| [Building Blocks For Foundation Model Training And Inference On Aws](ch11/136-building-blocks-for-foundation-model-training-and-inference.html) | AWS 训练/推理基础设施通用构件 | 上层基础设施视角，本文专注 GPU 节点驱动管理 |
| [Foundation Model Building Blocks](../ch03/097-foundation-model-building-blocks.html) | 训练/inference building blocks | 同上，更上层 |
| [在 Amazon Ec2 Gpu 实例上部署 Nvidia Nemoclaw 以 Amazon Bedrock 作为推理](ch11/181-amazon-bedrock.html) | Nemoclaw 安全沙箱 + Bedrock 混合推理 | 应用层，本文是底层 driver 管理 |
...
| [Gpu Virtualization Using Mig Technology On Amazon Sagemaker Hyperpod](../ch04/440-pod.html) | SageMaker HyperPod MIG 虚拟化 | SageMaker 路径，与 EKS 路径平行 |

**本文填补的空白**：EKS + GPU Operator + 自定义 driver 管理的完整版本矩阵 + 失败模式实证 + AI 运维闭环。现有 entities 中无 GPU Operator 主题覆盖。

## 相关主题

- [Building Blocks For Foundation Model Training And Inference On Aws](ch11/136-building-blocks-for-foundation-model-training-and-inference.html)
- [Foundation Model Building Blocks](../ch03/097-foundation-model-building-blocks.html)
- [在 Amazon Ec2 Gpu 实例上部署 Nvidia Nemoclaw 以 Amazon Bedrock 作为推理](ch11/181-amazon-bedrock.html)
...
- [Gpu Virtualization Using Mig Technology On Amazon Sagemaker Hyperpod](../ch04/440-pod.html)
- [Build Multi Tenant Ai Agent On Eks Graviton Openclaw K8S Practice](ch11/250-openclaw.html)
- [Openclaw Amazon Bedrock Eks Printer Qc](ch11/181-amazon-bedrock.html)
