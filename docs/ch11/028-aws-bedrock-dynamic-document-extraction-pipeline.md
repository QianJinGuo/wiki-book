...
### [Aws Bedrock Intelligence Message Defense](ch11/007-aws-bedrock.html) — 共享 prompt versioning 思想
该 entity 提到 "Prompt CI/CD" 概念但**没有给出具体服务限制数字**（50/10 prompts/region）也没有 batch 集成方案。本 entity 是其**具体实现 + 服务约束**的工程补全。

### [Process Financial Documents Using Amazon Bedrock Data Automa](ch11/152-amazon-bedrock.html) — 同主题但不同角度
...
- [from pdfs to insights: architecting an intelligent document](ch11/221-from-pdfs-to-insights-architecting-an-intelligent-document.html)
...
4. **DynamoDB 存储 performance metrics 是端到端可观测性的基础**
   - 核心观点：文章在结果存储之外额外记录 latency/token count 等性能指标，这不是论文式的"额外贡献"，而是生产可观测性的必要设计——与 [Aws Bedrock Intelligence Message Defense](ch11/007-aws-bedrock.html) 的 prompt A/B test 思路一致。
...
5. **按文档格式动态选 prompt 是提升提取精度的关键工程细节**
   - 核心观点：文章强调"地契文档有 3 种格式变体（编号列表/表格/图示），不假设一个 prompt 适用所有"。这与 [Optimize Blueprint Extraction Accuracy In Amazon Bedrock Dat](../ch12/046-optimize-blueprint-extraction-accuracy-in-amazon-bedrock-dat.html) 的"跨版面泛化"问题呼应，但本文给出了具体的工程解法。
...
5. **与 [Aws Bedrock Intelligence Message Defense](ch11/007-aws-bedrock.html) 联合参考** — 后者侧重 prompt versioning + A/B test 的 80/20 流量分配策略，前者侧重动态 prompt 路由 + 混合推理模式。两者构成完整的 Bedrock Prompt Management 最佳实践。
