...
9. **NFW 日志需开启 Flow + Alert 双通道** — Flow 日志记录所有流量会话，Alert 日志记录触发拦截/告警的规则匹配事件；两者结合才能完整还原一次攻击链。参见 [AWS NFW 规则冲突 AI 检测](ch11/122-aws-network-firewall-ai.md)。
...
- [AWS NFW 规则冲突 AI 检测](ch11/122-aws-network-firewall-ai.md) — 姐妹篇（AI 集成）
- [LiteLLM ECS/EKS 部署](../ch01/580-llm.md) — 同样部署在 VPC 中，受 NFW 保护
