...
## Ch16.036 LMSYS 在 H20 上服务 DeepSeek-V4-Pro 的场景化 Serving 优化

> 📊 Level ⭐⭐⭐ | 3.5KB | `entities/lmsys-deepseek-v4-pro-serving-h20-engine-optimization-2026-08.md`

# LMSYS 在 H20 上服务 DeepSeek-V4-Pro 的场景化 Serving 优化

## 概述

LMSYS 团队发表了一篇关于在 **NVIDIA H20 GPU**（而非 Blackwell）上服务 1.6T 参数 MoE 模型 **DeepSeek-V4-Pro** 的系统优化长文。核心论点是「一个模型需要多个 serving profile」：没有一种通用配置能同时满足长上下文 prefill、交互式 decode 的 TPOT、以及高并发吞吐的 KV 容量约束，必须按工作负载、SLO 与实测硬件行为分别选择部署拓扑与执行路径。

## 核心方法论：场景化 Serving Profile

文章提出「场景驱动」的 serving 方法论：prefill 依据实测 context-length 在 **PP2 与 PP4** 间切换，decode 则按低延迟/高吞吐/KV 容量分别优化。关键设计决策包括：
...
这篇文章是 LMSYS serving 优化系列的重要延续，与 [DFlash 投机解码](https://github.com/QianJinGuo/wiki/blob/main/entities/lmsys-dflash-speculative-decoding-2026-06.md)、[LMSYS Agent 辅助 SGLang 开发](https://github.com/QianJinGuo/wiki/blob/main/entities/agent-assisted-sglang-development-lmsys-2026-07.md) 同源同团队；其 DSpark 投机解码路径与 [DeepSeek DSpark V4](https://github.com/QianJinGuo/wiki/blob/main/entities/deepseek-dspark-v4-speculative-decoding-deepspec.md) 一脉相承。方法论层面可对照 [LLM 高效推理](https://github.com/QianJinGuo/wiki/blob/main/entities/ai-infra-llm-efficient-inference-vllm.md) 与 [vLLM Native Speed](https://github.com/QianJinGuo/wiki/blob/main/entities/native-speed-vllm-transformers-modeling-backend.md)，其「可预测通信优于不可预测不均衡」的 MoE-TP 论证是通用可迁移结论。

→ [原文存档](https://github.com/QianJinGuo/wiki-book/tree/main/docs/raw/articles/lmsys-deepseek-v4-pro-serving-h20-engine-optimization-2026-08.md)

---

## Ch16.037 Model Routing Is Simple. Until It Isn't — IBM Research 多目标优化路由
...
## Ch16.038 Native-speed vLLM transformers modeling backend
...
## Ch16.039 Nunchaku — 4-bit Diffusion 推理加速
...
## Ch16.042 PUMA — 语义保持的推理模型早停（Semantic-Preserving Early Exit for Reasoning Models）
