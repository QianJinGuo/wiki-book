...
## 相关实体
- [Vector Db Chroma Vs Qdrant](ch11/185-chroma-vs-qdrant.md)
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/chroma-to-qdrant-1m-vector-migration.md)
...
## Ch16.002 ACL 2026｜块越大，推理越差？扩散语言模型的新难题被T*破解了

> 📊 Level ⭐ | 2.0KB | `entities/acl-2026-diffusion-lm-block-size-reasoning-t-star.md`

# ACL 2026｜块越大，推理越差？扩散语言模型的新难题被T*破解了

ACL 2026 的一篇论文揭示了一个反直觉的现象：在扩散语言模型中，推理块（block）越大，模型的推理质量反而越差。与传统自回归语言模型的"长上下文 = 更好推理"不同，扩散模型在分块推理时面临独特的语义连贯性挑战。该研究提出了 T* 方法，通过动态块边界优化和跨块一致性约束，有效解决了大块推理的性能退化问题。

T* 的核心思路是不再使用固定大小的推理块，而是根据内容语义自动划分块边界，同时引入跨块注意力机制确保前后推理的一致性。实验表明，该方法在多个推理基准上比固定分块方案提升了 12-18% 的准确率，且推理速度仅下降 5%。这一发现对扩散语言模型的实际部署有重要指导意义——盲目增大推理块不仅无益，反而有害。

该工作由多所高校联合完成，已在 GitHub 开源代码和模型。论文地址与实验数据完整公开（论文链接见原文存档）。这一结果挑战了 LLM 推理中"越长越好"的朴素直觉，为扩散语言模型的推理策略设计提供了新的理论视角。

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/acl-2026块越大推理越差扩散语言模型的新难题被t破解了.md)

---

## Ch16.003 刚刚DeepSeek开源推理神器DSpark，V4最高提速85%，连底层训练全家桶都开源了

> 📊 Level ⭐ | 1.7KB | `entities/deepseek-dspark-v4-speculative-decoding-deepspec.md`

# 刚刚DeepSeek开源推理神器DSpark，V4最高提速85%，连底层训练全家桶都开源了

DeepSeek 开源了 DSpark V4 推理优化方法，同时发布了完整的训练框架 DeepSpec。DSpark V4 采用半自回归生成 + 置信度调度验证 + 异步零开销调度的架构，在生产环境（V4 模型）中实现了吞吐量和延迟提升最高 85%。

DeepSpec 是配套的完整工具链，包含数据准备、草稿模型实现、训练代码和评估脚本，当前支持 DSpark、DFlash 和 Eagle3 三种推测解码算法。这一开源举措使得社区可以完整复现和扩展推测解码技术，极大降低了推理优化门槛。

[已有 DSpark 实体](ch01/216-0.md) 涵盖了 DSpark 的整体架构设计，本文补充了 V4 版本的具体性能数据和 DeepSpec 全栈工具链的细节。

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/刚刚deepseek开源推理神器dsparkv4最高提速85连底层训练全家桶都开源了.md)

---

## Ch16.004 Build real-time voice applications with Amazon SageMaker AI and vLLM
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/build-real-time-voice-applications-with-amazon-sagemaker-ai.md)
→ [Voice Agent 设计 - Nova Sonic 多 Agent 工具与会话](ch03/045-agent.md)
→ [Nova Sonic WebRTC 实时语音流](ch11/257-amazon-nova.md)
→ [OpenAI Realtime Voice 架构](https://github.com/QianJinGuo/wiki/blob/main/concepts/openai-realtime-voice-architecture.md)

---

## Ch16.005 Apple Siri 私有推理（Private Inference）不私有：三个对抗者都不受加密学保护
...
> **Source**：[原文存档（Matthew Green / Cryptography Engineering, 2026-06-09）](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/apple-siri-private-inference-cryptography-green.md)
...
## 与 [Simon Willison vibe-coding](ch09/047-coding-agent.md) 的呼应
...
- [Vibe Coding Agentic Engineering Convergence Simon Willison](ch09/047-coding-agent.md)：lethal trifecta 概念同源
- [Apple Silicon Costs More Than Openrouter](ch01/440-apple-silicon-costs-more-than-openrouter.md)：Apple 硬件成本视角
- [Apple Corecrypto Formal Verification Blueprint](ch12/026-apple-corecrypto-formal-verification-blueprint-post-quantu.md)：Apple 加密学基础设施
...
## Ch16.006 GLM-5 Scaling 痛点与推理优化
...
--- See also [Karpathy Vibe Coding To Agentic Engineering](ch03/045-agent.md)
...
## Ch16.007 EAGLE-3 投机解码与 USP 长序列训练优化
...
- [小米AI — ICML 2026 论文矩阵（11篇）](ch04/069-ai.md)
- [OpenClacky — Prompt Cache 命中率 90% 的 Harness 工程实践](ch05/085-harness.md)
- [百度文心大模型后训练进化（ERNIE 3.0→5.0）](ch04/069-ai.md)

---

## Ch16.008 PithTrain：陈天奇 + CMU Flame Center 推出的 agent-native MoE 训练框架（11K Python / 双重效率）
...
- **与 [Skill 自进化三路线](ch04/258-skill.md)** 呼应：PithTrain 的 agent skills 哲学与 SkillOS/SkillOpt 一脉相承（>30% 重复劳动 → 技能化）
- **与 [SaaS-Bench](ch03/045-agent.md)** 平行：两者都创建"为 agent 量身定制的工程指标" —— SaaS-Bench 测 GUI Agent 完成率，PithTrain 测训练框架的 agent-task efficiency
- **与 [Matt Van Horn 22 黑客技巧](ch03/075-claude-code.md)** 呼应：Matt 的"任何做超过 2 次的事 → 技能"是 PithTrain agent skills 哲学的应用层验证
- **与 [Agent 六机制](ch03/045-agent.md)** 平行：双重效率思想是六机制中"系统-环境协同"的具体实现
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/pith-train-agent-native-moe.md)
...
- [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/embodied-intelligence-sim-to-real-active-inference-behavior-tree-intrinsic-motivation-chenzhiyan-2026-06-17.md)
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/how-to-calculate-the-inference-efficiency-ratio.md)

## 相关实体
> [主题导航](https://github.com/QianJinGuo/wiki/blob/main/queries/ai-model-research-latest-directions.md)

- [How Superset built the IDE for AI agents on Vercel](ch04/069-ai.md)
- [What Is Urban Density Design? A Clear Guide to How Cities Get Built Denser](ch01/1168-what-is-urban-density-design-a-clear-guide-to-how-cities-ge.md)
...
> 来源：[原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/continuous-async.md)
...
- [Lightseek Tokenspeed](ch01/1004-lightseek-tokenspeed.md)
...
## 相关页面
[GLM-5 Scaling Pain 推理复盘](ch04/069-ai.md) — 包含 HiCache BugFix #2 的详细分析
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/end-to-end-encrypted-ml-inference-with-amazon-sagemaker-ai-a.md)
...
> Source: [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/lmsys-dflash-speculative-decoding-2026-06.md)
...
- [automation anywhere collaborates with cisco, nvidia, okta, a](ch04/069-ai.md)
...
- [DDoSing Software Delivery Pipelines](ch01/216-0.md)
- [AI GPUs probably live longer than three years](ch04/069-ai.md)

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/lmsys-dflash-speculative-decoding-2026-06.md)

---

## Ch16.015 TLiveOmni vLLM 适配与量化方案
...
---
## 相关实体
- [ai-infra-auto-driven-skills v0.1.0：给 codex / claude code 的推理](ch04/258-skill.md)
- [gemma 4 multi token prediction drafters](ch01/331-gemma-4-multi-token-prediction-drafters.md)
- [tokenspeed agentic inference engine](ch04/587-tokenspeed-agentic-inference-engine.md)

---

## Ch16.016 vLLM V0→V1 迁移中的 logprob 差异修复
...
<!-- - [servicenow vllm correctness](ch01/1171-llm.md) -->
<!-- - [servicenow vllm correctness huggingface](ch01/1171-llm.md) -->
...
## Ch16.017 京东 JoyAI-VL-Interaction — 全栈开源视频语言交互模型
...
JoyAI-VL-Interaction 的"前台实时助手 + 后台智能大脑"架构，与 [Hermes Agent 技能设计](ch04/258-skill.md) 中的多层架构有异曲同工之处。前台模型类似于"感知-响应"的快速路径（System 1），后台模型类似于"推理-规划"的慢速路径（System 2）。
...
京东拥有全球领先的物理世界运营网络（仓储、配送、门店、直播、客服、售后），这些场景每天都在发生人、货、场的实时互动——对这些物理世界数据的理解和利用，是 JoyAI-VL-Interaction 的独特优势。这也与 [NVIDIA XR/AR 眼镜 Agent 基础设施](ch04/069-ai.md) 中讨论的"物理世界 AI 入口"趋势相互印证。
...
- [WWW 2026 频谱解耦与增强](ch01/216-0.md)
- [NVIDIA XR/AR 眼镜 Agent 基础设施](ch04/069-ai.md)
- [Hermes Agent 技能设计分析](ch04/258-skill.md)
- [Claude Code 系统工程能力](ch03/075-claude-code.md)
- [NVIDIA 多模态 RAG 知识系统](ch01/1019-rag.md)
- [Agent Harness 架构](ch05/085-harness.md)

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/全球首个京东全栈开源joyai-vl-interaction让大模型从一问一答走向边看边说.md)

---

## Ch16.018 ServiceNow vLLM V0→V1 正确性修复

> 📊 Level ⭐⭐⭐ | 8.3KB | `entities/servicenow-vllm-correctness-huggingface.md`

> -> [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/servicenow-vllm-correctness-huggingface.md)
...
- [ServiceNow vLLM Correctness（更完整的分析）](ch01/1171-llm.md)
- [vLLM V0→V1 迁移中的 logprob 差异修复](ch01/216-0.md)
...
## Ch16.019 Profiling in PyTorch (Part 2): From nn.Linear to a Fused MLP
...
- [Deepseek V4 Triton Fp4 Optimization](ch01/305-deepseek-v4.md) — 同样涉及 Triton kernel 优化，与本文的 pointwise fusion 优化角度互补
...
## Ch16.020 MiMo-V2.5 推理系统全链路优化：Hybrid SWA + MoE + 多模态生产级落地
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/mimo-v2-5-inference-system-optimization-hybrid-swa.md)
...
> -> [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/llm-inference-pipeline-internals.md)
...
→ 相关实体：[DeepSeek V4 本地推理](ch04/069-ai.md)
...
- → [DeepSeek V4 本地推理](ch04/069-ai.md)：V4 的 CSA/HCA 架构创新（本文第 6 节）与 antirez 的 ds4.c 本地推理引擎互补
- → [GLM-5 Scaling Pain](ch04/069-ai.md)：高并发推理下的竞态 Bug，是本文第 8 节"推理服务基础设施"的反面案例
- → [vLLM](ch01/825-vllm.md)：PagedAttention 的具体实现

---

## Ch16.022 elasticpp重塑elasticsearch查询性能的c内核引擎
...
- [Claude Code Hidden Settings 18](ch03/075-claude-code.md)
- [Alphaevolve交出一周年炸裂成绩单Ai自我改进不再科幻](ch04/069-ai.md)
- [Rag Chunking Optimization 2025](ch01/216-0.md)
- [Wangyunhe Harness Optimization Agentsoul](ch05/085-harness.md)

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/elasticpp重塑elasticsearch查询性能的c内核引擎.md)
...
## Ch16.023 腾讯混元 Hy3 preview 在 Hopper 卡上的推理优化实践
...
- [腾讯混元 Hy3 preview 发布](ch03/045-agent.md)：互补实体，该篇讲模型能力与发布，本篇讲推理优化技术细节
- [LLM 推理流水线](ch01/1171-llm.md)：推理优化基础知识，本篇是 Hy3 的具体工程实践
...
→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/tencent-hunyuan-hy3-preview-hopper-inference-optimization.md)

---

## Ch16.024 vLLM V0 to V1: Correctness Before Corrections in RL
...
## Ch16.025 Bonsai Image 4B: 1-bit 和 Ternary 量化
...
**2. 结合 [Ai Infra Llm Efficient Inference Vllm](ch04/069-ai.md) 进一步降低推理延迟**
...
## Ch16.026 Pytorch in Kernel Recsys Optimization

> 📊 Level ⭐⭐⭐ | 4.4KB | `entities/pytorch-in-kernel-recsys-optimization.md`

## 深度分析

**消除而非优化：Kernel 层设计的方法论突破：** IKBO 的核心洞察是"broadcast 是数据布局问题，而非计算必需"——传统方法在系统层面处理 broadcast 复制，浪费内存带宽和计算资源；而 IKBO 在计算原语层面消除复制，让 kernel 内部处理 mismatched batch sizes。这个思维转换将优化方向从"workaround 问题"转向"消除问题根源"，实现了 2/3 的延迟降低。这种**在根源处解决问题**而非在表面做修补的思想，对其他 AI 系统优化有普遍借鉴意义。 See also [Harness Production Agent Engineering Deficit](ch05/085-harness.md)
...
## Ch16.027 14× faster embeddings: how we rebuilt the ONNX path in Manticore
...
## Ch16.029 小米多篇论文入选 ECCV 2026 — 人脸修复、视频大模型推理加速

> 📊 Level ⭐⭐⭐ | 1.5KB | `entities/xiaomi-eccv-2026-face-restoration-video-llm-inference.md`

# 小米多篇论文入选 ECCV 2026 — 人脸修复、视频大模型推理加速

小米多篇论文入选 ECCV 2026，涵盖人脸视频修复、图像美学评估、视频大模型推理加速等多个方向。

核心成果包括：
- **TIGER**：基于 diffusion 的人脸视频修复方法，解决严重退化人脸重建问题
- **RED-Aes**：图像美学评估新方法
- **VST（Video Streaming Thinking）**：视频大模型推理加速方法，响应速度提升 15.7 倍

这项工作展示了在端侧视觉模型和视频理解领域的最新进展。

→ [原文存档](https://raw.githubusercontent.com/QianJinGuo/wiki/main/raw/articles/一步修复严重退化人脸视频边看边想让视频大模型响应速度提升157倍小米多篇论文入选-eccv-2026.md)
