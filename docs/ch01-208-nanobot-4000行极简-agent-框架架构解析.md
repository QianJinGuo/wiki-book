...
这与 [天猫团队的 AGENT.md 持续约定](../ch09-029-天猫新品营销技术团队ai编码实战指南-上)、Anthropic Skills、Claude Code 的 `.claude/` 目录是同一种范式。**Markdown-as-Config 正在成为 LLM 时代的事实标准**。
...
- [Loop Engineering Addy Osmani Challengehub](../ch05-002-loop-engineering-不再写提示词-而是设计替你写提示词的循环-先写刹车再写循环-13-来源深度合并-ad) — ReAct 循环工程化的另一视角
