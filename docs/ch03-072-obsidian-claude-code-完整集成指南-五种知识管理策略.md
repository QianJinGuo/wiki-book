...
- **策略 3（MCP 桥接）**：代码仓库和 Obsidian 完全分离，通过 [obsidian-claude-code-mcp](/ch09-097-obsidian-claude-code-integration-guide/) 这类插件让 Claude 按需访问 Obsidian
...
这与 [harness 工程](/ch05-041-harness-engineering-概念框架/)中的「索引优先于扫描」原则完全一致——当你的知识库足够大时，让 Agent 调用索引比让它读文件便宜一到两个数量级。对正在选型的人来说，这意味着「Obsidian 是否提供官方 CLI/Skill」会成为是否值得作为 Agent 后端的关键指标。
...
这套方案对应的不是「更好用的笔记」，而是「让每一次 Agent 对话都不是一次性消耗」。它和 [Agent 长期记忆架构](/ch04-029-agent-memory-architecture-past-influence-future-ruofei/)、Warp Oz 的 [cross-harness Agent Memory](/ch04-409-agent-orchestration/) 处于同一个赛道——都是在解决「Agent 失忆」这个根本问题，只是 QMD 选择了「本地优先 + Markdown 原生」的路径。
...
更深一层是：当 Vault 被 AI 自己生成的内容污染后，Claude 下次读 Vault 时会被自己之前生成的二手内容误导，形成「自我喂养」的反馈环。这是 Agent 记忆生命周期里被反复强调的失败模式。
...
- [Obsidian Claude Code Integration Guide](/ch09-097-obsidian-claude-code-integration-guide/) — 早期版本的集成指南
- [Obsidian Claude Code Integration](/ch09-097-obsidian-claude-code-integration-guide/) — 集成实践综述
- [Obsidian](/ch09-097-obsidian-claude-code-integration-guide/) — Obsidian 实体页
- [Obsidian Llm Wiki Local Kytmanov 2026](/ch01-826-obsidian-llm-wiki-local-obsidian本地ai知识图谱自动构建工具/) — 本地 LLM Wiki 实践
- [Claude Code 7 Layer Memory Architecture](/ch01-857-claude-code-7-layer-memory-architecture/) — Claude Code 的 7 层记忆架构
- [Agent Memory Architecture](/ch04-029-agent-memory-architecture-past-influence-future-ruofei/) — Agent 记忆架构综述
- [Harness Engineering Framework](/ch05-041-harness-engineering-概念框架/) — Harness 工程框架
- [Agent Memory Lifecycle Philosophies — Agent 记忆生命周期哲学](/ch04-087-agent-memory-架构本质/)
- [我用claude搭了个自动新闻简报，30天后比我刷了一年的信息还有用](/ch03-106-我用claude搭了个自动新闻简报-30天后比我刷了一年的信息还有用/)
- MOC
