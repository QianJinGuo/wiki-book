...
| `@cf/baai/bge-m3` | 多语言 Embedding 模型（1024维） |
| `@cf/qwen/qwen3-embedding-0.6b` | 轻量中文 Embedding（备选） |
...
### 为什么不用 embedding 做语义搜索
- 需要预计算 47K 文档的向量（一次性 47K 次 embedding 调用）
- 需要存储向量索引（额外 ~200MB）
- 冷启动问题：新文档需要重新 embedding
- Reranker 直接对 query + doc 对打分，精度高于 cosine similarity
...
构建时:
  for each doc in search_index.docs:
    embedding = Workers AI @cf/baai/bge-m3(doc.title + doc.text)
    存储: { doc_id, embedding: float[1024], metadata }

查询时:
  query_embedding = Workers AI @cf/baai/bge-m3(question)
...
### 存储方案
- 向量存入 R2（JSON 格式，~200MB）
- 或使用 Cloudflare Vectorize（如果可用）
...
| `wrangler.toml` 加 AI binding | 手动更新 |
